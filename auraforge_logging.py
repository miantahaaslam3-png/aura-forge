"""Centralized logging setup for Aura Forge Agent.
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``auraforge_cli.web_server``,
    ``auraforge_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Aura Forge (TUI, gateway, ``hy_memory`` server, MCP
...
from auraforge_constants import get_config_path, get_hermes_home
...
COMPONENT_PREFIXES = {
    ...
    "tools": ("tools",),
    "cli": ("auraforge_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "auraforge_cli.web_server",
        "auraforge_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Aura Forge logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hermes_home
        Override for the Aura Forge home directory.  Falls back to
    ...
    from auraforge_constants import mkdir_under_hermes_home
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from auraforge_cli.config import is_managed
...
# the module header): when several Aura Forge processes log to the same file, an
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse auraforge_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when auraforge_cli.config isn't importable (bare
        # auraforge_logging consumers).
        try:
            from auraforge_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
