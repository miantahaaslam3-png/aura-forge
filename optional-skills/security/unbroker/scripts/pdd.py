...
The Aura Forge agent orchestrates scanning and opt-out submission with native tools
...
def cmd_cdp(args) -> None:
    ...
    Aura Forge at the operator's real Chrome on a dedicated debug profile (see methods.md).
    """
    import shlex
    import time
    ...
    if live:
        _out({"running": True, "endpoint": f"127.0.0.1:{port}",
              ...
              "note": "a debuggable browser is already listening; point Aura Forge's browser tools at "
    ...
    _out({"running": bool(live), "launched_pid": pid, "browser": browser,
          ...
          "next": ([f"point Aura Forge's browser tools at 127.0.0.1:{port} (CDP)",
