...
Companion skill for the `unreal-engine` entry in the Aura Forge MCP catalog. The
...
Two halves, in this order: the editor side must be up before Aura Forge connects.
...
Do NOT use `ModelContextProtocol.GenerateClientConfig` for Aura Forge — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Aura Forge connects
...
2. Start the Aura Forge session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Aura Forge
they appear as:

| Aura Forge tool | Purpose |
|---|---|
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Aura Forge runs batched calls
...
- **Start order matters.** Editor + server up first, then the Aura Forge
...
- **Timeouts on long operations.** Aura Forge' per-call default is 120 s; asset
