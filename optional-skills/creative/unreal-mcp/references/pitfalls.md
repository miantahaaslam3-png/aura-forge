...
### 1. Start order: editor first, Aura Forge second

Aura Forge probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Aura Forge session.
...
restart the editor, restart the Aura Forge session.
...
server fails to bind (Output Log) or Aura Forge' probe times out. Fix: change
...
### 5. GenerateClientConfig is not for Aura Forge

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Aura Forge' connection
lives in `~/.hermes/config.yaml` via `hermes mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Aura Forge — just don't
mistake it for the Aura Forge setup step.
...
explicitly warns against overlapping calls. Aura Forge executes same-turn tool
...
### 9. Timeouts: Aura Forge gives up before Unreal does

Aura Forge' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Aura Forge runs elsewhere (SSH backend,
