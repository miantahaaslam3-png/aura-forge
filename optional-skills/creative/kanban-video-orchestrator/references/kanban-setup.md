...
2. **Create profiles** — `aura profile create <name> --clone`
...
6. **Fire the initial kanban task** — `aura kanban create` assigned to the director
...
```bash
aura profile create director --clone 2>/dev/null || true
```
...
```bash
aura kanban create "Direct production of <video title>" \
    --assignee director \
...
the Aura Forge `.env` (`${HERMES_HOME:-$HOME/.hermes}/.env`) and macOS Keychain
(if on macOS):
...
check_key ELEVENLABS_API_KEY aura ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY aura OPENROUTER_API_KEY || exit 1
# ...
```
