...
Maps each role archetype to the Aura Forge skills it should `always_load` and the
...
## Aura Forge skills relevant to video production
...
These are **not** Aura Forge skills but external CLIs / APIs that profiles invoke.
...
## Built-in Aura Forge tools for media review

These are native Aura Forge tools — not invoked via terminal but through their own
...
| Anthropic | `ANTHROPIC_API_KEY` | every Aura Forge profile (Claude) |
