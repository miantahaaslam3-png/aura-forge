...
description: Migrate a user's OpenClaw customization footprint into Aura Forge Agent. Imports Hermes-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Aura Forge Agent (Nous Research)
license: MIT
...
# OpenClaw -> Aura Forge Migration

Use this skill when a user wants to move their OpenClaw setup into Aura Forge Agent with minimal manual cleanup.
...
```bash
aura claw migrate              # Full interactive migration
aura claw migrate --dry-run    # Preview what would be migrated
aura claw migrate --preset user-data   # Migrate without secrets
aura claw migrate --overwrite  # Overwrite existing conflicts
aura claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `aura setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.
...
- merge OpenClaw command approval patterns into Aura Forge `command_allowlist`
- migrate Hermes-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Aura Forge working-directory configuration
- copy OpenClaw skills into `~/.hermes/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Aura Forge workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hermes/tts/`
- archive non-secret docs that do not have a direct Aura Forge destination
...
With `--migrate-secrets`, it will also import a small allowlisted set of Aura Forge-compatible secrets, currently:
...
Aura Forge CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Aura Forge. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into an Aura Forge workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Aura Forge skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Aura Forge unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Aura Forge targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Aura Forge destination. Unsupported auth blobs must still be reported as skipped.
...
- Aura Forge persona state imported
- Aura Forge memory files populated with converted OpenClaw knowledge
