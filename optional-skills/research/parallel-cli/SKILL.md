...
- It overlaps with Aura Forge native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Aura Forge native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Aura Forge native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
Recommended Aura Forge workflow:
...
## Recommended Aura Forge usage patterns
...
- Do not silently choose Parallel when Aura Forge native tools are already sufficient.
