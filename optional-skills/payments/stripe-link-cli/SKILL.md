...
version: 0.1.0
author: Teknium (teknium1), Aura Forge Agent
license: MIT
...
Wraps [@stripe/link-cli](https://github.com/stripe/link-cli) so Aura Forge can complete purchases on the user's behalf using one-time-use virtual cards or Shared Payment Tokens (SPT). Every spend is gated by an in-app approval in the Link mobile/web app — Aura Forge cannot self-approve.
...
The Link account, payment method, and spend-approval app do NOT need to be set up before Aura Forge attempts to pay — the CLI walks the user through them on first run:
