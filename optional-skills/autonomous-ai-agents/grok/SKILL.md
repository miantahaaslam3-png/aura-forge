...
version: 0.1.1
author: Matt Maximo (MattMaximo), Aura Forge Agent
license: MIT
...
# Grok Build CLI — Aura Forge Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Aura Forge terminal. Grok
...
1. Prepare stable input files first with Aura Forge tools (`read_file`,
...
2. **Don't conflate Aura Forge' xAI auth with the `grok` CLI's auth.** Aura Forge'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Aura Forge' xAI auth carries
   over.
