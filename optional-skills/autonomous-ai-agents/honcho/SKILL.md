...
# Honcho Memory for Aura Forge

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Aura Forge profile its own peer identity while sharing a unified view of the user.
...
3. **AI peer card** -- the identity card for this Aura Forge profile's AI peer
...
Honcho models conversations as interactions between **peers**. Aura Forge creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Aura Forge instance. Each profile gets its own AI peer so agents develop independent views.
...
Each Aura Forge profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
Guidelines for Aura Forge when Honcho memory is active.
