...
- smoke-test a server with the FastMCP CLI before wiring it into Aura Forge or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Aura Forge. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
When the goal is Aura Forge integration, either:
