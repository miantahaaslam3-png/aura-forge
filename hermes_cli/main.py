...
def _print_fast_version_info() -> None:
    ...
    print(f"Aura Forge v{__version__} ({__release_date__})")
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Aura Forge'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_postinstall(args):
    ...
    print("⚕ Aura Forge post-install bootstrap")
    print()
...
def cmd_uninstall(args):
    """Uninstall Aura Forge (or just the Chat GUI with --gui)."""
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        ...
        candidates = [
            release_dir / "win-unpacked" / "Aura Forge.exe",
            release_dir / "win-ia32-unpacked" / "Aura Forge.exe",
            release_dir / "win-arm64-unpacked" / "Aura Forge.exe",
        ]
    else:
        candidates = [
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Aura Forge is damaged and can't be opened." The
...
def _update_via_zip(args):
    """Update Aura Forge by downloading a ZIP archive.
...
def cmd_update(args):
    """Update Aura Forge to the latest version.
    ...
    if is_managed():
        managed_error("update Aura Forge")
        return
...
def _cmd_update_pip(args):
    ...
    print("✓ Update complete! Restart aura to use the new version.")
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Aura Forge windows/gateways and re-run: aura update")
            else:
                print("✓ Already up to date!")
            _resume_windows_gateways_after_update(_windows_gateway_resume)
            return
...
def _report_dashboard_status() -> int:
    ...
    if not pids:
        print("No aura dashboard processes running.")
        return 0

    print(f"{len(pids)} aura dashboard process(es) running:")
    for pid in pids:
...
def cmd_acp(args):
    """Launch Aura Forge as an ACP server."""
    try:
