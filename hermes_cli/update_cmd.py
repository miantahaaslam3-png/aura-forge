...
def _format_concurrent_instances_message(
    ...
    lines.append("")
    lines.append("  Close Aura Forge Desktop, exit any open `hermes` REPLs, and")
...
def _filter_non_gateway_concurrent_instances(
    ...
    TUI shell, a Aura Forge Desktop backend child, an unrelated ``hermes`` REPL)
...
def _ensure_acp_launcher() -> None:
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        ...
        try:
            ...
            shim = (
                "#!/usr/bin/env bash\n"
                "# Aura Forge Agent — ACP launcher (written by `hermes update`).\n"
