"""Aura Forge update pipeline — extracted from ``hermes_cli/main.py``.
...
def _orphaned_desktop_backend_pids(
    ...
    - its cmdline is a Aura Forge backend (``hermes_cli.main`` + ``serve`` /
      ``dashboard``), and
...
def _handoff_reapable_backend_pids(
    ...
    - Only Aura Forge backends (``hermes_cli.main`` + ``serve``/``dashboard``)
