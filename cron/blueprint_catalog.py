...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``auraforge://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``auraforge://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"auraforge://blueprint/{quote(blueprint.key)}{qs}"
