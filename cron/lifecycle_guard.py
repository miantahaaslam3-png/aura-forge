...
``auraforge gateway restart`` (or ``launchctl kickstart ai.auraforge.gateway``
or ``systemctl restart auraforge-gateway``).  When the cron fires, the
...
``auraforge cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``auraforge gateway``, ``launchctl ... auraforge-gateway``,
``systemctl ... auraforge-gateway``, ``pkill`` against the gateway) so it
...
primitive as a persistent KeepAlive job, not a one-shot task. ``auraforge gateway
...
The profile-flag form (``auraforge -p <profile> gateway restart|stop``, #78028)
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive `auraforge gateway` operations.
    # The destructive operations are restart, stop, and uninstall.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    # The lookbehind (#77173): `auraforge` must not be a path component or a
    # word tail. Excluding `/`, word chars, `.` and `-` keeps file paths
    # with embedded spaces (`/docs/auraforge gateway restart-notes.md`) from
    # matching via the `/auraforge` tail, while every real command position
    # (start of text, whitespace, `;`/`&`/`|`, `$(`, backtick, even a
    # U+FFFD from binary-content decoding) still matches.
    r"(?:(?<![/\w.\-])auraforge\s+gateway\s+(?:restart|stop|uninstall)\b)"
    # Branch B: launchctl ops on a auraforge-gateway label. macOS launchd
    # labels look like `ai.auraforge.gateway` / `auraforge-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated auraforge services (e.g.
    # `launchctl unload ai.auraforge.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.auraforge.gateway-<suffix> --
    ...
    # Branch C: systemctl ops on a auraforge-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bhermes[.\-]?gateway)"
    # Branch D: pkill / kill targeting the auraforge gateway process. Both
...
# lines (e.g. `launchctl submit \` / `  -l ai.auraforge.gateway-... \` / `  -- ...`,
...
# selector — `auraforge -p <profile> gateway restart|stop` / `--profile <name>`
# / `--profile=<name>`. The selector token between `auraforge` and `gateway`
...
# `auraforge -p venus gateway restart` operates on a sibling profile's gateway
...
# Branch B only catches `launchctl <verb> ... auraforge[.-]?gateway` when the
...
# command — `for item in 'ai.auraforge.gateway-apollo:...' 'ai.auraforge.gateway:...';
...
# is the unexpanded variable `$label`, not the string "auraforge.gateway". This
...
# EXISTING job, so anchoring to the auraforge-gateway label is still correct —
# `test_safe_commands` requires unrelated-label ops (e.g. `launchctl unload
# ai.auraforge.update-checker.plist`) to stay unblocked. The fix is checking
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    a ``auraforge``/``gateway`` token) — this function is the single choke
    ...
    # a runbook line "a human can run: auraforge gateway restart" inside such a
    ...
    if _GATEWAY_LIFECYCLE_PATTERN.search(normalized):
        return True
    # Profile-flag form (#78028): `auraforge -p <profile> gateway restart|stop`
    # bypasses Branch A because the selector sits between `auraforge` and
...
# for `systemctl restart auraforge-gateway` in syslog, a SQL LIKE literal over a
...
# blocking `grep -r 'systemctl restart auraforge-gateway' .` outright. Require a
...
# to execution (`grep 'systemctl restart auraforge-gateway' f | sh`); never mask
# such a line.
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.auraforge.svc-reload-tmp``)
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart auraforge-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart auraforge-gateway%'"
...
def _references_at(
    ...
    # (e.g. `Path.home() / ".auraforge"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    (``~/.auraforge/scripts/restart.sh``) but is passed as the bare name
...
def _read_script_for_scanning(script_path: str) -> str:
    ...
    if unsafe:
        return "auraforge gateway restart"
...
def check_gateway_lifecycle(
        ...
        if resolved_script is not None:
            ...
            if _is_cloud_placeholder_path(resolved_script) or _is_cloud_placeholder_path(
                ...
                raise GatewayLifecycleBlocked(
                    ...
                    "(e.g. ~/.auraforge/scripts/) and recreate the job."
                )
    ...
    if python_script:
        ...
        # `auraforge gateway restart` embedded in a .py script is still
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `auraforge gateway restart` from a shell outside "
