...
Uses a file-based lock (~/.auraforge/cron/.tick.lock) so only one tick
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "Fallback chain was exhausted or unavailable."
    return (
        "No fallback chain configured — add one with `auraforge fallback add`, "
...
def _failure_streak_nudge(job: dict) -> str:
    ...
    return (
        ...
        f"Fix its prompt/config, or pause it with `auraforge cron pause {job_ref}` "
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    if "skipped to prevent unintended spend: global inference config drifted" in lower:
        if "finite one-shot job is consumed" in lower:
            ...
            remediation = (
                "On the host running Aura Forge, pin it explicitly: "
                f"`auraforge cron edit {job_id} --provider <provider> "
                "--model <model>`."
            )
        return (
    ...
    # checkout was updated underneath it (interrupted `auraforge update`, manual
    ...
    if provider_reachable and re.search(
        ...
        if skew is not None:
            ...
            message += (
                ...
                "`auraforge gateway restart` to fix it."
            )
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``auraforge tools`` config for the ``cron`` platform.
...
def _write_usage_audit(record: dict) -> None:
    """Append a single JSONL line to ~/.auraforge/cron/usage_audit.jsonl.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``auraforge update`` / ``auraforge gateway stop`` / systemd restart, or an
...
def _get_hermes_home() -> Path:
    """Resolve Aura Forge home dynamically while preserving test monkeypatch hooks.
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"Aura Forge — {task_name}"
    try:
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    Runs ``auraforge [-p <profile>] chat --in ~ -c "Bot Chat" --create-if-missing
    ...
    hermes_bin = _shutil.which("auraforge")
    if hermes_bin:
        argv = [hermes_bin]
    else:
        try:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="auraforge-cron-botchat-",
            delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
...
def _resolve_bot_chat_target(job: dict, profile_arg: str) -> Optional[dict]:
    ...
    ``~/.auraforge/profiles/`` tree, so same-named profiles on other gateways
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    # at boot; a standalone process (manual `auraforge cron run` from the CLI,
    ...
    for target in targets:
        ...
        if transport is not None:
            ...
            # (`auraforge cron run`) has no live relay adapter, so surface the
...
def _run_job_script(
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Aura Forge-managed secrets are not inherited
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `auraforge setup`), or pin a "
            "working provider via `auraforge cron edit "
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            return (
                ...
                "`auraforge setup` or change the job's `deliver` target."
            )
    return None
...
def run_job(
    ...
    *,
    ...
    try:
        ...
        #
        # A kanban worker is a normal `auraforge chat -q` CLI agent whose default
        ...
        # re-read from storage every tick so a ``auraforge cron edit --model``
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"`auraforge cron edit {job_id} --model <name>` or set a "
                "default with `auraforge model <name>`."
            )
        ...
        if cron_model_drift_guard_enabled(_cfg):
            ...
            if _drift:
                ...
                if _finite_oneshot:
                    ...
                    _remediation = (
                        "To run on the new config, on the host running Aura Forge "
                        "pin it explicitly: "
                        f"`auraforge cron edit {job_id} --provider <provider> "
...
def run_one_job(
    ...
    *,
    ...
    if extra_prompt is None:
        # A gateway-forwarded manual run (`auraforge cron run --prompt` /
    ...
    *,
    ...
    try:
        ...
        if delivery_error:
            ...
            # exact signature via `auraforge cron incidents ack`.
...
def tick(
    ...
    *,
    ...
    try:
        # Global emergency stop (`auraforge pause`): skip dispatch entirely while
        # the ESTOP sentinel exists. Never touches in-flight runs — due jobs
        # simply wait for the next tick after `auraforge resume`. Logged once per
        ...
        # scheduler STARTUP. A one-shot `auraforge cron run` that claimed a job
