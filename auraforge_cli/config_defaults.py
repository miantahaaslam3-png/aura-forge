"""Default configuration data for Aura Forge Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from auraforge_cli/config.py. Must not import from auraforge_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    # Soft file-descriptor limit for long-running Aura Forge server processes.
    ...
    "agent": {
        ...
        # all normalized by auraforge_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # prompt's environment-hints block. Lets a host that wraps Aura Forge
        ...
        # app, ACP) in a code workspace, Aura Forge adds a coding operating brief
    ...
    "terminal": {
        ...
        # skipped. When empty, Aura Forge auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Aura Forge sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Aura Forge
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Aura Forge sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Aura Forge should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # single read_file page exceeds these limits, Aura Forge truncates the
    ...
    "compression": {
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Aura Forge' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Aura Forge does.
    ...
    "display": {
        ...
        # model — see auraforge_cli/focus_view.py.
        "focus_view": False,
    ...
    "dashboard": {
        ...
        # protocol ping (see auraforge_cli/web_server.py — an event-loop stall
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Aura Forge appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
    ...
    # "Hey Aura Forge" hands-free wake word. Always-on, on-device hotword
    ...
    # response. If not, Aura Forge feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Aura Forge auto-pauses the goal and
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Aura Forge Agent*" header.
    ...
    # replace Aura Forge' built-in platform hint for a single messaging platform
    ...
    "security": {
        ...
        # ``auraforge_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Aura Forge to lazy-install opt-in backend packages from PyPI
    ...
    #
    # Core Aura Forge tools (terminal, read_file, write_file, patch,
    ...
    # #8731).
    #
    # Provider keys accept the Aura Forge provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # behavior: files must live under the Aura Forge cache, under
    ...
    "computer_use": {
        ...
        # by default upstream. Aura Forge disables it for our users unless they
        # explicitly opt in here. When false (default), Aura Forge sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Aura Forge also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Aura Forge computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Aura Forge passes the
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Aura Forge mints short-lived tokens from. Falls back to "
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Aura Forge auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Aura Forge' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Aura Forge, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Aura Forge API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Aura Forge API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Aura Forge API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
