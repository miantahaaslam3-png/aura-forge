"""Migrate Aura Forge' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Aura Forge' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Aura Forge mcp_servers.<n>.command/args/env  → codex stdio transport
  Aura Forge mcp_servers.<n>.url/headers       → codex streamable_http transport
  Aura Forge mcp_servers.<n>.timeout           → codex tool_timeout_sec
...
What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Aura Forge' own
...
MIGRATION_MARKER = (
    "# managed by aura-forge-agent — `auraforge codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end aura-forge-agent managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Aura Forge config.")
...
# Aura Forge keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Aura Forge' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def _translate_one_server(
    ...
    """Translate one Aura Forge MCP server config to the codex inline-table dict
    ...
    if has_command:
        ...
        # Aura Forge' transport: sse hint is informational; codex auto-negotiates
    ...
    for key in hermes_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            ...
            skipped.append(f"{key} (unknown Aura Forge key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (AURA_FORGE_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Aura Forge)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Aura Forge' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Aura Forge' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Aura Forge
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _build_hermes_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Aura Forge' own
    ...
    # — that would override the launcher's AURA_FORGE_HOME and pin the subprocess
...
def migrate(
    ...
    *,
    ...
    """Translate Aura Forge mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        hermes_config: full ~/.aura-forge/config.yaml dict
        ...
        expose_hermes_tools: when True (default), register Aura Forge' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Aura Forge for tools
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append(
            "mcp_servers in Aura Forge config is not a dict; cannot migrate."
        )
        return report
    ...
    # Inject Aura Forge' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Aura Forge for the tools codex
