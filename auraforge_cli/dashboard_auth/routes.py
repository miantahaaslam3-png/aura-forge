...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      2. ``X-Forwarded-Prefix: /auraforge`` (Mission Control deploys) — we
...
def _prefix(request: Request) -> str:
    ...
    ``auraforge_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
    from auraforge_cli.dashboard_auth.prefix import prefix_from_request
...
async def auth_native_authorize(
    ...
    from auraforge_cli.dashboard_auth import native_flow
...
async def api_auth_ws_ticket(request: Request):
    ...
    from auraforge_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
...
async def auth_native_refresh(request: Request, body: _NativeRefreshBody):
    ...
    """
    from auraforge_cli.dashboard_auth import list_session_providers
    from auraforge_cli.dashboard_auth.base import RefreshExpiredError
