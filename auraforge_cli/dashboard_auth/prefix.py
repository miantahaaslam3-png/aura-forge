...
prefix (e.g. ``mission-control.tilos.com/auraforge/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /auraforge`` so the backend can
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    missing scheme (``auraforge.example.com`` instead of
    ``https://auraforge.example.com``) — the single most common cause of
    ...
    _log.warning(
        ...
        source,
        cleaned,
        cleaned.split("://")[-1] or "auraforge.example.com",
    )
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/auraforge"`` (no trailing slash) or ``""``
