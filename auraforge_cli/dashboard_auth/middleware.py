...
from auraforge_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /auraforge``, the
    ``login_url`` is prefixed (``/auraforge/login?next=...``) so the
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from auraforge_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["auraforge.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    if session is None:
        ...
        from auraforge_cli.dashboard_auth.cookies import clear_session_cookies
        from auraforge_cli.dashboard_auth.prefix import prefix_from_request
