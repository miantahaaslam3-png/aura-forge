...
def resolve_exec_command() -> str:
    ...
    """
    from auraforge_cli.relaunch import resolve_hermes_bin
    ...
    if bin_path:
        ...
        if _needs_interpreter(resolved):
            ...
            # sys.executable is the interpreter actually running Aura Forge (the
            # venv one), so prefix it explicitly.
            argv = [str(Path(sys.executable).resolve()), str(resolved), "desktop"]
        else:
            argv = [str(resolved), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "auraforge_cli.main", "desktop"]
    return " ".join(_quote_exec_arg(a) for a in argv)


def _needs_interpreter(bin_path: Path) -> bool:
    """Whether ``bin_path`` is a Python script that must run under
    ``sys.executable`` to see Aura Forge' venv (rather than its own shebang)."""
    try:
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Aura Forge\n"
        "GenericName=Aura Forge Desktop\n"
        "Comment=Launch Aura Forge Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=Aura Forge\n"
    )
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Aura Forge desktop entry. Return its path.
