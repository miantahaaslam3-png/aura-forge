...
from auraforge_constants import agent_browser_runnable
...
from auraforge_cli.models import _HERMES_USER_AGENT
from auraforge_cli.vercel_auth import describe_vercel_auth
from auraforge_constants import OPENROUTER_MODELS_URL
...
from auraforge_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Aura Forge containers"
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    locks a live connection is holding (see ``auraforge_cli.sqlite_safe_read``).
...
def _report_database_journal_modes(
    ...
    from auraforge_state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable
    ...
    try:
        ...
        check_warn(f"Could not list Aura Forge databases: {exc}")
        return
    exposed = []
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from auraforge_cli.auth import get_minimax_oauth_auth_status
...
from auraforge_cli.sizefmt import format_bytes as _human_bytes
...
def collect_relay_plugin_cutover_findings(
    ...
    """Return actionable findings for the removed Aura Forge Relay plugin."""
    from auraforge_cli.relay_plugin_cutover import (
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches auraforge_cli.__version__.
    ...
    """
    try:
        from auraforge_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != auraforge_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'auraforge update', or set "
            "auraforge_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from auraforge_cli.providers import normalize_provider as _normalize_provider
...
def check_macos_tcc_grants() -> None:
    ...
    Post-#73681 builds pin ``designated => identifier "com.auraforge.desktop"``
    ...
    check_info(
        ...
        "grant is stale — run `tccutil reset ScreenCapture com.auraforge.desktop` "
        "(repeat per affected service), toggle it ON in System Settings, then "
        "fully quit & relaunch Aura Forge once."
    )
...
def _desktop_app_bundle() -> Path | None:
    ...
    (``apps/desktop/release/mac-<arch>/AuraForge.app``) — the only layout whose
    ...
    candidates = [p for p in release_dir.glob("mac*/AuraForge.app") if p.is_dir()]
    if not candidates:
        return None
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    """
    try:
        from auraforge_cli import macos_tcc_anchor as tcc
...
def check_macos_full_disk_access() -> None:
    ...
    check_info(
        ...
        "app Full Disk Access and Aura Forge will never trip per-folder dialogs "
        ...
        "    then enable your terminal (and AuraForge.app if you use Desktop), "
        "and restart them once. With Aura Forge' stable signing identities the "
...
def run_doctor(args):
    ...
    if ack_target:
        from auraforge_cli.security_advisories import (
            ADVISORIES,
            ack_advisory,
        )
    ...
    print(color("│                 🩺 Aura Forge Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        import sqlite3
        from auraforge_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Aura Forge already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and auraforge_cli/__init__.py versions
    ...
    if env_path.exists():
        ...
        # matches auraforge_cli.env_loader._load_dotenv_with_fallback.
        try:
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from auraforge_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from auraforge_cli.config import is_provider_enabled
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from auraforge_cli.auth import PROVIDER_REGISTRY, get_auth_status
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from auraforge_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        ...
        try:
            from auraforge_cli.config import load_env, read_user_config_raw, remove_env_value
        ...
        try:
            from auraforge_cli.config import load_env as _load_env_depr
            from auraforge_cli.config import read_user_config_raw as _read_raw_depr
        ...
        try:
            from auraforge_cli.config import validate_config_structure
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Aura Forge' own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Aura Forge a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Aura Forge Agent Persona\n\n"
                "<!-- Edit this file to customize how Aura Forge communicates. -->\n\n"
                "You are Aura Forge, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    if state_db_path.exists():
        try:
            ...
            from auraforge_state import _db_opens_cleanly, repair_state_db_schema
            ...
            from auraforge_state import is_malformed_db_error, repair_state_db_schema
        ...
        # with a runaway WAL was previously invisible to every Aura Forge
        ...
        try:
            from auraforge_state import collect_state_db_stats, count_db_holders
    ...
    try:
        from auraforge_constants import is_container as _is_container
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Aura Forge one.
                        check_info(
    ...
    try:
        from auraforge_cli.config import read_user_config_raw as _read_raw_mem
    ...
    try:
        from auraforge_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
    ...
    try:
        from auraforge_cli.doctor_live import maybe_run_live_checks
