...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "Aura Forge"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Aura Forge"
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Aura Forge" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "Aura Forge")
    else:
        ...
        # ``uninstall_gui``.
        from auraforge_cli.linux_desktop_entry import desktop_entry_path
...
def agent_is_installed(aura_forge_home: Path) -> bool:
    ...
    if (agent_root / "auraforge_cli").is_dir():
        return True
...
def uninstall_gui(aura_forge_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``aura-forge-agent/auraforge_cli`` (agent source), ``venv/``, or any
    ...
    if sys.platform.startswith("linux"):
        ...
        try:
            from auraforge_cli.linux_desktop_entry import (
                desktop_entry_path,
