...
def _root() -> Path:
    ...
    # helper's token, including custom AURA_FORGE_HOME layouts.
...
def process_state(pid: int, creation_time_ns: int, hermes_path: str, spawn_nonce: str) -> dict[str, Any]:
    ...
    # argv[0] is either the auraforge exe directly, or (normal case) the base Python
    ...
    # `-c <bootstrap that runs auraforge_cli.main>`; also accept a plain `-m` launch.
    ...
    launches_module = (
        argv[1:3] == ["-m", "auraforge_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "auraforge_cli.main" in argv[2])
    )
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves auraforge_cli's own location and prepends its parent, because the
    ...
    # the auraforge_cli package parent must be explicit -- the launcher resolves it via
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the auraforge_cli package")
    return base, py_path
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not os.path.isabs(configured_path):
        raise ValueError("Aura Forge path must be absolute")
    ...
    if not os.path.isfile(python_entry):
        raise ValueError("Aura Forge Python runtime was not found")
    ...
    bootstrap = (
        ...
        "runpy.run_module('auraforge_cli.main',run_name='__main__',alter_sys=True)"
    )
...
def inspect_hermes(hermes_path: str) -> dict[str, Any]:
    ...
    if not os.path.isabs(hermes_path) or not os.path.isfile(path):
        raise ValueError("Aura Forge path is not an executable file")
