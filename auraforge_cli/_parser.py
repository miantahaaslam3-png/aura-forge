...
# tests/auraforge_cli/test_top_level_value_flags_parity.py fails CI if the parser
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``auraforge_cli.relaunch`` should carry over when
...
def build_top_level_parser():
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Aura Forge Agent",
    )
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/auraforge_cli/
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Aura Forge itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
