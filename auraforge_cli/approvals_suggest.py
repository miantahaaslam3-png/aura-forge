"""``auraforge approvals suggest`` — mine approval history into allowlist proposals.

Aura Forge has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.aura-forge/state.db``): every assistant ``terminal`` tool
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  auraforge approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.aura-forge/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``auraforge approvals suggest``."""
...
def approvals_command(args) -> int:
    """Dispatch ``auraforge approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from auraforge_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: auraforge approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `auraforge approvals <subcommand> -h` for details."
    )
    return 1
