"""``auraforge plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.aura-forge/plugins/``.
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other Aura Forge subprocess resolution: :func:`shutil.which` first,
...
def _read_manifest(plugin_dir: Path) -> dict:
    ...
    if not manifest_file.exists():
        ...
        if not portable_file.exists() and not portable_file.is_symlink():
            return {}
        try:
            from auraforge_cli.agent_plugins import read_agent_plugin_manifest
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.aura-forge/.env``."""
...
def _print_python_dependencies(manifest: dict, console) -> None:
    ...
    Declaration seam ONLY — Aura Forge never auto-installs plugin pip
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from auraforge_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    with tempfile.TemporaryDirectory(prefix=".install-", dir=plugins_dir) as tmp:
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update Aura Forge.",
                ) from None
        ...
        if target.exists() and not force:
            raise PluginOperationError(
                ...
                f"or run `auraforge plugins update {plugin_name}`."
            )
...
def _resolve_index_name(identifier: str, console) -> tuple[str, Optional[str]]:
    ...
    """
    from auraforge_cli.plugin_index import SECURITY_FOOTER, load_index, resolve_name
    ...
    if entry is None:
        if len(candidates) > 1:
            ...
            console.print(
                ...
                f"community index ({source}). Use `auraforge plugins search <term>` to "
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (target / "plugin.json").exists() and not (
        ...
        console.print(
            ...
            f"plugin.json, or __init__.py. It may not be a valid Aura Forge plugin.",
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `auraforge plugins enable {installed_name}` to activate.[/dim]",
        )
...
def cmd_update(name: str) -> None:
    ...
    if install_record.get("pinned") is True:
        ...
        console.print(
            ...
            f"`auraforge plugins install {recorded_source} --force "
        ...
        if allowed is not True:
            ...
            if scan_result.verdict == "dangerous":
                ...
                console.print(
                    ...
                    f"findings, then re-enable with `auraforge plugins enable {name}` "
                    f"if you trust them.",
                )
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``auraforge setup`` / ``auraforge plugins disable basic`` can park the plugin
...
def _resolve_plugin_key(name: str) -> Optional[str]:
    ...
    This is the single normalization point so ``auraforge plugins enable`` /
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    if name in LEGACY_RELAY_PLUGIN_KEYS:
        console.print(
            ...
            f"by Aura Forge core; configure {RELAY_PLUGINS_CONFIG_ENV} instead."
        )
        sys.exit(1)
    ...
    # Aura Forge core and are trusted; every other source needs operator opt-in.
    if source == "bundled":
        return
...
def _declared_capabilities_from_manifest(manifest: dict, plugin_name: str = "?") -> list:
    ...
    from auraforge_cli.plugin_capabilities import parse_declared_capabilities
    ...
    for entry in _discover_all_plugins():
        ...
        if entry[5] == key or entry[0] == key:
            if entry[3] == "entrypoint":
                from auraforge_cli.plugins import discover_entrypoint_manifests
    ...
    *,
    ...
    if not (sys.stdin.isatty() and sys.stdout.isatty()):
        console.print(
            ...
            f"`auraforge plugins capabilities {plugin_id}` to review and "
            f"`auraforge plugins enable {plugin_id}` to grant interactively."
        )
        return False
    ...
    console.print(
        ...
        f"`auraforge plugins enable {plugin_id}` to grant later.[/dim]"
    )
    return False


def cmd_capabilities(name: Optional[str] = None) -> None:
    """``auraforge plugins capabilities [<id>]`` — declared vs granted."""
    from rich.console import Console

    from auraforge_cli.plugin_capabilities import granted_capabilities
...
def _resolve_tool_override_grant(
    ...
    if allow_tool_override:
        ...
        console.print(
            ...
            f"`auraforge plugins enable {key} --allow-tool-override` to grant "
            "this later.[/dim]"
        )
...
def _discover_entrypoint_plugins() -> list[tuple[str, str, str, str]]:
    ...
    plugin directory under ``~/.aura-forge/plugins``. Include package metadata here
    so ``auraforge plugins list`` can show and enable them.
    """
    from auraforge_cli.plugins import ENTRY_POINTS_GROUP
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``auraforge plugins list`` CLI filters."""
    filtered = entries
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Aura Forge
    ...
    try:
        from auraforge_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def cmd_toggle() -> None:
    ...
    # plugins.disabled, but ``auraforge plugins enable web/firecrawl`` cleared
...
def _run_composite_ui(curses, plugin_keys, plugin_labels, plugin_selected,
    ...
    from auraforge_cli.curses_ui import flush_stdin
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.aura-forge/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir, allow_subdir=True)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.aura-forge/plugins/<name>``."""
    ...
    if install_record.get("pinned") is True:
        ...
        return {
            ...
            "error": (
                ...
                f"run `auraforge plugins install {recorded_source} --force "
    ...
    # Sibling of the CLI ``auraforge plugins update`` path: drop bytecode
...
def _clear_plugin_bytecode(target: Path) -> int:
    ...
    fingerprint sweep in ``auraforge_cli.main`` never covers them. After a
...
def _git_pull_plugin_dir(target: Path) -> tuple[bool, str]:
    ...
    has local changes"), and the same autostash approach ``auraforge update``
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.aura-forge/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def plugins_command(args) -> None:
    ...
    if action == "install":
        ...
        from auraforge_cli.plugin_packs import pack_command
