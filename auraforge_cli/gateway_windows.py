...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Aura Forge profile."""
    from auraforge_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "auraforge_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    """
    _assert_windows()
    # Local import to avoid circular module initialization during auraforge_cli boot.
    from auraforge_cli.gateway import _profile_suffix
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``python -m auraforge_cli.main [--profile X] gateway run``
    ...
    # if someone imports auraforge_constants-based logic during startup.
    ...
    prog_args = [python_exe_path, "-m", "auraforge_cli.main"]
    if profile_arg:
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Aura Forge builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "auraforge_cli.main"]
    if profile_arg:
...
def windowless_gateway_restart_spec(
    ...
    preserves every argument after the interpreter (``-m auraforge_cli.main
    ...
    from auraforge_cli.config import get_aura_forge_home
    from auraforge_cli.gateway import PROJECT_ROOT
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m auraforge_cli.main gateway run`` directly — NOT
...
def _prompt_install_choices(
    ...
    from auraforge_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from auraforge_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Aura Forge gateway install prompt.")
                if start_now:
