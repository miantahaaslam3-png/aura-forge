"""auraforge hooks — inspect and manage shell-script hooks.

Usage::

    auraforge hooks list
    auraforge hooks test <event> [--for-tool X] [--payload-file F]
    auraforge hooks revoke <command>
    auraforge hooks doctor

Consent records live under ``~/.aura-forge/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.aura-forge/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``auraforge hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: auraforge hooks {list|test|revoke|doctor}")
        print("Run 'auraforge hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.aura-forge/config.yaml.")
        print("See `auraforge hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.aura-forge/config.yaml.")
    else:
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `auraforge hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `auraforge hooks test` and `auraforge hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Aura Forge wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `auraforge hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `auraforge hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `auraforge hooks doctor`.")
