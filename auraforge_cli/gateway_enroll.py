"""``auraforge gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nous Portal access token from the existing login
     (``~/.aura-forge/auth.json``) — the same path ``auraforge dashboard register``
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.aura-forge/.env``. The per-gateway secret authenticates the WS upgrade;
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "token could not be verified — try `auraforge auth add nous` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    from auraforge_cli.config import is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `auraforge gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    save_env_value writes them to ~/.aura-forge/.env (0600 dir) and never logs.
    to_write = {
...
def _warn_if_secondary_multiplex_profile() -> bool:
    ...
    in auraforge_cli/gateway.py. Best-effort: any failure to determine the
