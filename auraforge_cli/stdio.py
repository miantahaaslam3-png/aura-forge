...
on Japanese locales, etc.).  Aura Forge's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``auraforge_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Aura Forge is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Aura Forge-managed Git install directories so
    ...
    # already-running shells don't see the change — which means auraforge
    ...
    # install without Aura Forge-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Aura Forge's interactive
...
def _default_windows_editor() -> str:
    ...
    Set this before launching Aura Forge (User env var in Windows Settings, or
    export in a PowerShell profile) and Aura Forge picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Aura Forge-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\auraforge\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Patch-up strategy: add the known Aura Forge-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "auraforge", "git", "cmd"),
        os.path.join(local_appdata, "auraforge", "git", "bin"),
        os.path.join(local_appdata, "auraforge", "git", "usr", "bin"),
        # Aura Forge venv Scripts directory — host of the auraforge.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes auraforge, but harmless to include.
        os.path.join(local_appdata, "auraforge", "aura-forge-agent", "venv", "Scripts"),
