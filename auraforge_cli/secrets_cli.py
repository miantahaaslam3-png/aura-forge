...
# this module is registered parse-time from ``auraforge_cli.main`` and must not
...
def __getattr__(name: str):
    ...
    ``auraforge_cli.secrets_cli.bw`` directly.  Resolving that attribute at
    ...
    the first actual attribute access, so ``import auraforge_cli.secrets_cli``
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``auraforge_cli.main`` as part of building the top-level
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — Aura Forge will skip BSM "
