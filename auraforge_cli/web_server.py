"""
Aura Forge Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m auraforge_cli.main web          # Start on http://127.0.0.1:9119
    python -m auraforge_cli.main web --port 8080
"""
...
from auraforge_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
try:
    ...
    # running `auraforge dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``auraforge gateway run`` — but the
    desktop app spawns a ``auraforge dashboard`` backend, not a gateway, so a cron
...
def _warm_gateway_module() -> None:
    ...
    ``auraforge_cli.gateway``. But the first WS connection and its initial
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from auraforge_cli.gateway import _get_restart_drain_timeout
...
def _eager_reconcile_own_session_db() -> None:
    ...
    bringing a store left behind by `auraforge update` current before the
    ...
    """
    try:
        from auraforge_state import SessionDB, _default_db_path
...
async def _lifespan(app: "FastAPI"):
    ...
    # so a store left behind by `auraforge update` kept 500ing every
    ...
    # Import auraforge_cli.gateway eagerly *before* the lifespan yield so the
    ...
    # model picker) can detect when `auraforge update` replaced the code
    ...
    if os.getenv("HERMES_DESKTOP") == "1":
        ...
        try:
            from auraforge_cli.gateway import _reap_unsupervised_gateway_orphans
...
app = FastAPI(title="Aura Forge Agent", version=__version__, lifespan=_lifespan)


# Memory-provider OAuth connect routes live in the memory layer, not here.
from auraforge_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
_SESSION_HEADER_NAME = "X-AuraForge-Session-Token"
...
#
# This list is defined in ``auraforge_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from auraforge_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-AuraForge-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    """
    from auraforge_cli.dashboard_auth.prefix import resolve_public_url
...
async def _dashboard_auth_gate(request: Request, call_next):
    from auraforge_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from auraforge_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `auraforge egress setup` "
            "and `auraforge egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
...
from auraforge_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    """
    from auraforge_cli.config import get_compatible_custom_providers
    from auraforge_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from auraforge_cli.model_normalize import normalize_model_for_provider
    from auraforge_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from auraforge_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    """
    from auraforge_state import _default_db_path
...
# AURA_FORGE_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to AURA_FORGE_HOME.
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no AURA_FORGE_HOME resolution, so it blocks these trees wherever
...
def _default_hermes_root_is_opt_data() -> bool:
    raw = os.environ.get("AURA_FORGE_HOME", "").strip()
    if not raw:
        return False
    try:
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``auraforge update``.
    ...
    if _default_hermes_root_is_opt_data():
        return True
    try:
        from auraforge_constants import is_container
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    ``AURA_FORGE_HOME/images/`` — the same directory ``clipboard.paste`` /
...
# gate + path hardening as /api/fs). Logic lives in ``auraforge_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from auraforge_cli import web_git as _web_git  # noqa: E402
...
from auraforge_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from auraforge_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``profile_platforms`` — ``{profile: platforms}`` runtime platform maps
    ...
    """
    try:
        from auraforge_cli.profiles import _check_gateway_running, profiles_to_serve
...
_PROFILE_PLATFORM_STATUS_KEY_RE = re.compile(
    # Profile segment mirrors auraforge_cli.profiles._PROFILE_ID_RE.  Platform
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # AURA_FORGE_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        # Windows install the first import of auraforge_cli.gateway blocks the
        ...
        # and which providers are registered so ``auraforge status`` and the
        ...
        auth_flows: list[str] = []
        try:
            from auraforge_cli.dashboard_auth import (
        ...
        try:
            from auraforge_cli.auth import get_nous_session_validity
        ...
        # disk-exhaustion incidents). One statvfs call on AURA_FORGE_HOME's
        ...
        try:
            from auraforge_state import SessionDB as _SDB
            from auraforge_constants import get_hermes_home as _ghh
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from auraforge_cli.auth import get_nous_auth_status_local
    ...
    features = []
    try:
        from auraforge_cli.nous_subscription import get_nous_subscription_features
...
# streamed to a per-action log file under ``~/.aura-forge/logs/<action>.log`` so
...
def _dashboard_spawn_executable() -> str:
    ...
    runpy.run_module('auraforge_cli.main', ...)"``) — so ``sys.executable`` is
...
def _spawn_hermes_action(
    ...
    *,
    ...
    """Spawn ``auraforge <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``auraforge_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "auraforge_cli.main", *subcommand]

    # The dashboard runs *inside* the gateway process, so os.environ carries
    # _HERMES_GATEWAY=1. Inheriting it makes a spawned `auraforge gateway restart`
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``auraforge gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``auraforge gateway restart`` children race each other on the
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``auraforge gateway restart`` in the background."""
    try:
...
async def speak_text(payload: TTSSpeakRequest, profile: Optional[str] = None):
    ...
    configured in ``~/.aura-forge/config.yaml`` under ``tts.``.
    """
...
async def get_update_receipt():
    ...
    if not receipt:
        raise HTTPException(
            status_code=404,
            detail="No update receipt found (no `auraforge update` run recorded).",
        )
...
from auraforge_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from auraforge_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from auraforge_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from auraforge_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
async def get_egress_status():
    ...
    from auraforge_cli.proxy_cli import format_status_text
...
# in auraforge_cli/config.py — listed here for deterministic ordering in the UI.
...
def _dashboard_code_skew_guard() -> Optional[str]:
    ...
    boot.  When ``auraforge update`` (or a manual ``git pull``) replaces the
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `auraforge model` does so GUI onboarding lands on a
    ...
    try:
        from auraforge_cli.inventory import build_models_payload, load_picker_context
        from auraforge_cli.models import pick_silent_default_model
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from auraforge_cli.moa_config import normalize_moa_config
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from auraforge_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.aura-forge/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (auraforge_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from auraforge_cli.nous_subscription import apply_nous_managed_defaults
        ...
        # endpoint, mirroring the ``auraforge model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from auraforge_cli.main import _auto_provider_name, _save_custom_provider
...
async def update_config(body: ConfigUpdate, profile: Optional[str] = None):
    def _run():
        ...
        # different AURA_FORGE_HOME than this process's gateway sessions.
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``auraforge model`` universe). This is what lets the desktop Keys tab
    ...
    try:
        from auraforge_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `auraforge model` provider — would be
        ...
        # Vertex — a `auraforge model` provider — would be invisible in the desktop
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `auraforge model` picker uses (not desktop-only prefix guesses).
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from auraforge_cli.credential_lifecycle import save_provider_env_credential
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    the process-level AURA_FORGE_HOME. Mirrors ``/api/config``'s profile scoping.
    """
    try:
        with _config_profile_scope(profile):
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from auraforge_cli.credential_lifecycle import remove_provider_env_credential
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "weixin": {
        ...
        "docs_url": "https://aura-forge-agent.nousresearch.com/docs/user-guide/messaging/weixin/",
    ...
    "webhook": {
        ...
        "docs_url": "https://aura-forge-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://aura-forge-agent.nousresearch.com/docs/user-guide/messaging/msgraph-webhook",
        "required_env": (),
    },
    "whatsapp_cloud": {
...
# auraforge_cli/setup_hidden_env.py. Shared with the `auraforge setup gateway`
# wizard so the surfaces ask for the same things.
from auraforge_cli.setup_hidden_env import (  # noqa: E402
...
def _messaging_platform_payload(
    ...
    # AURA_FORGE_HOME contextvar override (#56986 / #69143), so the profile's
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.aura-forge-agent.nousresearch.com"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Aura Forge Agent").strip() or "Aura Forge Agent"
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the AURA_FORGE_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized AURA_FORGE_HOME)
...
# the canonical ``auraforge auth add <provider>`` command so the dashboard
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "auraforge auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "auraforge auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "auraforge auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "auraforge auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "auraforge auth add xai-oauth",
        "docs_url": "https://aura-forge-agent.nousresearch.com/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "auraforge auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from auraforge_cli import auth as hauth
        if provider_id == "nous":
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``auraforge model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `auraforge model` universe (zero-edit for new plugins).
    try:
        from auraforge_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"auraforge auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    sync with the `auraforge model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    def _run():
        with _profile_scope(profile):
            ...
            try:
                from auraforge_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.aura-forge/.anthropic_oauth.json AND credential pool
...
async def _start_device_code_flow(
    ...
    if provider_id == "xai-oauth":
        from auraforge_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``auraforge auth add minimax-oauth``.
    """
    from auraforge_cli.auth import (
        _minimax_poll_token,
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `auraforge auth add xai-oauth`: first credential may become
            ...
            # prior ``auraforge auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``auraforge model`` re-login path in _login_xai_oauth).
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from auraforge_cli.auth import _save_codex_tokens
...
def _session_db_read_probe_statements() -> tuple:
    ...
    after `auraforge update` until the first message forced a writable open.
    """
    from auraforge_state_schema import schema_read_probe_statements
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from auraforge_state import SessionDB, is_malformed_schema_error
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``auraforge serve`` — it runs neither the
    ...
    try:
        ...
        from auraforge_cli.config import load_config as _load_full_config
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `auraforge sessions prune`)."""
    has_window = (
...
async def get_logs(
    ...
    from auraforge_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from auraforge_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, AURA_FORGE_HOME)."""
    from auraforge_cli import profiles as profiles_mod
...
from auraforge_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from auraforge_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    try:
        ...
        # yaml.safe_load of config.yaml (tests/auraforge_cli/
        # test_config_read_guard.py). The AURA_FORGE_HOME override scopes
...
def _gateway_intentionally_stopped(profile: Optional[str]) -> bool:
    ...
    commands (``auraforge gateway stop`` persists ``"stopped"``; start and
...
#
# Wraps the same config data layer the CLI uses (auraforge_cli.mcp_config), so
# servers managed here show up under `auraforge mcp list` and vice versa.  Secrets
...
from auraforge_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from auraforge_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from auraforge_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
#
# Wraps the same JSON store the CLI uses (auraforge_cli.webhook); the webhook
...
async def list_webhooks():
    import auraforge_cli.webhook as wh
...
# spawn the real `auraforge gateway <verb>` so behaviour matches the CLI exactly.
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `auraforge auth remove`) can seed again.
        # Mirrors the `auraforge auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``auraforge auth remove`` uses: each source cleans up its
...
async def list_hooks():
    ...
    """
    def _run():
        from auraforge_cli.config import load_config as _load_config
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``auraforge`` subprocess, and
...
from auraforge_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from auraforge_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
# Human-readable labels for each hub source id (matches `auraforge skills search`
...
def _profile_setup_command(name: str) -> str:
    ...
    return "auraforge setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local AURA_FORGE_HOME override so the write lands in the target
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local AURA_FORGE_HOME override (same mechanism as
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the AURA_FORGE_HOME override. Returns the
    number of skills newly disabled.
    """
    from auraforge_constants import set_hermes_home_override, reset_hermes_home_override
    from auraforge_cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_scope(profile: Optional[str]):
    ...
    imported the modules before a AURA_FORGE_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
from auraforge_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from auraforge_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
#
# The endpoint spawns the same ``auraforge --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from auraforge_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
...
from auraforge_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from auraforge_cli.dashboard_auth.audit import AuditEvent, audit_log
        from auraforge_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``auraforge --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``HERMES_TUI_RESUME`` env var —
    matching what ``auraforge_cli.main._launch_tui`` does for the CLI path.
    ...
    ``AURA_FORGE_HOME`` at the profile dir in the child env. Every spawned
    ...
    — the same propagation ``auraforge -p <name>`` performs. The in-process
    ...
    """
    from auraforge_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    if profile_dir is not None:
        env["AURA_FORGE_HOME"] = str(profile_dir)
    try:
        from auraforge_cli.config import (
    ...
    # inherits the profile AURA_FORGE_HOME set above.
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from auraforge_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from auraforge_cli.console_engine import HermesConsoleEngine
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`auraforge_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from auraforge_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    """
    # `auraforge serve` is the headless backend: it must NEVER serve the browser
    ...
    if _headless:
        ...
        async def no_frontend(full_path: str):
            ...
            # `auraforge update` replaced the backend) had no way to adopt the
    ...
    # (#82614): a long-lived `auraforge dashboard --skip-build` process that
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.aura-forge/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.aura-forge/dashboard-themes/*.yaml for user-created themes.
    ...
    live under the server's own ``AURA_FORGE_HOME``.
    """
...
async def get_dashboard_themes():
    ...
    from `~/.aura-forge/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as auraforge_cli.plugins):
    ...
    from auraforge_cli.plugins import get_bundled_plugins_dir
    ...
    # AURA_FORGE_HOME override (e.g. embedded /chat under --open-profile).
    #
    ...
    # ``auraforge_cli.plugins`` resolves plugin install locations. The
    ...
    # ``auraforge_cli/plugins.py`` and the documented user contract.
...
async def get_dashboard_plugins():
    ...
    def _run():
        ...
        try:
            from auraforge_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
            enabled_set = _get_enabled_set()
            disabled_set = _get_disabled_set()
        except Exception:
            enabled_set = set()
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if cached_result is False:
                        auth_required = True
                        auth_command = f"auraforge auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from auraforge_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from auraforge_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from auraforge_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from auraforge_cli.plugins_cmd import dashboard_remove_user_plugin
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.aura-forge/plugins/ if you trust it)",
...
from auraforge_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
# spawn (and any script wrapping ``auraforge serve``) cannot tell "port occupied"
...
def start_server(
    ...
    try:
        from auraforge_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with Aura Forge Agent exposes a
            ...
            if host in _LOOPBACK_HOST_VALUES:
                _public_url_for_msg = ""
                try:
                    from auraforge_cli.dashboard_auth.prefix import (
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `auraforge dashboard register` (Nous Portal) or "
            ...
            try:
                ...
                from auraforge_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`auraforge plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            try:
                from auraforge_cli.process_identity import reap_orphaned_mcp_helpers
            ...
            # for standalone `auraforge serve` (no HERMES_PARENT_PID env).
            ...
            # is what lets `auraforge update` relaunch a manually-started serve
