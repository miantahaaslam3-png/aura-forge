...
``auraforge_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Aura Forge web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Aura Forge web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Aura Forge Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # showed "Aura Forge couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Aura Forge Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Aura Forge backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Aura Forge backend server — the JSON-RPC/WebSocket gateway the "
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://auraforge.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
