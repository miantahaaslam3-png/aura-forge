"""``auraforge mcp`` subcommand parser.
...
from auraforge_cli.subcommands._shared import add_accept_hooks_flag
...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Aura Forge as an MCP server",
        description=(
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Aura Forge as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `auraforge mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `auraforge mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `auraforge mcp install n8n`)",
    )
