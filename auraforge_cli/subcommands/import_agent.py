...
Follows the ``auraforge claw`` pattern (see ``auraforge_cli/subcommands/claw.py``):
parser building lives here, the handler is injected to avoid importing
``main``, and the import logic itself lives in ``auraforge_cli/agent_import.py``.
"""
...
def build_import_agent_parser(subparsers, *, cmd_import_agent: Callable) -> None:
    ...
    parser = subparsers.add_parser(
        "import-agent",
        help="Import a Claude Code or Codex CLI setup into Aura Forge",
        description=(
            "One-command import of another coding agent's setup into Aura Forge. "
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite existing Aura Forge items on name conflicts (default: skip)",
    )
    parser.add_argument(
