"""``auraforge debug`` subcommand parser.

Extracted verbatim from ``auraforge_cli/main.py:main()`` (god-file Phase 2).
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Aura Forge Agent. Use 'auraforge debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    auraforge debug share              Upload debug report (asks for confirmation)
    auraforge debug share --yes        Skip confirmation (for scripts/CI)
    auraforge debug share --lines 500  Include more log lines
    auraforge debug share --expire 30  Keep paste for 30 days
    auraforge debug share --local      Print report locally (no upload)
    auraforge debug share --no-redact  Disable upload-time secret redaction
    auraforge debug share --nous       Upload to Nous-internal storage (private)
    auraforge debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'auraforge debug share'",
    )
