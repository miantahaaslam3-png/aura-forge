...
Cloned from ``auraforge_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    auraforge sync status                 show gate/opt-in/head state
    auraforge sync pull                   pull and materialize opted-in skills
    auraforge sync push                   push opted-in skills
    auraforge sync now                    reconcile: pull then push
    auraforge sync enable <skill>         opt a skill into sync
    auraforge sync disable <skill>        opt a skill out of sync
    auraforge sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    auraforge sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  auraforge sync status            what is synced, and from where\n"
            "  auraforge sync enable my-skill   include a skill in your sync\n"
            "  auraforge sync now               pull, then push\n"
            "  auraforge sync propose my-skill  share a skill with your team\n"
        ),
