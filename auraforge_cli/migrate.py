"""CLI handlers for ``auraforge migrate ...``.

Currently exposes only ``auraforge migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``auraforge migrate <subtype>``."""
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `auraforge migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `auraforge doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
