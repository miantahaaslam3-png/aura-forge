...
# IMPORTANT: auraforge_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``auraforge_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``auraforge update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``auraforge_bootstrap`` but the editable install's
...
from auraforge_cli._subprocess_compat import suppress_platform_ver_console
from auraforge_cli.cli_output import line_input
...
# Two lines of inline path math so ``python auraforge_cli/main.py`` (script
# mode — sys.path[0] is auraforge_cli/, not the repo root) can import the
# canonical helpers; everything else lives in auraforge_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from auraforge_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below.  When
# a prior ``auraforge update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from auraforge_cli.env_loader import ...`` / ``from auraforge_cli.config import
...
# package directory, so if IT can't import, nothing else in auraforge_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from auraforge_cli import _early_recovery as _early_recovery_mod
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from auraforge_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    ...
    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``auraforge tools`` or
...
# that run *before* auraforge_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("AURA_FORGE_HOME")
        if home:
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    cron jobs, pipes run ``auraforge … chat -q`` with stdio on a pipe. This
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`auraforge --tui … >log`, CI capture):
...
def _try_ultrafast_version() -> bool:
    """Handle ``auraforge --version`` before config/logging imports."""
...
from auraforge_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from auraforge_cli.subcommands.cron import build_cron_parser
from auraforge_cli.subcommands.sync import build_sync_parser
from auraforge_cli.subcommands.gateway import build_gateway_parser
from auraforge_cli.subcommands.profile import build_profile_parser
from auraforge_cli.subcommands.model import build_model_parser
from auraforge_cli.subcommands.setup import build_setup_parser

from auraforge_cli.subcommands.whatsapp import build_whatsapp_parser
from auraforge_cli.subcommands.slack import build_slack_parser
from auraforge_cli.subcommands.login import build_login_parser
from auraforge_cli.subcommands.logout import build_logout_parser
from auraforge_cli.subcommands.auth import build_auth_parser
from auraforge_cli.subcommands.status import build_status_parser
from auraforge_cli.subcommands.pause import build_pause_parser
from auraforge_cli.subcommands.webhook import build_webhook_parser
from auraforge_cli.subcommands.hooks import build_hooks_parser
from auraforge_cli.subcommands.doctor import build_doctor_parser
from auraforge_cli.subcommands.verify import build_verify_parser
from auraforge_cli.subcommands.security import build_security_parser
from auraforge_cli.subcommands.approvals import build_approvals_parser
from auraforge_cli.subcommands.dump import build_dump_parser
from auraforge_cli.subcommands.debug import build_debug_parser
from auraforge_cli.subcommands.backup import build_backup_parser
from auraforge_cli.subcommands.import_cmd import build_import_cmd_parser
from auraforge_cli.subcommands.import_agent import build_import_agent_parser
from auraforge_cli.subcommands.config import build_config_parser
from auraforge_cli.subcommands.skin import build_skin_parser
from auraforge_cli.subcommands.console import build_console_parser
from auraforge_cli.subcommands.update import build_update_parser
from auraforge_cli.subcommands.uninstall import build_uninstall_parser
from auraforge_cli.subcommands.dashboard import build_dashboard_parser
from auraforge_cli.subcommands.gui import build_gui_parser
from auraforge_cli.subcommands.logs import build_logs_parser
from auraforge_cli.subcommands.prompt_size import build_prompt_size_parser
from auraforge_cli.subcommands.memory import build_memory_parser
from auraforge_cli.subcommands.acp import build_acp_parser
from auraforge_cli.subcommands.tools import build_tools_parser
from auraforge_cli.subcommands.insights import build_insights_parser
from auraforge_cli.subcommands.monitoring import build_monitoring_parser
from auraforge_cli.subcommands.skills import build_skills_parser
from auraforge_cli.subcommands.pairing import build_pairing_parser
from auraforge_cli.subcommands.plugins import build_plugins_parser
from auraforge_cli.subcommands.mcp import build_mcp_parser
from auraforge_cli.subcommands.claw import build_claw_parser
...
# every subsequent ``os.getenv("AURA_FORGE_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.aura-forge/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set AURA_FORGE_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `auraforge mcp add ... --args <command argv>`.
    ...
    # after the subcommand (`auraforge chat -p coder`), so keep scanning broadly.
    # The exception is command-argv passthrough regions such as `mcp add --args`.
    from auraforge_cli._parser import top_level_value_flag_sets
    ...
    # Mirrors auraforge_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If AURA_FORGE_HOME is already set and no explicit flag was given, trust it
    ...
    # reserved ``gateway-default`` slot runs bare ``auraforge gateway run`` to mean
    # "the root AURA_FORGE_HOME profile". If the reserved default child read
    ...
    # 3. If we found a profile, resolve and set AURA_FORGE_HOME
...
# desktop updater's ``--keep-stash`` nothing restored them and ``auraforge``
...
# ``python -m auraforge_cli.main``. Costs a few stat calls when healthy; gates
...
if sys.platform == "win32":
    try:
        from auraforge_cli import _install_repair as _install_repair_mod
...
from auraforge_cli.config import get_aura_forge_home
...
# var BEFORE auraforge_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    # yaml.load — the SAME parse then serves auraforge_logging's
    # _read_logging_config and any later raw reads in this process, collapsing
    # 3-4 config.yaml parses per invocation into one.
    from auraforge_cli.config import read_raw_config as _read_raw_early
...
# Initialize centralized file logging early — all `auraforge` subcommands
...
try:
    from auraforge_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from auraforge_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if auraforge_constants not importable yet
...
# Provider model-selection wizard flows extracted to auraforge_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (auraforge_cli.main._model_flow_*) keep resolving unchanged.
from auraforge_cli.model_setup_flows import (
...
def _relative_time(ts) -> str:
    ...
    in :mod:`auraforge_cli.timefmt` so lightweight consumers don't have to
    import the whole CLI surface.
    """
    from auraforge_cli.timefmt import relative_time
...
def _has_any_provider_configured() -> bool:
    ...
    from auraforge_cli.config import get_env_path, get_aura_forge_home, load_config
    from auraforge_cli.auth import get_auth_status
    ...
    if isinstance(model_cfg, dict):
        ...
        if isinstance(_default, dict):
            from auraforge_cli.config import split_model_config_default
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``auraforge -c`` from repo A continues repo A's last session rather than the
...
def _resolve_continue_arg(args, *, use_tui: bool) -> None:
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            try:
                from auraforge_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _ensure_tui_node() -> None:
    ...
    from auraforge_constants import get_aura_forge_home
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``auraforge update`` (HEAD stays
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        from auraforge_constants import find_node_executable
    ...
    # auraforge_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            from auraforge_constants import with_hermes_node_path
        ...
        if result.returncode != 0:
            ...
            from auraforge_cli.npm_engine import maybe_repair_npm_engine
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from auraforge_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    try:
        from auraforge_cli.config import apply_terminal_config_to_env
    ...
    # in the user's shell would otherwise make a plain `auraforge --tui` try to
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `auraforge update` so
    ...
    if code == 42:
        from auraforge_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    (``auraforge kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``auraforge kanban boards switch`` from another session can flip the file
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    try:
        from auraforge_cli.kanban_db import get_current_board
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``auraforge … chat -q`` with stdout
...
def cmd_chat(args):
    ...
    try:
        ...
        from auraforge_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'auraforge doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'auraforge setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from auraforge_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    #
    # FIRST RUN is the exception: with an empty ~/.aura-forge/skills the banner
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import auraforge_cli.banner as _banner_mod
    ...
    # user's ~/.aura-forge/config.yaml and return built-in defaults. Set BEFORE
...
def cmd_gateway(args):
    ...
    from auraforge_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from auraforge_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from auraforge_constants import find_node_executable, with_hermes_node_path
    ...
    # has no creds.json.  Every subsequent `auraforge gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `auraforge gateway` skips it cleanly instead of paying a 30s
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``auraforge whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``auraforge_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from auraforge_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from auraforge_cli.models import clear_provider_models_cache
            clear_provider_models_cache()
            print("  Cleared model picker cache.")
        except Exception:
            pass
    from auraforge_cli.setup import run_setup_action_with_navigation
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``auraforge model``) and the setup wizard
    ...
    from auraforge_cli.providers import (
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'auraforge model' for "
                "available providers, or run 'auraforge doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in auraforge_cli/models.py). A multi-member group shows one
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``auraforge model``
    ...
    # a leftover OPENAI_BASE_URL in ~/.aura-forge/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.aura-forge/.env if the active provider is not 'custom'.
    ...
    """
    from auraforge_cli.config import get_env_value, save_env_value, load_config
...
# `auraforge model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `auraforge model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`auraforge_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from auraforge_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``auraforge model`` flow, then route aux tasks
    to them here.
    """
    from auraforge_cli.config import load_config
    from auraforge_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _aux_flow_provider_model(
    ...
    from auraforge_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from auraforge_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from auraforge_cli.runtime_provider import _detect_api_mode_for_url
...
def _remove_custom_provider(config):
    ...
    try:
        from auraforge_cli.curses_ui import curses_radiolist
...
# downstream call sites read `auraforge_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `auraforge` invocation — including
# fast paths like `auraforge --version` and slash-command dispatch that never
...
# surface so argparse wiring and test monkeypatches on auraforge_cli.main.<name>
# keep resolving unchanged. Importing those modules eagerly costs ~50ms on
# every `auraforge` invocation, including fast paths like `auraforge --version`
...
def _self():
    ...
    keeps monkeypatches on auraforge_cli.main.<name> working, exactly like a
...
def _prompt_api_key(
    ...
    """Shared API-key entry point for ``auraforge setup`` / ``auraforge model``.
    ...
    recover from a malformed paste without editing ``~/.aura-forge/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from auraforge_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from auraforge_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(
            f"  API key cleared.  Re-run `auraforge setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from auraforge_constants import display_aura_forge_home as _dhh_fn
    ...
    try:
        ...
        print("    4. Re-run:               auraforge model")
        print()
...
def cmd_login(args):
    """Authenticate Aura Forge CLI with a provider."""
    from auraforge_cli.auth import login_command
...
def cmd_auth(args):
    ...
    from auraforge_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from auraforge_cli.status import show_status
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.aura-forge/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `auraforge sync propose <skill>`. "
...
def cmd_webhook(args):
    ...
    from auraforge_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``auraforge slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            ...
            "\n"
            "Run `auraforge slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from auraforge_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from auraforge_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from auraforge_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from auraforge_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from auraforge_cli.doctor import run_doctor
...
def cmd_security(args):
    """Dispatch `auraforge security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from auraforge_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `auraforge approvals <subcmd>`."""
    from auraforge_cli.approvals_suggest import approvals_command
...
def cmd_dump(args):
    ...
    from auraforge_cli.dump import run_dump
...
def cmd_skin(args):
    ...
    from auraforge_cli.skin_cmd import skin_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from auraforge_cli.backup import run_quick_backup
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    # `auraforge --version` pre-import fast path (the `version` subcommand
...
def cmd_uninstall(args):
    """Uninstall Aura Forge Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from auraforge_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from auraforge_cli.uninstall import run_gui_uninstall
    ...
    from auraforge_cli.uninstall import run_uninstall
...
# Update pipeline lives in auraforge_cli/update_cmd.py (main.py decomposition,
...
# monkeypatches on auraforge_cli.main.<name> keep resolving unchanged without
...
# validated against. Lives next to the checkout (NOT in AURA_FORGE_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``auraforge update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``auraforge update``
    ...
    ``auraforge`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``auraforge update``
    ...
    ``auraforge_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
    ...
    dist_dir = project_root / "auraforge_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``auraforge`` launcher present but ``auraforge_cli`` not importable.
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``auraforge update`` to stash the
    lockfile — repeatedly.
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "auraforge_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``auraforge web``).
    ...
    # build path usable on a stock `py -m auraforge_cli.main web` invocation.
    ...
    # as `auraforge update`'s _update_node_dependencies() (ui-tui + web +
...
#   - ``auraforge update`` can unconditionally call ``auraforge desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``auraforge desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $AURA_FORGE_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _renderer_bundle_torn(dist_dir: Path) -> bool:
    ...
    the intact SOURCE tree, ``auraforge desktop`` skips the rebuild that would fix
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``auraforge update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_macos_setup_tcc_identity(identity: str = "Aura Forge Local Signing") -> bool:
    ...
    try:
        from auraforge_cli.config import set_config_value
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``auraforge desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Aura Forge Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Aura Forge Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _detect_linux_password_store() -> str | None:
    ...
    fails under `auraforge desktop` because the launcher environment doesn't look
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Aura Forge Desktop (Linux only, best-effort).
    ...
    shell. ``auraforge uninstall --gui`` removes it.
    """
    try:
        from auraforge_cli.linux_desktop_entry import install_desktop_entry, is_supported
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from auraforge_logging import setup_logging as _setup_logging_gui
    ...
    if source_mode:
        print("→ Launching Aura Forge Desktop from source build...")
    ...
    print(f"→ Launching packaged Aura Forge Desktop: {' '.join(launch_command)}")
    launch_result = subprocess.run(launch_command, cwd=desktop_dir, env=env, check=False)
    sys.exit(launch_result.returncode)


# Dashboard process-hygiene helpers live in auraforge_cli/dashboard_procs.py
...
# auraforge_cli.main.<name> keep resolving unchanged.
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    if any(
        ...
        for pattern in (
            "auraforge dashboard",
            "auraforge_cli.main dashboard",
            "auraforge_cli/main.py dashboard",
        )
    ):
        mode = "dashboard"
    elif any(
        ...
        for pattern in (
            "auraforge serve",
            "auraforge_cli.main serve",
            "auraforge_cli/main.py serve",
        )
    ):
        mode = "serve"
    if mode is None:
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``auraforge update``.
...
# =========================================================================
# Fork detection and upstream management for `auraforge update`
# =========================================================================
...
# Install-scoped breadcrumbs live next to the venv (not under $AURA_FORGE_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``auraforge update``.
    ...
    protocol stream (``auraforge acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_core_update_marker_locked() -> None:
    ...
    """
    print(
        "⚠ A previous `auraforge update` was interrupted mid-install — "
    ...
    try:
        from auraforge_cli import _install_repair as _ir
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    The child re-runs ``auraforge update``, so the whole remaining flow — the
    ...
    cmd = [str(python_exe), "-m", "auraforge_cli.main", *sys.argv[1:]]
...
def _default_venv_install_target() -> tuple[list[str], dict[str, str] | None]:
    ...
    try:
        ...
        uv_bin = None
    if uv_bin:
        from auraforge_constants import project_venv_dir
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``auraforge update`` is still progressing even if pip/uv itself is silent.
    """
...
def _venv_scripts_dir() -> Path | None:
    ...
    from auraforge_constants import project_venv_dir, venv_bin_dir
...
def _quarantine_running_hermes_exe(
    ...
    handles (recovers in <1s), or the Aura Forge Desktop backend child process
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Aura Forge Desktop, exit other `auraforge` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `auraforge update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    update; a failed *restore* leaves the install with no ``auraforge`` on PATH,
...
def _run_quarantined_install(
    ...
    *,
    ...
    try:
        ...
        # quarantined aside and `auraforge` missing from PATH after a green
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    ...
    2. **Concurrency.** A fresh quarantine file may belong to an update in
       ...
       ``auraforge update`` does exactly this). Leave anything inside the grace
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `auraforge update`:")
    print(
...
def _interpreter_scripts_dir() -> Path | None:
    ...
    """
    from auraforge_constants import venv_bin_dir
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    # hours later inside ``auraforge update``'s desktop-rebuild or skill-sync
    ...
    *,
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m auraforge_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``auraforge update``, not the venv we just wrote
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `auraforge update` output above.")
        return
...
class _UpdateOutputStream:
    """Stream wrapper used during ``auraforge update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.aura-forge/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``auraforge update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``auraforge update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.aura-forge/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``auraforge update --gateway``) the update is already
    ...
    try:
        ...
        # auraforge_cli.config.get_aura_forge_home to simulate setup failure.
        from auraforge_cli.config import get_aura_forge_home as _get_aura_forge_home
...
def cmd_update(args):
    """Update Aura Forge Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Aura Forge Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `auraforge update --branch=<x>` would actually pull.
    ...
    try:
        ...
        try:
            from auraforge_cli.update_receipt import finalize_pending_update_receipt
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``auraforge -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from auraforge_constants import display_aura_forge_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `auraforge profile` — show current profile status
        from auraforge_cli.profiles import format_profile_label
    ...
    if action == "list":
        ...
        from auraforge_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from auraforge_constants import get_aura_forge_home as _hh
        ...
        from auraforge_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `auraforge profile info {name}` for full manifest)")
        if alias_name:
        ...
        from auraforge_cli.profiles import profile_exists, validate_alias_name
        ...
        from auraforge_cli.profiles import normalize_profile_name, rename_profile
        ...
        from auraforge_cli.profiles import export_profile
        ...
        from auraforge_cli.profiles import import_profile
        ...
        import tempfile
        from auraforge_cli.profile_distribution import (
            plan_install,
        ...
        from auraforge_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from auraforge_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from auraforge_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    provider on the spot — or point them at ``auraforge dashboard register`` for
    OAuth.
    ...
    try:
        from auraforge_cli.web_server import should_require_dashboard_auth
    ...
    try:
        from auraforge_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `auraforge dashboard register`)")
    print("    [3] Cancel")
    print()
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from auraforge_cli.windows_ssh_runtime import read_token
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    ``.../app.asar.unpacked/dist``. A standalone ``auraforge dashboard`` that
...
def cmd_dashboard(args):
    ...
    # those vars then runs `auraforge dashboard` would otherwise:
    ...
    # launches the dashboard (`worker dashboard` → AURA_FORGE_HOME points into
    ...
    if (
        ...
        reexec_argv = [
            sys.executable, "-m", "auraforge_cli.main",
            "-p", "default",
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: AURA_FORGE_HOME is explicitly pinned to the
        ...
        try:
            from auraforge_constants import get_default_hermes_root
            env["AURA_FORGE_HOME"] = str(get_default_hermes_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop AURA_FORGE_HOME) rather than block the reroute.
            env.pop("AURA_FORGE_HOME", None)
    ...
    if _headless_backend:
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "auraforge_cli" / "web_dist"
        )
...
def cmd_dashboard_register(args):
    ...
    from auraforge_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from auraforge_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from auraforge_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from auraforge_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from auraforge_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    ...
    from auraforge_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from auraforge_cli.models import CANONICAL_PROVIDERS as _cp
...
def _first_positional_argv() -> str | None:
    ...
    ``auraforge -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``auraforge`` or only flags → defaults to ``chat``.
        return False
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``auraforge <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``auraforge photon``
...
def _prepare_agent_startup(args) -> None:
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from auraforge_cli.plugins import start_background_plugin_discovery
...
def _try_fast_chat_launch() -> bool:
    ...
    ``auraforge`` / ``auraforge -w -s foo --yolo`` / ``auraforge chat`` don't need the
    ...
    try:
        from auraforge_cli.config import get_container_exec_info
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `auraforge --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        from auraforge_constants import get_aura_forge_home, display_aura_forge_home
        ...
        from auraforge_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Aura Forge Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from auraforge_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from auraforge_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from auraforge_cli.tools_config import tools_command
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: aura-forge-agent[otlp])")
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from auraforge_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    elif getattr(args, "skills_action", None) in ("trust", "untrust"):
        _cmd_skills_trust(args)
    else:
        from auraforge_cli.skills_hub import skills_command

        skills_command(args)


def _cmd_skills_trust(args):
    """``auraforge skills trust [path]`` / ``auraforge skills untrust [path]``.
...
def cmd_plugins(args):
    from auraforge_cli.plugins_cmd import plugins_command
...
def cmd_claw(args):
    from auraforge_cli.claw import claw_command
...
def main():
    ...
    try:
        from auraforge_cli.stdio import configure_windows_stdio
    ...
    # Self-heal a venv left half-built by an interrupted ``auraforge update``
    ...
    # ``auraforge skills install update``) merely defers recovery one launch;
    # under-matching (missing ``auraforge -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in auraforge_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from auraforge_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from auraforge_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://aura-forge-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `auraforge model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    worktree_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://aura-forge-agent.nousresearch.com/docs/user-guide/cli#worktree-cleanup"
        ),
    )
    ...
    def _dispatch_worktree(_args):
        from auraforge_cli.worktree_cmd import cmd_worktree
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.aura-forge/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://aura-forge-agent.nousresearch.com/docs/user-guide/secrets/"
        ),
    )
    ...
    from auraforge_cli import secrets_cli as _secrets_cli
    from auraforge_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # `auraforge proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://aura-forge-agent.nousresearch.com/docs/user-guide/egress/iron-proxy"
        ),
    )

    from auraforge_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``auraforge proxy`` subparser (dest='proxy_command').
    ...
    # migrate command
    # =========================================================================
    from auraforge_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in auraforge_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in auraforge_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)


    # =========================================================================
    # whatsapp command  (parser built in auraforge_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `auraforge whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in auraforge_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from auraforge_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in auraforge_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in auraforge_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in auraforge_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in auraforge_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # pause / resume commands  (parser built in auraforge_cli/subcommands/pause.py)
    # =========================================================================
    build_pause_parser(subparsers)

    # =========================================================================
    # cron command  (parser built in auraforge_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)
    build_sync_parser(subparsers, cmd_sync=cmd_sync)

    # =========================================================================
    # webhook command  (parser built in auraforge_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from auraforge_cli.subcommands.peer import build_peer_parser
    ...
    # =========================================================================
    from auraforge_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from auraforge_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from auraforge_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in auraforge_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in auraforge_cli/subcommands/doctor.py)
    # =========================================================================
    build_doctor_parser(subparsers, cmd_doctor=cmd_doctor)

    # =========================================================================
    # verify command  (parser built in auraforge_cli/subcommands/verify.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in auraforge_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # approvals command  (parser built in auraforge_cli/subcommands/approvals.py)
    # =========================================================================
    build_approvals_parser(subparsers, cmd_approvals=cmd_approvals)

    # =========================================================================
    # dump command  (parser built in auraforge_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in auraforge_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in auraforge_cli/subcommands/backup.py)
    # =========================================================================
    ...
    from auraforge_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in auraforge_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # import-agent command  (parser: auraforge_cli/subcommands/import_agent.py)
    # =========================================================================
    ...
    # =========================================================================
    # config command  (parser built in auraforge_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # skin command  (parser built in auraforge_cli/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in auraforge_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in auraforge_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in auraforge_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from auraforge_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in auraforge_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``auraforge --help``, ``auraforge logs``,
    ...
    try:
        from auraforge_cli.curator import register_cli as _register_curator_cli
    ...
    try:
        from auraforge_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from auraforge_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in auraforge_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in auraforge_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `auraforge computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `auraforge tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `auraforge computer-use doctor` to run cua-driver's\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import os as _os
            import subprocess
            from auraforge_cli.tools_config import _cua_driver_contract_status
            ...
            if path:
                version = ""
                try:
                    from auraforge_cli.tools_config import _cua_driver_env
                    ...
                    pass
                from auraforge_cli.tools_config import _cua_version_summary
                version = _cua_version_summary(version)
                # Name the override here too. Without it the operator is told
                # to repair an install that `auraforge computer-use install` will
    ...
    # =========================================================================
    # mcp command  (parser built in auraforge_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    # cmd_sessions lives in auraforge_cli/sessions_cmd.py (main.py decomposition).
    ...
    # and lets monkeypatches on auraforge_cli.main.cmd_sessions keep working.
    ...
    # =========================================================================
    # insights command  (parser built in auraforge_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)
    build_monitoring_parser(subparsers, cmd_monitoring=cmd_monitoring)

    # =========================================================================
    # claw command  (parser built in auraforge_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # NOTE: the `auraforge version` subcommand was removed — `auraforge --version`
    # / `-V` now carries the full output including update status.

    # =========================================================================
    # update command  (parser built in auraforge_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in auraforge_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in auraforge_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in auraforge_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in auraforge_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # =========================================================================
    # gui command  (parser built in auraforge_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in auraforge_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in auraforge_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``auraforge -c Pokemon Agent Dev`` → ``auraforge -c 'Pokemon Agent Dev'``
    ...
    # ``auraforge egress start`` refusing when credential_source=bitwarden
