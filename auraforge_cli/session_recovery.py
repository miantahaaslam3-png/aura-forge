"""Offline, non-destructive recovery for a damaged Aura Forge session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``auraforge_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``auraforge_state._backup_db_file``.
    """
    from auraforge_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
