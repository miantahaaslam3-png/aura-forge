...
Scope of the repair is deliberately narrow. Aura Forge only upgrades an npm that
lives inside its **own** managed Node tree (``$AURA_FORGE_HOME/node``), installing
...
and their other projects; Aura Forge never modifies those. When the failing npm is
one of those foreign installs, Aura Forge instead provisions its own managed Node
...
def upgrade_managed_npm(
    ...
    *,
    ...
    ``prefix=~/.local`` into ``$AURA_FORGE_HOME/node/etc/npmrc`` so that global
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Aura Forge could not provision its own Node.js runtime and never\n"
...
def maybe_repair_npm_engine(
    ...
    *output* is the combined stdout/stderr of the npm command that just failed.
    ...
    if prefix is not None:
        # Aura Forge owns this npm — upgrade it in place. Only an npm-range
