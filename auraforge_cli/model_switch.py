...
- ``auraforge_cli.providers``        -- canonical provider identity + overlays
- ``auraforge_cli.model_normalize``  -- per-provider name formatting
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        for model_id in value:
            # Backward compat: pre-fix Aura Forge wrote sentinel keys inside the
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Aura Forge.

    The current shape is an entry-level ``models_discovered: true`` sibling of
    ``models``. Older Aura Forge versions wrote an in-mapping
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    ``_entry_models_discovered``): a catalog Aura Forge itself persisted after a
    ...
    *,
    ...
    try:
        ...
        for entry in providers:
            ...
            # A mapping Aura Forge itself discovered (``models_discovered: true``
...
_HERMES_MODEL_WARNING = (
    "Nous Research Aura Forge 3 & 4 models are NOT agentic and are not designed "
    "for use with Aura Forge Agent. They lack the tool-calling capabilities "
...
def is_nous_hermes_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Aura Forge 3/4 chat model.
...
def _check_hermes_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Aura Forge 3/4 chat model."""
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from auraforge_cli.model_switch import
...
def resolve_display_context_length(
    ...
    if config_context_length is not None and (
        ...
        try:
            from auraforge_cli.route_identity import should_clear_context_pin
...
async def resolve_display_context_length_async(
    ...
    ``auraforge_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        from auraforge_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from auraforge_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
        ...
        if not new_model:
            if pdef.base_url:
                from auraforge_cli.runtime_provider import _auto_detect_local_model
    ...
    # =================================================================
        ...
        try:
            from auraforge_cli.config import load_config
            from auraforge_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from auraforge_cli.providers import resolve_user_provider as _ruser
    ...
    # auraforge_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    # kimi) the same way.
    from auraforge_cli.models import opencode_provider_family as _oc_family_fn
    if _oc_family_fn(target_provider) is not None and isinstance(base_url, str):
        from auraforge_cli.models import normalize_opencode_base_url
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from auraforge_cli.config import normalize_extra_headers
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    :param provider_slugs: Aura Forge provider IDs to prefetch (e.g. ``["openrouter",
        "anthropic", "deepseek"]``).  Unknown providers are silently skipped.
    """
    from auraforge_cli.models import cached_provider_model_ids
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from auraforge_cli.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from auraforge_cli.auth import PROVIDER_REGISTRY, _load_auth_store
    from auraforge_cli.providers import HERMES_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from auraforge_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from auraforge_cli/models.py (OPENROUTER_MODELS,
    ...
    from auraforge_cli.config import coerce_provider_id, stringify_provider_map
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from auraforge_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    if "lmstudio" not in curated and (
        ...
        from auraforge_cli.models import fetch_lmstudio_models
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Aura Forge may not route yet.
        ...
        if not has_creds:
            try:
                from auraforge_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(hermes_id)
                )
    ...
    from auraforge_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Aura Forge provider ID.
    # HERMES_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Aura Forge IDs ("copilot").
    _mdev_to_hermes = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # Resolve Aura Forge slug — e.g. "github-copilot" → "copilot"
        hermes_slug = _mdev_to_hermes.get(pid, pid)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                from auraforge_cli.auth import get_provider_auth_state as _nous_state
    ...
    try:
        from auraforge_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Aura Forge writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Aura Forge's own writer uses (mirrors section-4 grouping).
        ...
        for grp in ep_groups.values():
            ...
            if _probe_live:
                ...
                try:
                    from auraforge_cli.models import cached_fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Aura Forge's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, auraforge_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from auraforge_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`auraforge_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from auraforge_cli.models import fetch_openrouter_models
