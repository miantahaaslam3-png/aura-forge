...
Usage examples::
    ...
    auraforge logs                    # last 50 lines of agent.log
    auraforge logs -f                 # follow agent.log in real time
    auraforge logs errors             # last 50 lines of errors.log
    auraforge logs gateway -n 100    # last 100 lines of gateway.log
    auraforge logs gui -f            # follow gui.log (dashboard/pty/ws)
    auraforge logs desktop -f        # follow desktop.log (Electron app boot/backend)
    auraforge logs --level WARNING    # only WARNING+ lines
    auraforge logs --session abc123   # filter by session ID substring
    auraforge logs --component tools  # only tool-related lines
    auraforge logs --since 1h         # lines from the last hour
    auraforge logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_aura_forge_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Aura Forge runs — try 'auraforge chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_aura_forge_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_aura_forge_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_aura_forge_home()}/logs/")
        return

    print(f"Log files in {display_aura_forge_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'auraforge chat' to generate logs)")
