...
from auraforge_constants import get_aura_forge_home, get_optional_mcps_dir
...
from auraforge_cli.cli_output import prompt as _prompt_input
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Aura Forge install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Aura Forge understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Aura Forge is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Aura Forge
        understands. Update Aura Forge to install this entry.
...
def _install_root() -> Path:
    ...
    root = get_aura_forge_home() / "mcp-installs"
...
def _build_server_config(
    ...
    by auraforge_cli/mcp_config.py."""
    cfg: dict = {}
    t = entry.transport
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from auraforge_cli.mcp_config import _bearer_auth_headers
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from auraforge_cli.mcp_config import _probe_single_server
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    from auraforge_cli.mcp_config import _save_mcp_server
    ...
    print(color(
        ...
        f"Start a new Aura Forge session to load its tools.",
        Colors.GREEN,
    ))
