...
store at ``~/.aura-forge/checkpoints/``.  Actions:
    ...
    auraforge checkpoints               # same as `status`
    auraforge checkpoints status        # total size, project count, breakdown
    auraforge checkpoints list          # per-project checkpoint counts + workdir
    auraforge checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    auraforge checkpoints clear [-f]    # nuke the entire base (asks first)
    auraforge checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    auraforge checkpoints
    auraforge checkpoints prune --retention-days 3 --max-size-mb 200
    auraforge checkpoints clear -f
...
from auraforge_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: auraforge checkpoints clear-legacy")
    return 0
