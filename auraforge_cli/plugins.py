...
from auraforge_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
from auraforge_cli.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    ...
    is passed. Does not touch the root logger or other Aura Forge loggers.
    """
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Aura Forge' shipped guidance lives in the evidence-based
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by auraforge_cli.kanban_db when a task
    ...
    #   result: auraforge_cli.kanban_db.DispatchResult (spawned, reclaimed,
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Aura Forge "
...
class PluginManifest:
    ...
    # ``auraforge_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
    ...
    # Declared pip dependencies. VALIDATED AND SURFACED ONLY — Aura Forge never
...
def _plugin_relative_segments(key: str) -> tuple[str, ...]:
    ...
    ``retry.policy``).  Full Aura Forge paths, traversal syntax, and the security-
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        # threads. Other Aura Forge config writers still retain their existing
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from auraforge_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        ...
        :func:`auraforge_cli.profiles.get_active_profile_name`, so it works in
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Aura Forge core) are trusted by default —
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        ``provider`` must be an instance of
        :class:`auraforge_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from auraforge_cli.dashboard_auth import DashboardAuthProvider
        from auraforge_cli.dashboard_auth.registry import (
    ...
    def register_slack_action_handler(
        ...
        Aura Forge' Slack adapter wires registered handlers into its
    ...
    def register_telegram_handler(self, factory: Callable) -> None:
        ...
        Aura Forge' Telegram adapter invokes registered factories at ``connect()``
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        # Lazy import to avoid circular: auraforge_cli.main imports plugins indirectly
        from auraforge_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
...
class PluginManager:
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        try:
            ...
            from auraforge_cli.env_loader import load_hermes_dotenv, reset_secret_source_cache
        except Exception:
            return
        try:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning(
                "Removed Aura Forge plugin %s is still listed in plugins.enabled; "
        ...
        for manifest in winners.values():
            ...
            # Relay lifecycle ownership now lives in the Aura Forge core. Loading
            ...
            if (
                ...
                loaded.error = (
                    "removed — Relay lifecycle is owned by Aura Forge core; configure "
                    f"{RELAY_PLUGINS_CONFIG_ENV} instead"
                )
                self._plugins[lookup_key] = loaded
                logger.warning(
                    "Refusing to load removed Aura Forge Relay plugin '%s'; %s",
                    lookup_key,
                    loaded.error,
                )
                continue
            ...
            # path actually asks for that platform. Every platform Aura Forge ships
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from auraforge_cli.approval_transport import RegisteredApprovalTransport
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from auraforge_cli.agent_plugins import _discover_mcp
    ...
    def _classify_entrypoint_kind(self, ep) -> str:
        ...
        module's import cost in every Aura Forge process (e.g. a pip
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        python_dependencies is a declaration seam ONLY: Aura Forge validates and
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Aura Forge does not install plugin dependencies "
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from auraforge_cli.agent_plugins import load_agent_plugin
...
# auraforge_constants.set_aura_forge_home_override). A process-wide single-slot
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Aura Forge profile/home.
...
def _get_pre_tool_call_directive_details(
    ...
    from auraforge_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
