...
`auraforge import` restores from a backup zip, overlaying onto the current
AURA_FORGE_HOME root.
"""
...
from auraforge_constants import get_default_hermes_root, get_aura_forge_home, display_aura_forge_home
from utils import (
    _preserve_file_mode,
...
from auraforge_cli.sizefmt import format_bytes as _format_size
...
# Where ``auraforge backup --quick`` / ``/snapshot`` / the pre-update safety net
...
# ``aura-forge-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/aura-forge-agent/``
...
# AURA_FORGE_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "aura-forge-agent",     # the codebase repo — re-clone instead
    ...
    "browser-profile",
    # Python dependency trees (plugin / MCP-server venvs under AURA_FORGE_HOME) —
...
# File names that ``auraforge import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE AURA_FORGE_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Aura Forge backup slot."""
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Aura Forge backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside AURA_FORGE_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``aura-forge-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/aura-forge-agent/`` — must be preserved.
        if part == "aura-forge-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside AURA_FORGE_HOME.
...
# to the user as a hung `auraforge update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # auraforge_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the Aura Forge home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: Aura Forge home directory not found at {hermes_root}")
        sys.exit(1)
...
def _run_backup_locked(args, hermes_root: Path) -> None:
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        # ``aura-forge-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "aura-forge-agent" and not is_root)
        ]
    ...
    # outside AURA_FORGE_HOME, so the walk above never sees it. Ask the active
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Aura Forge backup.
    ...
    if not found:
        return False, (
            "zip does not appear to be a Aura Forge backup "
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``auraforge import``, which is the
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``auraforge import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to Aura Forge' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Aura Forge configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under AURA_FORGE_HOME.
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # auraforge_cli.profiles might not be available (fresh install)
        ...
        try:
            from auraforge_cli.gateway import ensure_gateway_service, _is_service_running
...
# — `auraforge update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $AURA_FORGE_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside AURA_FORGE_HOME and the entry is silently
...
# Relative path of the cron job database inside AURA_FORGE_HOME. Kept in sync with
...
def create_pre_update_backup(
    ...
    but writes to ``<AURA_FORGE_HOME>/backups/pre-update-<timestamp>.zip`` and
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `auraforge claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    ``auraforge claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``auraforge import <archive>``.  Writes to
    ``<AURA_FORGE_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `auraforge import` and the
