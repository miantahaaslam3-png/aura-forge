...
from auraforge_cli.config import get_aura_forge_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.aura-forge/.env.
    ...
    exports. ``auraforge debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    ``HERMES_GIT_SHA`` build-arg (see ``auraforge_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``auraforge dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from auraforge_cli.gateway import get_gateway_runtime_snapshot
...
def run_dump(args):
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.aura-forge/.env: a managed
        ...
        # A credential added via `auraforge auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `auraforge auth list` shows it (#42130).
