...
def _split_line(line: str) -> list[str]:
    try:
        ...
        from auraforge_cli._subprocess_compat import split_command_line
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("auraforge_cli.main")
    builder = getattr(module, builder_name)
...
class HermesConsoleEngine:
    """Curated line-command executor for Aura Forge Console."""
        ...
        try:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Aura Forge Console does not run shell syntax. Use one supported "
                    "Aura Forge command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Aura Forge component status.", _status)
        self.register(("version",), "version", "Show Aura Forge version information.", _version)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Aura Forge logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Aura Forge configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Aura Forge Console."""

        extracted = {
            "dump": (
                "auraforge_cli.subcommands.dump",
                "build_dump_parser",
                "cmd_dump",
                [()],
                set(),
            ...
            "debug": (
                "auraforge_cli.subcommands.debug",
                "build_debug_parser",
                "cmd_debug",
                [("share",), ("delete",)],
                {("share",), ("delete",)},
            ...
            "prompt-size": (
                "auraforge_cli.subcommands.prompt_size",
            ...
            "insights": (
                "auraforge_cli.subcommands.insights",
            ...
            "security": (
                "auraforge_cli.subcommands.security",
            ...
            "backup": (
                "auraforge_cli.subcommands.backup",
                "build_backup_parser",
                "cmd_backup",
                [()],
                {()},
            ...
            "import": (
                "auraforge_cli.subcommands.import_cmd",
            ...
            "config": (
                "auraforge_cli.subcommands.config",
                "build_config_parser",
                "cmd_config",
                [("env-path",), ("check",)],
                set(),
            ...
            "tools": (
                "auraforge_cli.subcommands.tools",
                "build_tools_parser",
                "cmd_tools",
            ...
            "plugins": (
                "auraforge_cli.subcommands.plugins",
            ...
            "skills": (
                "auraforge_cli.subcommands.skills",
                "build_skills_parser",
                "cmd_skills",
                [
                    ("browse",),
            ...
            "memory": (
                "auraforge_cli.subcommands.memory",
                "build_memory_parser",
                "cmd_memory",
                [("status",), ("off",), ("reset",)],
                {("off",), ("reset",)},
            ...
            "auth": (
                "auraforge_cli.subcommands.auth",
                "build_auth_parser",
                "cmd_auth",
                [
                    ("list",),
            ...
            "pairing": (
                "auraforge_cli.subcommands.pairing",
            ...
            "webhook": (
                "auraforge_cli.subcommands.webhook",
            ...
            "hooks": (
                "auraforge_cli.subcommands.hooks",
                "build_hooks_parser",
                "cmd_hooks",
            ...
            "slack": (
                "auraforge_cli.subcommands.slack",
                "build_slack_parser",
                "cmd_slack",
                [("manifest",)],
                set(),
            ...
            "profile": (
                "auraforge_cli.subcommands.profile",
            ...
            "cron": (
                "auraforge_cli.subcommands.cron",
                "build_cron_parser",
                "cmd_cron",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Aura Forge configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "auraforge_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("auraforge_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "auraforge_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("auraforge_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("auraforge_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported Aura Forge Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Aura Forge Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        blocked_pairs = {
            ...
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Aura Forge Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Aura Forge Console.",
            ("skills", "publish"): "`skills publish` is not available in Aura Forge Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Aura Forge Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Aura Forge Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Aura Forge Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Aura Forge Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Aura Forge Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Aura Forge Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Aura Forge Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Aura Forge Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Aura Forge Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Aura Forge Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Aura Forge Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Aura Forge Console.")
...
def _version(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from auraforge_cli._startup_fast import print_fast_version_info
...
def _logs(_engine: HermesConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Aura Forge Console.")
    ...
    from auraforge_cli.logs import list_logs, tail_log
...
def _config_show(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from auraforge_cli.config import show_config
...
def _config_path(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from auraforge_cli.config import get_config_path
...
def _sessions_repair(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from auraforge_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    return _dispatch_extracted_subcommand(
        ...
        fixed=(),
        args=(),
        module_name="auraforge_cli.subcommands.profile",
...
def _cron_list(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    ns = parser.parse_args(args)
    from auraforge_cli.cron import cron_list
...
def _cron_status(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from auraforge_cli.cron import cron_status
...
def run_console_repl(
    *,
    ...
    engine = HermesConsoleEngine()
    if interactive:
        print("Aura Forge Console. Type `help` for commands, `exit` to quit.", file=stdout)
