...
Currently supports:
    auraforge debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.aura-forge/logs/*.log`` are not leaked into
...
def _pending_file() -> Path:
    """Path to ``~/.aura-forge/pastes/pending.json``.
    ...
    ran ``auraforge debug share`` repeatedly.

    Deletion is now driven by the gateway's cron ticker
    (``gateway/run.py::_start_cron_ticker``) which calls
    ``_sweep_expired_pastes`` once per hour.  ``auraforge debug share`` also
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``auraforge debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``auraforge debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `auraforge debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "aura-forge-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``auraforge debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.aura-forge/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``auraforge debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "aura-forge-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from auraforge_cli.logs import LOG_FILES
...
# desktop app talks to a remote/docker/SSH backend, `auraforge debug share` runs
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Aura Forge Desktop on the machine running the app, not by this "
...
def _capture_log_snapshot(
    ...
    *,
    ...
    ``auraforge debug share --no-redact``).
    """
...
def _capture_dump() -> str:
    """Run ``auraforge dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``auraforge dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``auraforge debug share`` (CLI) and the
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `auraforge debug share --local` to print the report instead.\n")
        sys.exit(1)
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``auraforge debug share --nous``: upload the bundle to Nous-S3.
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `auraforge debug share --local` to print the report instead, "
            "or `auraforge debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(
        ...
        "  GitHub Issues        https://github.com/miantahaaslam3-png/aura-forge/issues\n"
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``auraforge debug`` call.
    ...
    # best-effort — any failure is swallowed so ``auraforge debug`` stays
