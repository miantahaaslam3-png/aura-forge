...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"auraforge send: {file_path} is not a text file. --file reads the "
                ...
                f'  auraforge send --to telegram "MEDIA:{file_path}"\n'
                f'  auraforge send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  auraforge send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"auraforge send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"auraforge send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"auraforge send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"auraforge send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"auraforge send: no targets found for platform '{platform_filter}'. "
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    if not target:
        print(
            "auraforge send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  auraforge send --to telegram \"hello\"\n"
            "  auraforge send --to discord:#ops --file report.md\n"
            "  auraforge send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "auraforge send: no message provided. Pass text as a positional "
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  auraforge send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | auraforge send --to telegram:-1001234567890\n"
            "  auraforge send --to discord:#ops --file /tmp/report.md\n"
            "  auraforge send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  auraforge send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  auraforge send --list                  # all platforms\n"
            "  auraforge send --list telegram         # filter by platform\n"
            "\n"
