...
``auraforge_cli.config.migrate_config``. Each step is a function
...
``auraforge_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``auraforge_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``auraforge_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``auraforge_cli.config`` (e.g.
``patch("auraforge_cli.config.read_raw_config", ...)``) keep working, because
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_aura_forge_home()}/config.yaml and run `auraforge setup` to "
...
def _cfg():
    """Return the live ``auraforge_cli.config`` module (lazy, cycle-free)."""
    from auraforge_cli import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_aura_forge_home = _c.get_aura_forge_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        try:
            user_plugins_dir = get_aura_forge_home() / "plugins"
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    #      on top of ensure_aura_forge_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_aura_forge_home() / "logs" / "curator"
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from auraforge_cli.relay_plugin_cutover import legacy_relay_plugin_keys
