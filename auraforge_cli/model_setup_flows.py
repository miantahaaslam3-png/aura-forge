...
Extracted from ``auraforge_cli/main.py`` as part of the god-file decomposition
...
in main.py re-imports them (``from auraforge_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``auraforge_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from auraforge_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``auraforge_cli.main`` at import time -> no import cycle.
"""
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from auraforge_cli.main import _prompt_api_key
    ...
    from auraforge_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from auraforge_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    """
    from auraforge_cli.auth import _save_model_choice, deactivate_provider
    from auraforge_cli.config import load_config, save_config
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from auraforge_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from auraforge_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from auraforge_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from auraforge_cli.auth import (
    ...
    from auraforge_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from auraforge_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from auraforge_cli.auth import _save_model_choice, deactivate_provider
    from auraforge_cli.config import (
    ...
    if probe.get("used_fallback") and probe.get("resolved_base_url"):
        ...
        print(
            ...
            f"Aura Forge will still save it."
        )
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from auraforge_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from auraforge_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    print("Anthropic-style API endpoints.  Aura Forge will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Aura Forge will install it now (the preflight below "
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from auraforge_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from auraforge_cli.auth import _save_model_choice, deactivate_provider
    from auraforge_cli.config import load_config, normalize_extra_headers, save_config
...
def _model_flow_copilot(config, current_model=""):
    ...
    from auraforge_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from auraforge_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from auraforge_cli.config import save_env_value, load_config, save_config
    from auraforge_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from auraforge_cli.copilot_auth import copilot_device_code_login
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Aura Forge turns to `copilot --acp`.")
    print("  Aura Forge currently starts its own ACP subprocess for each request.")
    print("  Aura Forge uses your selected model as a hint for the Copilot ACP session.")
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from auraforge_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from auraforge_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from auraforge_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    from auraforge_cli.auth import _resolve_api_key_provider_secret, ProviderConfig
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    from auraforge_cli.config import load_config, save_config, get_env_value
    from auraforge_cli.models import _PROVIDER_MODELS
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``auraforge_cli.auth`` so it stays in
    sync with the probe list.

    Returns the selected base URL.  Falls back to *current_base* on cancel
    or error.
    """
    from auraforge_cli.main import _prompt_provider_choice
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Aura Forge typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Aura Forge, enable billing on your "
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from auraforge_cli.main import _run_anthropic_oauth_flow
    from auraforge_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
