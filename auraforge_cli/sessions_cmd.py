"""``auraforge sessions`` command — extracted from ``auraforge_cli/main.py``.
...
Helpers that stay in ``auraforge_cli.main`` (``get_aura_forge_home``,
...
``auraforge_cli.main.<name>`` keep reaching this code path, and so imports stay
...
def _prune_never_active_keyed(db, args):
    ...
    """
    from auraforge_cli.session_filters import format_epoch, parse_duration_seconds
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "import":
        from auraforge_cli.foreign_sessions import run_sessions_import
    ...
    if action == "list":
        from auraforge_state import workspace_key as _ws_key
        ...
        # Markdown / QMD export
        from auraforge_cli.session_export_md import (
