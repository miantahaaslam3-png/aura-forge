...
from auraforge_cli.urllib_security import open_credentialed_url, url_origin
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from auraforge_cli.config import get_custom_provider_tls_settings
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from auraforge_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Aura Forge-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # "opencode/latest"; we send honest Aura Forge attribution and don't
    ...
    # endpoint expects (see auraforge_cli/model_setup_flows.py).
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Aura Forge that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Aura Forge that doesn't ship
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_aura_forge_home() / "cache" / "nous_recommended_cache.json"
...
# In-repo fallback for the model Aura Forge silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from auraforge_cli.model_catalog import get_default_model_from_cache
...
# ``get_recommended_default_model`` in auraforge_cli/web_server.py and
...
def _reasoning_caps_disk_path() -> Path:
    from auraforge_constants import get_aura_forge_home
    return get_aura_forge_home() / "cache" / "reasoning_caps.json"
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from auraforge_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Aura Forge lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from auraforge_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from auraforge_constants import AI_GATEWAY_BASE_URL
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return auraforge-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; auraforge's picker expects
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from auraforge_cli.providers import custom_provider_slug
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from auraforge_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        nous:auraforge-3                           →  ("nous", "auraforge-3")
...
def _resolve_static_model_alias(
    ...
    try:
        from auraforge_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Aura Forge' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``auraforge_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Aura Forge should expose the /fast toggle for this model."""
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Aura Forge release.
    """
    requested = str(provider or "").strip().lower()
    if requested == "ollama":
        if force_refresh:
    ...
    if normalized == "openai-codex":
        from auraforge_cli.codex_models import get_codex_model_ids
        ...
        # a Aura Forge release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from auraforge_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Aura Forge release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            from auraforge_cli.providers import is_official_openai_host
...
def _provider_models_cache_path() -> Path:
    from auraforge_constants import get_aura_forge_home
    return get_aura_forge_home() / "provider_models_cache.json"
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Aura Forge' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Aura Forge' native Anthropic adapter. The live catalog may
...
def opencode_zen_free_headers() -> dict:
    ...
    """
    try:
        from auraforge_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
        ...
        "User-Agent": f"Aura ForgeAgent/{_v}",
    }
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=auraforge"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from auraforge_constants import get_aura_forge_home
    return get_aura_forge_home() / "ollama_cloud_models_cache.json"
    ...
    *,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "Aura Forge will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Aura Forge will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
    ...
    if normalized in {"openai-codex", "xai-oauth"}:
        ...
        # BEFORE the hidden-slug soft-accept below: the suffix is a Aura Forge
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Aura Forge cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if _variant_base is not None and _variant_base in set(api_models):
            ...
            # fetch) so we only accept models Aura Forge actually ships.  (#46850)
            #
