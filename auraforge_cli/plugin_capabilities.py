...
this. Capabilities govern the *host API surfaces* Aura Forge hands out (which
...
until the user re-consents (``auraforge plugins update`` surfaces the diff).
...
def parse_declared_capabilities(raw: Any, plugin_name: str = "?") -> List[str]:
    ...
    for a newer Aura Forge may declare ids this build doesn't know; they can
