"""
Multi-provider authentication system for Aura Forge Agent.
...
# httpx is imported lazily: it costs ~30ms at import time and auraforge_cli.auth
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # ("auraforge_cli.auth.httpx.Client", ...) keeps working in tests.
...
from auraforge_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from auraforge_cli import __version__ as _HERMES_CLI_VERSION
...
def normalize_actual_base_url(base_url: str) -> str:
    ...
    surface for Aura Forge' Responses transport.
    """
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    ...
    "anthropic": ProviderConfig(
        ...
        # from it), so removing it would stop Aura Forge finding a setup-token
...
def get_anthropic_key() -> str:
    ...
    """
    from auraforge_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from auraforge_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    try:
        ...
        # ``provider: anthropic`` as a MoA advisor/aggregator has opted Aura Forge
    ...
    # not by the user explicitly configuring anthropic in Aura Forge.
    ...
    # the user initiated inside Aura Forge (manual add / device-code / PKCE), plus
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    pointing Aura Forge at a non-production Nous Portal (e.g. a hosted agent
...
def login_spotify_command(args) -> None:
    ...
    print()
    print("Open this URL to authorize Aura Forge:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Aura Forge is waiting for the OAuth callback on {redirect_uri}")
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Aura Forge auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Aura Forge auth state."""
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Aura Forge auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. Aura Forge keeps its OWN
        ...
        # the Codex CLI (or another Aura Forge process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Aura Forge's own Codex token store.
    ...
    if should_refresh:
        # Re-read under lock to avoid racing with other Aura Forge processes
...
def _probe_codex_quota_restored(
    ...
    *,
    ...
    Aura Forge persists a Codex 429's ``reset_at`` locally and freezes the
...
def _nous_shared_auth_dir() -> Path:
    ...
    :func:`auraforge_constants.get_default_hermes_root` returns — so
...
def _nous_shared_store_path() -> Path:
    ...
    # real user's Aura Forge root, refuse rather than silently corrupt cross-profile
...
def _quarantine_nous_oauth_state(
    ...
    *,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — Aura Forge
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with Aura Forge's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Aura Forge's next refresh uses it, and the whole
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError(
                "Aura Forge is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError("Aura Forge is not logged into Nous Portal.",
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from auraforge_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Aura Forge' "
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from auraforge_cli.config import clear_model_endpoint_credentials
...
def _confirm_selection_guards(
    ...
    *,
    ...
    :mod:`auraforge_cli.model_selection_guards` and shows one [y/N] confirm with
...
def _prompt_model_selection(
    ...
    def _confirmed_selection(mid: str) -> Optional[str]:
        if not mid:
            return None
        # Unified guard registry (auraforge_cli.model_selection_guards): the cost
    ...
    from auraforge_cli.curses_ui import format_radio_item_ansi
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from auraforge_cli.config import save_config, load_config
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Aura Forge auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Aura Forge will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                print("Aura Forge will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Aura Forge gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Aura Forge creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Aura Forge auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Aura Forge auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Aura Forge creates its own local OAuth session)")
    print()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Aura Forge login via MiniMax ({region}) OAuth...")
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Aura Forge login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Aura Forge builds still see
