...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Aura Forge. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Aura Forge deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Aura Forge on Slack's older Assistant messaging
    ...
    """
    from auraforge_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Aura Forge in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Aura Forge in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Aura Forge agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``auraforge_cli/main.py``):
    ...
    """
    name = getattr(args, "name", None) or "Aura Forge"
    description = getattr(args, "description", None) or "Your Aura Forge agent on Slack"
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Aura Forge app\n"
