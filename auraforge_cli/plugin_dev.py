    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Aura Forge discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from auraforge_cli.plugins import SUPPORTED_MANIFEST_VERSION

    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Aura Forge supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Aura Forge never auto-installs plugin dependencies; "
    ...
    if schema:
        from auraforge_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Aura Forge' real scanner and registration path."""
    try:
