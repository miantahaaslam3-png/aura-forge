"""Dependency-light venv recovery that runs BEFORE auraforge_cli.main's imports.

The ``auraforge`` console entry point is ``auraforge_cli.main:main``.  Importing
``auraforge_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``auraforge_cli.env_loader``, ``yaml`` via ``auraforge_cli.config``, ...).  In
...
a corrupted venv.  ``auraforge_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``auraforge_cli.main`` to become
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which Aura Forge already vendors
    (``~/.aura-forge/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``auraforge acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``auraforge_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``auraforge_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``auraforge update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `auraforge update`: CLI and Desktop retries preserve
...
# every `auraforge` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``auraforge update`` (or the self-lock preflight, #83569) left the
    ...
    ``auraforge_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``auraforge acp`` included).
