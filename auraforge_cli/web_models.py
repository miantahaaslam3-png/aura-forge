"""Pydantic request/response models for the Aura Forge dashboard web server.

Extracted verbatim from ``auraforge_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from auraforge_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``auraforge model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `auraforge import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`auraforge -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the AURA_FORGE_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under AURA_FORGE_HOME.
    output: str = ""
