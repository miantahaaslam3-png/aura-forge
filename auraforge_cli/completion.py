"""Shell completion script generation for auraforge CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Aura Forge Agent bash completion
# Add to ~/.bashrc:
#   eval "$(auraforge completion bash)"

_hermes_profiles() {{
    local profiles_dir="$HOME/.auraforge/profiles"
...
complete -F _hermes_completion auraforge
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef auraforge
# Aura Forge Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(auraforge completion zsh)"

_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.auraforge/profiles" ]]; then
        profiles+=($HOME/.auraforge/profiles/*(N/:t))
    fi
...
{top_cmds_str}
            ...
            _describe 'auraforge command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Aura Forge Agent fish completion",
        "# Add to your config:",
        "#   auraforge completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hermes_profiles",
        "    echo default",
        "    if test -d $HOME/.auraforge/profiles",
        "        for d in $HOME/.auraforge/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c auraforge -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c auraforge -f -s p -l profile"
