...
from auraforge_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

HERMES_AGENT_LOGO = """[bold #C084FC] █████╗ ██╗   ██╗██████╗  █████╗    ███████╗ ██████╗ ██████╗  ██████╗ ███████╗[/]
[bold #B47CFF]██╔══██╗██║   ██║██╔══██╗██╔══██╗   ██╔════╝██╔═══██╗██╔══██╗██╔════╝ ██╔════╝[/]
[#9D74FF]███████║██║   ██║██████╔╝███████║   █████╗  ██║   ██║██████╔╝██║  ███╗█████╗  [/]
[#8B6BFF]██╔══██║██║   ██║██╔══██╗██╔══██║   ██╔══╝  ██║   ██║██╔══██╗██║   ██║██╔══╝  [/]
[#6F5BFF]██║  ██║╚██████╔╝██║  ██║██║  ██║   ██║     ╚██████╔╝██║  ██║╚██████╔╝███████╗[/]
...
def check_for_updates() -> Optional[int]:
    """Check whether a Aura Forge update is available.
    ...
    try:
        from auraforge_cli.config import detect_install_method, get_project_root
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``auraforge_cli/build_info.py``) and return it as a frozen
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from auraforge_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/miantahaaslam3-png/aura-forge/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Aura Forge checkout. Cached per-process. Release URL always points at the
    canonical miantahaaslam3-png/aura-forge repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Aura Forge Agent v{VERSION} ({RELEASE_DATE})"
...
def _format_update_notice(behind: int) -> str:
    ...
    from auraforge_cli.config import get_managed_update_command, recommended_update_command
    if behind > 0:
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    if not _has_native_mcp:
        try:
            from auraforge_cli.plugins import get_portable_mcp_server_names_nowait
    ...
    try:
        from auraforge_cli.codex_runtime_switch import get_current_runtime
        from auraforge_cli.config import load_config as _load_cfg
