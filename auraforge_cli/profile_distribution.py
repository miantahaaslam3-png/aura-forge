"""Profile distributions — shareable, packaged Aura Forge profiles via git.

A distribution is a Aura Forge profile published as a git repository (or
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "auraforge_state.db", "response_store.db",
...
def check_hermes_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Aura Forge {op}{target}, "
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Aura Forge profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Aura Forge distribution."
        )
