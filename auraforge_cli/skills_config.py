"""
Skills configuration for Aura Forge Agent.
`auraforge skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.aura-forge/config.yaml under:
...
from auraforge_cli.config import cfg_get, load_config, save_config
from auraforge_cli.colors import Colors, color
from auraforge_cli.platforms import PLATFORMS as _PLATFORMS
...
def save_disabled_skills(config: dict, disabled: Set[str], platform: Optional[str] = None):
    ...
    Essential skills (e.g. ``aura-forge-agent``) are silently dropped from the
...
def skills_command(args=None):
    """Entry point for `auraforge skills`."""
    from auraforge_cli.curses_ui import curses_checklist
