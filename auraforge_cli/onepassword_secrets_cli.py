"""CLI handlers for ``auraforge secrets onepassword ...``.
...
the CLI through OS package managers and signed installers, so Aura Forge expects
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        Panel.fit(
            ...
            "Aura Forge resolves [cyan]op://vault/item/field[/cyan] references through your\n"
    ...
    if binary and not token_set:
        ...
        if who:
            ...
            console.print(
                ...
                "Aura Forge will warn and skip 1Password on next startup.[/yellow]"
            )
    if not references:
        console.print(
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Takes effect on the next Aura Forge invocation."
    )
    if not op_cfg.get("enabled"):
        console.print(
            "[yellow]Note: the 1Password integration is currently disabled — "
            "run `auraforge secrets onepassword setup` to turn it on.[/yellow]"
        )
    return 0
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not op_cfg.get("enabled"):
        console.print(
            ...
            "`auraforge secrets onepassword setup` first.[/yellow]"
        )
        return 1
    ...
    if not references:
        console.print(
            ...
            "`auraforge secrets onepassword set ENV_VAR \"op://…\"`.[/yellow]"
        )
        return 0
