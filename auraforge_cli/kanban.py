...
from auraforge_cli import kanban_swarm as ks
...
def _check_dispatcher_presence(
    ...
    - ``running=True``: a gateway is alive for this AURA_FORGE_HOME and its
    ...
    Used by ``auraforge kanban create`` (and callers) to warn when a task
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`auraforge kanban daemon --force`)."
        )
    return (
        False,
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across Aura Forge profiles. "
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `auraforge kanban boards switch <slug>` or the "
            "HERMES_KANBAN_BOARD env var). Use `auraforge kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    b_import = boards_sub.add_parser(
        ...
        description=(
            "Import a .tar.gz produced by `auraforge kanban boards export`. "
    ...
    p_create.add_argument("--project", default=None,
                          help="Link to a project (id or slug). Anchors the task's "
                               ...
                               "deterministic branch. See `auraforge project list`.")
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `auraforge gateway start`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.aura-forge/profiles/ and current assignees on the board)",
    )
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``auraforge kanban …`` argparse dispatch.
    ...
    # auraforge_cli.kanban_db, because children can import DB mutators directly.
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `auraforge kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    # AURA_FORGE_HOME. Previously only `init` and `daemon` triggered
    ...
    with board_scope:
        ...
        # every `auraforge kanban repair` into "could not initialize database"
...
def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``auraforge kanban boards <action>``.
    ...
    DB, so a fresh AURA_FORGE_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `auraforge kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `auraforge kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `auraforge kanban boards switch {meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `auraforge kanban boards delete <slug>` (alias), the
...
def _cmd_boards_export(args: argparse.Namespace) -> int:
    from auraforge_cli import kanban_transfer
    ...
    print("Import it with `auraforge kanban boards import <archive>`.")
    return 0
...
def _cmd_boards_import(args: argparse.Namespace) -> int:
    ...
    if res["activated"]:
        ...
        print(f"  Switch to it with `auraforge kanban boards switch {res['board']}`.")
    return 0
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.aura-forge/profiles/.")
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `auraforge -p <name> setup`)")
        return 0
    # Header
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if len(all_boards) > 1:
        ...
        print(
            ...
            f"`auraforge kanban boards list`)\n"
        )
    if not tasks:
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``auraforge kanban show <id>``
...
def _goal_mode_handoff_rejection(task: Optional[kb.Task], evidence: str) -> Optional[str]:
    ...
    from auraforge_cli.goals import judge_goal
...
def _cmd_daemon(args: argparse.Namespace) -> int:
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`auraforge kanban list --status ready` / "
                    f"`auraforge kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Aura Forge profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from auraforge_cli import kanban_specify as spec
