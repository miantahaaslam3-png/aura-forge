"""Slash command definitions and autocomplete for the Aura Forge CLI.
...
from auraforge_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``auraforge_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``auraforge_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Aura Forge config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Aura Forge works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey Aura Forge' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Aura Forge is working", "Configuration",
    ...
    CommandDef("update", "Update Aura Forge Agent to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show Aura Forge Agent version", "Info", aliases=("v",),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`auraforge_cli.plugins.PluginContext.register_command`. They behave
...
# Telegram allows up to 100 BotCommands. Aura Forge ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Aura Forge
...
class SlashCommandCompleter(Completer):
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via auraforge_cli.personality."""
        try:
