"""``auraforge project`` CLI — manage first-class, multi-folder Projects.
...
convention. State lives in the per-profile ``$AURA_FORGE_HOME/projects.db`` store
(see :mod:`auraforge_cli.projects_db`).
...
def build_parser(
    ...
    p_create.add_argument("name", help="Human name, e.g. 'Aura Forge Agent'")
...
def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``auraforge project …`` argparse dispatch."""
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if not projs:
        print("No projects yet. Create one with `auraforge project create <name>`.")
        return 0
    for p in projs:
...
def _cmd_set_primary(args, conn, proj) -> int:
    if not pdb.set_primary(conn, proj.id, args.path):
        print(
            ...
            f"add it first with `auraforge project add-folder`.",
            file=sys.stderr,
        )
        return 1
