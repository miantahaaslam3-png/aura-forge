...
web_server - reached via the late-binding seam in :mod:`auraforge_cli.web_deps`
...
from auraforge_cli.web_deps import late, LateState
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from auraforge_cli.mcp_config import _replace_mcp_servers
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the AURA_FORGE_HOME
...
async def mcp_oauth_callback(
    ...
    if flow is None:
        return HTMLResponse("<h1>OAuth flow expired</h1><p>Return to Aura Forge and try again.</p>", status_code=404)
    try:
    ...
    if error:
        return HTMLResponse("<h1>Authorization failed</h1><p>Return to Aura Forge for details.</p>", status_code=400)
    return HTMLResponse("<h1>Authorization received</h1><p>You can close this tab and return to Aura Forge.</p>")
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `auraforge mcp catalog` / `auraforge mcp install` read.  ``profile`` scopes
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    from auraforge_cli.config import validate_env_var_name_for_write
    ...
    # The -p subprocess rebinds AURA_FORGE_HOME-derived paths in the child.
