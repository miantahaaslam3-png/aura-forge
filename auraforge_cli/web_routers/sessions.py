...
the late-binding seam in :mod:`auraforge_cli.web_deps` so tests that
...
async def search_sessions(
    ...
    try:
        ...
        try:
            ...
            # logs, or another Aura Forge surface. FTS can't find those unless the
...
async def import_sessions_endpoint(request: Request):
    ...
    restores a whole Aura Forge backup archive, while this endpoint is scoped to
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `auraforge sessions stats`).
