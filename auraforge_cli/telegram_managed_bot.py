...
manual BotFather token copy-paste. Aura Forge talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.aura-forge-agent.nousresearch.com"
...
DEFAULT_MANAGER_BOT = "Aura ForgeSetupBot"

DEFAULT_BOT_NAME = "Aura Forge Agent"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Aura Forge Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Aura Forge Telegram onboarding service.")
