"""
Single source of truth for provider identity in Aura Forge Agent.
...
2. **Aura Forge overlays** — transport type, auth patterns, aggregator flags,
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Aura Forge overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Aura Forge overlay
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The Aura Forge overlay still advertises openai_chat
...
def resolve_provider_full(
    ...
    # 0.5 Exact Aura Forge provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from auraforge_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
