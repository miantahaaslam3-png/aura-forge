"""
Baked-in build metadata for Aura Forge Agent.

Source installs report their git revision live via ``git rev-parse`` (see
``auraforge_cli/dump.py`` and ``auraforge_cli/banner.py``).  That doesn't work inside
...
To make ``auraforge dump`` and the startup banner identify the exact commit the
