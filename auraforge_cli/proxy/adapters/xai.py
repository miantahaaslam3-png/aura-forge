...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Aura Forge-managed OAuth credentials."""

    auth_hint = "auraforge auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`auraforge auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`auraforge auth reset xai-oauth` or re-authenticate with "
                    "`auraforge auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `auraforge auth add xai-oauth --type oauth`."
            )
