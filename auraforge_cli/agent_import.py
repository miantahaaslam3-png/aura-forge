...
``optional-skills/migration/openclaw-migration/scripts/openclaw_to_auraforge.py``):
...
# Same entry delimiter as the Aura Forge memory store and the openclaw migration
...
# ---------------------------------------------------------------------------
# Memory-entry primitives (ported from openclaw_to_auraforge.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Aura Forge command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Aura Forge glob.
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Aura Forge equivalent — "
...
def import_agent_command(args) -> None:
    """Handle ``auraforge import-agent`` (invoked from auraforge_cli.main)."""
    from auraforge_cli.config import get_config_path, load_config, save_config
    ...
    print(color("│          ⚕ Aura Forge — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))
...
def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    ...
    from auraforge_cli.setup import Colors, color, print_header, print_info
