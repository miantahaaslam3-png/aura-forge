...
Used by ``auraforge profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``auraforge_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Aura Forge Agent kanban board.
...
Rules:
  ...
  - Never write "Aura Forge Agent profile" or other meta-narration.
...
def describe_profile(
    ...
    *,
    ...
    try:
        if canon == "default":
            from auraforge_constants import get_aura_forge_home  # type: ignore
            profile_dir = Path(get_aura_forge_home())
        else:
