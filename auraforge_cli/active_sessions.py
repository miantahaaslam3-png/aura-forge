...
def active_session_limit_message(
    ...
    return (
        f"Aura Forge is at the active session limit ({active_count}/{max_sessions})."
...
class ActiveSessionLease:
    ...
    # root ``AURA_FORGE_HOME`` must release against the same registry even when
...
def release_active_session(lease: ActiveSessionLease) -> None:
    ...
    # running under a profile AURA_FORGE_HOME override (#85431).
...
def release_orphaned_leases(live_lease_ids: set[str]) -> int:
    ...
    that runs for days (``auraforge dashboard`` / ``serve``) never trips that
