"""Aura Forge update pipeline — extracted from ``auraforge_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``auraforge_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``auraforge_cli.main`` reference — so existing call sites and test monkeypatches
that target ``auraforge_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``auraforge_cli.main``.
...
Imports are one-way: ``auraforge_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``auraforge_cli.main`` reference.

    Lets callers keep patching ``auraforge_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``auraforge_cli.main`` -> ``auraforge_cli.update_cmd`` stays one-way
    at import time.
    """
    from auraforge_cli import main
...
_STALE_PURGE_PROTECTED = frozenset(
    {
        ...
        "auraforge_cli.auraforge_logging",
    }
)


def _purge_stale_hermes_modules() -> None:
    """Evict every cached Aura Forge module after the checkout changed in-place.

    ``auraforge update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from auraforge_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``auraforge_cli.gateway`` does
    ``from auraforge_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the Aura Forge
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale Aura Forge module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale Aura Forge modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``auraforge update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``auraforge update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``auraforge_cli.config_defaults``,
    ``auraforge_cli.config``, and ``auraforge_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``auraforge_cli._subprocess_compat`` and
    ``auraforge_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "auraforge_cli.config_defaults",
        "auraforge_cli.config",
        "auraforge_cli.config_migrations",
        "auraforge_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from auraforge_cli.config import check_config_version
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    shared checkout serves every profile, but ``auraforge update`` historically
    ...
    caller): scope config reads/writes via the context-local AURA_FORGE_HOME
    ...
    try:
        ...
        from auraforge_cli.profiles import _get_profiles_root, _PROFILE_ID_RE
...
def _check_and_apply_config_migration(
    *,
    ...
    try:
        ...
        print("     Run 'auraforge config migrate' to check manually.")
        return
    ...
    if version_bump_only:
        ...
        try:
            ...
            print("     Run 'auraforge config migrate' to retry.")
        ...
        if assume_yes:
            ...
            try:
                ...
                print(
                    ...
                    "Run 'auraforge config migrate' manually to configure."
                )
                response = "n"
        ...
        if response in {"", "y", "yes", "auto"}:
            ...
            if (gateway_mode or assume_yes or response == "auto") and missing_env:
                print("  ℹ API keys require manual entry: auraforge config migrate")
        else:
            print()
            print("Skipped. Run 'auraforge config migrate' later to configure.")
    else:
    ...
    # context-local AURA_FORGE_HOME override (never os.environ — other
    ...
    try:
        from auraforge_cli.backup import restore_cron_jobs_all_profiles
...
# Critical files that Aura Forge must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "auraforge_cli/main.py",
    "auraforge_cli/config.py",
    "auraforge_cli/__init__.py",
    "auraforge_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "auraforge_constants.py",
)
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    Skipping is safe because Aura Forge pins its editable finder to a *static*
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``auraforge`` startup; if any of them
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``auraforge update`` can be driven by a
    ...
    """
    from auraforge_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from auraforge_constants so this can't drift
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``auraforge update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `auraforge update`.
...
def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `auraforge update`.
    ...
    if interrupted:
        ...
        print(
            "  A previous `auraforge sessions optimize-storage` run was "
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``auraforge update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``auraforge update`` invocations skip the block until a newer
...
def _reload_process_scan_modules() -> None:
    ...
    ``from auraforge_cli._subprocess_compat import bounded_probe_run``. If the
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``auraforge_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `auraforge update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``auraforge_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``auraforge update``
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    print("  Stash or commit your changes, then rerun `auraforge update`.")
...
def _clear_stale_sqlite_sidecars(db_path: Path) -> None:
    ...
    database (a crashed writer, or a second Aura Forge process the updater's drain
...
def _print_update_summary(
    *,
    ...
    if node_failures or not desktop_build_ok:
        ...
        if not desktop_build_ok:
            print("  Run `auraforge desktop` to retry the desktop rebuild.")
    else:
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from auraforge_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    """Update Aura Forge Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `auraforge update "
            f"--branch {branch}`, or update against main with `auraforge update`."
        )
        _m().sys.exit(1)
    _abort_zip_update_if_dirty_tree()
    zip_url = (
        f"https://github.com/miantahaaslam3-png/aura-forge/archive/refs/heads/{branch}.zip"
    )
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for aura-forge-agent itself
        ...
        # GitHub ZIPs extract to aura-forge-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"aura-forge-agent-{branch}")
        ...
        try:
            for item in entries:
                ...
                # Aura Forge.exe) exists only in the LIVE tree. Swapping `apps`
        ...
        print(
            "  Re-run `auraforge update` to retry; if the agent won't start, "
            "reinstall from https://aura-forge-agent.nousresearch.com"
        )
        _m().sys.exit(1)
    finally:
    ...
    from auraforge_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if not import_ok:
        ...
        print("  Re-run `auraforge update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        from auraforge_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from auraforge_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
    ...
    try:
        from auraforge_cli.update_receipt import finalize_update_receipt
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``auraforge update --keep-stash`` (the desktop updater's mode): the
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Aura Forge behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Aura Forge couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Aura Forge couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Aura Forge behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Aura Forge couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Aura Forge couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/miantahaaslam3-png/aura-forge.git",
    "git@github.com:miantahaaslam3-png/aura-forge.git",
    "https://github.com/miantahaaslam3-png/aura-forge",
    "git@github.com:miantahaaslam3-png/aura-forge",
}

OFFICIAL_REPO_URL = "https://github.com/miantahaaslam3-png/aura-forge.git"
...
def _sync_with_upstream_if_needed(
    ...
    *,
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Aura Forge repository.")
        print("  This means you may miss updates from miantahaaslam3-png/aura-forge.")
        print()
        ...
        if assume_yes or (
            ...
            print(
                "  Add it later with: git remote add upstream https://github.com/miantahaaslam3-png/aura-forge.git"
            )
            return False
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/miantahaaslam3-png/aura-forge.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/miantahaaslam3-png/aura-forge.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``auraforge update``, every profile is now current.
    """
    homes = []
...
# ``fleet_restart_pending`` lives under AURA_FORGE_HOME (not next to the venv).
...
def _current_checkout_sha() -> str | None:
    ...
    try:
        from auraforge_cli.build_info import get_code_identity
...
def _receipt_reports_stale_runtime(expected_sha: str | None = None) -> bool:
    ...
    """
    try:
        from auraforge_cli.update_receipt import read_latest_receipt
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print(
        "⚠ A previous `auraforge update` pulled new code but did not "
    ...
    if startup:
        print(
            "  Run `auraforge update` or `auraforge gateway restart`.",
            file=stream,
        )
...
def _apply_pending_fleet_restart_catchup() -> None:
    """On an already-up-to-date ``auraforge update``, finish a skipped restart.
...
def _format_concurrent_instances_message(
    ...
    lines.append("")
    lines.append("  Close Aura Forge Desktop, exit any open `auraforge` REPLs, and")
    lines.append("  stop the gateway (`auraforge gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `auraforge update --force` if you've already")
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from auraforge_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _filter_non_gateway_concurrent_instances(
    ...
    ``auraforge update``. If every concurrent instance is a gateway, the pause
    ...
    TUI shell, a Aura Forge Desktop backend child, an unrelated ``auraforge`` REPL)
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``auraforge tools``."""
    try:
        from auraforge_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Aura Forge Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``auraforge tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Aura Forge Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Aura Forge Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``auraforge update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `auraforge update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `auraforge update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Aura Forge' editable-install extras or
    ...
    try:
        from auraforge_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from auraforge_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$AURA_FORGE_HOME/bin/uv``, but on Termux the official
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `auraforge update` is exactly the step
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `auraforge update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `auraforge update`.")
            failed = []
            if any(
    ...
    # Aura Forge profile using this checkout. Keep one per-checkout cache under the
    # shared Aura Forge root rather than rerunning npm once per named profile.
    ...
    # `auraforge update` case. Synchronous and can block ~11s on a true cold
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `auraforge update`.")
        return list(labels)
    ...
    # `auraforge update` comes from the *desktop* build, not this step; that
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.aura-forge/logs/update.log`` only, never the terminal.

    During ``auraforge update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``auraforge update --check``: fetch and report without installing.
    ...
    from auraforge_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``auraforge update`` without requiring a reinstall.  The
    ...
    path_comment = (
        "# Aura Forge Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    ...
    The shim simply delegates to the sibling ``auraforge`` launcher with the
    ...
    ``auraforge_cli._install_repair.ensure_windows_bin_launchers`` re-stages
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        ...
        try:
            ...
            shim = (
                "#!/usr/bin/env bash\n"
                "# Aura Forge Agent — ACP launcher (written by `auraforge update`).\n"
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of AURA_FORGE_HOME under
      ``backups/`` (restorable via ``auraforge import``; the #48200 wrong-path
    ...
    try:
        ...
        # NOTE: this function later does `from auraforge_constants import
        ...
        from auraforge_cli.config import get_aura_forge_home as _get_home
        ...
        try:
            from auraforge_cli.backup import create_pre_update_snapshots_all_profiles
            ...
            if _sibling_snaps:
                ...
                try:
                    from auraforge_cli.update_receipt import record_step
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``auraforge update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``auraforge update`` on a current checkout prints
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    Windows: the Desktop app's backend (``python.exe -m auraforge_cli.main
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m auraforge_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "auraforge_cli.main" in cmdline_low:
        ...
        # the `-m auraforge_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``auraforge_cli.main`` used to import
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `auraforge` launch will complete it")
...
def _hermes_holder_subcommand(cmdline: str) -> str | None:
    """The actual Aura Forge SUBCOMMAND a venv-holder argv runs, or None.

    Token-based, never substring (#90778: ``kanban --preserve-cache``
    contained \"serve\" and got labeled as the Desktop backend). Finds the
    ``auraforge_cli.main`` / ``auraforge(.exe)`` entry token, then returns the
    ...
    for i, token in enumerate(tokens):
        ...
        if low.endswith("auraforge_cli.main") and i > 0 and tokens[i - 1] == "-m":
            entry_idx = i
            break
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    (#90778): a standalone ``auraforge dashboard`` must not be labeled as the
    ...
    """
    lines = [
        "✗ Other Aura Forge processes are running from this install's venv:",
    ]
    hint_by_subcommand = {
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``auraforge update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from auraforge_cli._scan_venv_blockers import _is_pausable_gateway
...
def _ledger_manual_serve_holders(
    ...
    Aura Forge spawner). Returns the full ledger entries so the relauncher can
    rebuild the launch command from structured host/port/profile instead of
    parsing argv.
    """
    try:
        from auraforge_cli.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _orphaned_desktop_backend_pids(
    ...
    the update with "Aura Forge is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Aura Forge backend (``auraforge_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    def _is_backend(argv_low: str) -> bool:
        return "auraforge_cli.main" in argv_low and (
...
def _ledger_reapable_backend_pids(
    ...
    (``auraforge_cli.process_identity``). A holder qualifies when ALL of:
    ...
    """
    try:
        from auraforge_cli.process_identity import (
            REAPABLE_PURPOSES,
            ledger_entries,
            spawner_is_dead,
        )
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of Aura Forge ``serve``/``dashboard`` backends safe to reap during a
    ...
    - Only Aura Forge backends (``auraforge_cli.main`` + ``serve``/``dashboard``)
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``auraforge serve`` / ``auraforge dashboard`` argv.
    ...
    """
    if "auraforge_cli.main" not in (cmdline or "").lower():
        return False
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    print("→ Stopping Windows gateway process(es) before updating Aura Forge...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m auraforge_cli.main
...
def _cold_start_windows_gateway_after_update() -> bool:
    ...
    ``auraforge gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from auraforge_cli import gateway_windows
        from auraforge_cli.gateway import find_gateway_pids
    except Exception as exc:
        raise RuntimeError(
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from auraforge_cli.gateway import is_macos
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``auraforge_cli.gateway`` itself no longer imports, which is one of the
...
def _gateway_recovery_partition(
    ...
    only safe source here: re-importing ``auraforge_cli.gateway`` in the failing
...
def _recover_gateway_restart_after_abort(
    ...
    ``auraforge update`` normally performs the fleet restart in the interpreter
    ...
    command = [
        sys.executable,
        "-m",
        "auraforge_cli.update_restart_recovery",
        "--stdin",
    ]
    env = os.environ.copy()
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``auraforge_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``auraforge update`` never touched them, so installs created before the
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``auraforge update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``auraforge update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `auraforge desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "auraforge_cli.main", "desktop", "--build-only"]
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `auraforge desktop` to retry)")
        tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
        if tail:
            print(tail)
        from auraforge_constants import display_aura_forge_home as _dhh
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Aura Forge Agent...")
    print()
    ...
    try:
        from auraforge_cli.update_receipt import begin_update_receipt
    ...
    # running Aura Forge runtime, its supervisor, and its running code version —
    ...
    # (most commonly the Desktop app's `auraforge serve` backend) will keep .pyd
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned Aura Forge backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "Aura Forge backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            # `auraforge serve --host <ip>` powering a REMOTE Desktop holds the
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving Aura Forge backends are leaks,
            ...
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} Aura Forge backend process(es) "
    ...
    # process by design (a CLI `auraforge update` IS the venv python), and an
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://aura-forge-agent.nousresearch.com/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Aura Forge gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in auraforge_cli/config.py bricked
        # every user who ran ``auraforge update`` for the 7 minutes between
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``auraforge update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``auraforge update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``auraforge doctor`` healthy. Compare pre-pull
        ...
        # ``auraforge update`` can catch up even when git is already up to date.
        ...
        # the old bytecode (e.g. get_aura_forge_home added to auraforge_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``auraforge`` launch finishes the install
        ...
        # symbols in auraforge_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `auraforge` again "
        ...
        if not import_ok:
            ...
            print("    Run `auraforge update` again — if it persists, reinstall:")
            print("    https://aura-forge-agent.nousresearch.com")
        ...
        if sys.platform == "darwin" and had_desktop_app_before_update:
            print()
            print(
                "  ℹ macOS: if Aura Forge re-prompts for permissions you already "
                "granted (toggle shows ON), the stored grant is stale — run "
                "`tccutil reset ScreenCapture com.auraforge.desktop` (repeat "
        ...
        try:
            from auraforge_cli.macos_tcc_anchor import ensure_tcc_anchor
        ...
        # current — so copy it straight over ~/.aura-forge/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit AURA_FORGE_HOME so
        # it is not affected by sync_skills()'s module-level AURA_FORGE_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's AURA_FORGE_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from auraforge_cli.profiles import backfill_profile_envs
        ...
        # check `auraforge curator status`. Self-stamps after printing so it
        ...
        # --keep-stash (the desktop updater), never restored — `auraforge`
        ...
        # the managed clone under the default Aura Forge root).
        try:
            from auraforge_cli._install_repair import migrate_windows_bin_path
        ...
        # don't have it. Tying the refresh to ``auraforge update`` gives users a
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `auraforge update`
                # must stay fast; `auraforge computer-use install --upgrade`
        ...
        # When running as ``auraforge update --gateway`` (spawned by the gateway's
        ...
        #
        # Purge stale cached Aura Forge modules FIRST: the import below pulls
        ...
        try:
            ...
            try:
                from auraforge_cli.gateway import _get_restart_exit_wait_budget
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `auraforge update` restart it automatically, allow\n"
                        ...
                        # path in `auraforge gateway restart`
            ...
            try:
                from auraforge_cli.update_receipt import record_gateway_restart
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy Aura Forge gateway unit(s) detected:")
        ...
        print("  auraforge model              # Select provider and model")
        ...
        try:
            from auraforge_cli.update_receipt import (
        ...
        try:
            if _pre_update_plan is not None and _pre_update_plan.runtimes:
                ...
                if report_unaccounted_runtimes(_runtime_outcomes):
                    gateway_fleet_restart_incomplete = True
                try:
                    import auraforge_cli.update_receipt as _ur
        ...
        if gateway_fleet_restart_incomplete:
            ...
            # ``auraforge update`` still runs the catch-up restart.
            sys.exit(1)
        _clear_fleet_restart_pending_marker()
            ...
            if _called_process_error_is_python_dep_install(e):
                ...
                if _m()._is_windows():
                    ...
                    print(
                        ...
                        '"from auraforge_cli.main import main; main()" update --yes'
                    )
            try:
                from auraforge_cli.update_receipt import finalize_update_receipt
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``auraforge_cli.gateway`` no longer imports,
