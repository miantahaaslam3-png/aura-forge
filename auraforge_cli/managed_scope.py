...
A system-level directory (default ``/etc/auraforge``, root-owned and not
...
This is DISTINCT from ``auraforge_cli.config.is_managed()`` / ``HERMES_MANAGED``,
...
_DEFAULT_MANAGED_DIR = Path("/etc/auraforge")
...
def _under_pytest() -> bool:
    ...
    Used to ignore the system default ``/etc/auraforge`` during tests so a real
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      ...
      2. ``/etc/auraforge`` — POSIX default, when it exists. Ignored under pytest so
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through auraforge_cli.config.load_config) to honor managed
    scope. Mirrors auraforge_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from auraforge_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
