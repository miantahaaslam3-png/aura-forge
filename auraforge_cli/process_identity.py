...
2. **Spawn ledger** (``spawn-ledger.json`` at the machine Aura Forge root): every
...
def install_id(project_root: Optional[Path] = None) -> str:
    ...
    Lets a reaper reject processes from a different Aura Forge install on the
    same machine without path comparisons at scan time.
    """
    if project_root is None:
        try:
            from auraforge_constants import PROJECT_ROOT as _root
...
def register_child(
    ...
    *,
    ...
    ``npx``/binary servers never import Aura Forge code). The entry records the
