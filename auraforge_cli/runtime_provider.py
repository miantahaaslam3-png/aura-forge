...
from auraforge_cli import config as _config_mod
from auraforge_cli.providers import custom_provider_aliases, custom_provider_slug
from auraforge_constants import OPENROUTER_BASE_URL
from auraforge_cli.providers import is_official_openai_host


def load_config():
    """Late-bound delegate to :func:`auraforge_cli.config.load_config`.

    Deliberately NOT a module-level ``from auraforge_cli.config import
    load_config``: this module is often imported lazily (inside functions),
    so its first import can happen while a test has
    ``auraforge_cli.config.load_config`` patched — a from-import would then
    ...
    patching ``auraforge_cli.config.load_config`` OR
    ``auraforge_cli.runtime_provider.load_config`` behaves as expected.
    """
...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from auraforge_cli.auth import resolve_provider as _resolve_provider
...
def _copilot_runtime_api_mode(
    ...
    *,
    ...
    try:
        from auraforge_cli.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # instead of Aura Forge' tool dispatch. Gated behind config key
...
def _parse_api_mode(raw: Any) -> Optional[str]:
    ...
    if isinstance(raw, str):
        from auraforge_cli.config import _canonical_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from auraforge_cli.models import azure_foundry_model_api_mode
        ...
        if opencode_provider_family(provider) is not None:
            ...
            # Refs #16878.
            from auraforge_cli.models import opencode_model_api_mode
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the built-in. See tests/auraforge_cli/test_runtime_provider_resolution.py
...
def resolve_runtime_provider(
    *,
    ...
    from auraforge_cli.config import is_provider_enabled, load_config
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "auraforge-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from auraforge_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Aura Forge canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Aura Forge-compatible importers).
