...
Pure logic: walks a Aura Forge config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `auraforge doctor` and a future `auraforge migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Aura Forge config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Aura Forge config and return retirement issues.
...
def apply_migration(
    ...
    from auraforge_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `auraforge migrate xai` reports nothing to migrate
