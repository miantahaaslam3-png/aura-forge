"""
MCP Server Management CLI — ``auraforge mcp`` subcommand.

Implements ``auraforge mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.aura-forge/config.yaml under the ``mcp_servers`` key.
"""
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from auraforge_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.aura-forge/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``auraforge mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    # auraforge_cli/main.py for why the dest is renamed.
...
def cmd_mcp_remove(args):
    ...
    # earlier `auraforge mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``auraforge mcp login`` and
    ``auraforge mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `auraforge mcp remove` + `auraforge mcp add` to reconfigure auth.")
        return False
    ...
    #
    # force_interactive_oauth: `auraforge mcp login` is *explicitly* user-
    # initiated even when stdin isn't a TTY (Aura Forge desktop / agent-
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `auraforge mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``auraforge mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``auraforge mcp reauth --all`` discovers every ``auth: oauth`` server in
...
def mcp_command(args):
    """Main dispatcher for ``auraforge mcp`` subcommands."""
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `auraforge plugin`.
        from auraforge_cli.mcp_picker import run_picker
        run_picker()
