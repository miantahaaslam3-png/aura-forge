...
def auth_add_command(args) -> None:
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``auraforge auth add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``auraforge auth add xai-oauth`` overwrote the
    ...
    raise SystemExit(f"`auraforge auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Aura Forge reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `auraforge auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `auraforge auth` is called bare."""
