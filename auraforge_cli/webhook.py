"""auraforge webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    auraforge webhook subscribe <name> [options]
...
def _setup_hint() -> str:
  ...
  Then start the gateway: auraforge gateway run
"""
...
def webhook_command(args):
    """Entry point for 'auraforge webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: auraforge webhook {subscribe|list|remove|test}")
        print("Run 'auraforge webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (auraforge gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: auraforge webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from auraforge webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (auraforge gateway run)")
