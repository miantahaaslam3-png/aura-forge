...
from auraforge_cli.auth import AuthError, resolve_provider
from auraforge_cli.colors import Colors, color
from auraforge_cli.config import get_env_path, get_env_value, get_aura_forge_home, load_config
from auraforge_cli.models import provider_label
...
from auraforge_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``auraforge config``'s
...
def _estop_status_line():
    """One-line pause banner for `auraforge status`, or None when not paused.

    Cheap: a single stat on $AURA_FORGE_HOME/ESTOP via agent.estop.
    """
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{suffix}; `auraforge resume` to lift)"


def show_status(args):
    """Show status of all Aura Forge Agent components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Aura Forge Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `auraforge status`
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: auraforge model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: auraforge auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: auraforge auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: auraforge model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from auraforge_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'aura-forge-agent[vercel]')"
        ...
        # provider's doctor rows (fail-soft — never break `auraforge status`).
        try:
    ...
    try:
        from auraforge_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    print(color("  Run 'auraforge doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'auraforge setup' to configure", Colors.DIM))
    print()
