...
This module is imported by ``auraforge_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no auraforge_cli.config,
...
``auraforge_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from auraforge_cli.banner import format_banner_version_label

        print(format_banner_version_label())
    except Exception:
        from auraforge_cli import __release_date__, __version__

        print(f"Aura Forge Agent v{__version__} ({__release_date__})")
    ...
    try:
        from auraforge_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
