...
One read-only pass that answers, BEFORE any mutation: what Aura Forge runtimes
...
@dataclass
class RuntimeRecord:
    """One running (or expected) Aura Forge runtime on this machine."""
...
class UpdatePlan:
    ...
    update_mechanism: str = "auraforge update"
...
def _detect_supervisor_for_pid(
    ...
    if pid in service_pids:
        try:
            from auraforge_cli.gateway import is_macos, supports_systemd_services
...
def describe_restart_mechanism(mechanism: str, profile: str) -> str:
    ...
    if profile != "default":
        return f"auraforge -p {profile} gateway restart"
...
def collect_runtime_inventory() -> UpdatePlan:
    ...
    try:
        from auraforge_cli.gateway import find_windows_gateway_services
    ...
    try:
        from auraforge_cli.gateway import find_profile_gateway_processes
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running Aura Forge services: none detected — code swap only.")
        return
    ...
    try:
        ...
        for runtime in plan.runtimes:
            ...
            if r.profile in relaunched or r.profile in external:
                ...
                r.profile in unit or (r.profile == "default" and "auraforge-gateway" in unit)
                for unit in failed_set
            ):
                outcome = "failed"
            elif any(
                r.profile in svc or (r.profile == "default" and "auraforge-gateway" in svc)
