"""
auraforge fallback — manage the fallback provider chain.
...
Subcommands:
  auraforge fallback [list]   Show the current fallback chain (default when no subcommand)
  auraforge fallback add      Pick provider + model via the same picker as `auraforge model`,
                           then append the selection to the chain
  auraforge fallback remove   Pick an entry to delete from the chain
  auraforge fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.aura-forge/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from auraforge_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `auraforge model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  auraforge fallback add")
        print()
        return
    ...
    print("  Docs: https://aura-forge-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `auraforge model`, then append the selection to the chain."""
    from auraforge_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `auraforge model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `auraforge fallback list` to view, or `auraforge fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``auraforge fallback [subcommand]``."""
