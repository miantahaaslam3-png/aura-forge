"""Windows ConPTY bridge for the `auraforge dashboard` chat tab.

Drop-in counterpart to ``auraforge_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``auraforge_cli.web_server`` consumes: ``spawn``, ``read``,
