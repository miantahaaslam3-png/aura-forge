"""Helpers for loading Aura Forge .env files consistently across entrypoints."""
...
# several hot modules (cli.py, auraforge_cli/main.py, run_agent.py,
...
def _known_hermes_env_keys() -> set[str]:
    """Return the combined set of known Aura Forge env-var keys.
    ...
    """
    from auraforge_cli.config import _EXTRA_ENV_KEYS
    from auraforge_cli.config_defaults import OPTIONAL_ENV_VARS

    return set(OPTIONAL_ENV_VARS.keys()) | set(_EXTRA_ENV_KEYS)


# Behavioral routing keys a parent Aura Forge process injects into child env and
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    """Remove inherited profile-managed Aura Forge keys absent from ``.env``.
    ...
    parent Aura Forge process injects and that silently change *which provider
    ...
    ``tests/auraforge_cli/test_dump_env_visibility.py``), and a startup scrub
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``auraforge_cli.config._sanitize_env_lines`` normalizes line endings while
    treating content after the first ``=`` as opaque for boundary discovery.
    """
    if not path.exists():
        return
    try:
        from auraforge_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_hermes_dotenv(
    *,
    ...
    if user_env.exists():
        ...
        # Mirror reload_env() known-key cleanup so inherited Aura Forge keys
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Delegates to ``auraforge_cli.config.apply_terminal_config_to_env`` — the
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Aura Forge reads
    ...
    time from several hot modules (cli.py, auraforge_cli/main.py,
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # shared cache here lets main.py's early bridge and auraforge_logging reuse
