"""
Unified tool configuration for Aura Forge Agent.

`auraforge tools` and `auraforge setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.aura-forge/config.yaml under
...
from auraforge_cli.nous_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    NousSubscriptionFeatures,
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from auraforge_cli.nous_account import format_nous_portal_entitlement_message
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``auraforge tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`auraforge_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from auraforge_cli.cli_output import (  # noqa: E402 — late import block
...
# who want it opt in via `auraforge tools` → Video Generation, which walks
...
# `auraforge tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `auraforge tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from auraforge_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `auraforge tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``auraforge tools`` → "reconfigure existing" would
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from auraforge_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``auraforge tools`` checklist actually offers
    for ``platform``.
    ...
    so ``auraforge tools`` never claims to add or remove a toolset the user was
...
from auraforge_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Aura Forge routes X searches through xAI's built-in x_search "
...
# provider+model picker like `auraforge model` — NOT this single-key prompt, so
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Aura Forge telemetry policy applied.
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports Aura Forge' runtime contract."""
    import time
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $AURA_FORGE_HOME/bin is never on PATH, so a bare which()
    # misses the uv Aura Forge installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``auraforge update`` and
      by ``auraforge computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``auraforge update`` sets
    ...
    a further ~600s wait on Windows). ``auraforge computer-use install
    ...
    ``auraforge update`` already prints a contextual line before its update
    ...
    ``auraforge update`` and must not inherit the explicit install command's
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `auraforge update` calls this
    ...
    # An installed driver that fails Aura Forge' runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # Aura Forge' own minimum requirement IS the confirmation that an upgrade is
    ...
    # contract landed, with `auraforge update` declining to refresh).
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but Aura Forge cannot use its current "
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `auraforge computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `auraforge update` (require_confirmed_update=True) keeps the
    ...
    # `auraforge computer-use install --upgrade` falls through and re-runs the
    # installer as before.
...
# Optional refreshes launched by ``auraforge update`` are quiet and unattended.
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    # Unattended refreshes (installer_timeout set by `auraforge update`) fail
    ...
    def _reap_after_timeout(proc):
        ...
        ceiling stops bounding anything and ``auraforge update`` hangs past its
    ...
    try:
        # When not verbose (e.g. `auraforge update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `auraforge update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.aura-forge/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and installed_binary:
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Aura Forge process.")
            return True
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/aura-forge-agent:latest"
            )
            return
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.aura-forge/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.aura-forge/config.yaml")
        ...
        # Run the full `auraforge auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.aura-forge/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from auraforge_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: auraforge auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `auraforge auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: auraforge auth spotify")
        except Exception as exc:
        ...
        try:
            from auraforge_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
        ...
        try:
            from auraforge_cli.auth import get_xai_oauth_auth_status
        ...
        try:
            ...
            _print_info("    Run later: auraforge auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `auraforge auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: auraforge auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: auraforge auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``auraforge tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``auraforge tools post-setup <key>`` — non-interactive post-setup runner.
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``auraforge tools``
...
def _get_platform_tools(
    ...
    *,
    ...
    if has_explicit_config:
        ...
        # through ``auraforge tools`` to flip the toolset on. Only fires when
    ...
    # shipped as a bundled plugin but user must opt in via `auraforge tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `auraforge tools`
    ...
    # JSON-array string (e.g. "['memory']") from `auraforge config set` or a
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `auraforge tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Opening `auraforge tools` is the user's opt-in to reconfigure tools, so treat
...
def _estimate_tool_tokens() -> Dict[str, int]:
    ...
    from auraforge_constants import aura_forge_home_key
...
def _prompt_toolset_checklist(
    ...
    *,
    ...
    from auraforge_cli.curses_ui import curses_checklist
    from toolsets import resolve_toolset
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `auraforge tools` gets a silent no-op
...
# Python dependencies installed explicitly through ``auraforge tools`` are not
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``auraforge tools`` Python dependencies present in this runtime."""
    return [
        name
...
def _agent_browser_installed() -> bool:
    ...
    from auraforge_cli.nous_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``auraforge tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from auraforge_cli.nous_subscription import _has_agent_browser
...
# Kept in sync with the dashboard selects (auraforge_cli/web_server.py
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `auraforge model`).
    if managed_feature:
        from auraforge_cli.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``auraforge model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``auraforge model`` aux-task
    ...
    if not providers:
        _print_warning(
            ...
            "with `auraforge model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `auraforge model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `auraforge tools` and `auraforge setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Aura Forge Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://aura-forge-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
