"""
Interactive setup wizard for Aura Forge Agent.
...
from auraforge_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
from auraforge_cli.nous_subscription import get_nous_subscription_features
from tools.tool_backend_helpers import managed_nous_tools_enabled
from auraforge_constants import get_optional_skills_dir
...
from auraforge_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from auraforge_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Aura Forge Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
...
def is_noninteractive() -> bool:
    ...
    ``HERMES_NONINTERACTIVE=1`` (see ``auraforge_cli/web_server.py``). In that
...
def _print_setup_summary(config: dict, aura_forge_home):
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from auraforge_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
...
def _install_neutts_deps() -> bool:
    ...
    from auraforge_cli.tools_config import _pip_install
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Aura Forge auth store) > existing XAI_API_KEY > prompt the user.
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Aura Forge runs shell commands and code.")
...
def _setup_telegram_auto_result():
    ...
    try:
        from auraforge_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Aura Forge delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Aura Forge to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from auraforge_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from auraforge_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Aura Forge from anywhere.")
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Aura Forge profile and are not uploaded.")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``auraforge_cli.auth`` — lists every supported
    ...
    """
    try:
        from auraforge_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from auraforge_cli.config
    ...
    if section_key == "model":
        ...
        from auraforge_cli.gateway import _all_platforms, _platform_status
...
# Config values may have different semantics between OpenClaw and Aura Forge.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Aura Forge to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Aura Forge at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Aura Forge at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Aura Forge at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Aura Forge at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Aura Forge equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Aura Forge config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Aura Forge.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Aura Forge's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(aura_forge_home: Path) -> bool:
    ...
    print_info("Aura Forge can preview what would be imported before making any changes.")
    print()
    ...
    # Execute the migration — overwrite=False so existing Aura Forge configs are
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    print(color("│     ⚕ Aura Forge Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    try:
        from auraforge_cli.main import _model_flow_nous
...
def _run_setup_wizard_impl(args):
    ...
    """
    from auraforge_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Aura Forge Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Aura Forge Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Aura Forge Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Aura Forge configured.")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Aura Forge?",
            [
    ...
    def _gateway_step() -> None:
        ...
        from auraforge_cli.gateway import ensure_gateway_service
...
def _print_macos_fda_tip() -> None:
    ...
    print_info("  your terminal (and AuraForge.app if you use Desktop), or run:")
    print_info("    open \"x-apple.systempreferences:com.apple.preference"
               ".security?Privacy_AllFiles\"")
    print_info("  The grant is permanent — it survives every Aura Forge update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        from auraforge_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
