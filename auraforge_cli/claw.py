"""auraforge claw — OpenClaw migration commands.

Usage:
    auraforge claw migrate              # Preview then migrate (always shows preview first)
    auraforge claw migrate --dry-run    # Preview only, no changes
    auraforge claw migrate --yes        # Skip confirmation prompt
    auraforge claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    auraforge claw migrate --no-backup  # Skip pre-migration snapshot
    auraforge claw cleanup              # Archive leftover OpenClaw directories
    auraforge claw cleanup --dry-run    # Preview what would be archived
"""
...
from auraforge_cli.config import get_aura_forge_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Aura Forge may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print()
    print_error(
        "Aura Forge gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'auraforge gateway stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_auraforge.py script in known locations."""
...
def claw_command(args):
    """Route auraforge claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: auraforge claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Aura Forge")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'auraforge claw <command> --help' for options.")
...
def _cmd_migrate(args):
    ...
    # migrate-auraforge posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: auraforge claw migrate --source /path/to/.openclaw")
        return
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: auraforge claw migrate --yes")
            return
    ...
    # Delegates to auraforge_cli.backup.create_pre_migration_backup(), which
    ...
    if not no_backup:
        try:
            from auraforge_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(aura_forge_home=aura_forge_home)
            if backup_archive:
                size_str = _format_size(backup_archive.stat().st_size)
                print()
                print_success(f"Pre-migration backup: {backup_archive} ({size_str})")
                print_info(f"Restore with: auraforge import {backup_archive.name}")
        except Exception as e:
            print()
    ...
    # 'auraforge claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: auraforge claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: auraforge claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  auraforge claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  auraforge claw migrate --migrate-secrets")
            print()
