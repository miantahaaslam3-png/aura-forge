...
Usage:
    auraforge pairing list              # Show all pending + approved users
    auraforge pairing approve <platform> <request-id|code>  # Approve a pairing request
    auraforge pairing revoke <platform> <user_id> # Revoke user access
    auraforge pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle auraforge pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: auraforge pairing {list|approve|revoke|clear-pending}")
        print("Run 'auraforge pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: auraforge pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.aura-forge/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'auraforge pairing list' to see pending requests.\n")
