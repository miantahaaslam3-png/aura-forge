...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or auraforge -c.",
    ...
    "Ctrl+Z suspends Aura Forge to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "auraforge -c resumes your most recent CLI session. auraforge -c \"project name\" resumes by title.",
    "auraforge -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "auraforge -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "auraforge chat -t web,terminal enables only specific toolsets for a focused session.",
    "auraforge chat -s github-pr-workflow preloads a skill at launch.",
    "auraforge chat -q \"query\" runs a single non-interactive query and exits.",
    "auraforge chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "auraforge chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "auraforge --yolo bypasses all dangerous command approval prompts for the entire session.",
    "auraforge chat --source telegram tags the session for filtering in auraforge sessions list.",
    "auraforge -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "auraforge doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "auraforge dump outputs a compact setup summary — great for bug reports.",
    "auraforge config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "auraforge config edit opens config.yaml in your default editor.",
    "auraforge config check scans for missing or stale configuration options.",
    "auraforge sessions browse opens an interactive session picker with search.",
    "auraforge sessions stats shows session counts by platform and database size.",
    "auraforge sessions prune --older-than 30 cleans up old sessions.",
    "auraforge skills search react --source skills-sh searches the skills.sh public directory.",
    "auraforge skills check scans installed hub skills for upstream updates.",
    "auraforge skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "auraforge skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "auraforge mcp add github --command npx adds MCP servers from the command line.",
    "auraforge mcp serve runs Aura Forge itself as an MCP server for other agents.",
    "auraforge auth add lets you add multiple API keys for credential pool rotation.",
    "auraforge completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "auraforge logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "auraforge backup creates a zip backup of your entire Aura Forge home directory.",
    "auraforge profile create coder creates an isolated profile that becomes its own command.",
    "auraforge profile create work --clone copies your current config and keys to a new profile.",
    "auraforge update syncs new bundled skills to ALL profiles automatically.",
    "auraforge gateway install sets up Aura Forge as a system service (systemd/launchd).",
    "auraforge memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "auraforge webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: auraforge tools disables unused tools, auraforge skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "auraforge models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as auraforge -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Aura Forge tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'auraforge profile create coder' creates the 'coder' command.",
    "auraforge profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Aura Forge prints a resume command with session ID and stats.",
    "auraforge sessions export backup.jsonl exports all sessions for backup or analysis.",
    "auraforge -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "auraforge skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .auraforge/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: auraforge cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Aura Forge runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "auraforge gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Aura Forge via a local macOS server.",
    ...
    "Context files (.auraforge.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Aura Forge loads project context from .auraforge.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "auraforge mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "auraforge mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "auraforge mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — auraforge tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.aura-forge/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "auraforge chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "auraforge plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "auraforge acp runs Aura Forge as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "auraforge auth add nous or auraforge auth add openai-codex sets up OAuth-based providers.",
    ...
    "auraforge status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.aura-forge/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Aura Forge your own.",
    ...
    "Before a gateway session reset, Aura Forge auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.aura-forge/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Aura Forge state via AURA_FORGE_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "AURA_FORGE_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in AURA_FORGE_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.aura-forge/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'auraforge profile create ls' would be rejected.",
    "auraforge profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "auraforge claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via auraforge tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.aura-forge/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.aura-forge/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Aura Forge auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Aura Forge config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Aura Forge is working.',
    ...
    '/reload reloads ~/.aura-forge/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.aura-forge/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.aura-forge/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'auraforge curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "auraforge curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'auraforge curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'auraforge auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'auraforge dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'auraforge dashboard embeds the full Aura Forge TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.aura-forge/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.aura-forge/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.aura-forge/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.aura-forge/skills/ so drop-in skills appear without restarting the session.',
    ...
    'auraforge -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'auraforge chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'auraforge chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'auraforge chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "auraforge chat --source tool tags programmatic chats so they don't clutter auraforge sessions list.",
    'auraforge dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'auraforge sessions rename <ID> "new title" renames any past session; auraforge sessions delete <ID> removes one.',
    'auraforge import restores a full Aura Forge backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'auraforge fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'auraforge pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'auraforge setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'auraforge status --deep runs the full health sweep across every component; plain auraforge status is the quick view.',
    ...
    'HERMES_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.auraforge/plugins/ — trust-gated by design.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Aura Forge block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from auraforge sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.aura-forge/dashboard-plugins/.',
]
...
# any project or none — Aura Forge is not a coding-only agent.
# ---------------------------------------------------------------------------
