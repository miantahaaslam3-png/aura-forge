...
from auraforge_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime
...
_MACOS_MANAGED_PYTHON_IDENTIFIER = "com.auraforge.desktop.managed-python"
...
def _macos_sign_managed_python(python: Path) -> bool:
    ...
    TCC with a cdhash-only identity that changes whenever Aura Forge provisions a
...
class _UvResult(str):
    ...
    the call site from the *old*, already-imported ``auraforge_cli.main`` against
...
def _reload_hermes_constants():
    """Re-execute ``auraforge_constants`` from disk and return the fresh module.

    ``auraforge update`` imports ``auraforge_constants`` from the OLD checkout,
    ...
    contains it, which is what made this read as a contradiction:
        ...
        (~/.aura-forge/aura-forge-agent/auraforge_constants.py)
    ...
    return importlib.reload(auraforge_constants)
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Aura Forge generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("auraforge_cli.main")
    ...
    if holders:
        ...
        return True, f"other Aura Forge processes still hold the venv (PID {pids})"
    return False, ""
