...
This is an additive Aura Forge extension surfaced under ACP ``_meta.auraforge`` so
...
compression rotates the internal Aura Forge head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.auraforge.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_hermes_session_id: The live internal Aura Forge DB session id
    ...
    Returns:
        A dict suitable for ``{"auraforge": {"sessionProvenance": <dict>}}`` under
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"auraforge": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"auraforge": {"sessionProvenance": prov}}
