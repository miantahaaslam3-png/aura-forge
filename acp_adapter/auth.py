"""ACP auth helpers — detect and advertise Aura Forge authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "auraforge-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Aura Forge runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Aura Forge can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Aura Forge.
    ...
    not have Aura Forge provider credentials configured yet, so Aura Forge always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Aura Forge using the currently configured "
