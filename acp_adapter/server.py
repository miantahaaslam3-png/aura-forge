"""ACP agent server — exposes Aura Forge Agent via the Agent Client Protocol."""
...
# already uses (`auraforge_cli/moa_cmd.py`). This bounds each provider's row, not
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Aura Forge running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Aura Forge cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Aura Forge/OpenAI-compatible user content payload."""
...
class HermesACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Aura Forge AIAgent."""
    ...
    _SLASH_COMMANDS = {
        ...
        "version": "Show Aura Forge version",
    }
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Aura Forge version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Aura Forge maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            ...
            from auraforge_cli.models import normalize_provider, provider_label
            ...
            def choice_provider(model_id: str) -> str:
                ...
                if parts[:1] == ["custom"] and len(parts) > 1:
                    from auraforge_cli.models import _configured_custom_provider_ids
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from auraforge_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Aura Forge estimates ``used`` from the same
    ...
    async def _send_session_info_update(
        ...
        *,
        ...
        """Send ACP native session metadata after Aura Forge changes it.
        ...
        # auraforge_state.py schema — only started_at/ended_at). Use "now" as
    ...
    def _schedule_mcp_late_refresh(self, state: SessionState) -> None:
        ...
        """
        try:
            from auraforge_cli.mcp_startup import mcp_discovery_in_flight
        except Exception:
            return
        ...
        def _wait_then_refresh() -> None:
            try:
                from auraforge_cli.mcp_startup import join_mcp_discovery
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Aura Forge' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Aura Forge setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Aura Forge
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Aura Forge on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Aura Forge' local kawaii waiting/status
        ...
        try:
            # Snapshot the internal Aura Forge DB session id before the turn so we
        ...
        # Aura Forge' local "waiting for model response" interrupt status is metadata,
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # an unpinned handler bakes the Aura Forge install tree into the session's
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Aura Forge Agent v{HERMES_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Aura Forge has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
