...
measured in the production session DB) through `auraforge chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    HERMES_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Aura Forge home with credentials for the
...
(tracker: miantahaaslam3-png/aura-forge#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "auraforge_cli.main", "chat", "--query", q,
