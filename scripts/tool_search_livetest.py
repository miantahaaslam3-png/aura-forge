#!/usr/bin/env python3
"""Live test harness for Aura Forge Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any auraforge imports.
ORIGINAL_HOME = os.environ.get("HERMES_HOME")
ORIGINAL_AUTH = Path.home() / ".auraforge" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.auraforge/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.auraforge/.env`` so
    ...
    hermes_home = home_dir / ".auraforge"
    ...
    real_env_file = Path.home() / ".auraforge" / ".env"
    if real_env_file.exists():
        ...
        load_hermes_dotenv(hermes_home=str(Path.home() / ".auraforge"))
