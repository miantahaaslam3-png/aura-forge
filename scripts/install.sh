#!/bin/bash
# ============================================================================
# Aura Forge Agent Installer
# ============================================================================
# Installation script for Linux, macOS, and Android/Termux.
# Uses uv for desktop/server installs and Python's stdlib venv + pip on Termux.
#
# Usage:
#   bash install-auraforge.sh   # or: curl -fsSL <your-release-url>/install.sh | bash
#
# Or with options:
...
# Python-driven tool session (e.g. Aura Forge terminal tool). A pre-set PYTHONPATH
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            echo "Aura Forge Agent Installer"
            echo ""
            ...
            echo "                   write \$AURA_FORGE_HOME/.no-bundled-skills so future"
            echo "                   'auraforge update' runs never inject bundled skills either"
...
emit_manifest() {
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Aura Forge Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install Aura Forge command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Point a Aura Forge-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $AURA_FORGE_HOME/node/bin — which is NOT on
...
# Aura Forge's own global installs pass an explicit --prefix and are unaffected.
...
configure_managed_node_npm_prefix() {
    [ -x "$AURA_FORGE_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$AURA_FORGE_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$AURA_FORGE_HOME/node/etc/npmrc"
}

get_hermes_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/auraforge" ]; then
        echo "$link_dir/auraforge"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  Use the Windows AuraForge-Installer.exe from the release page"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Aura Forge owns its own uv at $AURA_FORGE_HOME/bin/uv.  Always install there —
    ...
    local _managed_uv="$AURA_FORGE_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $AURA_FORGE_HOME/bin ..."
    mkdir -p "$AURA_FORGE_HOME/bin"
    ...
    # directly into $AURA_FORGE_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$AURA_FORGE_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# replaced with the Aura Forge-managed Node $NODE_VERSION.
...
check_node() {
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Aura Forge-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Aura Forge-managed Node from a previous run over a too-old system one.
    if [ -x "$AURA_FORGE_HOME/node/bin/node" ] && [ -x "$AURA_FORGE_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$AURA_FORGE_HOME/node/bin/node" --version)"; then
        export PATH="$AURA_FORGE_HOME/node/bin:$PATH"
        log_success "Node.js $("$AURA_FORGE_HOME/node/bin/node" --version) found (Aura Forge-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Aura Forge-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (Aura Forge requires Node 22.22+, 24.11+, or 26+) — installing Aura Forge-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    log_info "Extracting to ~/.aura-forge/node/..."
    ...
    # the Aura Forge command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$AURA_FORGE_HOME/node"
    mkdir -p "$AURA_FORGE_HOME"
    mv "$extracted_dir" "$AURA_FORGE_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$AURA_FORGE_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$AURA_FORGE_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$AURA_FORGE_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$AURA_FORGE_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.aura-forge/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Aura Forge install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Aura Forge Agent itself does not require or retain root access."
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Aura Forge behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"aura-forge-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up auraforge command..."
    ...
    if [ "$USE_VENV" = true ]; then
        ...
        if [ -z "$HERMES_BIN" ]; then
            log_warn "auraforge not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$HERMES_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$HERMES_ENTRYPOINT" ]; }; then
        log_warn "Aura Forge launcher prerequisites not found"
    ...
    # Create a user-facing shim for the Aura Forge command.
    ...
    rm -f "$command_link_dir/hermes" "$command_link_dir/auraforge"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/auraforge" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
...
EOF
    ...
    chmod +x "$command_link_dir/auraforge"
    log_success "Installed auraforge launcher → $command_link_display_dir/auraforge"
...
EOF
    ...
    chmod +x "$command_link_dir/auraforge-agent"
    log_success "Installed auraforge-agent launcher → $command_link_display_dir/auraforge-agent"
    ...
    # this launcher those hosts report Aura Forge as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/hermes-acp" "$command_link_dir/auraforge-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/auraforge-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
...
EOF
    ...
    chmod +x "$command_link_dir/auraforge-acp"
    log_success "Installed auraforge-acp launcher → $command_link_display_dir/auraforge-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "auraforge command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        PATH_COMMENT='# Aura Forge Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Aura Forge Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Aura Forge Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
copy_config_templates() {
    ...
    mkdir -p "$AURA_FORGE_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.hermes/.env (top level, easy to find)
    if [ ! -f "$AURA_FORGE_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$AURA_FORGE_HOME/.env"
            log_success "Created ~/.hermes/.env from template"
        else
            touch "$AURA_FORGE_HOME/.env"
    ...
    chmod 600 "$AURA_FORGE_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.hermes/config.yaml (top level, easy to find)
    if [ ! -f "$AURA_FORGE_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$AURA_FORGE_HOME/config.yaml"
    ...
    # This MUST match DEFAULT_SOUL_MD in auraforge_cli/default_soul.py — the
    ...
    if [ ! -f "$AURA_FORGE_HOME/SOUL.md" ]; then
        cat > "$AURA_FORGE_HOME/SOUL.md" << 'SOUL_EOF'
You are Aura Forge Agent, an intelligent AI assistant created by Nous Research. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
    ...
    if [ "$NO_SKILLS" = true ]; then
        ...
        printf '%s\n' \
            ...
            > "$AURA_FORGE_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $AURA_FORGE_HOME/.no-bundled-skills"
        ...
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            ...
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$AURA_FORGE_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$AURA_FORGE_HOME/skills/" 2>/dev/null || true
...
strip_snap_browser_override() {
    ...
    local env_file="$AURA_FORGE_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Aura Forge Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Aura Forge will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Aura Forge Agent browser tools — explicit browser override."
...
install_computer_use_driver() {
    ...
    # so a stalled GitHub download can't hang the Aura Forge install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Aura Forge'
...
run_setup_wizard() {
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m auraforge_cli.main setup < /dev/tty
    else
        python -m auraforge_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$AURA_FORGE_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Aura Forge to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$AURA_FORGE_HOME/whatsapp/session/creds.json"
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            nohup $HERMES_CMD gateway > "$AURA_FORGE_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
...
write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.auraforge-bootstrap-complete, which tells the Aura Forge
    ...
    local marker_path="$INSTALL_DIR/.auraforge-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $AURA_FORGE_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $AURA_FORGE_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $AURA_FORGE_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}auraforge${NC}           Start chatting"
    echo -e "   ${GREEN}auraforge setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}auraforge config${NC}       View/edit configuration"
    echo -e "   ${GREEN}auraforge config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}auraforge gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}auraforge update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'auraforge' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'auraforge' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'auraforge' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$AURA_FORGE_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$AURA_FORGE_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$AURA_FORGE_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$AURA_FORGE_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Aura Forge` rename
...
install_desktop() {
    ...
    # supported Node lines and prepends the Aura Forge-managed Node to PATH, so
    ...
    if [ "$OS" = "linux" ]; then
        ...
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/AuraForge.app" \
            "$desktop_dir/release/mac/AuraForge.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # importing Python cannot reverse it by loading AURA_FORGE_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if AURA_FORGE_HOME="$AURA_FORGE_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from auraforge_cli.main import _desktop_macos_relaunchable_fixup
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Aura Forge-managed Node
            # provisioned during prerequisites/node-deps (at $AURA_FORGE_HOME/node/bin)
        ...
        complete)
            ...
            # $AURA_FORGE_HOME. $AURA_FORGE_HOME is a shared data dir (it can be
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $AURA_FORGE_HOME.
    # $AURA_FORGE_HOME is a shared data dir (it can be bind-mounted into a Docker
