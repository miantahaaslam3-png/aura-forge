#!/usr/bin/env python3
"""Aura Forge Agent Release Script
...
VERSION_FILE = REPO_ROOT / "auraforge_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@auraforge.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "kavi@local.auraforge": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (auraforge journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "auraforge.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.auraforge/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows auraforge-shim resolution + prefer --update on recovery; #52378)
    ...
    "auraforge@marian.local": "Schrotti77",
    ...
    "274096618+auraforge-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "auraforge@nousresearch.com": "NousResearch",
    ...
    "auraforge@noushq.ai": "benbarclay",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on auraforge mcp add)
    ...
    "auraforge@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "agent@auraforge.local": "jacdevos",
    ...
    "teknium@auraforge-agent": "teknium1",
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: auraforge-eval community link)
    ...
    "vigo@auraforge": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.auraforge@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Aura Forge runtime files)
    ...
    "zhchl@auraforge-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Aura Forge
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "auraforge@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/miantahaaslam3-png/aura-forge",
    ...
    lines.append(f"# Aura Forge Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Aura Forge Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Aura Forge Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  Aura Forge Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Aura Forge Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Aura Forge Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Aura Forge Agent v{new_version} ({calver_date})' "
