...
Background
----------
A Nous bootstrap session (client_id ``auraforge-cli-vps``) can take a terminal
...
Design constraints
------------------
- Pure stdlib, no auraforge_cli imports: runs early in the boot hook, before the
...
BOOTSTRAP_CLIENT_ID = "auraforge-cli-vps"
...
def _nous_entry_is_terminal(nous_state: Any) -> bool:
    ...
    Mirrors the ``terminal`` predicate in ``auraforge_cli.auth.get_nous_session_validity``:
