...
What it does
------------
1. Spawns a SCRATCH dashboard (``auraforge dashboard``) bound to loopback on a
   free port, with an ISOLATED ``HERMES_HOME`` (temp dir, minimal seeded state).
   It NEVER touches the live :9119 dashboard / ai.auraforge.dashboard / live
...
class ScratchDashboard:
    ...
    def __enter__(self) -> "ScratchDashboard":
        ...
        cmd = [
            python, "-m", "auraforge_cli.main", "dashboard",
...
def run_certify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    home = parent_tmp / "auraforge-home"
    seed_scratch_home(
        home,
