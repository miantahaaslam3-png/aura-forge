#!/usr/bin/env node
/**
 * Aura Forge Agent WhatsApp Bridge
 *
...
const DEFAULT_REPLY_PREFIX = '⚕ *Aura Forge Agent*\n────────────\n';
...
function enqueuePollUpdateEvent({ key, update, selectedOptions, aggregation }) {
  ...
  const pollId = key?.id
    ...
    || '';
  // Only surface votes on polls Aura Forge itself created (tracked when
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Aura Forge Agent', 'Chrome', '120.0'],
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into Aura Forge and trigger
      ...
      // Ignore Aura Forge' own reply messages in self-chat mode to avoid loops.
