...
#     -InstallRoot <path>   repo checkout (AURA_FORGE_HOME\aura-forge-agent)
...
# pass our foreground right on to the new Aura Forge.exe pid.
try {
...
function Show-ProgressWindow {
    ...
    try {
        ...
        $title.Text = "Updating Aura Forge"
...
function Show-ManualFinale([string]$Message) {
    ...
    # they reopen Aura Forge themselves.
    if ($script:UiServer) {
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Aura Forge.exe before writing the replacement,
    ...
    # DO NOT spawn Aura Forge.exe as our child: Electron/Chromium calls
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh Aura Forge process.
        try {
...
function Invoke-HermesStep([string]$Exe, [string[]]$AuraForgeArgs, [string]$Tag) {
    ...
    $arguments = ($AuraForgeArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
...
# The #90455 deadlock needs no update, no checkout and no Aura Forge install to
...
try {
    ...
    Publish-UiProgress "Waiting for Aura Forge to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Aura Forge window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Aura Forge fully and try again."
    ...
    Publish-UiProgress "Preparing Aura Forge files"
    ...
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Aura Forge install open (venv\Scripts\hermes.exe locked after 20s). Nothing was changed. Close other Aura Forge windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # from a user leaving a second Aura Forge window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Aura Forge.exe that no longer
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Aura Forge installer or `hermes doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    ...
    if ($res.Code -ne 0 -and $res.Code -ne 2) {
        ...
        # code in memory). Exit 2 ("close all Aura Forge windows") is not retryable.
    ...
    #   3. only then the terminal UI state — done means "Aura Forge is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. Aura Forge was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen Aura Forge."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening Aura Forge"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen Aura Forge to finish (it could not restart itself)."
