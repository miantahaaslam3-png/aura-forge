...
Extracted verbatim from auraforge_state.py so the SessionDB mixin modules
(auraforge_state_search / auraforge_state_schema / auraforge_state_portability) can
reference them without importing auraforge_state (which would be a cycle).
auraforge_state re-imports every name here for backward compatibility.
"""
...
#
# Semantics mirror `auraforge_state._cross_process_repair_lock` (the schema-
...
# rebuilder cannot wedge future rebuilds. It lives here (not auraforge_state)
# because the search/schema mixins cannot import auraforge_state (cycle).
#
...
logger = logging.getLogger("auraforge_state")
