...
three virtual model IDs so the ``auraforge tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `auraforge tools` → Image "
                    "Generation → OpenAI to configure, or `auraforge setup` "
                    "to add the key."
                ),
