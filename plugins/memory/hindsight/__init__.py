...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Aura Forge can degrade gracefully instead of repeatedly trying to start
...
def _event_timestamp() -> str:
    """Return the configured Aura Forge event time with an explicit UTC offset."""
    event_time = _hermes_now()
    # auraforge_time.now() guarantees an aware datetime. Keep this fallback so a
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the Aura Forge .env during
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Aura Forge profile name (from agent_identity)
      {workspace} — Aura Forge workspace name (from agent_workspace)
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Aura Forge Agent and the User"
    ...
    def post_setup(self, hermes_home: str, config: dict) -> None:
        ...
        from auraforge_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
    ...
    def _offer_starter_template(self, mode: str, provider_config: dict, env_writes: dict) -> None:
        """Offer to seed the bank with a Aura Forge starter template (best-effort)."""
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Aura Forge Agent and the User"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Aura Forge Agent and the User")
        ...
        # Aura Forge emits a "👁️ Hindsight — recalled N memories" status line so the
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                ...
                # Aura Forge get sluggish. (issue #13125)
                try:
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        # turn's queued recall. See miantahaaslam3-png/aura-forge#5820.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Aura Forge created/used that client on the shared async loop,
