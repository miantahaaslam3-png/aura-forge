...
Resolution order for config file:
  1. $HERMES_HOME/honcho.json  (instance-local, enables isolated Aura Forge instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for an Aura Forge profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Aura Forge profile.
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Aura Forge system prompt
    ...
    def from_global_config(
        ...
        When host is None, derives it from the active Aura Forge profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.aura block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Aura Forge session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Aura Forge session title (from /title command; non-per-session)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'aura honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        # the "go run aura honcho setup" hint they used to.
        try:
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `aura honcho setup` to configure)."
            )
