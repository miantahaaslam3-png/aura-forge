...
```bash
aura memory setup honcho   # configure Honcho directly (works on a fresh install)
aura memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
aura config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.hermes/.env
```

> `aura honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `aura memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$HERMES_HOME/honcho.json` | Profile-local (isolated Aura Forge instances) |
| 2 | `~/.hermes/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Aura Forge profile: `hermes` (default) or `hermes_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `aura honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Aura Forge session ID (`20260415_a3f2b1`) |
...
- **`per-directory`** — basename of `$PWD`. Opening aura in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Aura Forge session ID (timestamp + hex). Every `hermes` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Aura Forge profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Host key is derived from the active Aura Forge profile: `hermes` (default) or `hermes_<profile>` (e.g. `aura -p coder` -> host key `hermes_coder`). Older `hermes.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
|---------|-------------|
| `aura memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `aura honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `aura memory setup`) |
| `aura honcho status` | Show resolved config for active profile |
| `aura honcho enable` / `disable` | Toggle Honcho for active profile |
| `aura honcho mode <mode>` | Change recall or observation mode |
| `aura honcho peer --user <name>` | Update user peer name |
| `aura honcho peer --ai <name>` | Update AI peer name |
| `aura honcho tokens --context <N>` | Set context token budget |
| `aura honcho tokens --dialectic <N>` | Set dialectic max chars |
| `aura honcho map <name>` | Map current directory to a session name |
| `aura honcho sync` | Create host blocks for all Aura Forge profiles |
