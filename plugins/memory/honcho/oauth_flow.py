...
``auraforge://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Aura Forge OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "auraforge-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``auraforge-desktop`` / ``auraforge-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Aura Forge.</div>"
)
