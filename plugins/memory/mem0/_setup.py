...
def _write_env(env_path: Path, env_writes: dict[str, str]) -> None:
    ...
    if env_path.exists():
        ...
        # auraforge_cli/config.py. read_text() with no encoding falls back to the
...
def _setup_platform(hermes_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "auraforge-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "auraforge"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.auraforge/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.auraforge/.env (or unset it) or Aura Forge will keep "
...
def _setup_selfhosted(hermes_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "auraforge-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "auraforge")
...
def _setup_oss(hermes_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "auraforge-user")
    ...
    if env_writes:
        _write_env(Path(hermes_home) / ".env", env_writes)
    _save_mem0_json(hermes_home, {"mode": "oss", "user_id": user_id, "agent_id": "auraforge", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, hermes_home: str) -> str:
    ...
    if not existing:
        ...
        if env_path.exists():
            ...
            # auraforge_cli/config.py; a Notepad BOM on the first line would
...
_PGVECTOR_CONTAINER = "auraforge-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "auraforge"
...
def _setup_oss_interactive(hermes_home: str, config: dict) -> None:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'auraforge-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "auraforge-user")

    agent_id = input("  Agent ID [auraforge]: ").strip()
    agent_id = agent_id or "auraforge"
...
def post_setup(hermes_home: str, config: dict) -> None:
    """Entry point called by auraforge memory setup framework.
