...
``{title, url, description, position}`` rows every other Aura Forge web
provider produces.
...
prefers Aura Forge-managed xAI Grok OAuth (via ``auraforge auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.auraforge/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``auraforge tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `auraforge auth` to sign in with "
