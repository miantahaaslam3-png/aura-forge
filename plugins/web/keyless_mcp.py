...
two endpoints so a fresh Aura Forge install with **zero web credentials** still
...
def provider_tier(name: str) -> str:
    ...
    ``auraforge tools`` picker's Free/Paid rows). ``free`` forces the keyless
    ...
    try:
        ...
        return {
            ...
            "error": (
                ...
                "backend via `auraforge tools` for reliable service."
            ),
        }
    ...
    try:
        ...
        return {
            ...
            "error": (
                ...
                "via `auraforge tools` for reliable service."
            ),
        }
...
_KEENABLE_TITLE = "auraforge-agent"
