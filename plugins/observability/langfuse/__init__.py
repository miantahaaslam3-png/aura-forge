"""langfuse — Aura Forge plugin for Langfuse observability.

Traces Aura Forge conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Aura Forge plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``auraforge plugins enable
observability/langfuse`` or ``auraforge tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``auraforge tools`` or ~/.auraforge/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Aura Forge plugin system —
    ...
    with _LANGFUSE_CLIENT_LOCK:
        ...
        if Langfuse is None:
            logger.warning(
                ...
                "tracing is disabled. Run `auraforge tools` and configure Langfuse "
        ...
        # short-lived-process gap (kanban workers / auraforge chat -q / cron): exit
...
def _trace_key(
    ...
    *,
    ...
    Older Aura Forge paths only expose ``task_id``/``session_id``. Newer paths
...
def _canonical_usage_and_cost(
    ...
    *,
    ...
    """Translate canonical Aura Forge usage into Langfuse usage and cost maps."""
    ...
    # Aura Forge estimate rather than summing components because it also includes
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="Aura Forge turn",
                tags=["auraforge", "langfuse"],
            ):
                root_ctx = client.start_as_current_observation(
                    trace_context=trace_ctx,
                    name="Aura Forge turn",
                    as_type="chain",
                    input=trace_input,
                    metadata=metadata,
                    end_on_exit=False,
...
def _finalize_all_traces() -> None:
    ...
    kanban workers, ``auraforge chat -q`` one-shots, cron jobs — can exit while
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Aura Forge branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Aura Forge also has a turn-scoped
    ...
    # messages is a list only for legacy Aura Forge branches that fired
    # pre_llm_call with API messages directly. Current Aura Forge fires
...
def register(ctx) -> None:
    ...
    # Aura Forge versions.  pre_api_request / post_api_request fire per API
