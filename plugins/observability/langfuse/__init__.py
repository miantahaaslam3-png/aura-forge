...
load when listed in ``plugins.enabled`` (via ``aura plugins enable
observability/langfuse`` or ``aura tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``aura tools`` or ~/.hermes/.env):
