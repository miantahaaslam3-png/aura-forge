...
Optional Aura Forge observability plugin that maps Aura Forge observer hooks to
...
not replace Aura Forge Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Aura Forge emit NeMo Relay lifecycle events for
the work Aura Forge already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Aura Forge Agent can:

- Preserve Aura Forge execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
aura plugins enable observability/nemo_relay
```
...
`HERMES_HOME` is the Aura Forge profile/config home used by both
`aura plugins enable ...` and the later `aura chat ...` run. If unset,
Aura Forge uses the user's default home, usually `~/.hermes`. For isolated smoke
...
aura plugins enable observability/nemo_relay
aura chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
uv run aura plugins enable observability/nemo_relay
uv run aura chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
`HERMES_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Aura Forge does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Aura Forge keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Aura Forge routes LLM and tool
...
For the full generic Aura Forge middleware contract, see
...
This run starts a parent Aura Forge session, delegates to a child subagent, has the
...
export HERMES_NEMO_RELAY_ATIF_AGENT_NAME='Aura Forge Agent E2E'
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Aura Forge Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Aura Forge dispatches the read-only tools as one batch, and NeMo Relay
...
- Aura Forge sessions map to `agent` scopes.
- Aura Forge API request hooks map to `llm` scope start/end events.
- Aura Forge tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Aura Forge execution middleware to hand
...
When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Aura Forge routes execution
through these boundaries:

```text
Aura Forge provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Aura Forge provider adapter next_call(...)

Aura Forge tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Aura Forge tool dispatcher next_call(...)
```
...
middleware for a local Aura Forge run. This path requires a NeMo Relay runtime that
...
agent_name = "Aura Forge Middleware E2E"
agent_version = "local"
...
```json
{
  ...
  "session_id": "middleware-demo-session",
  "agent": {
    "name": "Aura Forge Middleware E2E",
    "version": "local",
