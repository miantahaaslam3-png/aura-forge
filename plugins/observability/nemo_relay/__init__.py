"""nemo_relay — optional Aura Forge plugin for NeMo Relay observability."""
...
class _Settings:
    ...
    atif_agent_name: str = "Aura Forge Agent"
...
class _Runtime:
    ...
    def _run_managed_with_downstream_preservation(
        ...
        # execution so Aura Forge retry classification still sees it. The LLM and tool
...
def _load_settings() -> _Settings:
    ...
    return _Settings(
        ...
        atif_agent_name=_env("HERMES_NEMO_RELAY_ATIF_AGENT_NAME") or "Aura Forge Agent",
...
def _original_downstream_error(exc: Exception) -> BaseException:
    # Aura Forge wraps downstream execution failures in a local/private exception
