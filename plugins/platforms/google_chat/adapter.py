...
# ``model_tools`` import time, so ``aura status``, ``aura chat``, etc.
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a aura MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a aura MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        ...
        body: Dict[str, Any] = {"text": "Aura Forge is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``aura setup``.
    ...
    print_info("Restart the gateway: aura gateway restart")
