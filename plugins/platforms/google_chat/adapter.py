...
# ``model_tools`` import time, so ``auraforge status``, ``auraforge chat``, etc.
...
#     fresh "Aura Forge is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a auraforge MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "auraforge-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        try:
            from auraforge_constants import get_hermes_home as _get_hermes_home
            _hermes_home = _get_hermes_home()
        except (ModuleNotFoundError, ImportError):
            _hermes_home = _Path.home() / ".auraforge"
        ...
        # they don't sit in the chat forever as "Aura Forge is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("HERMES_HOME", str(_Path.home() / ".auraforge"))
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a auraforge MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Aura Forge is thinking…' marker message.
        ...
        "Aura Forge is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Aura Forge is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Aura Forge is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Aura Forge is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``auraforge setup``.

    The setup wizard at ``auraforge_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.auraforge/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. auraforge-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.auraforge/.env")
    print_info("Restart the gateway: auraforge gateway restart")
...
def register(ctx) -> None:
    """Plugin entry point — called by the Aura Forge plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `auraforge setup` to install Google Chat support.",
        ...
        # the "Aura Forge is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Aura Forge is thinking…' marker message "
