"""
IRC Platform Adapter for Aura Forge Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Aura Forge agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "auraforge-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Aura Forge Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Aura Forge Agent shutting down")
...
def check_requirements() -> bool:
    ...
    # auraforge setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `auraforge gateway setup` flow for the IRC platform.
    ...
    print_info("Connect Aura Forge to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    nickname = prompt(
        "Bot nickname (e.g. auraforge-bot)",
    ...
    channel = prompt(
        "Channel to join (e.g. #auraforge — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.auraforge/.env")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "auraforge-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "auraforge-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Aura Forge Agent (cron)")
