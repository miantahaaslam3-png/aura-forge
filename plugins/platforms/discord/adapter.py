...
def _needs_server_members_intent(
    ...
    """Return True when Aura Forge must request Discord's Server Members intent.
...
def _format_privileged_intents_guidance(*, needs_members: bool) -> str:
    ...
    lines.extend(
        [
            ...
            "Docs: https://auraforge-agent.nousresearch.com/docs/user-guide/messaging/discord",
        ]
    )
    return "\n".join(lines)
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.auraforge/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Aura Forge gateway
    ...
    def _classify_connect_exception(self, error: Exception) -> tuple:
        ...
        if _is("PrivilegedIntentsRequired"):
            # Name the exact intents Aura Forge requested (#79430): Message
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage Aura Forge response. Emoji-only acks are
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs Aura Forge work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit Aura Forge/gateway outage notices."""
        text = (content or "").lower()
        subject = r"(?:auraforge|the agent|agent|the gateway|gateway|bmo)"
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Aura Forge manages."""
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Aura Forge pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Aura Forge session")
        ...
        @tree.command(name="status", description="Show Aura Forge session status")
        ...
        @tree.command(name="stop", description="Stop the running Aura Forge agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.auraforge/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Aura Forge Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Aura Forge gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Aura Forge session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Aura Forge in the thread",
        ...
        # auraforge_cli/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            from auraforge_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from auraforge_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Aura Forge skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from auraforge_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Aura Forge session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Aura Forge: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Aura Forge has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "Aura Forge"
        if len(content) > 80:
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Aura Forge: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Aura Forge semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        reason = "Aura Forge session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Aura Forge handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want Aura Forge to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ Aura Forge needs your input",
                description=body,
                color=discord.Color.orange(),
            )
            ...
            content = self._self_contained_prompt_content(
                "❓ **Aura Forge needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``auraforge update --gateway``
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ Aura Forge could not create a Discord thread for "
...
def _component_check_auth(
    ...
    the pairing store (``auraforge pairing approve``).
    ...
    # so users approved via ``auraforge pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``auraforge update`` prompts.
...
# is not in this process (e.g. ``auraforge cron`` running standalone) and no live
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``auraforge_cli.gateway.get_env_value`` at call time (not via
...
def register(ctx) -> None:
    """Plugin entry point — called by the Aura Forge plugin system."""
    ctx.register_platform(
        ...
        install_hint="Run `auraforge setup` to install Discord support.",
        # Interactive setup wizard — replaces the central
        # auraforge_cli/setup.py::_setup_discord function.  Same shape as Teams.
