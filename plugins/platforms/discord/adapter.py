...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run an Aura Forge skill",
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``aura update --gateway``
...
def _component_check_auth(
    ...
    the pairing store (``aura pairing approve``).
    ...
    # so users approved via ``aura pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``aura update`` prompts.
...
# is not in this process (e.g. ``aura cron`` running standalone) and no live
