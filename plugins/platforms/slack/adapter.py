...
# identify HermesAgent traffic — matching other Aura Forge outbound surfaces
...
def _rewrite_known_bang_command(text: str) -> str:
    ...
    try:
        from auraforge_cli.commands import is_gateway_known_command
...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Aura Forge has no listener for (e.g. user_change,
            ...
            from auraforge_cli.commands import slack_native_slashes
            import re as _re
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Aura Forge handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    async def send_native_task_card_progress(
        ...
        *,
        title: str = "Aura Forge is working",
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Aura Forge session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Aura Forge owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message_impl(
        ...
        # the shape produced by peer Aura Forge agents in Socket Mode on some
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        if slash_name in {"auraforge", ""}:
            ...
            from auraforge_cli.commands import slack_subcommand_map
...
# ``auraforge_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``auraforge_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        ...
        try:
            from auraforge_cli.slack_cli import _build_full_manifest
            from auraforge_constants import get_hermes_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Aura Forge",
                bot_description="Your Aura Forge agent on Slack",
            )
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``auraforge_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # Interactive setup wizard — replaces auraforge_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in auraforge_cli/gateway.py.
