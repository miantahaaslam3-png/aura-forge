...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `aura slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Aura Forge does not currently need to route them into the agent loop.
            ...
            # /model work directly without /aura prefix). A single regex
            ...
            # manifest (see `aura slack manifest`). In Socket Mode, Slack
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/aura <subcommand> [args]`` form is preserved for
        ...
        if slash_name in {"hermes", ""}:
            # Legacy /aura <subcommand> [args] routing + free-form questions.
            ...
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/aura   ``).
        ...
        # questions via "/aura <question>" must produce public replies so
...
def interactive_setup() -> None:
    ...
    def _write_slack_manifest_and_instruct() -> None:
        ...
        try:
            ...
            print_info(
                "   Re-run `aura slack manifest --write` anytime to refresh after "
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `aura update`)",
                True,
            ):
