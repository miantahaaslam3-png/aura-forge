...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Aura Forge command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Aura Forge accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Aura Forge ``/command`` text."""
...
from auraforge_constants import get_hermes_dir as _get_hermes_dir
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches Aura Forge
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Aura Forge Agent" become "Agent".
        """
        if not body:
            return ""
...
# + _PLATFORMS["matrix"] static dict in auraforge_cli/{setup,gateway}.py, and the
...
def interactive_setup() -> None:
    """Configure Matrix credentials. Replaces auraforge_cli/setup.py::_setup_matrix
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print_info("📬 Home Room: where Aura Forge delivers cron job results and notifications.")
...
def _is_connected(config) -> bool:
    ...
    configured. Read via auraforge_cli.gateway.get_env_value so setup-status
