"""
Microsoft Teams platform adapter for Aura Forge Agent.
...
Requires:
    ...
    manually: ``<auraforge-venv>/bin/pip install microsoft-teams-apps aiohttp``)
...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    Aura Forge owns dotenv loading; third-party import side effects must not.
    """
    try:
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Aura Forge"}),
            )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Aura Forge\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.auraforge/.env")
...
def _install_hint() -> str:
    ...
    ``feature_install_command(venv_pip=True)`` targets the actual Aura Forge
