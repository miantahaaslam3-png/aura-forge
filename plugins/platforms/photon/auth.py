...
Credential storage mirrors every other Aura Forge channel:

    * runtime SDK creds  -> ``~/.auraforge/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.auraforge/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a auraforge dependency
...
# registers Aura Forge as its own client_id.
...
# Default name of the project Aura Forge provisions for the operator.
DEFAULT_PROJECT_NAME = "Aura Forge Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of auraforge-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.auraforge/auth.json`` honouring the active Aura Forge profile."""
    try:
        from auraforge_constants import get_hermes_home
        return Path(get_hermes_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.auraforge")) / "auth.json"
...
def _save_auth(data: Dict[str, Any]) -> None:
    ...
    # auraforge_cli/auth.py:_save_auth_store (#19673, #21148).
    fd = os.open(
        str(tmp),
...
def store_photon_token(token: str) -> None:
    ...
    from auraforge_cli.auth import _auth_store_lock
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.auraforge/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.auraforge/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.auraforge/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: auraforge_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `auraforge photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `auraforge photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `auraforge photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `auraforge photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `auraforge photon setup`)"
