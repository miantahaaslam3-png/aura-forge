...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a aura dependency
...
def _auth_json_path() -> Path:
    """Resolve ``~/.hermes/auth.json`` honouring the active Aura Forge profile."""
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `aura photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `aura photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `aura photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `aura photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `aura photon setup`)"
