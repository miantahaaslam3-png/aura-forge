...
This plugin connects Aura Forge Agent to iMessage (and other Spectrum
...
TypeScript-only, Aura Forge runs it inside a small supervised Node sidecar and
...
aura photon setup --phone +15551234567

# Start the gateway
aura gateway start
```

`aura photon setup` does, in order:

1. **Device login** (RFC 8628, `client_id=photon-cli`) — opens
   `https://app.photon.codes/` for approval and stores the bearer token.
2. **Find or create** the `Aura Forge Agent` project on the Photon dashboard.
...
There is no separate `login` command; like every other Aura Forge channel,
...
Run `aura photon status` to see what's configured.
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `aura photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `aura photon status`, a DM and a group roundtrip,
