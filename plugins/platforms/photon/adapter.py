"""
Photon Spectrum (iMessage) platform adapter for Aura Forge Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already an Aura Forge dep
...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        # spectrum-ts not installed yet — `aura photon setup` will
        # install it.  check_fn still returns False so the gateway
        # surfaces the missing-deps state in `aura setup` / status.
        return False
    return True
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Aura Forge defaults).
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Aura Forge checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not (_SIDECAR_DIR / "node_modules").exists():
            raise RuntimeError(
                ...
                f"cd {_SIDECAR_DIR} && npm install   (or `aura photon setup`)"
            )
...
def register(ctx) -> None:
    ...
    # gateway-setup hook and the `aura photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: aura photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `aura gateway setup` alongside every other
    ...
    # Register CLI subcommands — `aura photon ...`
