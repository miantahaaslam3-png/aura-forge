...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Aura Forge dep
...
# artifacts so Aura Forge sees the link once, not a follow-up "(attachment)" prompt.
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so Aura Forge does not drop labels or rewrite intent.
    """
