"""
``auraforge photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Aura Forge
...
# wiring imports this module for `auraforge --help`).
...
# when called later (gateway start, auraforge status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `auraforge photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Aura Forge Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``auraforge photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Aura Forge Agent" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.auraforge/.env,
    ...
    try:
        from auraforge_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  auraforge gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `auraforge photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`auraforge photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.auraforge/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `auraforge photon telemetry on` / `auraforge photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.auraforge/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  auraforge gateway restart")
    return 0
...
#
# `auraforge gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``auraforge photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `auraforge gateway setup` wizard."""
