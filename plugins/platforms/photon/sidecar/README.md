...
Small Node helper that bridges Aura Forge Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Aura Forge is Python; Photon has no public HTTP
...
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Aura Forge can receive inbound messages
...
The Aura Forge plugin's `hermes photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Aura Forge is Python, so replies go through this sidecar
