"""
Email platform adapter for the Aura Forge gateway.

Allows users to interact with Aura Forge by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "auraforge-agent" "version" "{_hermes_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `auraforge config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `auraforge gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Aura Forge Agent")
        ...
        msg_id = f"<auraforge-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# auraforge_cli/gateway.py, and the _send_email dispatch in
