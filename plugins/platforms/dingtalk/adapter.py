...
class DingTalkAdapter(BasePlatformAdapter):
    ...
    async def send(
        ...
        payload = {
            "msgtype": "markdown",
            "markdown": {"title": "Aura Forge", "text": normalized},
        }
...
# dict in auraforge_cli/gateway.py, and the _send_dingtalk dispatch in
...
def interactive_setup() -> None:
    ...
    Replaces auraforge_cli/setup.py-era _setup_dingtalk + the static
    _PLATFORMS["dingtalk"] dict in auraforge_cli/gateway.py. CLI helpers are
    ...
    if method == 0:
        try:
            from auraforge_cli.dingtalk_auth import dingtalk_qr_auth
