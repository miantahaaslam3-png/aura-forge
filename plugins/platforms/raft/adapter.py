...
and injects content-free wake hints into Aura Forge' normal gateway session pipeline.
...
class RaftAdapter(BasePlatformAdapter):
    ...
    async def _handle_wake(self, request: "web.Request") -> "web.Response":
        ...
        # Aura Forge only verifies that wake hints are content-free.
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        """Accept Raft wake hints without interrupting an active Aura Forge turn."""
        if not self._message_handler:
            return
...
def interactive_setup() -> None:
    ...
    print_info("Connect Aura Forge to Raft as an external agent.")
