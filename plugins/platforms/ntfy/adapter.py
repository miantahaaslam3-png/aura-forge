"""ntfy platform adapter (Aura Forge plugin).
...
a Aura Forge dependency.

This adapter ships as a Aura Forge platform plugin under
``plugins/platforms/ntfy/``. The Aura Forge plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "auraforge-in"              # subscribe topic (incoming)
          publish_topic: "auraforge-out"     # optional — defaults to topic
...
_ECHO_TAG = "auraforge-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``auraforge cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Aura Forge dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `auraforge gateway status` without
