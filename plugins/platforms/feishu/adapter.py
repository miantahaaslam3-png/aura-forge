...
In single-bot mode (what Aura Forge currently supports), open_id works as a
...
class FeishuAdapter(BasePlatformAdapter):
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=10,
                    thread_name_prefix="auraforge-feishu-sdk",
                )
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local Aura Forge gateway is already using this Feishu app_id"
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that Aura Forge does not act on."""
        event = getattr(data, "event", None)
        message = getattr(event, "message", None)
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by Aura Forge webhook mode")
    ...
    async def _resolve_sender_profile(
        ...
        *,
        ...
        """Map Feishu's three-tier user IDs onto Aura Forge' SessionSource fields.
    ...
    async def _connect_websocket(self) -> None:
        ...
        self._ws_client = FeishuWSClient(
            ...
            # See https://github.com/miantahaaslam3-png/aura-forge/issues/50656
...
# Called by `auraforge gateway setup` via _setup_feishu() in auraforge_cli/gateway.py.
# =============================================================================
...
def _begin_registration(domain: str = "feishu") -> dict:
    ...
    if "?" in qr_url:
        qr_url += "&from=auraforge&tp=auraforge"
    else:
        qr_url += "?from=auraforge&tp=auraforge"
    return {
...
# dict in auraforge_cli/gateway.py, and the _send_feishu dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    """
    if not await asyncio.to_thread(_load_lark_oapi):
        return {"error": "Feishu dependencies not installed. Run `auraforge setup` to install Feishu support."}
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_feishu in auraforge_cli/gateway.py and the static
    ...
    if access_idx == 0:
        ...
        print_info("Unknown users can request access; approve with `auraforge pairing approve`.")
    elif access_idx == 1:
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `auraforge setup` to install Feishu support.",
