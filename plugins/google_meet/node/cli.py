"""`auraforge meet node ...` subcommand tree.

Wired into the existing ``auraforge meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``auraforge meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="auraforge-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``auraforge meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             auraforge meet node approve <name> ws://<host>:{args.port} {token}")
        try:
