"""disk-cleanup plugin — auto-cleanup of ephemeral Aura Forge session files.
...
def _extract_paths_from_terminal(args: Dict[str, Any], result: str) -> Set[str]:
    ...
    if isinstance(cmd, str) and cmd:
        # Tokenise the command — catches `touch /tmp/auraforge-x/test_foo.py`
        try:
...
All operations are scoped to HERMES_HOME and /tmp/auraforge-*.
...
def register(ctx) -> None:
    ...
    ctx.register_command(
        ...
        description="Track and clean up ephemeral Aura Forge session files.",
    )
