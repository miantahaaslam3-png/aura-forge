"""disk_cleanup — ephemeral file cleanup for Aura Forge Agent.
...
Scope: strictly HERMES_HOME and /tmp/auraforge-*
Never touches: ~/.auraforge/logs/ or any system directory.
"""
...
try:
    ...
    def get_hermes_home() -> Path:  # type: ignore[no-redef]
        ...
        return Path(val).resolve() if val else (Path.home() / ".auraforge").resolve()
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under HERMES_HOME or ``/tmp/auraforge-*``.
    ...
    try:
        ...
        pass
    # Allow /tmp/auraforge-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("auraforge-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "auraforge-agent", "backups", "profiles", ".worktrees",
...
def quick() -> Dict[str, Any]:
    ...
    # durable state trees.  Some installs place the Aura Forge checkout, venv,
...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        # Path isn't under HERMES_HOME (e.g. /tmp/auraforge-*) — fall through.
        pass
