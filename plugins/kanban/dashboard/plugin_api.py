...
``auraforge_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
the in-browser PTY bridge in ``auraforge_cli/web_server.py``.

This means ``auraforge dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    WS auth gate (``auraforge_cli.web_server._ws_auth_ok``).
    ...
    """
    try:
        from auraforge_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``auraforge_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from auraforge_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from auraforge_cli.kanban_diagnostics import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from auraforge_cli.kanban import _check_dispatcher_presence
...
from auraforge_cli.kanban_db import (  # noqa: E402
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``auraforge kanban
...
# spawn failures, stuck-blocked). See auraforge_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``auraforge kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``auraforge kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``auraforge kanban specify <task_id>``.
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from auraforge_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``auraforge kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.auraforge/config.yaml.
...
def _active_profile_name() -> str:
    """Return the current Aura Forge profile name for notify-sub ownership."""
    try:
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.auraforge/profiles/*`` on disk and every
...
def model_options():
    ...
    Thin wrapper around ``auraforge_cli.inventory.build_models_payload`` — the
    same substrate the dashboard Models page and the TUI picker use, so the
    dropdown can never offer a model/provider pair the rest of Aura Forge
...
def list_kanban_projects():
    """List first-class Aura Forge projects for board scoping.
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from auraforge_constants import get_hermes_home  # type: ignore
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``auraforge profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from auraforge_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    1:1 to ``auraforge kanban decompose <task_id>``.
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from auraforge_cli import kanban_decompose  # noqa: WPS433 (intentional)
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.auraforge/config.yaml.
