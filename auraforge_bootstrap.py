"""Windows UTF-8 bootstrap for Aura Forge entry points.
...
should be imported at the very top of every Aura Forge entry point
(``auraforge``, ``auraforge-agent``, ``auraforge-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``auraforge_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``auraforge_bootstrap``, never ``auraforge_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Aura Forge modules.

    Aura Forge ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Aura Forge source root
...
# Apply on import — entry points just need ``import auraforge_bootstrap``
# (or ``from auraforge_bootstrap import apply_windows_utf8_bootstrap``) at
