# Aura Forge Has Had "Routines" Since March
...
Aura Forge equivalent — works today:
```bash
aura cron create "0 2 * * *" \
...
Aura Forge equivalent — works today:
```bash
aura webhook subscribe auth-watch \
  --events "pull_request" \
...
Aura Forge equivalent — works today:
```bash
aura webhook subscribe alert-triage \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Aura Forge implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Aura Forge |
|---|---|---|
...
## Things Aura Forge Does That Routines Can't
...
Aura Forge has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Aura Forge is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.

```bash
curl -fsSL https://aura-forge.nousresearch.com/install.sh | bash
aura setup
```

Set up a scheduled task in 30 seconds:
```bash
aura cron create "0 9 * * 1" \
...
```bash
aura gateway setup    # enable webhooks
aura webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation blueprints gallery: [aura-forge.nousresearch.com/docs/reference/automation-blueprints-catalog](https://aura-forge.nousresearch.com/docs/reference/automation-blueprints-catalog)

Documentation: [aura-forge.nousresearch.com](https://aura-forge.nousresearch.com)

GitHub: [github.com/auraforge/aura-forge](https://github.com/auraforge/aura-forge)

---

*Aura Forge is built by [Aura Forge Team](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
