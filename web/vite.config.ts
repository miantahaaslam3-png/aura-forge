 ...
 * session token into `index.html` (see `auraforge_cli/web_server.py`). The
  ...
  return {
    ...
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[aura-forge] Could not find session token in ${BACKEND} — ` +
        ...
        console.warn(
          `[aura-forge] Dashboard at ${BACKEND} unreachable — ` +
