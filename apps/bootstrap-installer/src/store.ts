...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/AppData/Local/aura-forge/logs/bootstrap-installer.log')
    $hermesHome.set('~/AppData/Local/aura-forge')
  ...
  try {
    const [logPath, hermesHome, mode] = await Promise.all([
      ...
      invoke<string>('get_aura_forge_home'),
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Aura Forge" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (AuraForge-Setup.exe --update);
...
export async function launchAuraForgeDesktop(): Promise<void> {
  ...
  await invoke('launch_aura_forge_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Aura Forge repository' },
