...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating Aura Forge' : 'Setting up Aura Forge'
  const description = isUpdate
    ? 'Aura Forge is updating to the latest version — this only takes a moment.'
    : 'This is a one-time setup. The Aura Forge installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.'
  const pct = Math.round(progress.fraction * 100)

  return (
    <div className="aura-fade-in flex h-full flex-col">
