...
//!
//! Resolution order:
//!   1. Dev shortcut: a sibling repo checkout via $AURA_FORGE_SETUP_DEV_REPO_ROOT
...
//! Electron app's APP_ROOT/../.. trick, because Aura Forge-Setup.exe is meant
...
///
/// `pin` is the commit-or-branch from either Aura Forge-Setup's build-time
...
pub async fn resolve(
    ...
    // 1. Dev shortcut.
    if let Ok(repo_root) = std::env::var("AURA_FORGE_SETUP_DEV_REPO_ROOT") {
    ...
    // 2. Bundled. We ship the branded install.{ps1,sh} next to the
    //    AuraForge-Setup.exe so the installer never reaches out to the
    //    public Aura Forge repo for the script. This is what makes the
    //    Aura Forge rebrand actually take effect at install time.
    if let Ok(exe_path) = std::env::current_exe() {
        if let Some(exe_dir) = exe_path.parent() {
            let bundled = exe_dir.join(kind.filename());
            if bundled.exists() {
                emit_log(&format!(
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    let url = format!(
        "https://raw.githubusercontent.com/miantahaaslam3-png/aura-forge/{}/scripts/{}",
        commit_or_ref,
        kind.filename()
    );
    ...
    let response = reqwest::Client::new()
        .get(&url)
        .header("User-Agent", "aura-forge-setup/0.0.1")
        .send()
        .await
