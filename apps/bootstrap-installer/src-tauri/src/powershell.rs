...
/// Cancellation signal — `cancel_tx.send(true)` aborts the running script.
/// Uses a watch channel so the receiver can be cloned for every stage.
pub type CancelRx = watch::Receiver<bool>;

/// Spawns install.ps1 / install.sh with the given args and streams output.
///
/// `aura_forge_home_override` propagates to the child as $AURA_FORGE_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, aura_forge_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = aura_forge_home_override {
        cmd.env("AURA_FORGE_HOME", home);
    }
    ...
    loop {
        tokio::select! {
            ...
            _ = recv_cancel(&mut cancel_rx) => {
                tracing::warn!("cancellation received — killing child process tree");
                killed = true;
    ...
    // Drain remaining lines after the loop exited. Skip the drain entirely
    // when we cancelled: the process tree was force-killed and any remaining
    // output is irrelevant — blocking here would hang the cancel.
    if !killed {
...
fn stable_script_cwd<'a>(script_path: &'a Path, aura_forge_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = aura_forge_home_override {
...
async fn recv_cancel(rx: &mut Option<CancelRx>) {
    match rx {
        Some(r) => {
            // Already cancelled before we started watching?
            if *r.borrow() {
                return;
            }
            // Wait until the flag flips to true (or the sender is dropped).
            while !*r.borrow() {
...
/// Kill the child AND all of its descendants. On Windows, PowerShell spawns
/// grandchildren (git, uv, npm, node) that keep the stdout/stderr pipes open;
/// killing only powershell.exe leaves the drain loop blocked forever, which
/// is why the Cancel button appeared to do nothing. `taskkill /T /F` kills
/// the entire process tree in one shot.
#[cfg(target_os = "windows")]
fn kill_process_tree(child: &mut Child) {
    ...
    // Belt-and-braces: also ask tokio to kill the direct child.
    let _ = child.start_kill();
}

#[cfg(not(target_os = "windows"))]
fn kill_process_tree(child: &mut Child) {
    // On Unix the child is bash; killing it orphans grandchildren but they
    // don't hold our pipes in the same way. Best-effort direct kill.
    let _ = child.start_kill();
}
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_aura_forge_home() {
