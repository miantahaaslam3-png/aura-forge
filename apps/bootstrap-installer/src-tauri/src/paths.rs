...
//!
//! Mirrors `aura-forge_constants.get_aura_forge_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\aura-forge
//!   macOS:   ~/.aura-forge
//!   Linux:   ~/.aura-forge  (override via $AURA_FORGE_HOME)
//!
//! NOTE (macOS): Python's get_aura_forge_home(), scripts/install.sh, and the
//! Electron desktop's resolveAura ForgeHome() ALL use ~/.aura-forge on macOS — there
...
/// Returns the canonical Aura Forge home directory, respecting $AURA_FORGE_HOME if set.
pub fn aura_forge_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("AURA_FORGE_HOME") {
    ...
    {
        // %LOCALAPPDATA%\aura-forge — matches scripts/install.ps1's $Aura ForgeHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("aura-forge");
        }
    }

    // macOS + Linux + fallback: ~/.aura-forge (matches Python get_aura_forge_home(),
    // install.sh, and the Electron desktop's resolveAura ForgeHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".aura-forge");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".aura-forge")
}

pub fn log_dir() -> PathBuf {
    aura_forge_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    aura_forge_home().join("bootstrap-cache")
}
...
/// AURA_FORGE_HOME so it survives repo checkout deletion (unlike anything under
/// aura-forge-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\aura-forge\aura-forge-setup.exe`; on other
/// platforms the extension differs but the directory is the same.
pub fn installer_dest() -> PathBuf {
    let name = if cfg!(target_os = "windows") {
        "aura-forge-setup.exe"
    } else {
        "aura-forge-setup"
    };
    aura_forge_home().join(name)
}
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_aura-forge`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under AURA_FORGE_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves AURA_FORGE_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    aura_forge_home().join(".aura-forge-update-in-progress")
}
...
pub fn copy_self_to_aura_forge_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to AURA_FORGE_HOME");
    Ok(())
}
...
///   const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_AURA_FORGE_ROOT, '.aura-forge-bootstrap-complete')
/// We don't always know ACTIVE_AURA_FORGE_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".aura-forge-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under AURA_FORGE_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[aura-forge-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("AURA_FORGE_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_aura_forge_home() -> String {
    aura_forge_home().to_string_lossy().into_owned()
}
