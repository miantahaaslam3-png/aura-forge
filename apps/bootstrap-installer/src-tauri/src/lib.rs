//! Aura Forge Setup — Tauri entrypoint.
//!
...
pub fn run() {
    ...
    // Aura Forge is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Aura Forge installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Aura Forge is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Aura Forge" doubles as a normal launcher (first run installs, every
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::hermes_home().join("aura-forge-agent");
        ...
        .expect("error while running Aura Forge Setup");
}
