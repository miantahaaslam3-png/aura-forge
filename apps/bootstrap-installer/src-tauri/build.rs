...
fn main() {
    ...
    // local checkout hasn't pushed). Set AURA_FORGE_BUILD_PIN_COMMIT to bake an
    // immutable commit pin for reproducible/release installers.
    //
    // Commit pin resolution:
    //   - AURA_FORGE_BUILD_PIN_COMMIT, if set and non-empty. Accepts a SHA, tag,
    ...
    //   1. AURA_FORGE_BUILD_PIN_BRANCH, if set and non-empty.
    ...
    if let Some(c) = &commit {
        ...
        println!(
            "cargo:warning=aura-forge-bootstrap: pinning to commit {}",
            short(c)
        );
    }
    if let Some(b) = &branch {
        ...
        match &commit {
            Some(_) => println!("cargo:warning=aura-forge-bootstrap: pinning to branch {b}"),
            None => println!(
                "cargo:warning=aura-forge-bootstrap: following branch {b} HEAD (no commit pin; \
                 set AURA_FORGE_BUILD_PIN_COMMIT for an immutable pin)"
            ),
        }
    }
    if commit.is_none() && branch.is_none() {
        ...
        // build environment.
        println!(
            "cargo:warning=aura-forge-bootstrap: no pin resolved at build time; binary will fail at runtime without AURA_FORGE_SETUP_DEV_REPO_ROOT or runtime args"
        );
    }
    ...
    // AURA_FORGE_BUILD_PIN_COMMIT resolves a moving ref (tag/branch) to a SHA, a
    ...
    println!("cargo:rerun-if-env-changed=AURA_FORGE_BUILD_PIN_COMMIT");
    println!("cargo:rerun-if-env-changed=AURA_FORGE_BUILD_PIN_BRANCH");

    // -----------------------------------------------------------------
    // Tauri windows manifest. See aura-forge-setup.manifest for rationale —
    // declares level="asInvoker" so Windows's installer-detection
    // heuristic doesn't refuse to launch us without UAC elevation.
    // -----------------------------------------------------------------
    #[cfg(target_os = "windows")]
    let attrs = {
        let manifest = include_str!("aura-forge-setup.manifest");
    ...
    // -----------------------------------------------------------------
    // Bundle-stage: copy the rebranded install.ps1 / install.sh next to the
    // compiled AuraForge-Setup binary so the bootstrap runs OUR branded script
    // instead of downloading the upstream Aura Forge one from GitHub. The Rust
    // resolver (install_script.rs) prefers a script sitting beside the exe.
    // -----------------------------------------------------------------
    // CARGO_MANIFEST_DIR = <repo>/apps/bootstrap-installer/src-tauri
    // scripts/ lives at <repo>/scripts (two levels up from src-tauri).
    // OUT_DIR always points inside the real target dir, e.g.
    //   <target>/<profile>/build/aura-forge-bootstrap-<hash>/out
    // so walking up to its `target` ancestor finds the exe dir.
    if let (Ok(manifest_dir), Ok(out_dir)) = (
        ...
        if scripts_dir.exists() {
            // Find the target root: the OUT_DIR contains "/build/" — the exe
            // lives in the parent of "build".
            let out = std::path::Path::new(&out_dir);
            if let Some(idx) = out.components().position(|c| {
                c.as_os_str().to_string_lossy().to_lowercase() == "build"
            }) {
                if let Some(target_root) = out.components().take(idx).collect::<std::path::PathBuf>().parent() {
                    for profile in ["debug", "release"] {
                        let exe_dir = target_root.join(profile);
                        for fname in ["install.ps1", "install.sh"] {
                            let src = scripts_dir.join(fname);
                            if src.exists() {
...
fn resolve_commit_pin() -> Option<String> {
    ...
    // asks for one via AURA_FORGE_BUILD_PIN_COMMIT. With no env var, we return
    // None and the installer follows the branch HEAD at install time.
    let requested = std::env::var("AURA_FORGE_BUILD_PIN_COMMIT").ok()?;
    ...
    panic!(
        "AURA_FORGE_BUILD_PIN_COMMIT={requested:?} could not be resolved to a commit \
...
fn resolve_branch_pin() -> Option<String> {
    if let Ok(v) = std::env::var("AURA_FORGE_BUILD_PIN_BRANCH") {
        if !v.trim().is_empty() {
