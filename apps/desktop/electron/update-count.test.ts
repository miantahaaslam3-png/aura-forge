...
function createTempGitRepo() {
  ...
  try {
    ...
    git('config', 'user.name', 'Aura Forge Test')
...
test('compareApiUrl builds the GitHub compare URL for HTTPS origins', () => {
  assert.equal(
    compareApiUrl({
      currentSha: SHA_A,
      originUrl: 'https://github.com/miantahaaslam3-png/aura-forge.git',
      targetSha: SHA_B
    }),
    `https://api.github.com/repos/miantahaaslam3-png/aura-forge/compare/${SHA_A}...${SHA_B}`
  )
})
...
test('compareApiUrl handles SSH origin forms', () => {
  for (const originUrl of [
    ...
    'ssh://git@github.com/miantahaaslam3-png/aura-forge.git',
