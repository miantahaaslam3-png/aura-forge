 ...
 * venv Python interpreter and the Aura Forge launcher that drives `hermes update`.
 ...
 * venv python invoked as `python -m auraforge_cli.main <backendArgs>` — but
...
export function resolveVenvHermesCommand(
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Aura Forge at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
