...
/**
 * Map an uninstall mode to the `python -m auraforge_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `auraforge_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'auraforge_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/AuraForge.app/Contents/MacOS/Hermes  → …/AuraForge.app
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/AuraForge.app/Contents/MacOS/Hermes → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/AuraForge.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Aura Forge.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Aura Forge$/i.test(dir) || /[\\/]hermes-desktop$/i.test(dir)) {
      return dir
    }
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import auraforge_cli`
 ...
 * modes + `pythonPath`=agentRoot so `import auraforge_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Aura Forge install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
