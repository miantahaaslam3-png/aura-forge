...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[aura-forge] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[aura-forge] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[aura-forge] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[aura-forge] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[aura-forge] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}
...
function resolveHermesHome() {
  ...
  if (process.env.AURA_FORGE_HOME) return normalizeHermesHomeRoot(process.env.AURA_FORGE_HOME)
  ...
  if (IS_WINDOWS) {
    ...
    if (fromRegistryAura) return normalizeHermesHomeRoot(fromRegistryAura)
...
// electron-builder derives from the build `appId` (com.auraforge.desktop) —
...
function unwrapWindowsVenvHermesCommand(command, backendArgs) {
  ...
  return {
    label: `existing Aura Forge Python at ${python}`,
    command: python,
...
// findGitBash — locate bash.exe on Windows. Aura Forge's terminal tool requires
...
// HERMES_HOME/aura-forge-setup.exe on a successful install (see
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'aura-forge-setup.exe' : 'aura-forge-setup'
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri aura-forge-setup. Unlike Windows
    ...
    if (!updater) {
      ...
      // aura-forge-setup.exe into HERMES_HOME). They DO have a working `hermes`
    ...
    if (!lock.unlocked) {
      ...
      const message =
        ...
        '(a second Aura Forge window or a terminal running auraforge?). Close it and retry.'
...
async function applyUpdatesPosixInApp() {
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Aura Forge (git + dependencies)…', percent: 10 })
  ...
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Aura Forge to retry.',
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Aura Forge…', percent: 100 })
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Aura Forge to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Aura Forge desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Aura Forge to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Aura Forge.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Aura Forge.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Aura Forge to load the new version.',
      percent: 100
    })
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Aura Forge to load the new version.',
      percent: 100
    })
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Aura Forge install was cancelled.')
...
async function waitForHermes(baseUrl, token) {
  ...
  throw new Error(`Aura Forge backend did not become ready: ${lastError?.message || 'timeout'}`)
}
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Aura Forge gateway',
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Aura Forge gateway uses OAuth, but you are not signed in. ' +
    ...
    if (!rawEnvToken) {
      throw new Error(
        'HERMES_DESKTOP_REMOTE_URL is set but HERMES_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Aura Forge backend.'
      )
    }
...
async function spawnPoolBackend(profile, entry) {
  ...
  child.once('exit', (code, signal) => {
    ...
    if (!ready) {
      rejectStart?.(
        new Error(`Aura Forge backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
...
async function startHermes() {
  ...
  connectionPromise = (async () => {
    ...
    hermesProcess.once('exit', (code, signal) => {
      ...
      if (!backendReady) {
        const message = `Aura Forge backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
...
ipcMain.handle('hermes:connection:revalidate', async () => {
  ...
  try {
    ...
    rememberLog('Cached remote Aura Forge backend failed liveness probe; dropping stale connection.')
    resetHermesConnection()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Aura Forge' TUI into
...
ipcMain.handle('hermes:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Aura Forge.')
  }
