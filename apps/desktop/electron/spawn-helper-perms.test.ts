...
test('rewrites an archived node-pty root to the matching unpacked tree exactly once', () => {
  assert.equal(
    writableNodePtyRoot('/AuraForge.app/Contents/Resources/app.asar/dist/node_modules/node-pty'),
    '/AuraForge.app/Contents/Resources/app.asar.unpacked/dist/node_modules/node-pty'
  )
  assert.equal(
    writableNodePtyRoot('/AuraForge.app/Contents/Resources/app.asar.unpacked/dist/node_modules/node-pty'),
    '/AuraForge.app/Contents/Resources/app.asar.unpacked/dist/node_modules/node-pty'
  )
})

test('uses the unpacked helper when resolution reports an app.asar node-pty root', () => {
  const archivedRoot = '/AuraForge.app/Contents/Resources/app.asar/dist/node_modules/node-pty'
