 ...
 * runtime, remote gateways, Aura Forge Cloud instances, SSH hosts) that are all
...
/**
 * Whether "Update all instances" may drive this connection. Aura Forge Cloud
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Aura Forge Cloud' : 'Remote gateway'),
