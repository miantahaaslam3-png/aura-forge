 ...
 *     `X-AuraForge-Session-Token` header; WS uses `?token=`.
...
// auraforge_cli/dashboard_auth/cookies.py.
//
...
// The Nous portal (NAS) does NOT use Aura Forge gateway session cookies — it is a
...
// Keep this aligned with auraforge_cli.profiles.validate_profile_name(). `default`
...
/** Which Aura Forge profile the remote SSH dashboard should actually run as.
...
// Aura Forge Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  // name used by the remote Aura Forge installation. Preserve an explicit mapping
  // when it is a valid Aura Forge profile identifier; otherwise fall back to the
 ...
 * the Aura Forge gateway session cookies, so cloud sign-in / discovery liveness
