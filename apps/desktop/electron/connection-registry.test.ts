...
test('primary SSH reuse rejects a descriptor with a different remote Aura Forge path', async () => {
  const registry = migrateV1ToRegistry({
    mode: 'ssh',
...
test('registry primary reuses a matching primary backend descriptor', () => {
  const registry = normalizeRegistry({
    ...
    connections: [
      ...
      { id: 'hermes-vps', kind: 'ssh', label: 'Aura Forge VPS', host: 'hermes-vps' }
    ]
  })
