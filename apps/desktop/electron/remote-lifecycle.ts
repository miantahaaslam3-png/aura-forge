 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Aura Forge dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Aura Forge install on the remote (login-shell probe),
...
async function locateHermes(ssh, remoteHermesPath) {
  const resolveLauncher = async (candidate: string) => {
    ...
    //     the Aura Forge version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/miantahaaslam3-png/aura-forge/issues/74411
    return candidate
  }
  ...
  if (remoteHermesPath) {
    ...
    const err: any = new Error(
      `The Aura Forge path you set is not an executable on the remote host: "${remoteHermesPath}". ` +
  ...
  const err: any = new Error(
    'Aura Forge is not installed on the remote host (could not find a `hermes` executable). ' +
      'Install it on the remote with:  curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh  ' +
      '— or set the Aura Forge path explicitly in the SSH connection settings.'
  )
...
// e.g. "Aura Forge Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH hermes a
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Aura Forge Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
async function probeRemoteHermesHome(ssh) {
  try {
    ...
    const error: any = new Error('Could not resolve the remote Aura Forge home.')
 ...
 * bytes; it never imports or executes code from the changing Aura Forge checkout.
...
async function assertRemoteInstallUpdateClear(ssh, hermesHome) {
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Aura Forge install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Aura Forge update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Aura Forge update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteHermesProfiles(ssh) {
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Aura Forge profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Aura Forge home.')
...
async function spawnRemoteDashboard(
  ...
  if (!(await remoteSupportsSshOwnership(ssh, hermesPath))) {
    const err: any = new Error(
      'The remote Aura Forge install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Aura Forge on the remote host to continue using Desktop SSH mode.'
    )
...
async function connect(deps) {
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Aura Forge Desktop build (${lock.reason}). ` +
