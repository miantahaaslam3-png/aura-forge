// Backend subcommand routing for the desktop-managed Aura Forge process.
//
 ...
 * @param {string} [profile] optional Aura Forge profile to pin via `--profile`.
 */
 ...
 * `-m auraforge_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `auraforge_cli/subcommands/dashboard.py` source registers
