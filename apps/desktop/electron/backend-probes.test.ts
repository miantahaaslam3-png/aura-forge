...
// have auraforge_cli" (since `node -c "import auraforge_cli"` will exit
...
test('canImportHermesCli returns false when interpreter cannot run -c', () => {
  // node IS an interpreter, but `node -c "import auraforge_cli"` is a
...
test('hermes runtime import probe checks config dependencies', () => {
  ...
  // (auraforge_cli/env_loader.py); a mid-update venv missing python-dotenv
  // passed the old probe and produced an unrecoverable boot loop.
  assert.match(probe, /\bimport dotenv\b/)
  assert.match(probe, /\bimport auraforge_cli\.config\b/)
})

test('explicit Aura Forge override is authoritative', () => {
  assert.equal(shouldTrustHermesOverride('/nix/store/abc/bin/hermes'), true)
})

test('empty Aura Forge override is not authoritative', () => {
