// window-below.ts — which OS window sits directly underneath a Aura Forge window.
//
...
export function enumerationFailureNote(platform: string, env: NodeJS.ProcessEnv, detail?: string): string {
  ...
  if (env.HYPRLAND_INSTANCE_SIGNATURE) {
    return (
      ...
      'Check that `hyprctl clients` works from the same session Aura Forge is ' +
      'running in.'
    )
  }
  ...
  if (wayland) {
    return (
      ...
      'X11/Xorg session, or run Aura Forge under XWayland with DISPLAY set.'
    )
  }
 ...
 *
 * Walks past every window owned by our own process (all Aura Forge windows share
...
export async function readWindowBelow(
  ...
  if (process.platform === 'darwin' && !titlesAvailable) {
    result.note =
      ...
      'Screen Recording permission, which Aura Forge does not request for this.'
  }
