...
test('Windows probe validates Aura Forge and Python topology before selection', async () => {
  let script = ''
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Aura Forge is not installed on the remote Windows host.')
      })
    ),
