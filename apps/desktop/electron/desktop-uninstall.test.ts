...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'auraforge_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'auraforge_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'auraforge_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/AuraForge.app/Contents/MacOS/Hermes', 'darwin'),
    '/Applications/AuraForge.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/AuraForge.app/Contents/MacOS/Hermes', 'darwin'),
    '/Users/x/Applications/AuraForge.app'
  )
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/AuraForge.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/AuraForge.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    ...
    uninstallArgs: ['-m', 'auraforge_cli.uninstall', '--mode', 'gui'],
  ...
  assert.match(script, /'-m' 'auraforge_cli\.uninstall' '--mode' 'gui'/)
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    agentRoot: '/home/x/.hermes/hermes-agent',
    uninstallArgs: ['-m', 'auraforge_cli.uninstall', '--mode', 'full'],
    appPath: null,
    hermesHome: '/home/x/.hermes'
  })

  // System python + source on PYTHONPATH so import auraforge_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.hermes\/hermes-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'auraforge_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'auraforge_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    hermesHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'auraforge_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  ...
  // PYTHONPATH set so a system python can import auraforge_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\hermes;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "auraforge_cli\.uninstall" "--mode" "full"/)
