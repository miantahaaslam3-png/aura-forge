 ...
 * path) but no auraforge_cli in its site-packages: the resolver hands back
 * a backend the spawn step can't actually run, and the user gets a
 * dead-on-arrival "ModuleNotFoundError: No module named 'auraforge_cli'"
...
/**
 * Return the Python snippet used to verify Aura Forge can import far enough to
...
/**
 * Return true iff the Aura Forge runtime import probe exits 0.
 *
 * Used to gate the "fallback to system Python with auraforge_cli installed"
 * rung of resolveHermesBackend. Without this, a system Python 3.11-3.13
 * registered in PEP 514 makes findSystemPython() succeed regardless of
 * whether auraforge_cli has actually been pip-installed into its
 * site-packages -- and the resolver returns a backend that immediately
 * dies on spawn.
 *
 * The probe intentionally imports auraforge_cli.config, not just the top-level
