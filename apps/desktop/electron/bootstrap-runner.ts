 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Aura Forge Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.auraforge-bootstrap-complete'), 'utf8')
...
// checkout under ~/.aura-forge/aura-forge-agent. Used as a last-resort fallback when
...
function installedAgentInstallScript(hermesHome) {
  ...
  const candidate = path.join(hermesHome, 'aura-forge-agent', 'scripts', installScriptName())
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/miantahaaslam3-png/aura-forge/${ref}/scripts/${scriptName}`
