...
<p align="center">
  <a href="https://github.com/miantahaaslam3-png/aura-forge/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
**The native desktop app for [Aura Forge Agent](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Aura Forge surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Aura Forge and hear it back.</td></tr>
...
### Install with Aura Forge (recommended)

Already have the Aura Forge CLI? Just run:

```bash
hermes desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Aura Forge gateway or install Aura Forge locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Aura Forge Desktop website.](https://hermes-agent.nousresearch.com/).
...
first launch it can install the Aura Forge Agent runtime into `HERMES_HOME`
...
- **Aura Forge Agent** runs as a headless `hermes serve` process and exposes the
...
5. a system Python that can import the Aura Forge runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Aura Forge
...
screen offers **Connect to existing Aura Forge** before starting the local installer.
...
terminal commands, and file operations run against the remote Aura Forge host, not
...
`Host`, `Origin`, `Referer`, and `X-AuraForge-Session-Token`.
...
tccutil reset Microphone com.auraforge.desktop
```
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\hermes\hermes-agent\.auraforge-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\hermes\hermes-agent\venv"
```

> The default Aura Forge home on Windows is `%LOCALAPPDATA%\hermes`. Set the `HERMES_HOME` env var if you've relocated it.
