/**
 * Shared E2E fixtures for the Aura Forge desktop Playwright suite.
 *
 * Two fixture modes:
 *
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { HERMES_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Aura Forge backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Aura Forge.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'AuraForge.app', 'Contents', 'MacOS', 'Aura Forge')
  }
 ...
 * progress without spawning a real Aura Forge backend.
 *
