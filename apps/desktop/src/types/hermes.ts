...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Aura Forge doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in auraforge_cli/provider_catalog.py). When present, the Keys tab groups by
...
// non-desktop) Aura Forge simply carries none of it. See store/profile-share.ts.
...
// folders. Mirrors auraforge_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Aura Forge, 'hub' = installed. */
 ...
 *  not Aura Forge); elsewhere, driver health from `cua-driver doctor`. `null`
