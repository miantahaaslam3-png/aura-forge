...
export const ja = defineLocale({
  ...
  settings: {
    ...
    gateway: {
      ...
      intro:
        'Aura Forge Desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Aura Forge バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。以下でプロファイルを選択して、それぞれのリモートホストを設定します。',
      appliesTo: '適用対象',
  ...
  shell: {
    ...
    statusbar: {
      ...
      gatewayTitle: 'Aura Forge 推論ゲートウェイのステータス',
      agents: 'エージェント',
