...
describe('speaker labels', () => {
  it('the default profile speaks as Aura Forge in transcripts, not @default', async () => {
    ...
    expect(line).toBe('Aura Forge: hello room')
    ...
    ).toBe('Aura Forge (you): hi')
    expect(
      rounds.formatGroupChatLine({ from: { kind: 'member', name: 'builder' }, text: 'yo' } as GroupMessage, 'research')
    ).toBe('builder: yo')
  })

  it('honor friendly identity: Bot Mode title, then display_name, never a stale Aura Forge', async () => {
    ...
    // "Aura Forge is thinking…" in group rooms.
    ...
    // Untitled rows keep today's behavior: default → Aura Forge, others verbatim.
    data.$botMeta.set({})

    expect(chat.groupSpeakerLabel('default')).toBe('Aura Forge')
