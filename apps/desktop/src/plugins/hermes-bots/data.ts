 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "Aura Forge"
...
export function newBotChat(bot: RosterRow) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({
      kind: 'error',
      message:
        getPluginCtx()?.i18n?.t('bot.openAnotherChatUnsupported') ?? 'Update Aura Forge Desktop to open another Bot chat.'
    })
