 ...
 *    desktop shell fires only while the user is AWAY from Aura Forge. This is the
...
function notifyOne(kind: string, spec: { titleKey: string; toast: ToastKind }, ev: CompletionEvent): void {
  ...
  // is away from Aura Forge (the toast above covers the foreground case). Isolated:
