...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Aura Forge gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.hermesDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Aura Forge Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<HermesGateway | null> {
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Aura Forge gateway is not connected" on the Sessions `+` action (#88880).
