...
export function handleStatusEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === 'notification.show') {
    ...
    // native OS notification when Aura Forge is backgrounded; dispatch is gated
  ...
  if (event.type === 'error') {
    const errorMessage = payload?.message || 'Aura Forge reported an error'
