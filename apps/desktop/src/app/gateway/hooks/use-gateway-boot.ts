...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    // composer stayed disabled forever on "Starting Aura Forge...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Aura Forge…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Aura Forge backend'
        )
        ...
        // as an opaque "Could not connect to Aura Forge gateway". resolveGatewayWsUrl
    ...
    async function boot() {
      try {
        ...
        // hanging "Starting Aura Forge…" forever.
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          // retry it), not brick boot behind the "Aura Forge couldn't start"
