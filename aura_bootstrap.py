"""Windows UTF-8 bootstrap for Aura entry points.
...
should be imported at the very top of every Aura entry point
(``aura``, ``aura-forge``, ``aura-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Aura modules.

    Aura ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Aura source root
    ...
    """
    root = src_root or os.environ.get("AURA_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``AURA_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("AURA_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import aura_bootstrap``
# (or ``from aura_bootstrap import apply_windows_utf8_bootstrap``) at
