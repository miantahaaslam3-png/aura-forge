...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.auraforge to the active profile home."""
    ...
    if raw == "~/.auraforge":
        return hermes_home
    if raw.startswith("~/.auraforge/"):
        return hermes_home / raw.removeprefix("~/.auraforge/")
...
def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    ...
    if raw_text == "~/.auraforge" or raw_text.startswith("~/.auraforge/"):
