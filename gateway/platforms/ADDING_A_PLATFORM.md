...
There are two ways to add a platform to the Aura Forge gateway:
...
**zero changes to core Aura Forge code**.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `auraforge_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Aura Forge core.
...
## 12. Status Display (`auraforge_cli/status.py`)
...
## 13. Gateway Setup Wizard (`auraforge_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ auraforge_cli/ toolsets.py \
  --include="*.py" -l | sort -u
