...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Aura Forge-Session-Id header; opt-in long-term memory scoping via X-Aura Forge-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Aura Forge-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists auraforge-agent and any configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible Aura Forge sessions
- POST /api/sessions               — create an empty Aura Forge session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to auraforge-agent
...
_BROWSER_CONTROL_WS_PROTOCOL = "auraforge-browser-control-v1"
_BROWSER_CONTROL_TICKET_PROTOCOL_PREFIX = "auraforge-browser-control-ticket."
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``auraforge-agent``)
    ...
    ``direct_model_requests`` config value here, while Aura Forge-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously Aura Forge-aware and is
    always honored.
    """
...
def _derive_chat_session_id(
    ...
    the same Aura Forge session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through auraforge-agent's AIAgent.
    """
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send an explicit ``provider`` — and the Aura Forge-native session-chat
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        try:
            ...
            pass
        return resolve_effective_model(explicit, profile_name, "auraforge-agent")
    ...
    def _resolve_request_profile(self, request: "web.Request"):
        ...
        if not getattr(cfg, "multiplex_profiles", False):
            ...
            # `auraforge peer dm mini/researcher` answered by the mini's default
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Aura Forge-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Aura Forge-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Aura Forge-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Aura Forge-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Aura Forge-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``auraforge sessions list``
    ...
    def _stored_session_model(self, session: Any) -> Optional[str]:
        ...
        The advertised virtual model (usually ``auraforge-agent``) means "use
        ...
        400s ("auraforge-agent is not a valid model ID") — the same filter
    ...
    def _session_runtime_request_from_body(self, body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # The virtual model alias (self._model_name, e.g. "auraforge-agent") is
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the auraforge-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-Aura Forge-Session-Key``).  Unlike ``session_id``
        ...
        # (e.g. user ran `auraforge auth add openai-codex` without `auraforge model`),
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "auraforge-agent", "version": _hermes_version()}
        )
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list auraforge-agent and any configured model_routes aliases.
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return Aura Forge provider/model inventory.
        ...
        Aura Forge provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Aura Forge version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            "object": "auraforge.api_server.capabilities",
            "platform": "auraforge-agent",
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Aura Forge AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-Aura Forge-Session-Id",
                "session_key_header": "X-Aura Forge-Session-Key",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Aura Forge sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        # Exact-title lookup, used by `auraforge peer dm` to resolve a peer's
        ...
        if title_filter:
            ...
            if not sessions:
                ...
                # which would fail `auraforge peer dm` resolution and mint
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Aura Forge session row.
        ...
        # (e.g. "provider::auraforge-agent") are split, and the virtual model
        # alias (self._model_name, e.g. "auraforge-agent") is nulled out
        # there — a bare "auraforge-agent" is not a model_routes alias, so a
        ...
        return web.json_response({"object": "auraforge.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "auraforge.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "auraforge.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "auraforge.session", "session": self._session_response(fork)}, status=201)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-Aura Forge-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-Aura Forge-Session-Key"] = gateway_session_key
        runtime = {}
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        headers = {
            ...
            "X-Aura Forge-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-Aura Forge-Session-Key"] = gateway_session_key
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "auraforge.session.model_lock",
            "session_id": session_id,
            "runtime": runtime,
        })
    @_admit_api_agent_request
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Aura Forge-Session-Key.  This
        # is independent of X-Aura Forge-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Aura Forge-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Aura Forge-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Aura Forge-Session-Id rejected: "
            ...
            # conversation map to the same Aura Forge session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``auraforge.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-Aura Forge-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Aura Forge-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["auraforge"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-Aura Forge-Completed"] = "false"
            response_headers["X-Aura Forge-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Aura Forge-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Aura Forge-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Aura Forge-Error"] = _redact_api_error_text(err_msg, limit=200)
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Aura Forge-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Aura Forge-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: auraforge.tool.progress`` SSE event so
                frontends can display them without storing the markers in
                conversation history.  See #6972 for the original event,
                #16588 for the ``toolCallId``/``status`` lifecycle fields.
                """
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    await response.write(_sse_frame(item[1], event="auraforge.tool.progress"))
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
                        "created": created, "model": model,
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # response and the X-Aura Forge-Session-Id header.  Without this,
        ...
        response_headers = {"X-Aura Forge-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-Aura Forge-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _handle_run_job(self, request: "web.Request") -> "web.Response":
        ...
        # `auraforge cron run` / cronjob(action='run', prompt=...) — same length
            ...
            if role == "assistant" and msg.get("tool_calls"):
                for tc in msg["tool_calls"]:
                    ...
                    items.append({
                        ...
                        # Aura Forge agent; they are replayed for structured tool
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    # (e.g. X-Aura Forge-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
                    ...
                    with self._profile_scope(request_profile):
                        try:
                            ...
                            session_tokens = self._bind_api_server_session(
                                ...
                                # X-Aura Forge-Session-Id equivalent) exactly like
        ...
        response_headers = (
            {"X-Aura Forge-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "auraforge.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def _handle_steer_run(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "auraforge.run.steer", "run_id": run_id, "accepted": True})
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local Aura Forge-Relay
            ...
            # that is the exact surface the auraforge-0day campaign abused to write
            # ~/.auraforge/config.yaml and plant persistence. Sandboxing (Docker /
