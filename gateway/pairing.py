...
# working (miantahaaslam3-png/aura-forge#93449).
#
...
class PairingStore:
    ...
    as ``auraforge -p <name> pairing ...``. This keeps multiplex gateways and
    ...
    def __init__(self, profile: Optional[str] = None):
        ...
        if profile:
            ...
            # ``auraforge -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `auraforge` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> auraforge pairing approve ...` as root, "
                    "re-run with `docker exec -u auraforge <container> ...` and "
                    "chown the existing file to the auraforge user, or restart the "
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``auraforge
