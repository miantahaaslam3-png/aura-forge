...
def _kanban_dispatch_allowed() -> bool:
    """Return False while the global emergency stop (`auraforge pause`) is engaged.
...
class GatewayKanbanWatchersMixin:
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        no separate `auraforge kanban daemon` process needed. When false, the
            ...
            try:
                ...
                if _is_corrupt_board_db_error(exc):
                    ...
                    logger.error(
                        ...
                        "then run `auraforge kanban init` if you need a fresh board.",
                        slug,
                        fingerprint[0],
                    )
                    return None
        ...
        def _ready_nonempty() -> bool:
            ...
            task on ANY board whose assignee maps to a real Aura Forge profile
            ...
            PATH, missing venv, credential loss for a real Aura Forge profile).
            """
        ...
        def _auto_decompose_tick(auto_decompose_per_tick: int) -> int:
            ...
            """
            try:
                from auraforge_cli import kanban_decompose as _decomp
        ...
        while self._running:
            ...
            try:
                # Global emergency stop (`auraforge pause`): skip auto-decompose
                ...
                if bad_ticks >= HEALTH_WINDOW:
                    ...
                    if now - last_warn_at >= 300:
                        logger.warning(
                            ...
                            "`auraforge kanban list --status ready`.",
                            bad_ticks,
                        )
                        last_warn_at = now
