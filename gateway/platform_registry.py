...
class PlatformEntry:
    ...
    # status displays (``auraforge setup``, ``auraforge status``, the dashboard
    ...
    # Env vars this platform needs (for ``auraforge setup`` display).
    ...
    # built-ins).  Used by ``auraforge gateway setup`` to auto-enable the
    ...
    # hard-casing in Aura Forge core.
    #
    # Signature:
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `auraforge`
        # invocation -- including plain `auraforge chat`, which never touches any
        ...
        # `auraforge setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``auraforge setup``/``gateway status``, channel
