"""Relay/connector support package for the Aura Forge gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``auraforge gateway enroll`` writes them to
    ``~/.auraforge/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``auraforge gateway enroll``).
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from auraforge_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # multi-agent scope it would prefix every reply "**Aura Forge Agent:**" —
...
def _resolve_relay_identity_token() -> str:
    ...
    ``auraforge gateway enroll`` CLI. Three modes, in precedence order:
    ...
    if not token_url:
        ...
        from auraforge_cli.auth import resolve_nous_access_token
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.auraforge/.env``.
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``auraforge gateway enroll``: has a PINNED
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'auraforge'}"
