...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from auraforge_cli.profiles import normalize_profile_name, validate_profile_name
...
# dashboard's pre-write mutation validation (auraforge_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Aura Forge is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        if multiplex_profiles is None and isinstance(nested_gateway, dict):
            ...
            # ``auraforge config set gateway.multiplex_profiles true``.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.auraforge/config.yaml (primary user-facing config)
    3. ~/.auraforge/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through auraforge_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `auraforge config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `auraforge config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``auraforge config set gateway.streaming.*``
