...
This document is the formal interface between the **Aura Forge gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `aura gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`aura gateway enroll` → connector `/relay/enroll`, or
