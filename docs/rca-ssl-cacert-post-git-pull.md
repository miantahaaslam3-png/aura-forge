# RCA: SSL CA cert bundle corruption after `aura update`
...
When the venv is partially refreshed, or when one of those env vars points at a file that no longer exists, provider client construction can fail before Aura Forge has enough context to produce a useful message.
...
For a normal corrupted Aura Forge venv, reinstall the affected client dependencies:

```bash
python -m pip install --force-reinstall certifi openai httpx
```

For a custom/corporate CA setup, fix the env var so it points at a real PEM bundle, or unset it if Aura Forge should use the bundled `certifi` store.
