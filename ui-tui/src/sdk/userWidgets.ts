...
/**
 * User widget apps — Aura Forge authors its own TUI widgets, mirroring the
 ...
 *  every change, so a widget Aura Forge writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Aura Forge) ever writes must hot-load too — a 10s poll here read
