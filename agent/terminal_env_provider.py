...
Providers live in ``~/.auraforge/plugins/<name>/`` (user, opt-in via
...
* ``cache_path_base`` — where auto-synced ``~/.auraforge/cache`` files land
  inside the backend (e.g. ``"~/.auraforge"`` for home-synced backends,
  ``"/root/.auraforge"`` for root-homed containers), or ``None`` when host
...
class TerminalEnvironmentProvider(abc.ABC):
    ...
    def cache_path_base(self) -> Optional[str]:
        """Base dir for synced Aura Forge cache files inside the backend."""
        return None
    ...
    def setup_instructions(self) -> List[str]:
        """Lines printed by ``auraforge setup`` after this backend is selected.
