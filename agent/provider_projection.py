"""Fold an agent-as-provider's own activity back into Aura Forge' turn state.

Most providers are models: they ask Aura Forge to run a tool and Aura Forge runs it, so
...
time Aura Forge sees the response that work is already done.

Those calls must never come back as pending ``tool_calls`` — Aura Forge would re-run
...
* the **skill-review nudge**, whose counter (``_iters_since_skill``) only moves
  on Aura Forge tool iterations, of which there are none.
