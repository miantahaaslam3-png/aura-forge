...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("auraforge-verify-", "auraforge-ad-hoc-")
...
def record_verify_run(
    *,
    ...
    session_id: str | None = None,
    ok: bool,
    command: str = "auraforge verify",
    scope: str = "full",
    output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``auraforge verify`` run as verification evidence.

    Explicit CLI-side write: unlike :func:`record_terminal_result` there is
    nothing to classify — the caller (the ``auraforge verify`` command) already
    ...
    evidence = VerificationEvidence(
        command=command,
        canonical_command="auraforge verify",
        kind="verify",
