...
KANBAN_GUIDANCE = (
    ...
    "every assignee in a real profile (`aura profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `aura kanban <verb>` for board operations. Use "
...
#
# The Aura Forge runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to Aura Forge's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `aura computer-use doctor` and share the output. That command "
...
def build_environment_hints() -> str:
    ...
    # Aura Forge desktop GUI — any agent running under the desktop app should know
    ...
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Aura Forge desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "first. It has the actual commands (e.g. `aura config set …`, `aura tools`, "
            "`aura setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: aura setup, aura setup tools, aura setup terminal, aura status.",
        ]
    )
    return "\n".join(lines)
