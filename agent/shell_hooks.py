...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`auraforge_cli.plugins.invoke_hook` and its aggregators.  Python
...
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``auraforge_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
from auraforge_cli._subprocess_compat import IS_WINDOWS, kill_process_tree, windows_hide_flags
...
# auraforge_cli.plugins.get_pre_tool_call_block_message / _get_pre_tool_call_
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``auraforge_cli.config.load_config``
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    """
    from auraforge_cli.plugins import SHELL_UNSUPPORTED_HOOKS, VALID_HOOKS
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a Aura Forge wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical Aura Forge
    ``{"action": "block", "message": "..."}`` shape expected by
    :func:`auraforge_cli.plugins.get_pre_tool_call_block_message`.  This is
    ...
    if event == "pre_verify":
        # "continue" (Aura Forge) / "block" (Claude-Code Stop: block the stop) both
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ Aura Forge is about to register a shell hook that will run a\n"
...
def run_once(
    ...
    ``kwargs`` is the same dict that :func:`auraforge_cli.plugins.invoke_hook`
