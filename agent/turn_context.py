...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``auraforge logs``.
    ...
    if (
        ...
        # Aura Forge only initiates compaction in "auraforge" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Aura Forge preflight compression for codex app-server "
                "(mode=%s); Aura Forge will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Aura Forge (via _emit_status), so it always
