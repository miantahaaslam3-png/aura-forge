...
that instructs the live agent to:
  ...
  1. Gather the sources the user named, using the tools it already has
     ...
     material).
  2. Author a single ``SKILL.md`` via ``skill_manage`` that follows the Aura Forge
     skill-authoring standards (description <=60 chars, the modern section
     order, Aura Forge-tool framing, no invented commands).
...
- version: 0.1.0
- author: always the literal value `Aura Forge`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as Aura Forge.
...
4. "## How to Run" — the canonical invocation, framed through Aura Forge tools.
