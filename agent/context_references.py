...
from auraforge_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Aura Forge path and cannot be attached")
    ...
    # message text into reference expansion, so `@file:~/.auraforge/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Aura Forge path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
