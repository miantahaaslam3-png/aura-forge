"""Unified removal contract for every credential source Aura Forge reads from.

Aura Forge seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.auraforge/.env
    claude_code   — ~/.claude/.credentials.json
    hermes_pkce   — ~/.auraforge/.anthropic_oauth.json
    ...
    manual        — user ran `auraforge auth add`
...
unify here is **removal**:
    ...
    ``auraforge auth remove <provider> <N>`` must make the pool entry stay gone.
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.auraforge/.env  → clear it
    ...
    """
    from auraforge_cli.config import get_env_path, remove_env_value
    ...
    try:
        ...
        if env_path.exists():
            ...
            # canonical reader in auraforge_cli/config.py. read_text() with no
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.auraforge/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Aura Forge.",
            f"  The pool entry is now suppressed — Aura Forge will ignore "
            f"{env_var} until you run `auraforge auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Aura Forge stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `auraforge auth add anthropic` to re-enable if needed.",
    ])


def _remove_hermes_pkce(provider: str, removed) -> RemovalResult:
    """~/.auraforge/.anthropic_oauth.json is ours — delete it outright."""
    ...
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Aura Forge Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `auraforge auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `auraforge auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``auraforge auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `auraforge model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Aura Forge auth store is not enough — _seed_from_singletons() would
    ...
    via ``auraforge auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `auraforge auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `auraforge auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Aura Forge stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `auraforge auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        ...
        description="~/.auraforge/.anthropic_oauth.json",
    ))
    register(RemovalStep(
