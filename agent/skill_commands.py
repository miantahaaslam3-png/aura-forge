...
#
# When a user invokes a /skill (or /bundle), Aura Forge expands the turn into a
...
def _resolve_skill_commands_home() -> str:
    """Return the effective Aura Forge home the skill scan should be scoped to.
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.auraforge.config``
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.auraforge/skills/ and return a mapping of /command -> skill info.
        ...
        for scan_dir in dirs_to_scan:
            ...
            for skill_md in _iter:
                ...
                try:
                    ...
                    # with a core Aura Forge slash command (name or alias). The
                    ...
                    if resolve_command(cmd_name) is not None:
                        logger.warning(
                            ...
                            "collides with a core Aura Forge command; skipping "
...
def get_skill_commands() -> Dict[str, Dict[str, Any]]:
    ...
    active profile's Aura Forge home changes (e.g. Desktop switching profiles
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.auraforge/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``auraforge -s <skill>`` or
