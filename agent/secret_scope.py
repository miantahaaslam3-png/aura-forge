...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Aura Forge writes
    ...
    ``auraforge_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Aura Forge parser: save_env_value
    ...
    from auraforge_cli.config import _parse_env_value
...
def build_profile_secret_scope(hermes_home: Path) -> Dict[str, str]:
    ...
    try:
        from auraforge_cli.env_loader import get_secret_source_values
