...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Aura Forge with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.auraforge/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Aura Forge
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``auraforge tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``auraforge setup`` for
