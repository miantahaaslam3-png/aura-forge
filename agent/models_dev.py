...
Data resolution order:
  ...
  2. Disk cache (~/.auraforge/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Aura Forge ↔ models.dev
# ---------------------------------------------------------------------------

# Aura Forge provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; Aura Forge' provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → Aura Forge ids (built lazily; many-to-one,
...
def _models_dev_to_hermes_ids(mdev_id: str) -> List[str]:
    """Return the Aura Forge provider ids that map to *mdev_id* (may be [])."""
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``auraforge config refresh``, the
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the Aura Forge provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by Aura Forge id.
...
def _get_provider_models(
    ...
    """Resolve a Aura Forge provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Aura Forge agent traffic.
...
def get_provider_info(
    ...
    Accepts either a Aura Forge provider ID (e.g. "kilocode") or a models.dev
    ...
    ``allow_network=False``.
    """
    # Resolve Aura Forge ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Aura Forge or models.dev provider ID.  Tries exact match then
