"""OpenAI-shape bridge shared by Aura Forge' ACP clients.

An ACP agent (``copilot --acp``, and the ACP CLIs that reach Aura Forge as
...
the agent's *own* tool notifications. Aura Forge' agentic surface — ``memory``,
...
* :func:`completion_to_stream_chunks` — re-shape a one-shot ACP response as
  ...
  is inherently one-shot from Aura Forge' perspective).
...
of its own (Copilot) forwards everything Aura Forge offers; a CLI that is an
autonomous agent with its own read/edit/execute tools must forward only Aura Forge'
agent-level tools, because re-offering the overlapping ones makes Aura Forge re-run
...
class StreamChunks(list):
    ...
    Aura Forge reads provider-level extras off the object returned by
...
def tool_specs_from_openai_tools(
    ...
    *,
    ...
    name is in it survive — that is how a client forwards just Aura Forge'
