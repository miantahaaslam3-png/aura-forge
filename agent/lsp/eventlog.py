...
Grep recipe::
    ...
    tail -f ~/.auraforge/logs/agent.log | rg 'lsp\\['
"""
...
event_log = logging.getLogger("auraforge.lint.lsp")
...
def log_server_unavailable(server_id: str, binary_or_pkg: str) -> None:
    ...
    if _announce_once(_announced_unavailable, key):
        _emit(
            ...
            "(install via `auraforge lsp install <id>` or set lsp.servers.<id>.command)",
        )
    else:
