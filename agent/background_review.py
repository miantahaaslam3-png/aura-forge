...
See the ``auraforge-agent-dev`` skill (``references/self-improvement-loop.md``)
...
def _parent_can_emit_tool_calls(agent: Any) -> bool:
    ...
    calls. A provider that IS an autonomous agent reaches Aura Forge through a client
    shim, and a shim that cannot carry Aura Forge tool calls back turns the fork into
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Bundled skills (shipped with Aura Forge, e.g. 'auraforge-agent').\n"
    "  • Hub-installed skills (installed via 'auraforge skills install').\n"
    "  • Skills in skills.external_dirs (externally owned).\n"
    "  • PINNED skills (marked via 'auraforge curator pin'). You are an "
    ...
    "'auraforge curator adopt <name>' — do not try to patch it.\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "  • Hub-installed skills (installed via 'auraforge skills install').\n"
    "  • Skills in skills.external_dirs (externally owned).\n"
    "  • PINNED skills (marked via 'auraforge curator pin'). Pin blocks "
    ...
    "reply and recommend 'auraforge curator adopt <name>' instead.\n"
    "If the only skills that need updating are protected, say\n"
    "'Nothing to save.' and stop.\n\n"
...
def _run_review_in_thread(
    ...
    # An agent-as-provider whose client can't carry Aura Forge tool calls back would
    ...
    if not _parent_can_emit_tool_calls(agent) and not bool(
        ...
        logger.warning(
            "Background review skipped: provider %r cannot emit Aura Forge tool calls, "
