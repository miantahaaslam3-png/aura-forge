"""Projects codex app-server events into Aura Forge' messages list.

The translator that lets Aura Forge' memory/skill review keep working under the
...
Aura Forge' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Aura Forge
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Aura Forge' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Aura Forge stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Aura Forge already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Aura Forge' messages store text only.
        text_parts: list[str] = []
