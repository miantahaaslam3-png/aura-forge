...
Owns one Codex thread per Aura Forge session. Drives `turn/start`, consumes
...
# Aura Forge' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Aura Forge/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Aura Forge session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        into Aura Forge' messages shape.
        ...
        # supports rich content but Aura Forge' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Aura Forge' approval
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Aura Forge approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Aura Forge approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Aura Forge returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Aura Forge approval mode/timeout resolution
...
def _get_hermes_version() -> str:
    """Best-effort Aura Forge version string for codex's userAgent line."""
    try:
