...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in auraforge_state.py — kept local
# to avoid importing auraforge_state at module load time (its module-level
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Aura Forge tool use: "
    ...
    return (
        ...
        f"context, but Aura Forge needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Aura Forge config, set `model.ollama_num_ctx: 65536` "
...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    def host_info_value(label: str) -> str:
        ...
        occurrence, so only Aura Forge' own emitted block can satisfy the read.
        """
        prefix = f"{label}:"
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from auraforge_cli.moa_config import decode_moa_turn
    ...
    # 'disk', or 'unknown' — see auraforge_state.classify_persistence_error).
    ...
    # all run inside Codex). Default Aura Forge path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Aura Forge internals.
        #
        # Aura Forge invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Aura Forge tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                try:
                    from auraforge_cli.middleware import apply_llm_request_middleware
                ...
                try:
                    from auraforge_cli.lifecycle import (
                        has_hook,
                ...
                from auraforge_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Aura Forge/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if (
                    ...
                    from auraforge_constants import display_hermes_home as _dhh_fn
                    _dhh = _dhh_fn()
                    _body_text = ""
                    try:
                ...
                # and omits the field, and Aura Forge' local compression takes
                ...
                if getattr(api_error, "status_code", None) == 404:
                    try:
                        from auraforge_cli.model_normalize import suggest_prefixed_model_id
                ...
                # providers (DeepSeek, Kimi, MiMo, Aura Forge).  A 429 can
                ...
                # which Aura Forge' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Aura Forge' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # Aura Forge may add API-only content not present in persisted
                ...
                if is_client_error:
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        _policy_response = (
                            ...
                            "(not a Aura Forge/gateway failure).\n\n"
        ...
        try:
            ...
            # with the iterations Aura Forge never saw. Appended before this turn's
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates Aura Forge mid-turn, resume logic still sees the
                ...
                try:
                    ...
                    from auraforge_cli.plugins import get_pre_verify_continue_message
