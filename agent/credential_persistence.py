...
``auth.json``.  They intentionally have no dependency on ``auraforge_cli.auth`` so
...
# Sources Aura Forge owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Aura Forge-owned OAuth/device-code state)
