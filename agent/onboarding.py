...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Aura Forge starts and finds ``~/.openclaw/``.

    Points users at ``auraforge claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``auraforge claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Aura Forge, run "
        "`auraforge claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `auraforge claw cleanup` (renames it to ~/.openclaw.pre-migration — "
