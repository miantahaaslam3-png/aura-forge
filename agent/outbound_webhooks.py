...
(``gateway/platforms/webhook.py``): inbound wakes Aura Forge when the world
changes; outbound tells the world when Aura Forge does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Aura Forge-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
Config schema (``~/.auraforge/config.yaml``)::

    hooks:
      outbound:
        - url: https://ci.example.com/auraforge-events
          ...
          name: ci-notify   # optional label for logs / `auraforge hooks list`
...
Headers::
    ...
    User-Agent:              Aura Forge-Agent-Outbound-Webhook
    X-Aura Forge-Event:          <hook event name>
    X-Aura Forge-Delivery:       <delivery_id>
    X-Aura Forge-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Aura Forge-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-Aura Forge-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
