"""Core NeMo Relay adapters for physical Aura Forge provider attempts."""
...
def _relay_operation_name(provider_name: str, metadata: dict[str, Any] | None) -> str:
    """Return Relay's canonical operation name when Aura Forge knows the API mode."""
...
def execute_current(
    ...
    *,
    ...
    """Run a provider attempt under the inherited Aura Forge turn when present."""
...
def stream_current(
    ...
    *,
    ...
    """Run a provider stream under the inherited Aura Forge turn when present.
...
class ManagedLlmStream(Iterator[Any]):
    """Drive Relay's async stream from Aura Forge's provider worker thread."""
...
class AnthropicStreamAccumulator:
    ...
    def response(self, base: Any = None) -> Any:
        """Return the attribute-shaped response consumed by Aura Forge."""
    ...
    *,
    ...
    with turn.finalize_lock:
        ...
        try:
            ...
            logger.warning(
                "Aura Forge Relay logical LLM finalization failed",
                exc_info=True,
            )
            return
        with turn.logical_llm_lock:
