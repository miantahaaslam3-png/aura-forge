...
Every Aura Forge request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Aura Forge Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=auraforge-agent",
        "client=auraforge-client-v<__version__>",
    ]

The version is sourced live from ``auraforge_cli.__version__`` so it auto-aligns
...
``auraforge_cli.__version__`` is the canonical source of truth.
"""
...
def _hermes_version() -> str:
    """Return the current Aura Forge release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``auraforge_cli`` cannot be imported (should
...
def hermes_client_tag() -> str:
    ...
    Format: ``client=auraforge-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=auraforge-client-v{_hermes_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Aura Forge session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Aura Forge conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.auraforge/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Aura Forge
    ...
    """
    tags = ["product=auraforge-agent", hermes_client_tag()]
