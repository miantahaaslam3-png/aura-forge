...
Plugins built on Aura Forge Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Aura Forge
...
:class:`~auraforge_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Aura Forge' main config
...
handles every provider, fallback chain, and per-task override Aura Forge
supports.
"""
...
def _resolve_task_ownership(plugin_id: str) -> tuple[frozenset, frozenset]:
    ...
    try:
        ...
        pass
    try:
        from auraforge_cli.main import _AUX_TASKS
...
class PluginLlm:
    ...
    Instances are constructed by :class:`auraforge_cli.plugins.PluginContext`
