"""Upload a Aura Forge session transcript to Hugging Face as an agent trace.

Aura Forge stores sessions in its own SQLite store (``auraforge_state.SessionDB``),
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Aura Forge' secret redactor (``force=True``) unless the
...
# ---------------------------------------------------------------------------
# Conversion: Aura Forge OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Aura Forge' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Aura Forge conversation messages as Claude Code JSONL text.
