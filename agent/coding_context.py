"""Coding-context awareness — base Aura Forge, every interactive surface.

When the user runs Aura Forge inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Aura Forge shifts into a **coding posture**. This module is the
...
from auraforge_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("auraforge.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "auraforge", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
def is_coding_context(
    *,
    ...
    """Whether Aura Forge should operate in its coding posture right now."""
