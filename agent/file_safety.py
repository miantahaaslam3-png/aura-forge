...
def _hermes_home_path() -> Path:
    ...
    try:
        from auraforge_constants import get_hermes_home  # local import to avoid cycles
...
def _hermes_root_path() -> Path:
    ...
    try:
        from auraforge_constants import get_default_hermes_root  # local import to avoid cycles
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.auraforge/.env`` and exfiltrate the file. The read-deny exists
...
#
# Reference: May 2026 incident where a auraforge-security profile session
# edited skills under both ``~/.auraforge/profiles/auraforge-security/skills/``
# AND ``~/.auraforge/skills/`` (the default profile's skills) without realizing
...
#
#   <HERMES_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.auraforge/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.auraforge/…`` segment and warns
...
# (the agent's view inside the container is plain ``/root/.auraforge/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.auraforge`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.auraforge/…`` and returns the
    index where the inner Aura Forge-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .auraforge / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".auraforge":
            return i + 4
    return None
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    ...
    Otherwise returns a dict with:
      ...
      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.auraforge``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.auraforge`` (what the
...
# full ``…/sandboxes/<backend>/<task>/home/.auraforge/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.auraforge/…``.  The root:root
