...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    not inheriting ``~/.auraforge/.env``. See miantahaaslam3-png/aura-forge#2765.
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'auraforge memory status'. Note: "
        "systemd/gateway services do not inherit ~/.auraforge/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from auraforge_cli.providers import HERMES_OVERLAYS, get_provider
    ...
    try:
        ...
        from auraforge_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: auraforge config set compression.codex_gpt55_autoraise false"
    )
...
def _refuse_checkpoint_required_on_codex_app_server(
    ...
    the default — Aura Forge never even initiates the compaction), so no
    ...
    Aura Forge compression entirely.
    """
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .auraforge.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / HERMES_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.auraforge/SOUL.md as the primary
    ...
    if api_mode in {"chat_completions", "codex_responses", "anthropic_messages", "bedrock_converse", "codex_app_server"}:
        ...
        try:
            from auraforge_cli.providers import host_mandated_api_mode as _host_mandated_api_mode
    ...
    agent._cache_ttl = "5m"
    try:
        from auraforge_cli.config import load_config_readonly as _load_pc_cfg
    ...
    # both live under ~/.auraforge/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from auraforge_logging import setup_logging, setup_verbose_logging
    ...
    if agent.verbose_logging:
        ...
        # on the fact that auraforge_logging.setup_logging() does not
        ...
        # handler level inside auraforge_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from auraforge_cli.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `auraforge model` to "
                        "select a provider, or run `auraforge setup` for first-time "
                        "configuration."
                    )
    ...
    # Session logs go into ~/.auraforge/sessions/ alongside gateway sessions
    hermes_home = get_hermes_home()
    agent.logs_dir = hermes_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.auraforge/sessions/session_{sid}.json)
    ...
    try:
        from auraforge_cli.config import load_config_readonly as _load_sess_cfg
    ...
    # `auraforge --resume <id>` can restore the bypass (CLI resume paths read
    ...
    try:
        from auraforge_cli.config import load_config_readonly as _load_agent_config
    ...
    # management API (the historical Aura Forge behavior) or left to LM Studio's
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "auraforge"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Aura Forge' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "auraforge", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, auraforge, off.",
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from auraforge_cli.auth import resolve_provider as resolve_auth_provider
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from auraforge_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from auraforge_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Aura Forge Agent.  Choose a model with at least "
    ...
    # Nous Aura Forge 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from auraforge_cli.model_switch import _check_hermes_model_warning

            _hermes_warn = _check_hermes_model_warning(agent.model or "")
            if _hermes_warn:
                _user_msg = (
                    "⚠ Nous Research Aura Forge 3 & 4 models are NOT agentic — they "
