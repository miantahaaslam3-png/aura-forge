...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.auraforge/auth.json active provider)
...
from auraforge_constants import OPENROUTER_BASE_URL
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `auraforge update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Aura Forge owns auxiliary retry + provider/model fallback policy (the
    ...
    # Aura Forge sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Aura Forge control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="auraforge-protected-aux-provider",
        daemon=True,
    ).start()
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Aura Forge triggers summarization.  Higher values delay
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    """
    try:
        from auraforge_cli.auth import resolve_api_key_provider_credentials
        from auraforge_cli.models import fetch_models_with_pricing
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "Aura ForgeAgent",
}
...
# in lockstep with auraforge_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``auraforge_cli.__version__`` is
...
def _codex_cloudflare_headers(
    ...
    the official endpoint always send Aura Forge' originator and version. Custom
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Aura Forge Agent)",
    ...
    if _is_official_codex_base_url(base_url):
        ...
        headers.update({
            "User-Agent": f"Aura ForgeAgent/{__version__}",
            "originator": "auraforge-agent",
        })
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from auraforge_cli.auth import _nous_inference_env_override
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Strip the Aura Forge-side ``-900k`` large-context picker suffix —
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Aura Forge plumbing (_reasoning_config
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``auraforge_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.auraforge/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from auraforge_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from auraforge_cli.auth import _agent_key_is_usable
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``auraforge auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``auraforge_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from auraforge_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Aura Forge auth store.
    ...
    try:
        from auraforge_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from auraforge_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `auraforge setup` or `auraforge model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_azure_foundry(
    *,
    ...
    :func:`auraforge_cli.runtime_provider._resolve_azure_foundry_runtime` —
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second auraforge
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``auraforge config aux reset``)."""
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        if normalized == "copilot":
            from auraforge_cli.copilot_auth import (
                _jwt_cache,
                _token_fingerprint,
        ...
        if normalized == "openai-codex":
            from auraforge_cli.auth import resolve_codex_runtime_credentials
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``auraforge_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Aura Forge' built-in discovery
...
def _resolve_auto_route(
    ...
    #    scenario where a user switches providers via `auraforge model` but the
    #    old OPENAI_BASE_URL lingers in ~/.auraforge/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: auraforge model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.auraforge/.env",
    ...
    if (main_provider and main_model
        ...
        if runtime_base_url and main_provider == "custom":
            ...
            try:
                from auraforge_cli.runtime_provider import _get_named_custom_provider
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    # See _create_openai_client: disable SDK-internal retries so Aura Forge owns
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: auraforge auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        from auraforge_cli.providers import nous_api_mode
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: auraforge model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: auraforge model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: auraforge doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("auraforge_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        try:
            from auraforge_cli.models import opencode_zen_free_runtime as _oc_free_rt
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`auraforge_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def resolve_compression_fast_lane(
    ...
    *,
    ...
    route is the one Aura Forge will call. A requested model covers a compressor
...
def _build_call_kwargs(
    ...
    # Anthropic Messages adapters translate Aura Forge reasoning into native
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Aura Forge reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `auraforge model`."
                    )
