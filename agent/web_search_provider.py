...
``kind: backend``) or ``~/.auraforge/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
def get_provider_env(name: str) -> str:
    ...
    Resolves *name* via :func:`auraforge_cli.config.get_env_value` (checks
    ``os.environ`` first, then ``~/.auraforge/.env``) so credentials set
    through Aura Forge' config layer are visible even when they were never
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``auraforge tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``auraforge tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``auraforge tools`` picker.

        Used by ``auraforge_cli/tools_config.py`` to inject this provider as a
