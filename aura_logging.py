"""Centralized logging setup for Aura Forge.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.hermes/logs/`` (profile-aware via ``get_aura_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``aura_cli.web_server``,
    ``aura_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Aura (TUI, gateway, ``hy_memory`` server, MCP
...
from aura_constants import get_config_path, get_aura_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_aura_session_injector", False):
    ...
    _session_record_factory._aura_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``aura logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``aura logs --component gateway``.
    "gateway": ("gateway", "aura_plugins", "plugins.platforms"),
...
def setup_logging(
    *,
    ...
    """Configure the Aura logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    aura_home
        Override for the Aura home directory.  Falls back to
        ``get_aura_home()`` (profile-aware).
    log_level
    ...
    home = aura_home or get_aura_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_aura_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._aura_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the aura group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from aura_cli.config import is_managed
...
def _read_logging_config():
    ...
    try:
        ...
        if config_path.exists():
            ...
            try:
                from aura_cli import managed_scope
