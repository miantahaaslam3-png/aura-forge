# Política de Seguridad de Aura Forge Agent

Este documento describe el modelo de confianza de Aura Forge Agent, identifica el
...
Reporta de forma privada a través de [GitHub Security Advisories](https://github.com/miantahaaslam3-png/aura-forge/security/advisories/new)
o **security@nousresearch.com**. No abras issues públicos para
vulnerabilidades de seguridad. **Aura Forge Agent no opera un programa de
...
Aura Forge Agent es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta Aura Forge Agent,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso a Aura Forge Agent al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de Aura Forge Agent
...
Aura Forge Agent admite dos posturas de aislamiento a nivel de SO. Abordan
...
Aura Forge Agent admite esto de dos maneras:

- **La propia imagen Docker de Aura Forge Agent y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de Aura Forge Agent
...
Aura Forge Agent filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en Aura Forge Agent
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de Aura Forge Agent
...
**Superficies en Aura Forge Agent:**
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   Aura Forge Agent no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de Aura Forge Agent o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  Aura Forge Agent ha documentado una postura sobre cómo su salida debe ser
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de Aura Forge Agent
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en Aura Forge Agent. Los errores en la ruta de
  instalación de habilidades o plugins de Aura Forge Agent que impidan al
...
- Aura Forge Agent incluye guardias de cadena de suministro para lanzamientos de servidores
