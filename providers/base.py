...
def _profile_user_agent() -> str:
    """Return a ``auraforge-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from auraforge_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"auraforge-cli/{_ver}"
    except Exception:
        return "auraforge-cli"
...
class ProviderProfile:
    ...
    def fetch_models(
        ...
        *,
        ...
        from auraforge_cli.urllib_security import open_credentialed_url
        ...
        # auraforge-cli UA so the catalog endpoint is reachable.
