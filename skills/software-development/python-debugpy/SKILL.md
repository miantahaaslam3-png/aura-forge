...
- A long-running process (aura gateway, tui_gateway) misbehaves and you can't restart it
...
The aura test runner and pytest both support this:
...
For long-lived processes: Aura Forge gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Aura Forge you have two practical options:
...
### `tui_gateway` subprocess (spawned by `aura --tui`)
...
debugpy.wait_for_client()
```
Start `aura --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Aura Forge subagents, debug one process at a time.
