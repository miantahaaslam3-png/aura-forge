...
author: Real Name (github-handle), Aura Forge Agent
license: MIT
...
- Credit the **human first**, then "Aura Forge Agent" as secondary collaborator: `Ben Barclay (benbarclay), Aura Forge Agent`.
- Never `author: Aura Forge Agent` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), Aura Forge Agent`.
...
|---|---|
| Aura Forge tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
- [ ] Body follows the modern section order; commands framed through Aura Forge tools
