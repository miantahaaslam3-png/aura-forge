...
Set via `auraforge model` (picker) or `auraforge setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://auraforge-agent.nousresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `auraforge auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `auraforge auth add openai-codex` |
| qwen-oauth | OAuth | `auraforge auth add qwen-oauth` |
| minimax-oauth | OAuth | `auraforge auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`auraforge auth`).
Fallback chain when the primary fails: `auraforge fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`auraforge_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   auraforge config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
