# Themes / Skins — Author a Aura Forge Color Theme

Author a Aura Forge **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`auraforge_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.auraforge/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Aura Forge itself.
...
- Write access to the Aura Forge home dir — `~/.auraforge` by default, or `$AURA_FORGE_HOME`
  / the active profile's dir. Skins live in `<auraforge-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `auraforge config set`).
...
3. `write_file` it to `<auraforge-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `auraforge config set display.skin default` (or they can `/skin default`).
...
```
auraforge skin set <key> <hex>      # e.g. auraforge skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `auraforge skin set background "#08201f"` changes
only the background; `auraforge skin use <name>` / `auraforge skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.auraforge`** when a profile is active — resolve the real home
  from `$AURA_FORGE_HOME` first, falling back to `~/.auraforge`.
...
- **Never hand-edit `config.yaml` to activate.** Use `auraforge config set
...
- **You apply it, not the user.** `auraforge config set display.skin <name>` is
...
- `read_file` the written `<auraforge-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `auraforge config get display.skin` and confirm it reports `<name>`.
