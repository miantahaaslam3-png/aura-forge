...
For occasional contributors and PR authors. Full developer docs: https://auraforge-agent.nousresearch.com/docs/developer-guide/
...
Config: `~/.auraforge/config.yaml` (settings), `~/.auraforge/.env` (API keys) — both under `$AURA_FORGE_HOME` when it is set.
...
never hardcode `~/.auraforge`. For custom/local-only tools, write a plugin in
`~/.auraforge/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `auraforge_cli/commands.py`
...
- Tests auto-redirect `AURA_FORGE_HOME` to temp dirs — never touch real `~/.auraforge/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/auraforge_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_hermes_home()` from `auraforge_constants` for all paths (profile-safe)
