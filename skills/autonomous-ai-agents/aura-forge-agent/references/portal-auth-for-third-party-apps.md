...
## Layer 1 — Is this thing a Aura Forge plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Aura Forge process**. Its LLM calls go through Aura Forge's already-configured provider. | Already uses Portal if user's Aura Forge is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Aura Forge. That LLM call is already authenticated
via Aura Forge's provider config. The plugin's own server auth (e.g.
...
What Aura Forge Agent has that *feels* like OAuth — `auraforge login --provider nous`
opening a browser, user signs in, token lands in `~/.auraforge/auth.json` — is a
**Aura Forge-specific browser flow**. Under the hood it produces a credential
Aura Forge uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Aura Forge's existing Portal credential out of `~/.auraforge/auth.json`.
...
rides on the credential Aura Forge already holds.

Where this could live in Aura Forge:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Aura Forge itself.
...
1. First decide: Aura Forge plugin (runs inside Aura Forge) or separate app? If plugin,
   it already uses Portal via Aura Forge's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Aura Forge agent itself") is fully supported. Aura Forge treats agent
...
pitch in the Tool Gateway announcement is specifically about Aura Forge Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Aura Forge is configured to use Portal as its provider. It is
