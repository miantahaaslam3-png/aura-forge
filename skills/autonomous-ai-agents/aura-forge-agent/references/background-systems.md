...
the `cronjob` tool, the `auraforge cron` CLI (`list`, `add`, `edit`,
...
User docs: https://auraforge-agent.nousresearch.com/docs/user-guide/features/cron
...
- **CLI:** `auraforge curator <verb>` — `status`, `usage`, `run`, `pause`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `auraforge curator run --consolidate`. Routine background curation costs
  zero tokens.
- **Telemetry:** sidecar at `~/.auraforge/skills/.usage.json` holds
  per-skill `use_count`, `view_count`, `patch_count`,
  `last_activity_at`, `state`, `pinned`.

Config: `curator.*` (`enabled`, `interval_hours`, `min_idle_hours`,
`stale_after_days`, `archive_after_days`, `backup.*`).
User docs: https://auraforge-agent.nousresearch.com/docs/user-guide/features/curator

### Kanban (multi-agent work queue)

Durable SQLite board for multi-profile / multi-worker collaboration.
Users drive it via `auraforge kanban <verb>`; dispatcher-spawned workers
...
User docs: https://auraforge-agent.nousresearch.com/docs/user-guide/features/kanban
