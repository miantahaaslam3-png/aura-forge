...
**three** code paths in Aura Forge that cap a batch. If none of them fired, the
cap came from the **model itself** — not from Aura Forge — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in Aura Forge
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "Aura Forge is
   refusing my value."
...
appears, **the cap is the model**, not Aura Forge.
