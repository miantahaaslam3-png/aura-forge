...
Author widget apps for the Aura Forge TUI (`auraforge --tui`): glanceable ambient
...
- The TUI must be in use (`auraforge --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.auraforge/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.auraforge/logs/tui_gateway_crash.log` if a widget never appears.
