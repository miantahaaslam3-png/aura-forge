...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Aura Forge terminal. Codex is OpenAI's autonomous coding agent CLI.
...
OAuth from `~/.hermes/auth.json` after `aura auth add openai-codex`. For the
...
## Aura Forge Gateway Caveat

When invoking the Codex CLI from a Aura Forge gateway/service context (for example,
