...
metadata:
  auraforge:
    ...
    related_skills: [auraforge-agent]
---
...
- Both sides' intent sources: kanban completion summaries (`terminal` running
  `auraforge kanban show <task-id>`), PR bodies, or at minimum each branch's
  commit messages.
...
- Collect each side's intent: `auraforge kanban show <task-id>` for completion
