#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Aura Forge Agent.
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Aura Forge profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Aura Forge Docker image's venv is
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Aura Forge home."""
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Aura Forge")
