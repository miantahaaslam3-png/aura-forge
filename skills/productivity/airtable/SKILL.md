...
4. Store the token in `${AURA_FORGE_HOME:-~/.auraforge}/.env` (or via `auraforge setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Aura Forge. Pipe through `python -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
## Important Notes for Aura Forge
...
- **`AIRTABLE_API_KEY` flows from `${AURA_FORGE_HOME:-~/.auraforge}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
