...
2. If it does not resolve, install and verify an isolated CLI under a writable, persistent Aura Forge runtime directory. Prefer the current Aura Forge home at `tools/box-cli`; `HERMES_HOME` is optional, and Aura Forge uses its platform default when it is unset (`~/.hermes` on macOS/Linux and `%LOCALAPPDATA%\hermes` on Windows).
