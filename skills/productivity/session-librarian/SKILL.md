...
| List/filter by metadata (age, source, cost, tokens, workspace) | `auraforge sessions list` / `stats` via terminal |
| Rename | `auraforge sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `auraforge sessions archive <filters>` |
| Delete (destructive) | `auraforge sessions delete` / `auraforge sessions prune <filters>` |
| Export before deleting anything valuable | `auraforge sessions export --session-id <id> --format md` |
...
`auraforge sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `auraforge sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `auraforge sessions list`
