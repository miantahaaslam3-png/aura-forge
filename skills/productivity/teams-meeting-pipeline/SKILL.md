...
Everything operator-facing is a `auraforge teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${AURA_FORGE_HOME:-~/.auraforge}/.env`:
...
```bash
auraforge teams-pipeline validate              # config snapshot — run first after any change
auraforge teams-pipeline token-health          # Graph token status
auraforge teams-pipeline token-health --force-refresh   # force a fresh token acquisition
auraforge teams-pipeline list                  # recent meeting jobs
auraforge teams-pipeline list --status failed  # only failed jobs
auraforge teams-pipeline show <job-id>         # full detail of one job
auraforge teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
auraforge teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
auraforge teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
auraforge teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
auraforge teams-pipeline fetch --join-web-url "<url>" --organizer-user-id <id>   # organizer-scoped lookup (required for /meet/ short URLs)
```

### Subscription management

```bash
auraforge teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

auraforge teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
auraforge teams-pipeline delete-subscription <sub-id>
auraforge teams-pipeline maintain-subscriptions            # renew near-expiry ones
auraforge teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `auraforge teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `auraforge cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
