# Contribuir a Aura Forge Agent

¡Gracias por contribuir a Aura Forge Agent! Esta guía cubre todo lo que necesitas: configurar tu entorno de desarrollo, entender la arquitectura, decidir qué construir y conseguir que tu PR sea aceptado.
...
2. **Compatibilidad entre plataformas** — macOS, diferentes distribuciones de Linux y WSL2 en Windows. Queremos que Aura Forge funcione en todas partes.
...
Las habilidades incluidas (en `skills/`) se envían con cada instalación de Aura Forge. Deben ser **ampliamente útiles para la mayoría de los usuarios**:
...
├── auraforge_state.py           # Base de datos de sesiones SQLite con búsqueda de texto completo FTS5, títulos de sesión
...
│
├── auraforge_cli/               # Implementaciones de comandos CLI
...
- **Persistencia de sesión**: Todas las conversaciones se almacenan en SQLite (`auraforge_state.py`) con búsqueda de texto completo y títulos de sesión únicos.
...
2. **Las herramientas referenciadas en el cuerpo de SKILL.md deben ser herramientas nativas de Aura Forge o servidores MCP que la habilidad espere explícitamente.** Usa los nombres de herramientas en comillas invertidas: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, etc.
...
Aura Forge usa un sistema de skins basado en datos — no se necesitan cambios de código para añadir una nueva skin.
...
Añade al dict `_BUILTIN_SKINS` en `auraforge_cli/skin_engine.py`. Usa el mismo esquema que arriba pero como dict de Python.
...
Aura Forge se ejecuta en Linux, macOS y Windows nativo (además de WSL2). Al escribir código
...
Aura Forge tiene acceso al terminal. La seguridad importa.
...
- Usa [GitHub Issues](https://github.com/miantahaaslam3-png/aura-forge/issues)
- Incluye: SO, versión de Python, versión de Aura Forge (`hermes --version`), traza de error completa
