...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.auraforge/auraforge-agent/utils.py)
...
2. ``auraforge update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``auraforge_cli/main.py`` (the ``auraforge update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``auraforge_constants`` after the
