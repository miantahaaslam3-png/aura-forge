...
"No LLM provider configured. Run `auraforge model`..." (resume failed) or
...
def _make_agent_with_override(override, monkeypatch, config, model_cfg=None):
    ...
    # Keep credential-pool resolution off the developer's real AURA_FORGE home.
...
# found" (the b200/auraforge-ultra-sft report). The stored MODEL NAME is the one
...
ULTRA_LEGACY_CONFIG = {
    ...
    "custom_providers": [
        {
            "name": "auraforge-ultra",
            "base_url": ULTRA_URL,
            "api_key": "sk-ultra",
            "model": "auraforge-ultra-sft",
        }
    ],
}
...
class TestModelNameRecoversEntryIdentity:
    def test_identity_by_model_from_providers_dict_models_list(self, monkeypatch):
        ...
        assert (
            rp.find_custom_provider_identity_by_model("auraforge-ultra-sft")
            == "custom:auraforge-ultra"
        )
