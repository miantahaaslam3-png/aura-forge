...
def server():
    ...
    # e.g. auraforge_cli.active_sessions would bind the mocked get_hermes_home
    ...
    with patch.dict("sys.modules", {
        "auraforge_constants": MagicMock(get_hermes_home=MagicMock(return_value="/tmp/hermes_test")),
        "auraforge_cli.env_loader": MagicMock(),
        "auraforge_cli.banner": MagicMock(),
        "auraforge_state": MagicMock(),
    }):
        import importlib
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/auraforge-agent-dev": {"name": "auraforge-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "auraforge-agent-dev", "session_id": sid},
        })
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `auraforge config set` does)
    ...
    import auraforge_cli.skin_engine as skin_engine
    ...
    # Activate midnight, as `auraforge config set display.skin midnight` would.
