...
Aura Forge's own top-level modules (``utils``, ``proxy``, ``ui``).  entry.py must
neutralize this before any non-stdlib import is resolved, by delegating to the
shared ``auraforge_bootstrap.harden_import_path`` guard.
...
def test_entry_calls_shared_harden_guard_before_heavy_imports():
    """entry.py must call auraforge_bootstrap.harden_import_path() before it
    ...
    assert harden_call_line is not None, (
        "entry.py must call auraforge_bootstrap.harden_import_path()"
    )
...
def test_guard_handles_absolute_cwd_path():
    ...
    path, ahead of the Aura Forge root.  harden_import_path must relocate the
    Aura Forge root to the front so ``from utils import ...`` resolves to Aura Forge."""
    import sys

    original = sys.path[:]
    try:
        sys.path[:] = ["/home/user/tg-ws-proxy", "/opt/auraforge", "/usr/lib"]
        auraforge_bootstrap.harden_import_path(src_root="/opt/auraforge")
