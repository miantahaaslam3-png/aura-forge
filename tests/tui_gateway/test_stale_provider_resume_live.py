...
def live_home(monkeypatch):
    ...
    # auraforge_constants caches the resolved home at first read — the env var
    ...
    import auraforge_cli.config as hconfig
    ...
    # conftest also re-pins auraforge_state.DEFAULT_DB_PATH per test; pin it to
    # THIS home so server._get_db() opens the same real state.db we seed.
    monkeypatch.setattr(auraforge_state, "DEFAULT_DB_PATH", home / "state.db")
