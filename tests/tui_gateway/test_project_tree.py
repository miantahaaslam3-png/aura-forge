...
def test_user_worktree_under_dotworktrees_is_its_own_lane_not_kanban():
    ...
    sessions = [
        _session("/repo", branch="main"),
        _session("/repo/.worktrees/test-gui-stuff", branch="auraforge/test-gui-stuff"),
    ]
...
def test_junk_root_never_becomes_an_auto_project():
    ...
    resolve = _resolver(
        {
            "/home/me/.auraforge": ("/home/me/.auraforge", "/home/me/.auraforge"),
            "/www/app": ("/www/app", "/www/app"),
        }
    )
    junk = _session("/home/me/.auraforge", branch="main")
    real = _session("/www/app", branch="main")
    is_junk = lambda root: root == "/home/me/.auraforge"
...
def test_broad_default_non_git_cwd_stays_unscoped():
    detached = _session("/home/test/.auraforge")
    ...
    tree = pt.build_tree(
        ...
        hydrate=True,
        is_junk_cwd=lambda path: path in {"/home/test", "/home/test/.auraforge"},
    )
...
def test_deleted_sibling_worktree_folds_into_parent_home_checkout():
    ...
    resolve = _resolver({"/www/auraforge-agent": ("/www/auraforge-agent", "/www/auraforge-agent")})
    sessions = [
        _session("/www/auraforge-agent", branch="main"),
        _session("/www/auraforge-agent-session-links"),
    ]

    tree = pt.build_tree([], sessions, [], resolve, hydrate=True)
    project = tree["projects"][0]

    assert [p["id"] for p in tree["projects"]] == ["/www/auraforge-agent"]
    assert _lane_ids(project) == ["/www/auraforge-agent::branch::main"]
    main = project["repos"][0]["groups"][0]
    assert main["isMain"] and main["path"] == "/www/auraforge-agent"
