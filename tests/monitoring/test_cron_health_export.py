...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("auraforge.gateway.up"), _M("auraforge.gateway.active_agents"),
        _M("auraforge.gateway.busy"), _M("auraforge.gateway.drainable"),
        _M("auraforge.gateway.restart_requested"),
        _M("auraforge.platform.up"), _M("auraforge.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("auraforge.cron.scheduler.heartbeat_age_seconds"),
        _M("auraforge.cron.scheduler.last_success_age_seconds"),
        _M("auraforge.cron.scheduler.catch_up_occurrences"),
        _M("auraforge.cron.jobs.enabled"), _M("auraforge.cron.jobs.running"),
        _M("auraforge.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Aura Forge Agent-owned Relay transport health" in text
