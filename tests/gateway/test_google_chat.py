...
class TestExtractMessagePayload:
    ...
    def test_relay_flat_format_synthesizes_chat_api_shape(self):
        ...
        GCP credentials off the Aura Forge host. The relay flattens Chat
...
class TestTypingLifecycle:
    ...
    async def test_send_typing_concurrent_calls_create_only_one_card(self, adapter):
        ...
        'Aura Forge is thinking…' stuck in chat. Race fix via
    ...
    async def test_orphan_typing_cards_reaped_on_completion(self, adapter):
        ...
        'Aura Forge is thinking…' messages."""
        from plugins.platforms.google_chat.adapter import _TYPING_CONSUMED_SENTINEL
...
class TestGoogleChatInteractiveSetup:
    def test_interactive_setup_uses_shared_cli_prompt_helpers(self, monkeypatch):
        ...
        monkeypatch.setattr("auraforge_cli.config.get_env_value", fake_get_env_value)
        monkeypatch.setattr("auraforge_cli.config.save_env_value", fake_save_env_value)
        monkeypatch.setattr("auraforge_cli.cli_output.prompt", fake_prompt)
        monkeypatch.setattr(
            "auraforge_cli.cli_output.prompt_yes_no", lambda *_a, **_kw: True
        )
        monkeypatch.setattr(
            "auraforge_cli.cli_output.print_info", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "auraforge_cli.cli_output.print_success", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "auraforge_cli.cli_output.print_warning", lambda *_a, **_kw: None
        )
