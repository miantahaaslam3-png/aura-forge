...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "auraforge_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "auraforge_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr("auraforge_constants.get_hermes_home", lambda: hermes_home)
    monkeypatch.setattr("auraforge_cli.config.get_hermes_home", lambda: hermes_home)
    return cfg_path
