...
class TestProfileScopedDiscovery:
    def test_list_approved_scopes_platform_discovery_to_profile_dir(self, tmp_path):
        ...
        # A profile's store anchors to the auraforge ROOT, not the current
        ...
        with patch("gateway.pairing.PAIRING_DIR", global_dir), patch(
            ...
            # consolidated layout a standalone `auraforge -p alice` resolves —
...
class TestProfileScopedStorage:
    ...
    under each profile's own Aura Forge home so a multiplexing gateway can keep
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        """Explicit profile stores use that profile's normal Aura Forge layout."""
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
