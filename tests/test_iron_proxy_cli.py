"""Unit tests for ``auraforge_cli.proxy_cli`` command handlers.
...
from auraforge_cli import proxy_cli
...
# ---------------------------------------------------------------------------
# _load_env_file_into_environ — setup discovers keys kept only in ~/.auraforge/.env
# ---------------------------------------------------------------------------
...
def test_cmd_disable_uses_public_status_pid_not_private_read_pid(
    ...
    # mutation only.
    from auraforge_cli.config import load_config as _lc
    cfg2 = _lc()
...
def test_register_cli_uses_egress_command_dest():
    ...
    the inbound OAuth 'auraforge proxy' subparser (dest='proxy_command').
    A future grep-and-refactor on proxy_command should not hit this
    subparser by accident."""

    parser = argparse.ArgumentParser(prog="auraforge egress")
...
def test_cmd_setup_audit_log_failure_is_warning_not_abort(hermes_home, monkeypatch):
    ...
    monkeypatch.setattr(ip, "discover_provider_mappings", lambda **kw: [
        ip.TokenMapping(
            proxy_token="auraforge-proxy-deadbeef",
