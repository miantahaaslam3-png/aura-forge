...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"auraforge-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "auraforge-agent-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "auraforge-agent-dev"
...
# `auraforge curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `auraforge curator pin <umbrella>` in the rename block so
