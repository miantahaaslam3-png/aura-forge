"""Regression tests for miantahaaslam3-png/aura-forge#2765.
...
service that didn't inherit ``~/.auraforge/.env``) used to be dropped silently.
...
def test_warns_once_and_dedupes(caplog):
    ...
    assert "auraforge memory status" in msg
