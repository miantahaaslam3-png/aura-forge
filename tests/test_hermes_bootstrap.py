"""Tests for auraforge_bootstrap — Windows UTF-8 stdio shim.
...
Key invariants covered by these tests:
  ...
  5. Load order: every Aura Forge entry point imports auraforge_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported auraforge_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("auraforge_bootstrap", None)
    import auraforge_bootstrap  # noqa: WPS433
    return auraforge_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Aura Forge CLI
...
class TestEntryPointsImportBootstrap:
    """Every Aura Forge entry point must import auraforge_bootstrap as its
    ...
    # import auraforge_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "auraforge_cli/main.py",   # auraforge CLI (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import auraforge_bootstrap' and that
        ...
        ``auraforge_bootstrap`` as a top-level module) leaves auraforge
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # auraforge start even when auraforge_bootstrap hasn't been
        ...
        assert first_import_name == "auraforge_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'auraforge_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import auraforge_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Aura Forge's own top-level modules — covering both
