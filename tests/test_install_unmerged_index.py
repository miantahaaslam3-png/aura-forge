...
The ``auraforge update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m auraforge_cli.main gateway run`` — image name
    ``pythonw``, not ``auraforge.exe`` — so the ``taskkill /IM auraforge.exe`` guard
    ...
    # The auraforge.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM auraforge.exe' in text
