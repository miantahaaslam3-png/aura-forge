...
2. ``local_delivery_command`` hardcoded ``"auraforge"``, relying on PATH —
   ...
   basename so resolved absolute paths (and ``auraforge.exe``) still take
...
def test_waiter_windows_path_compiles_after_backslash_folding():
    ...
    code = _waiter_code("C:\\Users\\joshu\\.auraforge")
...
def test_waiter_raw_prefix_keeps_injection_defense():
    ...
    code = _waiter_code(Path("/tmp/auraforge-home"), inj)
...
def test_local_delivery_uses_shutil_which_when_no_sibling(tmp_path, monkeypatch):
    ...
    which_hit = str(tmp_path / "usr-local-bin" / "auraforge")
    monkeypatch.setattr(
        bot_relay.shutil, "which", lambda name: which_hit if name == "auraforge" else None
    )
...
def test_delivery_lock_recognizes_resolved_cli_paths(tmp_path, monkeypatch):
    ...
    now that argv[0] may be a resolved absolute path (or auraforge.exe)."""
    acquired = []
    ...
    with bot_mode_dm._delivery_lock(
        [str(tmp_path / "venv" / "bin" / "auraforge"), "-p", "ops", "chat"],
        stdin_file=False,
    ):
        pass
    with bot_mode_dm._delivery_lock(["auraforge", "-p", "ops", "chat"], stdin_file=False):
        pass
    with bot_mode_dm._delivery_lock(
        ["C:\\venv\\Scripts\\auraforge.exe", "-p", "ops", "chat"], stdin_file=False
    ):
        pass
