...
# ---------------------------------------------------------------------------
# E2E regression (#93082): cron/no_agent children keep bare `auraforge` on PATH
# ---------------------------------------------------------------------------
...
def test_e2e_scrubbed_env_resolves_bare_hermes_under_minimal_parent_path(monkeypatch):
    ...
    minimal PATH (no auraforge console-script dir) must still hand cron job
    children an env whose PATH resolves bare ``auraforge``.
    ...
    if not bin_dir or not os.path.isfile(
        os.path.join(bin_dir, "auraforge.exe" if os.name == "nt" else "auraforge")
    ):
        pytest.skip("no real auraforge console-script install available")

    # Simulate the service-manager minimal PATH: auraforge dir absent.
    minimal_path = os.pathsep.join(["/usr/bin", "/bin"])
    monkeypatch.setenv("PATH", minimal_path)
    assert shutil.which("auraforge", path=minimal_path) is None

    env = build_subprocess_env(scrub_secrets=True)  # cron _run_job_script path

    resolved = shutil.which("auraforge", path=env.get("PATH", ""))
    assert resolved is not None, (
        f"bare 'auraforge' must resolve from the child PATH {env.get('PATH')!r}"
    )
