...
See: https://github.com/miantahaaslam3-png/aura-forge/issues/1002
See: https://github.com/miantahaaslam3-png/aura-forge/issues/1264
"""
...
class TestProviderEnvBlocklist:
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a Aura Forge inference secret
        ...
        models`` run inside a Aura Forge terminal enumerated the entire Bedrock
    ...
    def test_bare_hermes_resolves_from_sanitized_subprocess_path(self):
        """Cron children can resolve Aura Forge even when the gateway PATH cannot."""
...
class TestActiveVenvMarkerStripping:
    ...
    into the Aura Forge venv path instead of the project's own ``.venv`` —
    silently clobbering the Aura Forge environment (and, when the other project
    pins a different Python, breaking the gateway outright). The Aura Forge venv
...
class TestPythonpathSelectiveStrip:
    ...
    entries Aura Forge itself owns (repo root, own venv site-packages),
    ...
    def test_non_owned_runtime_shaped_entries_preserved(self):
        ...
        different interpreter version, a descendant of the Aura Forge venv
    ...
    def test_base_python_sanitizer_uses_validated_separate_runtime_venv(self, tmp_path, monkeypatch):
        ...
        This deliberately uses a synthetic Aura Forge venv separate from the test
        runner: sys.prefix represents base Python, while validated VIRTUAL_ENV
        identifies ``<repo>/venv`` as the Aura Forge runtime producer contract.
        """
    ...
    def test_builders_strip_hermes_venv_pythonpath(self, builder):
        ...
        Aura Forge venv site-packages is stripped, user entries survive.
        """
    ...
    def test_scrub_child_env_strips_hermes_venv_pythonpath(self):
        """execute_code's _scrub_child_env path: after scrubbing, Aura Forge venv
    ...
    def test_execute_code_composition_strips_inherited_hermes_entries(self, same_env):
        ...
        Seeds a contaminated inherited PYTHONPATH (Aura Forge repo root + Aura Forge
        ...
        * inherited Aura Forge venv site-packages never survive into the sandbox;
        ...
        * user entries survive after the controlled entries.
        ...
        assert norm_venv not in norm_parts, \
            "inherited Aura Forge venv site-packages must be stripped"
...
class TestPythonhomeSanitized:
    """PYTHONHOME must not leak from the Aura Forge runtime into subprocesses.
    ...
    inherits it redirects its stdlib search to the Aura Forge venv and crashes
...
class TestBlocklistCoverage:
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to Aura Forge, and blocklisting
    ...
    def test_claude_code_oauth_token_is_inheritable(self):
        ...
        (subscription OAuth), not a Aura Forge inference credential. Stripping it
...
class TestHermesInternalDynamicSecrets:
    """Dynamically-named Aura Forge secrets injected at gateway/CLI startup must
