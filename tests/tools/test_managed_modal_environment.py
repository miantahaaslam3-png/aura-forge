...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "auraforge_cli"}
        or name.startswith("tools.")
        or name.startswith("agent.")
        or name.startswith("auraforge_cli.")
    }
    try:
        yield
    finally:
        _reset_modules(("tools", "agent", "auraforge_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    auraforge_cli = types.ModuleType("auraforge_cli")
    auraforge_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["auraforge_cli"] = auraforge_cli
    sys.modules["auraforge_cli.config"] = types.SimpleNamespace(
        get_hermes_home=lambda: Path(tempfile.gettempdir()) / "auraforge-home",
    )
...
def test_managed_modal_rejects_host_credential_passthrough():
    _install_fake_tools_package(
        credential_mounts=[{
            ...
            "container_path": "/root/.auraforge/token.json",
        }]
    )
    managed_modal = _load_tool_module("tools.environments.managed_modal", "environments/managed_modal.py")
