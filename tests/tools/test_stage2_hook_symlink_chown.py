...
def test_chown_helper_repairs_real_directories(stage2_text: str, tmp_path: Path) -> None:
    ...
    assert log_path.read_text().splitlines() == [
        f"-R auraforge:auraforge {target}",
    ]
...
def test_chown_helper_refuses_symlinked_directories(stage2_text: str, tmp_path: Path) -> None:
    ...
    real_home.mkdir()
    symlinked_home = tmp_path / "auraforge-home"
    try:
...
def test_stage2_uses_symlink_safe_helper_for_hermes_home_trees(stage2_text: str) -> None:
    ...
    assert 'chown -R auraforge:auraforge "$HERMES_HOME/$sub"' not in stage2_text
    assert 'chown -R auraforge:auraforge "$HERMES_HOME/profiles"' not in stage2_text
    assert 'chown -R auraforge:auraforge "$HERMES_HOME/cron"' not in stage2_text
