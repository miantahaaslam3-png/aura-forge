...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="auraforge-spill-test-")
    ...
    def test_default_directory_uses_hermes_home(self):
        ...
        test_home = tempfile.mkdtemp(prefix="auraforge-home-")
        try:
            ...
            # ~/.auraforge/hook_outputs depending on get_hermes_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.auraforge/hook_outputs/sess")),
            ]
