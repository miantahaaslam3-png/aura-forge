...
A package-manager install (NixOS, and anything else shipping Aura Forge from a
...
def test_nixos_install_fails_fast_without_touching_the_installer(monkeypatch):
    monkeypatch.setattr("auraforge_cli.config.get_managed_system", lambda: "nixos")
...
def test_unreadable_config_fails_open(monkeypatch):
    ...
    monkeypatch.setattr("auraforge_cli.config.get_managed_system", _raise)
