"""Tests for tools.wake_word — the "Hey Aura Forge" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey auraforge"}) == "hey auraforge"
    assert ww.wake_phrase({}) == "hey auraforge"
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "auraforge_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey auraforge"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey auraforge"
...
def test_bundled_hey_hermes_model_ships_on_disk():
    # The "hey auraforge" wake word works out of the box only if the model is
