...
Older images made /opt/auraforge/ui-tui and /opt/auraforge/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV HERMES_TUI_DIR=/opt/auraforge/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R auraforge:auraforge /opt/auraforge/ui-tui" not in text
    assert "chown -R auraforge:auraforge /opt/auraforge/node_modules" not in text
