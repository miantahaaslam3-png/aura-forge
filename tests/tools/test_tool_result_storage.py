...
class TestWriteToSandbox:
    def test_success(self):
        ...
        result = _write_to_sandbox("hello world", "/tmp/auraforge-results/abc.txt", env)
    ...
    def test_large_content_via_stdin(self):
        ...
        env.execute.return_value = {"output": "", "returncode": 0}
        big = "x" * 200_000
        _write_to_sandbox(big, "/tmp/auraforge-results/big.txt", env)
        cmd = env.execute.call_args[0][0]
...
class TestResolveStorageDir:
    ...
    def test_uses_env_temp_dir_when_available(self):
        ...
        assert _resolve_storage_dir(env) == "/data/data/com.termux/files/usr/tmp/auraforge-results"
...
class TestBuildPersistedMessage:
    def test_structure(self):
        msg = _build_persisted_message(
            ...
            file_path="/tmp/auraforge-results/test123.txt",
        )
        ...
        assert "/tmp/auraforge-results/test123.txt" in msg
    ...
    def test_large_size_shows_mb(self):
        msg = _build_persisted_message(
            ...
            file_path="/tmp/auraforge-results/big.txt",
        )
        assert "MB" in msg
...
class TestMaybePersistToolResult:
    ...
    def test_tool_use_id_cannot_escape_storage_dir(self):
        ...
        assert "Full output saved to: /tmp/auraforge-results/outside_whoami_x_" in result
        assert "/tmp/auraforge-results/../" not in result
        assert target.startswith("/tmp/auraforge-results/outside_whoami_x_")
...
class TestSpillover:
    ...
    def test_remote_env_probe_failure_falls_back_to_sandbox_write(self):
        ...
        assert PERSISTED_OUTPUT_TAG in result
        assert "/tmp/auraforge-results/tc_remote_2.txt" in result
...
class TestRecoveryHint:
    def test_preview_teaches_recovery_not_refetch(self):
        msg = _build_persisted_message(
            ...
            file_path="/tmp/auraforge-results/r.txt",
        )
