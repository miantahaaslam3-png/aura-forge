...
the SAME provider universe as ``aura model`` (the CLI/TUI picker).
...
def test_every_aura_forge_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `aura model` universe."""
    ...
    assert not missing, (
        "providers shown in `aura model` but not configurable in the desktop "
