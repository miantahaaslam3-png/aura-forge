...
def container_env(tmp_path, monkeypatch):
    """Set up a fake AURA_HOME with .container-mode file."""
    aura_home = tmp_path / ".aura-forge"
    aura_home.mkdir()
    monkeypatch.setenv("AURA_HOME", str(aura_home))
    monkeypatch.delenv("AURA_DEV", raising=False)

    container_mode = aura_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=aura-forge\n"
        "exec_user=aura-forge\n"
        "aura_bin=/data/current-package/bin/aura\n"
    )
    return aura_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("aura_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "aura-forge"
    assert info["exec_user"] == "aura"
    assert info["aura_bin"] == "/data/current-package/bin/aura"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("aura_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_aura_forge_dev(container_env, monkeypatch):
    """Returns None when AURA_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("AURA_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_aura_forge_dev_zero(container_env, monkeypatch):
    """AURA_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("AURA_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        aura_home = Path(tmpdir) / ".aura-forge"
        aura_home.mkdir()
        (aura_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("aura_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_aura_home": lambda: aura_home}), \
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=aura-forge-custom\n"
        "exec_user=myuser\n"
        "aura_bin=/opt/aura/bin/aura\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "aura-custom"
    assert info["exec_user"] == "myuser"
    assert info["aura_bin"] == "/opt/aura/bin/aura"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from aura_cli.main import _exec_in_container
