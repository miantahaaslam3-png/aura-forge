"""Tests for the unified provider catalog (aura_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``aura model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from aura_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
...
def test_catalog_covers_every_aura_forge_model_provider():
    """PARITY CONTRACT: the catalog == the `aura model` universe."""
    slugs = {d.slug for d in provider_catalog()}
    for entry in CANONICAL_PROVIDERS:
        assert entry.slug in slugs, (
            f"{entry.slug} is shown in `aura model` but missing from provider_catalog()"
        )


def test_catalog_has_no_providers_outside_aura_forge_model():
    """The catalog must not invent providers `aura model` doesn't show."""
