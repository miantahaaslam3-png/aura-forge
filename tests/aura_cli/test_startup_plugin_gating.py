...
``aura_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``aura --help``, ``aura version``,
``aura logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``aura --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from aura_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["aura", "--tui", "chat"], "chat"),
        (["aura", "-w", "logs"], "logs"),
        (["aura", "chat", "hello world"], "chat"),
        (["aura", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["aura", "-m", "gpt5", "chat"], "chat"),
        (["aura", "--model", "gpt5", "chat", "hi"], "chat"),
        (["aura", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["aura", "-z", "hello world"], None),
        (["aura", "-z", "hello", "chat"], "chat"),
        (["aura", "--model=gpt5", "chat"], "chat"),     # inline form
        (["aura", "--", "chat"], "chat"),               # -- terminator
        (["aura", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["aura", "some-plugin-cmd"], "some-plugin-cmd"),
        (["aura", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["aura"],                          # bare → chat
        ["aura", "--help"],                # top-level help
        ["aura", "-h"],
        ["aura", "version"],               # known built-in
        ["aura", "logs"],
        ["aura", "gateway", "run"],
        ["aura", "--tui"],
        ["aura", "-w", "--tui"],
        ["aura", "chat", "hi"],
        ["aura", "help"],                  # accepted built-in-ish
        ["aura", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["aura", "meet", "join"],          # potential google_meet plugin
        ["aura", "honcho", "status"],      # potential memory plugin
        ["aura", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``aura help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"aura_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
