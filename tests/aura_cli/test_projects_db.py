"""Tests for the per-profile Projects store (aura_cli/projects_db)."""
...
from aura_cli import projects_db as pdb
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Aura Forge", folders=["/tmp/aura"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "aura-forge"
    assert proj.name == "Aura Forge"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/aura"
    assert [f.path for f in proj.folders] == ["/tmp/aura"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "aura-forge").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Aura Forge")
    pdb.create_project(conn, name="Aura Forge")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["aura-forge", "aura-forge-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' AURA_HOME.
...
def test_db_path_under_aura_home():
    # Resolves under AURA_HOME (set by the autouse isolation fixture).
