...
The terminal `aura model` flow already exposes `custom_providers`, but the
shared slash-command pipeline (`/model` in CLI/gateway/Telegram) historically
only looked at `providers:`.
"""

import aura_cli.providers as providers_mod
...
def test_list_authenticated_providers_includes_custom_providers(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "AURA_OVERLAYS", {})
...
def test_switch_model_accepts_explicit_bare_custom_current_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("aura_cli.models.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION)
    monkeypatch.setattr("aura_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("aura_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
...
def test_switch_model_accepts_explicit_named_custom_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "aura_cli.runtime_provider.resolve_runtime_provider",
        lambda **kwargs: {
...
def test_list_enumerates_dict_format_models_alongside_default(monkeypatch):
    ...
    Regression: Aura's own writer stores configured models as a dict
...
def test_lmstudio_picker_probes_active_config_base_url(monkeypatch):
    ...
    monkeypatch.setattr("aura_cli.models.fetch_lmstudio_models", _fake_fetch)
...
def test_resolve_custom_provider_passes_key_env():
    ...
    """
    from aura_cli.providers import resolve_custom_provider
