"""Tests for ``aura update`` / ``--check`` inside the Docker container.
...
fix, ``aura update`` would fall through to ``"✗ Not a git repository.
...
from aura_cli.main import _cmd_update_check, cmd_update
...
@patch("aura_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``aura update`` inside Docker → friendly message + exit 1, no git calls."""
    ...
    assert "docker pull nousresearch/hermes:latest" in out
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``aura update --check`` inside Docker → same message + exit 1, no fetch."""
    with pytest.raises(SystemExit) as excinfo:
...
def test_format_docker_update_message_contents():
    ...
    """
    from aura_cli.config import format_docker_update_message

    msg = format_docker_update_message()

    # Primary command — the entire reason this message exists.
    assert "docker pull nousresearch/hermes:latest" in msg
    ...
    assert "AURA_HOME" in msg or "/opt/data" in msg, (
