...
Covers:
- Defense 1: gateway stop/restart refuse when _AURA_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "aura gateway restart",
        "aura gateway stop",
        "aura-forge  gateway  restart",         # double spaces
    ...
    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.aura-forge.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.aura-forge.gateway.plist",
        "launchctl stop ai.aura-forge.gateway",
        "systemctl restart aura-gateway",
        "systemctl stop aura-gateway.service",
        "systemctl start aura-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill aura gateway process",
        "pkill -f aura-forge.*gateway",
        "pkill -f gateway.*aura-forge",          # inverse token order
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "aura cron list",
        "aura update",
        "aura config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `aura gateway start` is benign — starting a gateway from inside a
        ...
        "aura gateway start",
        "aura gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway aura-forge
        # services must not be falsely blocked (the old `.*aura-forge` matched any
        # aura-forge token).
        "launchctl unload ai.aura-forge.update-checker.plist",
        "launchctl restart ai.aura-forge.daemon",
        "systemctl restart aura-meta.service",
        "systemctl restart aura-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_aura_forge_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade aura-forge then run aura gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.aura-forge.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_aura_forge_gateway_once.sh). The script must live under
        # AURA_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("AURA_HOME", str(tmp_path / ".aura-forge"))
        scripts_dir = tmp_path / ".aura-forge" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify aura gateway stop/restart refuse when _AURA_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_AURA_GATEWAY", "1")
        from aura_cli.gateway import gateway_command
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_AURA_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _AURA_GATEWAY=1.

    Issue #37453: systemctl --user restart aura-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart aura-gateway",
        "systemctl --user restart aura-gateway",
        "systemctl stop aura-gateway.service",
        "aura gateway restart",
        "launchctl kickstart gui/501/ai.aura-forge.gateway",
        "pkill -f aura-forge.*gateway",
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart aura-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-aura-forge systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _AURA_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart aura-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart aura-gateway"]
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run aura gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("aura gateway stop\n")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeaura gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under AURA_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.aura-forge.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    aura_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run aura gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AURA_HOME", str(tmp_path / ".aura-forge"))
        (tmp_path / ".aura-forge").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run aura gateway restart nightly",
        ))
