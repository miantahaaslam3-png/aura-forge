"""Tests for aura_cli configuration management."""
...
class TestGetAuraForgeHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AURA_HOME", None)
            home = get_aura_home()
            assert home == Path.home() / ".aura-forge"

    def test_env_override(self):
        with patch.dict(os.environ, {"AURA_HOME": "/custom/path"}):
            home = get_aura_home()
            assert home == Path("/custom/path")


class TestEnsureAuraForgeHome:
    def test_creates_subdirs(self, tmp_path):
        with patch.dict(os.environ, {"AURA_HOME": str(tmp_path)}):
            ensure_aura_home()
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from aura_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    def test_preserves_legacy_template_with_user_persona(self, tmp_path):
        ...
        from aura_cli.default_soul import _LEGACY_TEMPLATE_SOULS
    ...
    def test_existing_named_profile_still_bootstraps_subdirs(self, tmp_path):
        profile_home = tmp_path / ".aura-forge" / "profiles" / "coder"
        profile_home.mkdir(parents=True)
        with patch.dict(os.environ, {"AURA_HOME": str(profile_home)}):
            ensure_aura_home()
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``aura logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from aura_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"AURA_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="aura_cli.config"):
                config = load_config()
            ...
            # stands out at aura-forge startup before logging is configured)
            captured = capsys.readouterr()
            assert "aura config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"AURA_HOME": str(tmp_path)}):
            ...
            assert "aura config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"AURA_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"AURA_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"AURA_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"AURA_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"AURA_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"AURA_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_api_key_registered(self):
        ...
        from aura_cli.config import OPTIONAL_ENV_VARS
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from aura_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """AURA_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `aura setup`) lets a
        ...
        via config.yaml; AURA_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from aura_cli.config import OPTIONAL_ENV_VARS
        assert "AURA_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `aura setup` surface it (previously only Honcho was listed, leaving
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``AURA_*`` runtime flag.
    ...
    def _aura_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        "allowed_key",
        [
            "AURA_LANGFUSE_PUBLIC_KEY",
            "AURA_SPOTIFY_CLIENT_ID",
            "AURA_QWEN_BASE_URL",
            "AURA_MAX_ITERATIONS",
        ],
    )
    def test_hermes_integration_keys_still_writable(self, allowed_key):
        """``AURA_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``AURA_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``AURA_*``) keep working. The denylist
