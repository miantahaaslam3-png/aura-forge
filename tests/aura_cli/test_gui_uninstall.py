"""Tests for aura_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import aura_cli.gui_uninstall as gu


def _make_agent(aura_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = aura_home / "aura-forge"
    (agent_root / "aura_cli").mkdir(parents=True)
    (agent_root / "aura_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(aura_home: Path) -> None:
    """Create the source-built GUI artifacts a `aura desktop` run produces."""
    desktop = aura_home / "aura-forge" / "apps" / "desktop"
    ...
    (aura_home / "aura-forge" / "node_modules").mkdir(parents=True)
    (aura_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(aura_home: Path) -> None:
    (aura_home / "config.yaml").write_text("x: 1\n")
    (aura_home / ".env").write_text("KEY=secret\n")
    (aura_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    aura_home = tmp_path / ".aura-forge"
    aura_home.mkdir()
    assert gu.agent_is_installed(aura_home) is False
    _make_agent(aura_home)
    assert gu.agent_is_installed(aura_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    aura_home = tmp_path / ".aura-forge"
    (aura_home / "aura-forge" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(aura_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    aura_home = tmp_path / ".aura-forge"
    _make_gui_build(aura_home)
    artifacts = gu.source_built_gui_artifacts(aura_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(aura_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(aura_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    aura_home = tmp_path / ".aura-forge"
    agent_root = _make_agent(aura_home)
    _make_gui_build(aura_home)
    _make_user_data(aura_home)
    ...
    removed = gu.uninstall_gui(aura_home)
    ...
    assert not (aura_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "aura_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (aura_home / "config.yaml").exists()
    assert (aura_home / ".env").exists()
    assert (aura_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    aura_home = tmp_path / ".aura-forge"
    _make_agent(aura_home)
    userdata = tmp_path / "Aura Forge-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(aura_home, remove_userdata=False)
...
def test_uninstall_gui_removes_packaged_bundle(tmp_path, monkeypatch):
    aura_home = tmp_path / ".aura-forge"
    _make_agent(aura_home)
    bundle = tmp_path / "Aura Forge.app"
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(aura_home)
    ...
    assert summary["aura_home"] == str(aura_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Aura Forge"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Aura Forge"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Aura Forge"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Aura Forge"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    AURA_HOME with all the destructive externals stubbed out.
    """
    import aura_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_aura_home", lambda: aura_home)
    ...
    from aura_cli import gui_uninstall as gu_mod
    ...
    assert not (desktop / "release").exists()
    assert (aura_home / "config.yaml").exists()
    assert aura_home.exists()


def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole AURA_HOME non-interactively."""
    import aura_cli.uninstall as uninstall

    aura_home = tmp_path / ".aura-forge"
    (aura_home / "aura-forge" / "aura_cli").mkdir(parents=True)
    (aura_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not aura_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m aura_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_aura_home", lambda: aura_home)
    ...
    assert not (desktop / "release").exists()
    assert not (aura_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "aura_cli").exists()
    assert (aura_home / "config.yaml").exists()
