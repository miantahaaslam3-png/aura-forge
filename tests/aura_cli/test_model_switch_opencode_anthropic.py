...
``aura_cli.runtime_provider.resolve_runtime_provider`` already strips
``/v1`` at fresh agent init (PR #4918), but the ``/model`` mid-session
switch path in ``aura_cli.model_switch.switch_model`` was missing the
...
from aura_cli.model_switch import switch_model
...
def _run_opencode_switch(
    ...
    with (
        patch("aura_cli.model_switch.resolve_alias", return_value=None),
...
class TestStaleConfigDefaultDoesNotWedgeResolver:
    ...
    def test_kimi_switch_keeps_v1_despite_claude_config_default(self, tmp_path, monkeypatch):
        ...
        # Re-import with the new AURA_HOME so config cache is fresh.
        import aura_cli.config as _cfg_mod
        importlib.reload(_cfg_mod)
        import aura_cli.runtime_provider as _rp_mod
        importlib.reload(_rp_mod)
        import aura_cli.model_switch as _ms_mod
