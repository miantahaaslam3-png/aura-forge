...
subparser, placing the flag BEFORE the subcommand (e.g. 'aura-forge --yolo chat')
...
def _build_parser():
    """Build the aura argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("aura_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "aura_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets AURA_YOLO_MODE regardless of flag position.
    ...
    def _clean_env(self):
        os.environ.pop("AURA_YOLO_MODE", None)
        yield
        os.environ.pop("AURA_YOLO_MODE", None)
    ...
    def test_no_yolo_no_env(self):
        ...
        args = parser.parse_args(["chat"])
        self._simulate_cmd_chat_yolo_check(args)
        assert os.environ.get("AURA_YOLO_MODE") is None
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `aura gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `aura <argv>` must exit 0 (help) rather than
        failing with `unrecognized arguments`."""
        import subprocess
        result = subprocess.run(
            [sys.executable, "-m", "aura_cli.main", *argv],
            capture_output=True,
            text=True,
            timeout=15,
        )
