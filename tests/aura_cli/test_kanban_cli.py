"""Tests for the kanban CLI surface (aura_cli.kanban)."""
...
from aura_cli import kanban as kc
...
def test_run_slash_session_filter(kanban_home):
    """`aura kanban list --session <id>` filters by the originating
...
def test_board_override_is_isolated_per_concurrent_call(kanban_home, monkeypatch):
    ...
    parser = argparse.ArgumentParser(prog="aura", add_help=False)
...
def test_kanban_bypasses_active_session_guard():
    from aura_cli.commands import should_bypass_active_session

    assert should_bypass_active_session("kanban")


def test_kanban_in_autocomplete_table():
    from aura_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_kanban_autocomplete_includes_live_subcommands():
    ...
    from aura_cli.commands import SlashCommandCompleter
...
def test_kanban_not_gateway_only():
    ...
    from aura_cli.commands import COMMAND_REGISTRY
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    kb.create_board("alpha")
    kb.create_board("beta")
    monkeypatch.setenv("AURA_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("AURA_KANBAN_BOARD") == "beta"
