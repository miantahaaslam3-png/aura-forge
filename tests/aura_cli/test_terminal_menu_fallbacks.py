...
def test_prompt_model_selection_falls_back_on_menu_runtime_error(monkeypatch):
    from aura_cli.auth import _prompt_model_selection

    monkeypatch.setattr("aura_cli.curses_ui.curses_radiolist", _raise_menu)
    responses = iter(["2"])
...
def test_prompt_reasoning_effort_falls_back_on_menu_runtime_error(monkeypatch):
    from aura_cli.main import _prompt_reasoning_effort_selection
...
def test_remove_custom_provider_falls_back_on_menu_runtime_error(tmp_path, monkeypatch):
    from aura_cli.main import _remove_custom_provider
...
def test_named_custom_provider_model_picker_falls_back_on_menu_runtime_error(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AURA_HOME", str(tmp_path))
    monkeypatch.setattr("aura_cli.curses_ui.curses_radiolist", _raise_menu)
    monkeypatch.setattr("aura_cli.models.fetch_api_models", lambda *args, **kwargs: ["model-a", "model-b"])
    monkeypatch.setattr("aura_cli.auth.deactivate_provider", lambda: None)
