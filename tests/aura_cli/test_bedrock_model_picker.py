...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (AURA_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via AURA_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in AURA_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from aura_cli.providers import AURA_OVERLAYS
        assert "bedrock" in AURA_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from aura_cli.providers import AURA_OVERLAYS
        assert AURA_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from aura_cli.providers import AURA_OVERLAYS
        assert AURA_OVERLAYS["bedrock"].auth_type == "aws_sdk"
