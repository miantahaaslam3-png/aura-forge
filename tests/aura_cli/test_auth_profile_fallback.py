...
When ``AURA_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path
    * Global root -> tmp_path/.aura-forge            (has its own auth.json fixture)
    * Profile     -> tmp_path/.hermes/profiles/coder   (active, AURA_HOME points here)
    ...
    global_root = tmp_path / ".aura-forge"
    global_root.mkdir()
...
def test_profile_with_zero_entries_falls_back_to_global(profile_env):
    ...
    from aura_cli.auth import read_credential_pool
...
def test_provider_auth_state_falls_back_to_global_when_profile_has_none(profile_env):
    from aura_cli.auth import get_provider_auth_state
...
# ``_load_provider_state`` itself, those helpers raise ``"Aura is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from aura_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    aura_home = tmp_path / "classic"
    aura_home.mkdir()
    monkeypatch.setenv("AURA_HOME", str(aura_home))

    _write(aura_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (AURA_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.aura-forge as the "real home" — which is NOT equal
    # to the AURA_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    aura_home = tmp_path / "classic"
    ...
    _write(aura_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from aura_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: AURA_HOME is set to a custom path that is NOT under
    # ~/.hermes/profiles/ — get_default_aura_root() returns AURA_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from aura_cli.auth import read_credential_pool, write_credential_pool
