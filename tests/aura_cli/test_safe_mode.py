"""Tests for `aura chat --safe-mode` — pristine troubleshooting runs.
...
_VARS = ("AURA_SAFE_MODE", "AURA_IGNORE_USER_CONFIG", "AURA_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in aura_cli/main.py.
        class Args:
            safe_mode = True

        args = Args()
        if getattr(args, "safe_mode", False):
            os.environ["AURA_SAFE_MODE"] = "1"
            os.environ["AURA_IGNORE_USER_CONFIG"] = "1"
            os.environ["AURA_IGNORE_RULES"] = "1"

        assert os.environ.get("AURA_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under AURA_SAFE_MODE=1."""

    def test_discovery_skipped(self, monkeypatch):
        monkeypatch.setenv("AURA_SAFE_MODE", "1")
        from aura_cli.plugins import PluginManager
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("AURA_SAFE_MODE", raising=False)
        from aura_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under AURA_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `aura chat`."""
