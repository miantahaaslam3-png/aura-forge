"""Tests for aura_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_aura_home):
    """Drop the example-dashboard fixture into the per-test AURA_HOME
    ...
    ``AURA_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, aura-achievements, model providers
    ...
    """
    from aura_constants import get_aura_home
    from aura_cli import web_server

    user_plugins_dir = get_aura_home() / "plugins"
    ...
    # plugin in exactly as a real operator would with `aura plugins
    # enable example`.
    from aura_cli.config import load_config, save_config
    _cfg = load_config()
    ...
    #      and start serving requests against a torn-down AURA_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Aura vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Aura env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    """The desktop shell mints AURA_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import importlib
        import aura_cli.web_server as ws

        monkeypatch.setenv("AURA_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("AURA_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_aura_home):
        """Create a TestClient and isolate the state DB under the test AURA_HOME."""
        try:
        ...
        import aura_state
        from aura_constants import get_aura_home
        from aura_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(aura_state, "DEFAULT_DB_PATH", get_aura_home() / "state.db")
    ...
    def test_get_status(self):
        ...
        assert "aura_home" in data
        assert "active_sessions" in data
        assert data["can_update_aura"] is True
    ...
    def test_dashboard_update_capability_allows_git_in_container(self, monkeypatch):
        """A git checkout inside a container (e.g. bind-mounted in aura-webui)
    ...
    def test_get_memory_provider_config_returns_safe_defaults(self):
        ...
        assert fields["bank_id"]["value"] == "aura"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from aura_constants import get_aura_home
        from aura_cli.config import load_config, load_env
        ...
        config_path = get_aura_home() / "hindsight" / "config.json"
    ...
    def test_put_moa_models_persists_provider_model_slots(self):
        from aura_cli.config import load_config
    ...
    def test_get_sessions_uses_only_persisted_cwd(self, monkeypatch):
        ...
        """
        from aura_state import SessionDB
    ...
    def test_get_sessions_forwards_min_messages(self, monkeypatch):
        ...
        monkeypatch.setattr("aura_state.SessionDB", _FakeDB)
    ...
    def test_sessions_endpoint_reads_requested_profile(self):
        ...
        from aura_state import SessionDB
        from aura_cli import profiles as profiles_mod
    ...
    def test_desktop_audio_routes_registered(self):
        ...
        without failing CI.
        """
        from aura_cli.web_server import app
    ...
    def test_update_aura_returns_docker_guidance_without_spawning(self, monkeypatch):
        import aura_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn aura update")

        # Bypass the managed-externally gate so we reach the docker install check.
        monkeypatch.setattr(web_server, "_dashboard_local_update_managed_externally", lambda: False)
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_aura_action", fail_spawn)
        web_server._ACTION_PROCS.pop("aura-update", None)
        web_server._ACTION_RESULTS.pop("aura-update", None)

        resp = self.client.post("/api/aura/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "aura-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/hermes:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/aura-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nousresearch/hermes:latest" in line for line in status_data["lines"])

    def test_update_aura_returns_managed_runtime_guidance_without_spawning(self, monkeypatch):
        import aura_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("managed runtime update guard should not spawn aura update")
    ...
    def test_update_aura_spawns_on_non_docker_install(self, monkeypatch):
        import aura_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "git")
        monkeypatch.setattr(web_server, "_spawn_aura_action", fake_spawn)
        web_server._ACTION_PROCS.pop("aura-update", None)
        web_server._ACTION_RESULTS.pop("aura-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "aura-update"}
        assert calls == [(["update"], "aura-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("aura-update", None)
        web_server._ACTION_RESULTS.pop("aura-update", None)
        web_server._ACTION_PROCS["aura-update"] = proc

        resp = self.client.get("/api/actions/aura-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "aura-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["aura-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from aura_cli.web_server import _channel_managed_env_keys
    ...
    def test_get_env_vars_surfaces_catalog_providers(self):
        ...
        tencent-tokenhub, copilot were configurable via `aura model` but
        invisible in the desktop Providers → API keys tab.
        """
        from aura_cli.provider_catalog import provider_catalog
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from aura_cli.web_server import (
    ...
    def test_model_set_requires_confirmation_for_expensive_model(self, monkeypatch):
        monkeypatch.setattr(
            "aura_cli.model_cost_guard.expensive_model_warning",
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Aura provider — keep the user's aggregator instead of writing a
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        `aura import` doesn't auto-abort at the overwrite prompt."""
        import aura_cli.web_server as ws
        ...
        monkeypatch.setattr(ws, "_spawn_aura_action", fake_spawn)
    ...
    def test_ops_backup_defaults_to_dashboard_downloadable_archive(self, monkeypatch):
        ...
        import aura_cli.web_server as ws
        from aura_cli.config import get_aura_home
        ...
        assert archive.parent == get_aura_home() / "backups"
        assert archive.name.startswith("aura-backup-")
        assert archive.suffix == ".zip"

    def test_ops_backup_uses_hosted_aura_home(self, tmp_path, monkeypatch):
        from pathlib import Path

        import aura_cli.web_server as ws

        hosted_home = tmp_path / "opt-data"
        monkeypatch.setenv("AURA_HOME", str(hosted_home))
        captured = {}
    ...
    def test_ops_backup_download_streams_dashboard_backup(self, tmp_path):
        ...
        archive = backup_dir / "aura-backup-test.zip"
    ...
    def test_ops_import_upload_rejects_invalid_zip(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_aura_action", fail_spawn)
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from aura_cli.config import save_env_value
        from aura_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from aura_cli.config import save_env_value
        from aura_cli.web_server import _SESSION_TOKEN
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://aura-forge.nousresearch.com/docs/user-guide/messaging/weixin/"
        )
    ...
    def test_teams_messaging_metadata_links_setup_guide(self):
        ...
        from aura_cli.web_server import _build_catalog_entry

        teams = _build_catalog_entry("teams")
        assert teams["docs_url"] == (
            "https://aura-forge.nousresearch.com/docs/user-guide/messaging/teams"
        )
    ...
    def test_google_chat_messaging_metadata_links_setup_guide(self):
        ...
        assert google_chat["docs_url"] == (
            "https://aura-forge.nousresearch.com/docs/user-guide/messaging/google_chat"
        )
    ...
    def test_update_messaging_platform_saves_slack_allowed_users(self):
        from aura_cli.config import load_env
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "Aura Forge"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("AuraForgeDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "aura_forge_pair123_bot",
                "deep_link": "https://t.me/newbot/AuraForgeSetupBot/aura_pair123_bot",
                "qr_payload": "https://t.me/newbot/AuraForgeSetupBot/aura_pair123_bot",
        ...
        resp = self.client.post(
            ...
            json={"bot_name": "Hosted Aura Forge"},
        )
    ...
    def test_telegram_onboarding_ready_and_apply_never_returns_bot_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "aura_forge_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/AuraForgeSetupBot/aura_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/AuraForgeSetupBot/aura_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "aura_forge_pair_ready_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_aura_action", fake_spawn_action)
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "aura_forge_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/AuraForgeSetupBot/aura_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/AuraForgeSetupBot/aura_pair_restart_fails_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "aura_forge_pair_restart_fails_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
        ...
        monkeypatch.setattr(ws, "_spawn_aura_action", fail_spawn_action)
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``aura gateway restart`` child (e.g. when a stale
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "aura_forge_pair_reuse_bot",
                    "deep_link": "https://t.me/newbot/AuraForgeSetupBot/aura_pair_reuse_bot",
                    "qr_payload": "https://t.me/newbot/AuraForgeSetupBot/aura_pair_reuse_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            return {
                "status": "ready",
                "bot_username": "aura_forge_pair_reuse_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "aura_forge_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/AuraForgeSetupBot/aura_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/AuraForgeSetupBot/aura_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "aura_forge_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/AuraForgeSetupBot/aura_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/AuraForgeSetupBot/aura_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/aura_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_spa_assets_are_read_as_utf8(self, monkeypatch, tmp_path):
        ...
        css_resp = spa_client.get("/assets/app.css", headers={"x-forwarded-prefix": "/aura"})
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        import aura_cli.nous_subscription as ns
        ...
        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "nous", "model": "aura-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from aura_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from aura_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``aura model`` custom flow. Regression for the desktop loop where a
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg = load_config()
        cfg["model"] = {"provider": "nous", "default": "aura-4"}
        cfg["auxiliary"] = {
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `aura model`), not the first curated paid entry."""
        import aura_cli.models as models_mod
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from aura_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from aura_cli.web_server import _build_schema_from_config
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from aura_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_aura_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_list_includes_default(self):
        from aura_constants import get_aura_home
        get_aura_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from aura_constants import get_aura_home
        import aura_cli.profiles as profiles_mod

        aura_home = get_aura_home()
        aura_home.mkdir(parents=True, exist_ok=True)
        (aura_home / "config.yaml").write_text(
            "model:\n  provider: openrouter\n  name: anthropic/claude-sonnet-4.6\n",
            encoding="utf-8",
        )
        named = aura_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_aura_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profile_setup_command_uses_aura_forge_for_default_profile(self):
        from aura_constants import get_aura_home
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "aura setup"
    ...
    def test_profiles_create_creates_wrapper_alias_when_safe(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(profiles_mod.shutil, "which", lambda name: "/opt/aura/bin/aura")
        ...
        if is_windows:
            assert lines == ["@echo off", "aura -p writer %*"]
        else:
            assert lines == ["#!/bin/sh", 'exec /opt/aura/bin/aura -p writer "$@"']
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        (get_aura_home() / "config.yaml").write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        )
        default_skill = get_aura_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_root = get_aura_home() / "profiles" / "cloned"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_aura_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_aura_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_aura_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_aura_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_aura_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        from aura_constants import (
            get_aura_home,
            set_aura_home_override,
            reset_aura_home_override,
        )
        from aura_cli.config import load_config
        from aura_cli.skills_config import get_disabled_skills
        import aura_cli.profiles as profiles_mod
        import aura_cli.web_server as web_server
        ...
        prof_dir = get_aura_home() / "profiles" / "builder"
        token = set_aura_home_override(str(prof_dir))
        try:
            cfg = load_config()
            ...
            reset_aura_home_override(token)
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_aura_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_profile_describe_auto_success(self, monkeypatch):
        ...
        from aura_cli import profile_describer
        monkeypatch.setattr(
            profile_describer,
            "describe_profile",
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import aura_cli.skills_config as skills_config
        import aura_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import aura_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from aura_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from aura_cli.web_server import _denormalize_config_from_web
        from aura_cli.config import save_config
...
class TestGatewayBusyReadout:
    ...
    def test_restart_drain_timeout_surfaced_and_numeric(self, monkeypatch):
        ...
        monkeypatch.setenv("AURA_RESTART_DRAIN_TIMEOUT", "90")
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from aura_cli.web_server import _normalise_theme_definition
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_aura_home):
        ...
        monkeypatch.setattr(
            aura_state, "DEFAULT_DB_PATH", get_aura_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``aura_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in aura_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_aura_home):
        ...
        # Pin the SessionDB to the isolated AURA_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_aura_home, _install_example_plugin):
        ...
        installs it into the per-test ``AURA_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into AURA_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, aura-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (aura-achievements is loaded by default).
        resp = self.client.get("/api/plugins/aura-achievements/overview")
        assert resp.status_code == 401
...
# or ``sh -c 'printf …'``) instead of the real ``aura --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        import aura_cli.main as main_mod
        ...
        assert env["AURA_TUI_DASHBOARD"] == "1"
        assert env["AURA_TUI_INLINE"] == "1"
        assert env["AURA_TUI_DISABLE_MOUSE"] == "1"

    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_aura_home
    ):
        import aura_cli.main as main_mod

        config_path = Path(os.environ["AURA_HOME"]) / "config.yaml"
        config_path.write_text(
            "\n".join(
                [
                    "terminal:",
                    "  backend: docker",
                    "  docker_image: example/aura-tools:latest",
                    "  docker_extra_args:",
                    "    - --network=host",
                ]
            ),
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/aura-tools:latest"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"aura-ws-ok" in buf:
                    break
            assert b"aura-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from aura_cli.pty_bridge import PtyUnavailableError
        ...
        import aura_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        AURA_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from aura_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import aura_cli.main as cli_main
    import aura_cli.web_server as ws
    ...
    gateway_url = env.get("AURA_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_aura_home, _install_example_plugin):
        ...
        the per-test ``AURA_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_aura_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("AURA_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under AURA_DESKTOP=1"

    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_aura_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.delenv("AURA_DESKTOP", raising=False)
