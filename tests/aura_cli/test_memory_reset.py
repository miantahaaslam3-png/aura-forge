"""Tests for the `aura memory reset` CLI command.
...
- Profile-scoped reset (uses AURA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake AURA_HOME with memory files."""
    aura_home = tmp_path / ".aura-forge"
    memories = aura_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("AURA_HOME", str(aura_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nAura repo is at ~/.hermes/aura-forge\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `aura memory reset` is run.
    """
    from aura_constants import get_aura_home

    mem_dir = get_aura_home() / "memories"
    files_to_reset = []
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        aura_home = tmp_path / ".aura-forge"
        (aura_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("AURA_HOME", str(aura_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's AURA_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_aura_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
