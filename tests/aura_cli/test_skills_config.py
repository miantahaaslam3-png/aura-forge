"""Tests for aura_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("aura_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from aura_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("aura_cli.config.load_config")
    ...
    @patch("aura_cli.config.load_config")
    @patch.dict("os.environ", {"AURA_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("AURA_PLATFORM", raising=False)
        monkeypatch.delenv("AURA_SESSION_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """AURA_SESSION_PLATFORM should be used when AURA_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
        ...
        monkeypatch.setenv("AURA_HOME", str(tmp_path))
        monkeypatch.delenv("AURA_PLATFORM", raising=False)
        monkeypatch.setenv("AURA_SESSION_PLATFORM", "discord")
    ...
    def test_hermes_platform_takes_precedence(self, tmp_path, monkeypatch):
        """AURA_PLATFORM should win over AURA_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("AURA_HOME", str(tmp_path))
        monkeypatch.setenv("AURA_PLATFORM", "telegram")
        monkeypatch.setenv("AURA_SESSION_PLATFORM", "discord")
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AURA_HOME", str(tmp_path))
        monkeypatch.setenv("AURA_PLATFORM", "telegram")
        monkeypatch.setenv("AURA_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from aura_cli.skills_config import _get_categories
        skills = [
