"""Tests for aura_cli.gateway_windows."""

from pathlib import Path

import pytest

import aura_cli.gateway as gateway
import aura_cli.gateway_windows as gateway_windows
import aura_cli.setup as setup
...
def test_exec_schtasks_decodes_with_replace_errors(monkeypatch):
    ...
    code, out, err = gateway_windows._exec_schtasks(["/Query", "/TN", "Aura Forge_Gateway"])
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda aura_home: "")
    monkeypatch.setattr("aura_cli.config.get_aura_home", lambda: str(aura_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "aura_cli.main"]
    assert cwd == str(aura_home.resolve())
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_aura_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".aura-forge"
        home.mkdir()
        monkeypatch.setattr("aura_cli.config.get_aura_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".aura-forge"
        project = tmp_path / "checkout"
        monkeypatch.setattr("aura_cli.config.get_aura_home", lambda: missing)
        assert gateway_windows._stable_gateway_working_dir(project) == str(project)


def test_write_task_script_anchors_cmd_cd_at_aura_home(monkeypatch, tmp_path):
    project = tmp_path / "project"
    aura_home = tmp_path / "aura-home"
    aura_home.mkdir()
    ...
    assert f"cd /d {gateway_windows._quote_cmd_script_arg(str(aura_home.resolve()))}" in content
...
def _arrange_startup_fallback(monkeypatch, tmp_path, running_pids):
    script_path = tmp_path / "Aura Forge_Gateway_alice.cmd"
    startup_entry = tmp_path / "Startup" / "Aura Forge_Gateway_alice.cmd"
    calls = []

    monkeypatch.setattr(gateway_windows, "_prompt_install_choices", lambda *args, **kwargs: (False, True))
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Aura Forge_Gateway_alice")
...
def test_gateway_cmd_script_uses_pythonw_without_replace_or_start_churn(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda exe: (exe.replace("python.exe", "pythonw.exe"), r"C:\\Aura Forge\\aura-forge\\venv", []),
    )

    content = gateway_windows._build_gateway_cmd_script(
        r"C:\\Aura Forge\\aura-forge\\venv\\Scripts\\python.exe",
        r"C:\\Aura Forge\\aura-forge",
        r"C:\\AuraForgeHome\\profiles\\alice",
        "--profile alice",
    )
...
def test_elevated_gateway_command_uses_pythonw_hidden_console(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Aura Forge\venv\Scripts\python.exe")
...
def test_install_scheduled_task_recreates_instead_of_change(monkeypatch, tmp_path):
    ...
    ok, detail = gateway_windows._install_scheduled_task("Aura Forge_Gateway_alice", script_path)

    assert ok is True
    assert "/Change" not in [arg for call in calls for arg in call]
    assert calls[0][:4] == ("/Delete", "/F", "/TN", "Aura Forge_Gateway_alice")
    ...
    assert "Aura Forge_Gateway_alice.vbs" in xml_seen["text"]
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "pythonw.exe" in content
    assert "aura_cli.main" in content
    assert "gateway run" in content
    assert ", 0, False" in content  # hidden window, detached/async
    for var in ("AURA_HOME", "PYTHONIOENCODING", "AURA_GATEWAY_DETACHED", "VIRTUAL_ENV", "PYTHONPATH"):
...
def test_install_scheduled_task_success_start_now_uses_direct_spawn_not_task_run(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_install_scheduled_task",
        lambda task_name, script_path: (True, "Created Scheduled Task 'Aura Forge_Gateway_alice'"),
    )
...
def test_install_access_denied_launches_elevated_install_before_startup_fallback(monkeypatch, tmp_path, capsys):
    ...
    assert "Launched elevated Aura gateway install prompt" in out
...
def test_install_startup_fallback_does_not_auto_spawn_when_gateway_stopped(monkeypatch, tmp_path, capsys):
    ...
    assert "aura-forge --profile alice gateway start" in out
...
def test_uninstall_access_denied_prompts_before_elevating(monkeypatch, tmp_path, capsys):
    ...
    assert "UAC is Windows' admin approval prompt" in out
    assert "Launched elevated Aura gateway uninstall prompt" in out
...
# resume_pending=True) never fires when `aura gateway stop` kills the
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("aura_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("aura_cli.gateway._get_restart_drain_timeout", lambda: 5.0)
...
def test_stop_escalates_to_force_kill_when_drain_times_out(monkeypatch):
    ...
    `aura gateway start` fails with "another instance is running".
    """
    pid = 77777
    events = []
