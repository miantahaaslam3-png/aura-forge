...
Before the fix, ``aura whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``aura gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    aura-forge = home / ".aura-forge"
    aura-forge.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("AURA_HOME", str(aura-forge))
...
def _env_value(aura_home: Path, key: str) -> str | None:
    env_file = aura_home / ".env"
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from aura_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("aura_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``aura whatsapp`` with an existing paired session and
