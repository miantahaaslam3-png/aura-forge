...
@pytest.fixture
def cli():
    obj = AuraCLI.__new__(AuraCLI)  # bypass __init__ (no full app needed)
...
def test_billing_logged_out(cli, monkeypatch, capsys):
    ...
    assert "aura-forge portal" in out


def test_billing_overview_non_interactive_renders_text_not_modal(cli, monkeypatch, capsys):
    monkeypatch.setattr(AuraCLI, "_prompt_text_input_modal", _boom_modal, raising=False)
    state = BillingState(
        logged_in=True,
        org_name="Acme",
        role="OWNER",
