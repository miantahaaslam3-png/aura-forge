...
def aura_home(tmp_path, monkeypatch):
    ...
    # /etc/aura on the dev/CI box can't influence the test.
    monkeypatch.setenv("AURA_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(aura_home, monkeypatch):
    from aura_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(aura_home, monkeypatch):
    from aura_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(aura_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    from aura_cli.env_loader import load_aura_dotenv

    home = tmp_path / "home"
    home.mkdir()
    (home / ".env").write_text("FOO_TOKEN=from_user_env\n", encoding="utf-8")
    monkeypatch.setenv("FOO_TOKEN", "from_shell")
    load_aura_dotenv(aura_home=str(home))
