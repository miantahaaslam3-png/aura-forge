...
``aura setup`` / ``aura model`` when an API key already exists in ``.env``.
...
def _run_prompt(existing_key, choice, new_key="", provider_id="", pconfig_name="deepseek"):
    ...
    with patch("builtins.input", return_value=choice), \
         patch("aura_cli.secret_prompt.masked_secret_prompt", return_value=new_key):
...
def test_replace_saves_new_key(profile_env):
    from aura_cli.config import get_env_value, save_env_value
...
def test_lmstudio_first_time_empty_uses_placeholder(profile_env):
    from aura_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from aura_cli.config import get_env_value
