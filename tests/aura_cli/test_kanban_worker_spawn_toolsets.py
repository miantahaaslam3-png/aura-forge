...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    AURA_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    monkeypatch.setattr(kb, "_resolve_aura_forge_argv", lambda: ["aura"])
    ...
    assert pid == 4242
    assert captured["env"]["AURA_HOME"] == str(profile)
    assert captured["env"]["AURA_KANBAN_TASK"] == "t_spawn_tools"
