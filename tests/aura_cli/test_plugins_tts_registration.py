...
``$AURA_HOME/plugins/``, runs ``PluginManager().discover_and_load()``,
and asserts the registration result.

Mirrors the structure of
``tests/aura_cli/test_plugin_scanner_recursion.py::TestRegisterImageGenProvider``.
"""
...
class TestRegisterTTSProvider:
    ...
    def test_accepts_valid_provider(self):
        ...
        aura_home = Path(os.environ["AURA_HOME"])
        _write_plugin(
            aura_home / "plugins",
            "my-tts-plugin",
            register_body=(
        ...
        _enable(aura_home, "my-tts-plugin")
    ...
    def test_rejects_non_provider(self, caplog):
        ...
        _enable(aura_home, "bad-tts-plugin")
    ...
    def test_rejects_builtin_shadow(self, caplog):
        ...
        _enable(aura_home, "shadow-tts-plugin")
