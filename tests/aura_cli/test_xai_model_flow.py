...
def test_xai_model_flow_reauth_uses_standard_radio_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "aura_cli.auth.get_xai_oauth_auth_status",
        lambda: {"logged_in": True},
    )
    monkeypatch.setattr(
        "aura_cli.setup._curses_prompt_choice",
    ...
    monkeypatch.setattr("aura_cli.auth._login_xai_oauth", _fake_login)
    monkeypatch.setattr(
        "aura_cli.auth.resolve_xai_oauth_runtime_credentials",
...
def test_xai_model_flow_cancel_skips_reauth(monkeypatch):
    ...
    monkeypatch.setattr(
        "aura_cli.auth._login_xai_oauth",
