"""Tests for `aura memory setup [provider]` routing.
...
``aura memory setup honcho``) without the interactive picker — which matters
because the per-provider ``aura <provider>`` subcommand is only registered
...
from aura_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "aura-forge memory setup" in out
