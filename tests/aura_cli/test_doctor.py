...
import aura_cli.doctor as doctor
import aura_cli.gateway as gateway_cli
from aura_cli import doctor as doctor_mod
from aura_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `aura doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "AURA_HOME", aura_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_marks_kanban_available_only_when_missing_worker_env_gate(self, monkeypatch):
        monkeypatch.setattr(doctor, "_honcho_is_configured_for_doctor", lambda: False)
        monkeypatch.delenv("AURA_KANBAN_TASK", raising=False)
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("AURA_KANBAN_TASK", "probe")
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "AURA_HOME", aura_home)
    monkeypatch.delenv("AURA_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("AURA_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_aura_home(self, tmp_path, provider=""):
        """Create a minimal AURA_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_aura_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "AURA_HOME", home)
        ...
        try:
            from aura_cli import auth as _auth_mod
...
def test_run_doctor_accepts_aura_forge_provider_ids_that_catalog_aliases(
...
class TestGitHubTokenCheck:
    ...
    def test_no_token_and_not_gh_authenticated_shows_warn(self, monkeypatch, tmp_path):
        ...
        from aura_cli.doctor import run_doctor
        import io, contextlib
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from aura_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("aura_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "aura_cli.doctor", raising=False)
        from aura_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale AURA_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        aura_home = tmp_path / ".aura-forge"
        aura_home.mkdir(parents=True)
        (aura_home / "config.yaml").write_text(
            f"agent:\n  max_turns: {cfg_turns}\n", encoding="utf-8"
        )
        env_lines = ["OPENAI_API_KEY=sk-test\n"]
        if ghost is not None:
            env_lines.append(f"AURA_MAX_ITERATIONS={ghost}\n")
        (aura_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "AURA_HOME", aura_home)
        monkeypatch.setattr(doctor_mod, "get_aura_home", lambda: aura_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("AURA_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("AURA_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale AURA_MAX_ITERATIONS" in out
        env_after = (aura_home / ".env").read_text(encoding="utf-8")
        assert "AURA_MAX_ITERATIONS" not in env_after
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`aura doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # left thinking a crashing command means a broken Aura install.
