...
A credential added via `aura auth add openrouter` lives in the credential
...
def _seed_openrouter_pool(token: str = "sk-or-FAKEKEY123") -> None:
    """Mimic `aura auth add openrouter <token>` — a manual pool entry."""
...
def test_auto_detects_openrouter_from_pool(tmp_path, monkeypatch):
    ...
    (tmp_path / "aura").mkdir(parents=True, exist_ok=True)
...
def test_no_credentials_still_raises(tmp_path, monkeypatch):
    ...
    from aura_cli.auth import AuthError, resolve_provider
