"""Tests for --ignore-user-config and --ignore-rules flags on `aura chat`.
...
files. In Aura the equivalent isolation is:
...
argparse → cmd_chat → cli.main() → AuraCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("AURA_IGNORE_USER_CONFIG", "AURA_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``AURA_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<aura_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._aura_home at tmp_path and return a fresh load_cli_config."""
        import cli
        monkeypatch.setattr(cli, "_aura_home", tmp_path)
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("AURA_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``AuraCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting AURA_IGNORE_RULES=1 flips AuraCLI.ignore_rules True."""
        monkeypatch.setenv("AURA_IGNORE_RULES", "1")

        # Import AuraCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of AuraCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.AuraCLI)
        # Replicate the exact logic from cli.py AuraCLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("AURA_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True

    def test_constructor_flag_alone_enables_ignore_rules(self, monkeypatch):
        monkeypatch.delenv("AURA_IGNORE_RULES", raising=False)
        import cli
        obj = object.__new__(cli.AuraCLI)
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``aura_cli/main.py`` must set
    ...
    def _simulate_cmd_chat_env_setup(self, args):
        ...
        if getattr(args, "ignore_user_config", False):
            os.environ["AURA_IGNORE_USER_CONFIG"] = "1"
        if getattr(args, "ignore_rules", False):
            os.environ["AURA_IGNORE_RULES"] = "1"

    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("AURA_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("AURA_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("AURA_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("AURA_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("AURA_IGNORE_USER_CONFIG") == "1"
        assert "AURA_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "AURA_IGNORE_USER_CONFIG" not in os.environ
        assert "AURA_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real aura-forge parser accepts both flags."""
        from aura_cli._parser import build_top_level_parser
        ...
        import inspect
        import aura_cli.main as hm
        src = inspect.getsource(hm)
        assert "AURA_IGNORE_USER_CONFIG" in src
        assert "AURA_IGNORE_RULES" in src
