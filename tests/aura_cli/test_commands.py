...
class TestSlackNativeSlashes:
    ...
    def test_hermes_catchall_is_first(self):
        """``/aura`` must be reserved as the first slot so the legacy
        ``/aura <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "aura"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /aura <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
    ...
    def test_includes_aliases_as_first_class_slashes(self):
        ...
        aliases stay reachable via ``/aura <alias>``.
        """
        slashes = slack_native_slashes()
    ...
    def test_telegram_parity(self):
        ...
        # Commands deliberately routed through /aura <command> on Slack only
        # (Slack's 50-slash cap) are expected to be absent from native slashes.
        via_aura_forge_norm = {_norm(n) for n in _SLACK_VIA_AURA_ONLY}
        missing = (tg_norm - slack_norm) - reserved_norm - via_aura_forge_norm
        assert not missing, (
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `aura slack manifest`)."""
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_completes_toolset_names(self, monkeypatch):
        ...
        from aura_cli import commands as commands_mod

        # `web` is enabled, `spotify` is disabled — enabling should only offer
        # the disabled ones.
        monkeypatch.setattr(
            "aura_cli.tools_config._get_platform_tools",
            lambda *_a, **_k: {"web", "file"},
        )
        monkeypatch.setattr("aura_cli.config.load_config", lambda: {})
        monkeypatch.setattr(
            "aura_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
from aura_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch aura_cli.plugins.get_plugin_commands() to a fixed dict."""
        from aura_cli import plugins as _plugins_mod
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/aura metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
    ...
    def test_is_gateway_known_command_recognizes_plugin_commands(self, monkeypatch):
        ...
        from aura_cli.commands import is_gateway_known_command
