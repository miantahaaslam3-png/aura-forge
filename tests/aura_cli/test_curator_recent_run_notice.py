...
The notice prints the most recent curator run summary on `aura update`,
...
session start) so users normally never see the rename map. `aura update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(aura_constants)
    from agent import curator
    importlib.reload(curator)
    from aura_cli import main as aura_forge_main
    importlib.reload(aura_forge_main)

    yield {
        "curator": curator,
        "main": aura_forge_main,
        "capsys": capsys,
    }
...
def test_prints_multiline_summary_with_rename_map(curator_env):
    ...
    summary = (
        ...
        "full report: aura-forge curator status"
    )
    _set_state(
        curator_env["curator"],
        last_run_at=now,
