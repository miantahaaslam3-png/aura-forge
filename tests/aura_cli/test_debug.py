"""Tests for ``aura debug`` CLI command and debug utilities."""
...
def aura_home(tmp_path, monkeypatch):
    """Set up an isolated AURA_HOME with minimal logs."""
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO aura_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from aura_cli.debug import _upload_paste_rs
        ...
        with patch("aura_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "aura_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from aura_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from aura_cli.debug import upload_to_pastebin

        with patch("aura_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("aura_cli.debug._upload_paste_rs",
             ...
             patch("aura_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, aura_home):
        from aura_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, aura_home):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (aura_home / "logs" / "agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, aura_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = aura_home / "logs" / "agent.log"
        from aura_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, aura_home):
        ...
        (aura_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, aura_home):
    ...
    def test_unknown_log_returns_none(self, aura_home):
        from aura_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, aura_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from aura_cli.debug import _capture_log_snapshot

        logs_dir = aura_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, aura_home):
    ...
    def test_falls_back_when_primary_empty(self, aura_home):
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of AURA_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def aura_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated AURA_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".aura-forge"
        home.mkdir()
        monkeypatch.setenv("AURA_HOME", str(home))
        ...
        monkeypatch.delenv("AURA_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, aura_home_with_secret):
        from aura_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, aura_home_with_secret):
        from aura_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, aura_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when AURA_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("AURA_REDACT_SECRETS", "false")

        from aura_cli.debug import _capture_log_snapshot

        assert os.environ.get("AURA_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, aura_home_with_secret
    ):
        from aura_cli.debug import _capture_log_snapshot

        log_path = aura_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, aura_home_with_secret):
        from aura_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from aura_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, aura_home):
        from aura_cli.debug import collect_debug_report

        with patch("aura_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- aura dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- aura dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, aura_home):
        from aura_cli.debug import collect_debug_report

        with patch("aura_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, aura_home):
        from aura_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, aura_home):
        from aura_cli.debug import collect_debug_report
    ...
    def test_report_includes_gui_log(self, aura_home):
        from aura_cli.debug import collect_debug_report
    ...
    def test_report_includes_desktop_log(self, aura_home):
        from aura_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, aura_home, capsys):
        """Slash-command path should sweep old pending deletes before uploading."""
        from aura_cli.debug import run_debug_share
        ...
        with patch("aura_cli.dump.run_dump"), \
             patch("aura_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("aura_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, aura_home, capsys):
        ...
        with patch("aura_cli.dump.run_dump"), \
             patch(
                 "aura_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, aura_home, capsys):
    ...
    def test_share_uploads_five_pastes(self, aura_home, capsys):
        ...
        with patch("aura_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- aura dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- aura dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- aura dump ---" in gateway_paste
        assert "--- full gateway.log ---" in gateway_paste
        gui_paste = uploaded_content[3]
        assert "--- aura dump ---" in gui_paste
        assert "--- full gui.log ---" in gui_paste
        desktop_paste = uploaded_content[4]
        assert "--- aura dump ---" in desktop_paste
        assert "--- full desktop.log ---" in desktop_paste

    def test_share_keeps_report_and_full_log_on_same_snapshot(self, aura_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from aura_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("aura_cli.dump.run_dump"), \
             patch("aura_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("aura_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, aura_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, aura_home, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, aura_home_with_secret, capsys
    ):
        ...
        with patch("aura_cli.dump.run_dump"), \
             patch("aura_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("aura_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        from aura_cli.debug import run_debug
        ...
        out = capsys.readouterr().out
        assert "aura debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, aura_home):
        from aura_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from aura_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from aura_cli.debug import delete_paste
        ...
        with patch("aura_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    handle the DELETE requests synchronously on the next ``aura debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, aura_home):
        ...
        import ast
        import inspect
        from aura_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, aura_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from aura_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, aura_home):
    ...
    def test_merges_with_existing_pending(self, aura_home):
        """Subsequent calls merge into existing pending.json."""
        from aura_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, aura_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, aura_home):
        from aura_cli.debug import _sweep_expired_pastes

        deleted, remaining = _sweep_expired_pastes()
        assert deleted == 0
        assert remaining == 0

    def test_sweep_deletes_expired_entries(self, aura_home):
        from aura_cli.debug import (
        ...
        with patch("aura_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, aura_home):
        from aura_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("aura_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, aura_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, aura_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, aura_home):
        from aura_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("aura_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, aura_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: aura debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from aura_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("aura_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("aura_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, aura_home, capsys):
        from aura_cli.debug import run_debug_share
        ...
        with patch("aura_cli.dump.run_dump"), \
             ...
             patch("aura_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, aura_home, capsys):
        from aura_cli.debug import run_debug_share
        ...
        with patch("aura_cli.dump.run_dump"), \
             patch("aura_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("aura_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
    ...
    def test_local_no_privacy_notice(self, aura_home, capsys):
        from aura_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    Backs both ``aura debug share`` (CLI) and ``POST /api/ops/debug-share``
    (dashboard). The dashboard renders ``urls`` as real, copyable links, so the
    contract here is the return value, not stdout.
    """

    def test_returns_structured_urls(self, aura_home):
        from aura_cli.debug import build_debug_share, DebugShareResult
        ...
        with patch("aura_cli.dump.run_dump"), patch(
            "aura_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("aura_cli.debug._schedule_auto_delete"):
    ...
    def test_skips_missing_logs_without_failure(self, aura_home):
        from aura_cli.debug import build_debug_share

        # Remove desktop.log so it should be neither uploaded nor reported failed.
        (aura_home / "logs" / "desktop.log").unlink()

        with patch("aura_cli.dump.run_dump"), patch(
            "aura_cli.debug.upload_to_pastebin",
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, aura_home):
        from aura_cli.debug import build_debug_share
    ...
    def test_optional_log_failure_is_collected_not_raised(self, aura_home):
        from aura_cli.debug import build_debug_share
    ...
    def test_required_report_failure_raises(self, aura_home):
        from aura_cli.debug import build_debug_share
...
class TestCollectShareBundle:
    def test_returns_report_and_logs(self, aura_home):
        from aura_cli.debug import collect_share_bundle
    ...
    def test_no_redact_omits_banner(self, aura_home):
        from aura_cli.debug import collect_share_bundle
    ...
    def test_redaction_keeps_secrets_out(self, aura_home):
        from aura_cli.debug import collect_share_bundle
    ...
    def test_build_debug_share_uses_collector(self, aura_home):
        # build_debug_share must produce the same report text the collector does
        # (i.e. the refactor preserved paste.rs behaviour).
        from aura_cli.debug import build_debug_share, collect_share_bundle
...
class TestBuildNousBundle:
    def test_envelope_shape_and_gzip(self, aura_home):
        import gzip
        import json as _json

        from aura_cli.debug import build_nous_bundle
        ...
        assert envelope["format"] == "aura-debug-share/1"
...
class TestRunDebugShareNous:
    ...
    def test_nous_success_prints_view_url(self, aura_home, capsys):
        from aura_cli.debug import run_debug_share
        ...
        with patch("aura_cli.dump.run_dump"), patch(
            "aura_cli.diagnostics_upload.share_to_nous", return_value=res
        ) as share:
    ...
    def test_nous_failure_suggests_local(self, aura_home, capsys):
        from aura_cli.debug import run_debug_share

        with patch("aura_cli.dump.run_dump"), patch(
            "aura_cli.diagnostics_upload.share_to_nous",
    ...
    def test_nous_does_not_touch_pastebin(self, aura_home):
        from aura_cli.debug import run_debug_share

        res = {"id": "id-1", "viewUrl": "https://v"}
        with patch("aura_cli.dump.run_dump"), patch(
            "aura_cli.diagnostics_upload.share_to_nous", return_value=res
        ), patch("aura_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
...
class TestDebugSlashCommand:
    ...
    ``AuraCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    which parses an optional destination word and builds the args namespace
    handed to ``run_debug_share``.
    """

    def _handler(self):
        from aura_cli.cli_commands_mixin import CLICommandsMixin
    ...
    def _captured(self, cmd_original):
        ...
        with patch("aura_cli.debug.run_debug_share", _fake_run):
...
class TestShareConsentGate:
    """`aura debug share` requires explicit consent before uploading.
    ...
    def test_aborts_on_user_decline(self, aura_home, capsys, monkeypatch):
        ...
        with patch("aura_cli.dump.run_dump"), \
             patch("aura_cli.debug.upload_to_pastebin") as mock_upload:
            run_debug_share(self._args())

        mock_upload.assert_not_called()
        assert "Aborted" in capsys.readouterr().out

    def test_proceeds_on_user_accept(self, aura_home, capsys, monkeypatch):
    ...
    def test_yes_flag_skips_prompt(self, aura_home, capsys, monkeypatch):
    ...
    def test_non_interactive_requires_yes(self, aura_home, capsys, monkeypatch):
    ...
    def test_non_interactive_with_yes_succeeds(self, aura_home, capsys, monkeypatch):
    ...
    def test_nous_path_also_gated(self, aura_home, capsys, monkeypatch):
        ...
        with patch("aura_cli.dump.run_dump"), \
             patch("aura_cli.diagnostics_upload.share_to_nous") as mock_nous:
            run_debug_share(self._args(nous=True))

        mock_nous.assert_not_called()
        assert "Aborted" in capsys.readouterr().out

    def test_local_never_prompts(self, aura_home, capsys, monkeypatch):
