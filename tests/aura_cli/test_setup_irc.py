"""Tests for IRC gateway configuration via `aura setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Aura install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Aura install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#aura-forge")
            monkeypatch.setenv("IRC_NICKNAME", "aura-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `aura setup gateway` experience with IRC present."""
