"""Tests for the top-level `./aura` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./aura` should use `aura_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "aura"
    called = []

    fake_main_module = types.ModuleType("aura_cli.main")

    def fake_main():
        called.append("aura_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "aura_cli.main", fake_main_module)
