"""Regression tests for #34107 — Docker UID/GID handling in ensure_aura_home.

When Aura runs in Docker with ``AURA_UID=1000`` / ``AURA_GID=911``,
the entrypoint chowns the top-level ``AURA_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_aura_home()`` — especially
...
The fix is a ``_chown_to_aura_forge_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_aura_forge_uid_gid
# ---------------------------------------------------------------------------


class TestResolveAuraForgeUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("AURA_UID", "1000")
        monkeypatch.setenv("AURA_GID", "911")
        from aura_cli.config import _resolve_aura_forge_uid_gid
        uid, gid = _resolve_aura_forge_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("AURA_UID", raising=False)
        monkeypatch.delenv("AURA_GID", raising=False)
        from aura_cli.config import _resolve_aura_forge_uid_gid
        uid, gid = _resolve_aura_forge_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("AURA_UID", "not-a-number")
        monkeypatch.setenv("AURA_GID", "911")
        from aura_cli.config import _resolve_aura_forge_uid_gid
        uid, gid = _resolve_aura_forge_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("AURA_UID", "")
        monkeypatch.setenv("AURA_GID", "")
        from aura_cli.config import _resolve_aura_forge_uid_gid
        uid, gid = _resolve_aura_forge_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToHermesUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("AURA_UID", "1000")
        monkeypatch.setenv("AURA_GID", "911")
        from aura_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_aura_forge_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_aura_forge_uid(d)  # must not raise
