"""Tests for aura_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
from aura_cli.telegram_managed_bot import (
    DEFAULT_MANAGER_BOT,
...
class TestGenerateBotUsername:
    def test_secure_default_format(self):
        ...
        assert name.startswith("aura_forge_")
        assert name.endswith("_bot")
        assert len(name) == len("aura_forge_") + 16 + len("_bot")
        assert len(name) <= 32
    ...
    def test_slug_uses_telegram_safe_base32_chars(self):
        name = generate_bot_username()
        slug = name.removeprefix("aura_forge_").removesuffix("_bot")
        assert len(slug) == 16
...
class TestGenerateDeepLink:
    ...
    def test_defaults(self):
        ...
        assert "aura_forge_" in link

    def test_name_url_encoded(self):
        link = generate_deep_link(
            manager_bot="Bot",
            suggested_username="test_bot",
            suggested_name="Aura & Friends",
        )
        assert "Aura Forge+%26+Friends" in link
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "suggested_username": "aura_forge_abcdefghijklmnop_bot",
            "deep_link": "https://t.me/newbot/AuraForgeSetupBot/aura_abcdefghijklmnop_bot?name=Aura Forge+Agent",
            "qr_payload": "https://t.me/newbot/AuraForgeSetupBot/aura_abcdefghijklmnop_bot?name=Aura Forge+Agent",
            "expires_at": "2026-05-18T00:00:00.000Z",
        }

        with patch(
            "aura_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
            pairing = create_pairing("https://api.example.com", bot_name="Aura Forge")
...
class TestPollForToken:
    def pairing(self):
        return TelegramPairing(
            ...
            poll_token="secret-token",
            suggested_username="aura_forge_abcdefghijklmnop_bot",
            deep_link="https://t.me/newbot/AuraForgeSetupBot/aura_abcdefghijklmnop_bot",
            qr_payload="https://t.me/newbot/AuraForgeSetupBot/aura_abcdefghijklmnop_bot",
        )
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "aura_forge_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
        ...
        with patch(
            "aura_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("aura_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_setup_result_includes_owner_user_id(self):
        ...
        with patch("aura_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("aura_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_invalid_ready_token_returns_none(self):
        ...
        with patch("aura_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("aura_cli.telegram_managed_bot.time.sleep"):
                with patch(
                    "aura_cli.telegram_managed_bot.time.monotonic"
                ) as mock_time:
    ...
    def test_eventual_success(self):
        ...
        with patch("aura_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("aura_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    def test_setup_helper_exists(self):
        from aura_cli.setup import _setup_telegram_auto

        assert callable(_setup_telegram_auto)

    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        from aura_cli import setup

        seen = {}
        profile_home = tmp_path / ".aura-forge" / "profiles" / "oracle"
        profile_home.mkdir(parents=True)

        monkeypatch.setattr(setup, "get_aura_home", lambda: profile_home)
        ...
        monkeypatch.setattr(
            "aura_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from aura_cli.setup import _profile_name_from_aura_home

        assert (
            _profile_name_from_aura_home(
                PureWindowsPath(r"C:\Users\test\AppData\Local\aura-forge\profiles\oracle")
            )
            == "oracle"
        )
