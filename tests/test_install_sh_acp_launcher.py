"""`setup_path()` must also install a `auraforge-acp` launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving a command
name against the login-shell PATH. The `auraforge-acp` console script generated by
the packaging metadata lives inside the install's venv, which is not on that
PATH, so those hosts report Aura Forge as not installed. `setup_path()` therefore
writes a `auraforge-acp` launcher next to the `auraforge` one.
...
ACP_BLOCK = re.compile(
    r'(    rm -f "\$command_link_dir/auraforge-acp".*?'
    r'log_success "Installed auraforge-acp launcher[^\n]*\n)',
    re.S,
)
...
def _extract_acp_shim_block() -> str:
    ...
    assert match, (
        "could not locate the auraforge-acp launcher block in scripts/install.sh — "
...
def _run_block(tmp_path: Path, use_venv: str) -> Path:
    ...
    return command_link_dir / "auraforge-acp"
...
def test_acp_launcher_does_not_follow_a_symlink_into_the_venv(tmp_path):
    ...
    An older install could leave `auraforge-acp` as a symlink to the venv console
    ...
    shim_path = command_link_dir / "auraforge-acp"
    ...
    assert console_script.read_text(encoding="utf-8") == marker, (
        "venv/bin/auraforge-acp was overwritten — symlink-stomp regression (#21454)"
    )
    assert not shim_path.is_symlink(), (
        "command_link_dir/auraforge-acp must be replaced with a regular file"
    )


# ---------------------------------------------------------------------------
# auraforge-agent launcher regression (#74819)
# ---------------------------------------------------------------------------

HERMES_AGENT_BLOCK = re.compile(
    r'(rm -f "\$command_link_dir/auraforge-agent".*?'
    r'log_success "Installed auraforge-agent launcher[^\n]*\n)',
    re.S,
)
...
def _extract_hermes_agent_shim_block() -> str:
    ...
    assert match, (
        "could not locate the auraforge-agent launcher block in scripts/install.sh — "
        "if it was renamed, update this test with it"
    )
    return match.group(1)
...
def _run_hermes_agent_block(tmp_path: Path, use_venv: str) -> Path | None:
    """Execute the extracted auraforge-agent block with the env vars setup_path() sets."""
    if use_venv == "false":
        # --no-venv: auraforge-agent is NOT installed by this block (handled
    ...
    assert result.returncode == 0, (
        f"auraforge-agent shim block failed:\nstdout={result.stdout}\nstderr={result.stderr}"
    )
    return command_link_dir / "auraforge-agent"


def test_venv_install_writes_executable_hermes_agent_launcher(tmp_path):
    """venv install must write a user-executable auraforge-agent launcher."""
    ...
    assert "unset PYTHONPATH" in text
    assert "unset PYTHONHOME" in text
    assert "run_agent.py" in text, "auraforge-agent must dispatch to run_agent.py"
...
def test_hermes_agent_launcher_cleanup_on_uninstall(tmp_path):
    ...
    local_shim.write_text("#!/usr/bin/env bash\nexec auraforge-agent\n", encoding="utf-8")

    with patch.object(Path, "home", return_value=tmp_path):
        removed = remove_wrapper_script()

    assert local_shim in removed, "local auraforge-agent wrapper must be removed"
