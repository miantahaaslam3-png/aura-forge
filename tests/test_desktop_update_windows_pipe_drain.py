...
``.auraforge-update-result.json`` is never written, ``.auraforge-update-in-progress``
is never cleared, and the Desktop is never relaunched -- so the app sits on
"Updating Aura Forge" until the user kills the gateway by hand, and the stale marker
...
step writing to both pipes took 18.3s vs 0.29s. ``auraforge update`` is exactly this
...
def test_update_step_survives_pipe_leak_flood_and_live_child_stall(
    ...
    *logstall* -- a step that is silent on its pipes but appends to the
    ...
    This is the shape of every real ``auraforge update`` build: output streams to
    ...
    assert "PIPE-DRAIN SELF-TEST: PASS" in result.stdout, (
        ...
        "Aura Forge' forever) or metered a chatty step (backpressure on the running "
