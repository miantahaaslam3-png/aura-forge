...
Regression test for https://github.com/miantahaaslam3-png/aura-forge/issues/18594.

When HERMES_HOME is unset but an active_profile file indicates a non-default
profile is active, get_hermes_home() should:
  1. STILL return ~/.auraforge (raising would brick 30+ module-level callers)
...
def fresh_constants(monkeypatch, tmp_path):
    """Import auraforge_constants fresh and reset the one-shot warn flag."""
    import importlib
    import auraforge_constants
    importlib.reload(auraforge_constants)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.delenv("HERMES_HOME", raising=False)
    return auraforge_constants
...
class TestGetHermesHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        """Classic mode: no active_profile file → silent, returns ~/.auraforge."""
    ...
    def test_hermes_home_set_suppresses_warning(
        ...
        (tmp_path / ".auraforge" / "active_profile").write_text("coder\n")
