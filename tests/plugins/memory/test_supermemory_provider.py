...
def test_format_prefetch_context_deduplicates_overlap():
    result = _format_prefetch_context(
        ...
        dynamic_facts=["Jordan prefers short answers", "Uses Aura Forge"],
        search_results=[{"memory": "Uses Aura Forge", "similarity": 0.9}],
        max_results=10,
    )
    assert result.count("Jordan prefers short answers") == 1
    assert result.count("Uses Aura Forge") == 1
...
def test_prefetch_includes_profile_on_first_turn(provider):
    provider._client.profile_response = {
        ...
        "search_results": [{"memory": "Working on Aura Forge memory provider", "similarity": 0.88}],
    }
...
def test_merge_metadata_stamps_sm_source():
    # sm_source routes Aura Forge writes into the "Aura Forge" Space in the Supermemory
    ...
    assert merged["sm_source"] == "auraforge"
    ...
    assert merged2["sm_source"] == "auraforge"
...
def test_identity_template_resolved_in_container_tag(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("SUPERMEMORY_API_KEY", "test-key")
    monkeypatch.setattr("plugins.memory.supermemory._SupermemoryClient", FakeClient)
    _save_supermemory_config({"container_tag": "auraforge-{identity}"}, str(tmp_path))
    p = SupermemoryMemoryProvider()
...
def test_ingest_conversation_uses_client_base_url(monkeypatch, base_url, expected_url):
    ...
    client._container_tag = "auraforge"
    client._timeout = 1.0
...
def test_probe_supermemory_connection_missing_key(tmp_path):
    ...
    assert status["container_tag"] == "auraforge"
...
def test_post_setup_writes_config_and_prints_summary(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        "auraforge_cli.memory_setup._prompt",
    ...
    monkeypatch.setattr("auraforge_cli.config.save_config", fake_save_config)
