...
from auraforge_cli.memory_setup import _CANCELLED
...
def test_normalize_retain_tags_accepts_csv_and_dedupes():
    assert _normalize_retain_tags("agent:fakeassistantname, source_system:auraforge-agent, agent:fakeassistantname") == [
        "agent:fakeassistantname",
        "source_system:auraforge-agent",
    ]
...
class TestConfig:
    ...
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Aura Forge Agent and the User"
    ...
    def test_custom_config_values(self, provider_with_config):
        ...
        assert p._retain_source == "auraforge"
...
class TestPostSetup:
    def test_setup_cancel_at_mode_picker_writes_nothing(self, tmp_path, monkeypatch):
        ...
        run = MagicMock()
        monkeypatch.setattr("auraforge_cli.memory_setup._curses_select", lambda *args, **kwargs: _CANCELLED)
        ...
        assert not (user_home / ".hindsight" / "profiles" / "auraforge.env").exists()
    ...
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("auraforge_cli.memory_setup._curses_select", lambda *args, **kwargs: next(selections))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "sk-local-test")
        saved_configs = []
        monkeypatch.setattr("auraforge_cli.config.save_config", lambda cfg: saved_configs.append(cfg.copy()))
        ...
        profile_env = user_home / ".hindsight" / "profiles" / "auraforge.env"
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config, monkeypatch):
        ...
        item = call_kwargs["items"][0]
        assert item["context"] == "conversation between Aura Forge Agent and the User"
        ...
        assert item["metadata"]["source"] == "auraforge"
...
class TestBankIdTemplate:
    def test_sanitize_bank_segment_passthrough(self):
        assert _sanitize_bank_segment("auraforge") == "auraforge"
    ...
    def test_resolve_empty_template_uses_fallback(self):
        result = _resolve_bank_id_template(
            "", fallback="auraforge", profile="coder"
        )
        assert result == "auraforge"


    def test_resolve_sanitizes_placeholder_values(self):
        result = _resolve_bank_id_template(
            "user-{user}", fallback="auraforge",
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        config = {
            ...
            "bank_id_template": "auraforge-{profile}",
        }
        config_path = tmp_path / "hindsight" / "config.json"
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        ...
        p.initialize(
            ...
            agent_workspace="auraforge",
        )
        assert p._bank_id == "auraforge-coder"
        assert p._bank_id_template == "auraforge-{profile}"
...
class TestPostSetupEnvEncoding:
    def _run_cloud_post_setup(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("auraforge_cli.memory_setup._curses_select",
                            lambda *a, **kw: 0)  # cloud mode
        monkeypatch.setattr("auraforge_cli.config.save_config", lambda c: None)
