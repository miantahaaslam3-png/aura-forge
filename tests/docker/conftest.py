...
IMAGE_TAG = os.environ.get("HERMES_TEST_IMAGE", "auraforge-agent-harness:latest")
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged auraforge user
# ---------------------------------------------------------------------------
#
# Background: every Aura Forge runtime path inside the container drops to UID
# 10000 (the ``auraforge`` user) via ``s6-setuidgid auraforge``. ``docker exec``
...
# fails (PermissionError swallowed) for auraforge, so a test that ran as root
...
# helpers below run every probe under ``-u auraforge`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: auraforge).
