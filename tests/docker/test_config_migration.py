...
is migrated by docker_config_migrate.py on boot, running as the auraforge
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    docker_config_migrate.py, running as the auraforge user."""
    # Start container
    ...
    r = docker_exec_sh(
        container_name,
        "test -f /opt/auraforge/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by auraforge (migration ran as auraforge)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "auraforge", (
        f"config.yaml not owned by auraforge (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
