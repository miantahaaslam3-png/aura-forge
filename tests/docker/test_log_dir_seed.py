...
owned by the auraforge user after container boot.
...
registered later runs its log service as the dropped auraforge user and
...
def test_logs_gateways_seeded_and_hermes_owned(
    ...
    """logs/ and logs/gateways/ must exist and be owned by auraforge after boot."""
    ...
    # Both must be owned by auraforge
    r = docker_exec_sh(
        container_name,
    ...
    assert "logs=auraforge" in r.stdout, (
        f"logs/ not owned by auraforge: {r.stdout}"
    )
    assert "gateways=auraforge" in r.stdout, (
        f"logs/gateways/ not owned by auraforge: {r.stdout}"
    )
