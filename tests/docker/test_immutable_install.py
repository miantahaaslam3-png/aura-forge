...
Build the real image and verify at runtime:
  ...
  1. /opt/auraforge is not writable by the auraforge user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and HERMES_DISABLE_LAZY_INSTALLS are set
  3. /opt/auraforge/.install_method contains "docker" (code-scoped stamp)
...
def test_install_tree_not_writable_by_hermes(
    ...
    """The auraforge user must not be able to modify /opt/auraforge.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/auraforge as the auraforge user
        "touch /opt/auraforge/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"auraforge user can write to /opt/auraforge (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/auraforge/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"auraforge user can write to /opt/auraforge/.venv: {r.stdout}"
    )
