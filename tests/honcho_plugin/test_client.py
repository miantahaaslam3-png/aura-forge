...
class TestResolveActiveHost:
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            # Temporarily remove auraforge_cli.profiles to simulate import failure
            saved = sys.modules.get("auraforge_cli.profiles")
            sys.modules["auraforge_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "auraforge"
            finally:
                if saved is not None:
                    sys.modules["auraforge_cli.profiles"] = saved
                else:
                    sys.modules.pop("auraforge_cli.profiles", None)
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("auraforge_cli.profiles.get_active_profile_name", return_value="default"), \
