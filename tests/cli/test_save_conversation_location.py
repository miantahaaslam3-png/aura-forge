...
file to be discoverable via ``auraforge sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.auraforge/sessions/saved/`` and prints
...
def hermes_home(tmp_path, monkeypatch):
    ...
    if hasattr(auraforge_constants, "_hermes_home_cache"):
        auraforge_constants._hermes_home_cache = None
    return home
...
def test_save_conversation_writes_under_hermes_home(hermes_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.auraforge/sessions/saved/, not CWD."""
    ...
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "auraforge_constants"]:
    ...
    # File MUST be under ~/.auraforge/sessions/saved/
    ...
    assert "auraforge --resume 20260101_120000_abc123" in out, out
