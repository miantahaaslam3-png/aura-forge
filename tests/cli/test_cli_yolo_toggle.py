...
prompt. Only ``auraforge --yolo`` (process-start env), ``HERMES_YOLO_MODE=1``,
and ``auraforge config set approvals.mode off`` actually bypassed.
...
def _clear_approval_state(monkeypatch):
    ...
    # Aura Forge-driven test runs may inherit HERMES_YOLO_MODE=1 from the parent
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``auraforge --yolo`` sets ``HERMES_YOLO_MODE`` before tool imports, so
