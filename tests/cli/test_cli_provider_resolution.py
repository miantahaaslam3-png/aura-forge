...
def test_provider_flag_uses_named_custom_default_model(monkeypatch):
    ...
    monkeypatch.setattr("auraforge_cli.config.load_config", lambda: config)
    monkeypatch.setattr("auraforge_cli.runtime_provider.load_config", lambda: config)
...
def test_provider_flag_logs_when_custom_default_model_cannot_resolve(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        "auraforge_cli.runtime_provider._get_named_custom_provider",
        _boom,
    )
...
def test_hermes_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    ...
    monkeypatch.setattr("auraforge_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("auraforge_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("auraforge_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("auraforge_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_model_flow_nous_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    ...
    config_home = tmp_path / "auraforge"
    config_home.mkdir()
    ...
    monkeypatch.setattr(
        "auraforge_cli.auth.get_provider_auth_state",
        lambda provider: {
    ...
    monkeypatch.setattr(
        "auraforge_cli.models.get_curated_nous_model_ids",
        lambda: [selected_model],
    )
    monkeypatch.setattr("auraforge_cli.models.get_pricing_for_provider", lambda provider: {})
    monkeypatch.setattr("auraforge_cli.models.check_nous_free_tier", lambda **kwargs: False)
    monkeypatch.setattr(
        "auraforge_cli.models.union_with_portal_paid_recommendations",
    ...
    monkeypatch.setattr(
        "auraforge_cli.nous_subscription.prompt_enable_tool_gateway",
        lambda config: None,
    )
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "auraforge_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("auraforge_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("auraforge_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("auraforge_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("auraforge_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr("auraforge_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("auraforge_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("auraforge_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr(
        "auraforge_cli.models.probe_api_models",
        lambda api_key, base_url: {
    ...
    saved_env = {}
    monkeypatch.setattr("auraforge_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("auraforge_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "auraforge_cli.config.save_env_value",
        lambda key, value: saved_env.__setitem__(key, value),
    )
    monkeypatch.setattr(
        "auraforge_cli.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("auraforge_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    ...
    monkeypatch.setattr("auraforge_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("auraforge_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("auraforge_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("auraforge_cli.auth.resolve_provider", lambda requested, **kwargs: "nous")
    monkeypatch.setattr("auraforge_cli.auth.get_provider_auth_state", lambda provider_id: None)
    ...
    monkeypatch.setattr("auraforge_cli.auth._login_nous", _fake_login)

    hermes_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.nousresearch.com",
            inference_url="https://inference.nousresearch.com/v1",
            client_id="auraforge-local",
...
def test_auto_provider_name_localhost():
    from auraforge_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from auraforge_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "auraforge_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("auraforge_cli.config.save_config", _save)
...
def test_save_custom_provider_references_the_key_instead_of_inlining_it(monkeypatch, tmp_path):
    ...
    saved = {}
    monkeypatch.setattr("auraforge_cli.config.save_config", lambda cfg: saved.update(cfg))
...
def test_custom_endpoint_key_env_is_a_valid_posix_name_for_ip_endpoints():
    ...
    from auraforge_cli.config import _ENV_VAR_NAME_RE, custom_endpoint_key_env
