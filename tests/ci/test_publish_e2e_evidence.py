...
def test_publish_marks_evidence_upload_failure_in_pr_comment(tmp_path, monkeypatch):
    comment = {
        "id": 123,
        "body": "before\n<!-- auraforge-e2e-evidence:start -->\npending\n<!-- auraforge-e2e-evidence:end -->\nafter",
    }
    updates = []
    ...
    assert updates == [
        (
            "https://api.github.com/repos/miantahaaslam3-png/aura-forge/issues/comments/123",
            "github-token",
            "PATCH",
            {
...
def test_find_review_comment_requires_the_evidence_marker():
    pending = "<!-- auraforge-ci-review-bot -->\n<!-- auraforge-e2e-evidence:start -->\npending\n<!-- auraforge-e2e-evidence:end -->"

    assert _mod._find_review_comment([{"body": "<!-- auraforge-ci-review-bot --> no evidence"}]) is None
