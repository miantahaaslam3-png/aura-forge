"""Tests for the top-level `./auraforge` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./auraforge` should use `auraforge_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "auraforge"
    called = []

    fake_main_module = types.ModuleType("auraforge_cli.main")

    def fake_main():
        called.append("auraforge_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "auraforge_cli.main", fake_main_module)
    ...
    assert called == ["auraforge_cli.main"]
