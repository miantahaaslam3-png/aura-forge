"""Tests for `auraforge memory setup [provider]` routing.
...
``auraforge memory setup honcho``) without the interactive picker — which matters
because the per-provider ``auraforge <provider>`` subcommand is only registered
...
from auraforge_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "auraforge memory setup" in out
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        with _patch.dict(os.environ, {"HERMES_DISABLE_LAZY_INSTALLS": "0"}), \
             ...
             patch("auraforge_cli.tools_config.shutil.which", side_effect=which_side_effect), \
             patch("auraforge_cli.tools_config.subprocess.run", fake_run):
