...
def test_show_status_termux_gateway_section_skips_systemctl(monkeypatch, capsys, tmp_path):
    ...
    assert "Start with:   auraforge gateway" in output
...
class TestShowStatusXaiOAuth:
    """xAI OAuth row in auraforge status."""
    ...
    def test_logged_in_shows_auth_store(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(auth_mod, "get_xai_oauth_auth_status",
                            lambda: {"logged_in": True, "auth_store": "/home/u/.auraforge/auth.json"},
                            raising=False)

        status_mod.show_status(SimpleNamespace(all=False, deep=False))
        out = capsys.readouterr().out

        assert "Auth file:  /home/u/.auraforge/auth.json" in out
    ...
    def test_status_function_returns_none_does_not_crash(self, monkeypatch, capsys, tmp_path):
        ...
        assert "not logged in (run: auraforge auth add xai-oauth)" in out


def test_show_status_reports_gateway_session_last_activity(monkeypatch, capsys, tmp_path):
    """auraforge status should surface freshest gateway last_active (#72016)."""
    from auraforge_cli import status as status_mod
    import auraforge_cli.auth as auth_mod
    import auraforge_cli.gateway as gateway_mod
    ...
    monkeypatch.setattr(auraforge_state, "SessionDB", _FakeDB)
