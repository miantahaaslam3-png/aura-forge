...
`auraforge kanban …`) inherit the same board the in-process kanban tools resolve.
Without this, a concurrent `auraforge kanban boards switch` from another session
...
def test_pin_writes_resolved_board_when_env_unset(monkeypatch):
    main_mod = importlib.import_module("auraforge_cli.main")

    import auraforge_cli.kanban_db as kdb
