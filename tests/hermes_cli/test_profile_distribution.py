"""Tests for auraforge_cli.profile_distribution — git-based profile installs.
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/auraforge_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestInstall:
    ...
    def test_update_respects_distribution_owned_allowlist(self, profile_env):
        ...
        from auraforge_cli.profile_distribution import read_manifest as _read
    ...
    def test_install_enforces_hermes_requires(self, profile_env, monkeypatch):
        # Pin current Aura Forge version to something well below the requirement
        import auraforge_cli
        monkeypatch.setattr(auraforge_cli, "__version__", "0.1.0", raising=False)
...
class TestUpdate:
    ...
    def test_update_missing_manifest_errors(self, profile_env):
        ...
        from auraforge_cli.profiles import create_profile
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "auraforge_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from auraforge_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from auraforge_cli.profiles import create_profile, list_profiles, get_profile_dir
