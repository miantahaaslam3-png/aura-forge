...
# unprivileged auraforge user from every lifecycle op. These tests run
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``auraforge gateway run``, an agent action, the Open WebUI helper) holds
    ...
    # Root profile: bare `auraforge gateway run --replace` (no -p flag).
    assert "auraforge gateway run --replace" in default_text
    assert "auraforge -p default" not in default_text
...
#
# `auraforge gateway stop` inside a container dispatches through
...
def test_s6_log_run_creates_leaf_as_hermes_without_chown(
    ...
    """log/run must not root-chown/unlink volume paths; create leaf as auraforge.

    #45258 parent ownership is stage2's job (``logs/gateways`` seeded as
    auraforge). Restartable log/run must not pathname-chown or pathname-rm a
    auraforge-writable tree from root — that is a symlink TOCTOU hole.
    """
    ...
    assert not any(line.lstrip().startswith("chown ") for line in log_text.splitlines()), (
        "restartable log/run must not invoke chown on auraforge-writable paths; "
        f"saw: {log_text!r}"
    )
    assert 's6-setuidgid auraforge mkdir -p "$log_dir"' in log_text
    assert 's6-setuidgid auraforge rm -f "$log_dir/lock"' in log_text
    assert 'else\n  mkdir -p "$log_dir"\n  rm -f "$log_dir/lock"\nfi\n' in log_text
    ...
    mkdir_as_hermes_idx = log_text.index('s6-setuidgid auraforge mkdir -p "$log_dir"')
    rm_as_hermes_idx = log_text.index('s6-setuidgid auraforge rm -f "$log_dir/lock"')
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    # a real auraforge uid cannot delete a foreign root-owned lock.
    ...
    (bin_dir / "rm").write_text(
        "#!/bin/sh\n"
        # Privilege-dropped: no-op. Models that auraforge cannot unlink a foreign
