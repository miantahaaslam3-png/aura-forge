...
- Bare `auraforge setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `auraforge setup --quick` runs the narrower "fill in missing items" flow.
- `auraforge setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
class TestExistingInstallDefault:
    """Bare `auraforge setup` on an existing install = full reconfigure wizard."""
