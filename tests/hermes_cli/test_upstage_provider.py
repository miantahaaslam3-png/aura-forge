...
Regression guard for the bug where `auraforge model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`auraforge_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `auraforge_cli/main.py` (cmd_model): read
