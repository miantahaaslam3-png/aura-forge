...
checkout where auraforge_cli/gateway.py newly imports `line_input` from
auraforge_cli.cli_output, but the updater process had cli_output cached from
before that symbol existed. The function-level `from auraforge_cli.gateway
...
the Aura Forge package prefixes so later imports rebuild a self-consistent
...
def _restore_sys_modules():
    ...
    The purge under test evicts real Aura Forge modules from the cache; later
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_hermes_modules()
    assert sys.modules.get("auraforge_cli.update_cmd") is update_cmd
    assert sys.modules.get("auraforge_cli.main") is cli_main
    assert "auraforge_cli" in sys.modules
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["auraforge_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_hermes_modules()
    finally:
        sys.modules.pop("auraforge_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from auraforge_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
