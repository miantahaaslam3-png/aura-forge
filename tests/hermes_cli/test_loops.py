"""Tests for auraforge_cli/loops.py — /loop recurring in-session wakeups."""
...
class TestParseIntervalToken:
    def test_minutes(self):
        from auraforge_cli.loops import parse_interval_token
...
class TestParseLoopArgs:
    def test_fixed_interval(self):
        from auraforge_cli.loops import parse_loop_args
...
class TestFormatInterval:
    def test_render(self):
        from auraforge_cli.loops import format_interval
...
class TestResponseSignalsComplete:
    def test_marker_on_own_line(self):
        from auraforge_cli.loops import response_signals_complete
...
class TestLoopStateSerde:
    def test_round_trip(self):
        from auraforge_cli.loops import LoopState
...
class TestPersistence:
    def test_save_load_clear(self, hermes_home):
        from auraforge_cli.loops import LoopManager, load_loop
    ...
    def test_list_active_loops(self, hermes_home):
        from auraforge_cli.loops import LoopManager, list_active_loops
    ...
    def test_migrate_to_session(self, hermes_home):
        from auraforge_cli.loops import LoopManager, load_loop, migrate_loop_to_session
...
class TestTickLifecycle:
    ...
    def test_until_judge_done_stops(self, hermes_home):
        ...
        state.next_due_at = time.time() - 1
        mgr.fire_tick()
        with patch("auraforge_cli.goals.judge_goal", return_value=("done", "suite green", False, None, False)):
    ...
    def test_until_judge_continue_loops(self, hermes_home):
        ...
        state.next_due_at = time.time() - 1
        mgr.fire_tick()
        with patch("auraforge_cli.goals.judge_goal", return_value=("continue", "3 failures", False, None, False)):
    ...
    def test_until_judge_error_fails_open(self, hermes_home):
        ...
        state.next_due_at = time.time() - 1
        mgr.fire_tick()
        with patch("auraforge_cli.goals.judge_goal", side_effect=RuntimeError("api down")):
...
class TestControls:
    def test_min_interval_enforced(self, hermes_home):
        from auraforge_cli.loops import LoopManager, min_interval_seconds
...
class TestDispatchLoopCommand:
    ...
    def test_route_stored(self, hermes_home):
        from auraforge_cli.loops import LoopManager, dispatch_loop_command, load_loop
