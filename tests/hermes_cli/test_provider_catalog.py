"""Tests for the unified provider catalog (auraforge_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``auraforge model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from auraforge_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
