...
def test_classify_local_preview_args_preserves_full_directory_label_and_port() -> None:
    args = [
        r"C:\Aura Forge\venv\Scripts\python.exe",
        "-m",
        "http.server",
        "8766",
        "--bind",
...
#
# `auraforge-setup` always invokes `auraforge update --yes --gateway`, whose
...
def test_main_gateway_with_long_managed_runtime_path_is_exempt(monkeypatch, capsys):
    ...
    """
    long_exe = (
        r'"C:\Users\u\AppData\Local\auraforge\auraforge-agent\.auraforge-runtime\python'
