"""Tests for Codex auth — tokens stored in Aura Forge auth store (~/.auraforge/auth.json)."""
...
def _setup_hermes_auth(hermes_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Aura Forge auth store."""
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``auraforge auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``auraforge auth add openai-codex``.
    ...
    (hermes_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `auraforge auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``auraforge auth add openai-codex`` for accounts B and C
    ...
    (hermes_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``auraforge auth add openai-codex`` (account B and account C).
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Aura Forge auth store, not ~/.codex/."""
    hermes_home = tmp_path / "auraforge"
    ...
    _save_codex_tokens({"access_token": "auraforge-at", "refresh_token": "auraforge-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Aura Forge store
    assert not (codex_home / "auth.json").exists()

    # Aura Forge auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "auraforge-at"
...
def test_resolve_returns_hermes_auth_store_source(tmp_path, monkeypatch):
    ...
    assert creds["source"] == "auraforge-auth-store"
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    assert "auraforge auth" not in rendered
