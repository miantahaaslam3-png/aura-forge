...
1. The venv-repair import probes (early recovery + `auraforge update`) must
...
2. ``auraforge doctor`` must fail the certificate check in that state, and
   ``auraforge doctor --fix`` must repair by force-reinstalling certifi and
   re-verifying.
"""
...
class TestUpdateProbeScriptChecksBundle:
    """The subprocess probe used by `auraforge update`'s venv repair must apply
...
# =========================================================================
# 2. auraforge doctor: detection and --fix repair
# =========================================================================
...
class TestSslGuardRepairHint:
    def test_missing_bundle_error_mentions_doctor_fix(self, monkeypatch, tmp_path):
        ...
        assert "auraforge doctor --fix" in message
