...
def test_list_authenticated_providers_enumerates_dict_format_models(monkeypatch):
    ...
    (canonical Aura Forge write format) should surface every key in the picker.
...
def test_list_authenticated_providers_accepts_base_url_and_singular_model(monkeypatch):
    """providers: dict entries written in canonical Aura Forge shape
    ...
    ``default_model``, so new-shape entries written by Aura Forge's own writer
