"""Uninstall must not leave a dangling ``auraforge`` command on Windows.

Every uninstall mode deletes the code checkout, but the launchers install.ps1
staged in the managed binary dir (the default Aura Forge root's ``bin``, shared
with the managed uv) live outside it. A surviving launcher makes ``auraforge``
...
def managed_bin(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    (bin_dir / "auraforge.exe").write_bytes(b"MZ launcher")
    (bin_dir / "auraforge-acp.cmd").write_text("@echo off\r\n", encoding="ascii")
...
def test_removes_both_launcher_forms_and_keeps_managed_uv(managed_bin: Path):
    ...
    assert sorted(p.name for p in removed) == ["auraforge-acp.cmd", "auraforge.exe"]
    assert not (managed_bin / "auraforge.exe").exists()
    assert not (managed_bin / "auraforge-acp.cmd").exists()
...
def test_noop_on_posix(managed_bin: Path):
    ...
    assert (managed_bin / "auraforge.exe").exists()
...
def test_launcher_names_stay_in_lockstep_with_install_ps1():
    ...
    for name in _WINDOWS_BIN_LAUNCHERS:
        assert name.startswith("auraforge")  # never a generic name it could clobber
...
class TestManagedBinPathMarker:
    ...
    HOME = r"C:\Users\me\AppData\Local\auraforge"
    BIN_ENTRY = r"C:\Users\me\AppData\Local\auraforge\bin"
