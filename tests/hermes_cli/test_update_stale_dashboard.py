...
def _refresh_bindings_against_live_module():
    """Rebind module-level names to the *current* ``auraforge_cli.main``.

    Other tests in the suite (notably ``test_env_loader.py`` and
    ``test_skills_subparser.py``) reload or delete ``auraforge_cli.main`` from
    ``sys.modules``.  When that happens on the same xdist worker before we
    run, our top-of-file ``from auraforge_cli.main import ...`` bindings end
    up pointing at the *old* module object.  ``patch(\"auraforge_cli.main.X\")``
    ...
    live = sys.modules.get("auraforge_cli.main")
    if live is None:
        live = importlib.import_module("auraforge_cli.main")
...
def test_update_cleanup_spares_backend_owned_by_valid_ssh_lock(tmp_path, monkeypatch):
    ...
    with patch(
        "auraforge_cli.main._find_stale_dashboard_pids",
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m auraforge_cli.main dashboard"),
...
class TestKillStaleDashboardPosix:
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        with patch("subprocess.run", side_effect=fake_run), \
             patch("auraforge_cli.main._find_stale_dashboard_pids", return_value=[12345]) as find_pids, \
...
class TestDashboardUpdateCleanup:
    ...
    def test_all_failed_stops_do_not_claim_the_dashboard_was_stopped(self, capsys):
        with patch(
            "auraforge_cli.main._kill_stale_dashboard_processes",
...
class TestWindowsWmicEncoding:
    ...
    def test_wmic_routed_through_bounded_probe_run_with_ignore_errors(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("auraforge_cli._subprocess_compat.bounded_probe_run") as mock_probe:
            mock_probe.return_value = subprocess.CompletedProcess(
                args=["wmic"],
                returncode=0,
                stdout=(
                    "CommandLine=python -m auraforge_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
    ...
    def test_probe_failure_fails_open_to_empty_list(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("auraforge_cli._subprocess_compat.bounded_probe_run",
                   return_value=None):
            assert _find_stale_dashboard_pids() == []
...
class TestSupervisedBackendRestart:
    ...
    def _live(self):
        return sys.modules["auraforge_cli.main"]
...
class TestManualBackendRespawn:
    ...
    def test_non_orphan_fixed_port_still_respawns(self, capsys):
        ...
        with patch.object(live, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("auraforge_cli.dashboard_procs._hermes_home_for_pid", return_value=None), \
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_skips_serve_port_zero(self):
        from auraforge_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_dedupes_identical_normalized_cmdlines(self):
        ...
        a = ["/usr/bin/python3", "-m", "auraforge_cli.main", "dashboard", "--port", "8300"]
        b = ["/other/python", "-m", "auraforge_cli.main", "dashboard", "--port", "8300"]
    ...
    def test_own_home_defaults_to_get_hermes_home(self, monkeypatch):
        ...
        monkeypatch.setattr(
            auraforge_constants, "get_hermes_home", lambda: Path("/home/u/.auraforge")
        )
        argv = ["auraforge", "serve", "--port", "9118"]
...
class TestCmdlineCapture:
    ...
    def test_reads_proc_cmdline_when_available(self, tmp_path, monkeypatch):
        ...
        assert argv == ["/usr/bin/python3", "-m", "auraforge_cli.main", "serve"]
...
class TestPostUpdateStaleModuleReload:
    ...
    adds a new symbol to ``auraforge_cli._subprocess_compat`` (as #87134 added
    ``bounded_probe_run``), the post-update dashboard cleanup's lazy
    ``from auraforge_cli._subprocess_compat import bounded_probe_run`` hits the
    ...
    def test_reload_restores_missing_symbol(self):
        ...
        import auraforge_cli._subprocess_compat as compat
        from auraforge_cli import update_cmd
        ...
        try:
            ...
            stale = sys.modules["auraforge_cli._subprocess_compat"]
            assert hasattr(stale, "bounded_probe_run")
        finally:
            importlib.reload(sys.modules["auraforge_cli._subprocess_compat"])
            importlib.reload(sys.modules["auraforge_cli.dashboard_procs"])
    ...
    def test_config_reload_list_includes_process_scan_modules(self):
        ...
        assert "auraforge_cli._subprocess_compat" in reloaded
        assert "auraforge_cli.dashboard_procs" in reloaded
