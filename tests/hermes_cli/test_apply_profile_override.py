...
When HERMES_HOME is set to the auraforge root (e.g. systemd hardcodes
HERMES_HOME=/root/.auraforge), _apply_profile_override must still read
...
def _run_apply_profile_override(
    ...
    or None if unset.
    """
    hermes_root = tmp_path / ".auraforge"
...
class TestApplyProfileOverrideHermesHomeGuard:
    ...
    Verifies that HERMES_HOME pointing to the auraforge root does NOT suppress
    ...
    def test_hermes_home_at_root_with_active_profile_is_redirected(
        ...
        """HERMES_HOME=/root/.auraforge + active_profile=coder must redirect
        HERMES_HOME to .../profiles/coder.

        Bug scenario from #22502: systemd sets HERMES_HOME to the auraforge root
        and the user switches to a profile via `auraforge profile use`.
    ...
    def test_sudo_explicit_profile_resolves_invoking_users_profile(self, tmp_path, monkeypatch):
        ...
        user_home = tmp_path / "home" / "auraforge"
        profile_dir = user_home / ".auraforge" / "profiles" / "elias"
        profile_dir.mkdir(parents=True, exist_ok=True)
        (root_home / ".auraforge").mkdir(parents=True, exist_ok=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("SUDO_USER", "auraforge")
        monkeypatch.delenv("HERMES_HOME", raising=False)
        monkeypatch.setattr(os, "geteuid", lambda: 0, raising=False)
        monkeypatch.setattr(sys, "argv", ["auraforge", "-p", "elias", "gateway", "install", "--system"])
        ...
        assert sys.argv == ["auraforge", "gateway", "install", "--system"]
...
class TestSupervisedChildIgnoresStickyProfile:
    ...
    bare ``auraforge gateway run`` (no ``-p``) to mean "the root HERMES_HOME
    ...
    def test_non_supervised_run_still_follows_active_profile(
        ...
        """Without the sentinel, a normal `auraforge gateway run` still honors
