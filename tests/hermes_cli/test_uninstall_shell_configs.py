...
This rewrites files Aura Forge does not own (``~/.bashrc``, ``~/.zshrc``, ...) and
...
def fake_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setenv("HERMES_HOME", str(home / ".auraforge"))
    return home
...
class TestHappyPath:
    def test_hermes_path_block_is_removed(self, fake_home: Path):
        ...
        assert "# Aura Forge Agent" not in text
...
class TestCrashDurability:
    ...
    def test_symlinked_shell_config_stays_a_symlink(self, fake_home: Path):
        ...
        assert "# Aura Forge Agent" not in real.read_text(encoding="utf-8")
