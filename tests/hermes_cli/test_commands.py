...
class TestSlackNativeSlashes:
    ...
    def test_telegram_parity(self):
        ...
        # Commands deliberately routed through /auraforge <command> on Slack only
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `auraforge slack manifest`)."""
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_skips_already_listed(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "auraforge_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
from auraforge_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch auraforge_cli.plugins.get_plugin_commands() to a fixed dict."""
        from auraforge_cli import plugins as _plugins_mod
