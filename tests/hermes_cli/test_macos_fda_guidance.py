...
reports the state and prints the one-switch setup; `auraforge setup` surfaces
...
import auraforge_cli.doctor as doctor_mod
from auraforge_cli.setup import _print_macos_fda_tip
...
class TestSetupFdaTip:
    ...
    def test_tip_printed_when_denied(self, monkeypatch, tmp_path):
        ...
        assert "Full Disk Access" in out
        assert "Privacy_AllFiles" in out
        assert "survives every Aura Forge update" in out
