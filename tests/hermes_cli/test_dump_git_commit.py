"""Tests for auraforge_cli.dump._get_git_commit — git SHA resolution for ``auraforge dump``.

``auraforge dump`` prints the running commit so support bug reports identify the
...
def test_get_git_commit_uses_live_git_when_available(tmp_path):
    ...
    with patch("auraforge_cli.dump.subprocess.run", return_value=git_result) as mock_run, \
         patch("auraforge_cli.build_info.get_build_sha") as mock_build:
...
def test_get_git_commit_output_format_identical_between_sources(tmp_path):
    ...
    so support tooling that parses ``auraforge dump`` doesn't have to special-case
    ...
    with patch("auraforge_cli.dump.subprocess.run", return_value=git_result):
        live = dump._get_git_commit(repo_dir)

    # Baked-SHA path.
    failed = MagicMock(returncode=128, stdout="")
    with patch("auraforge_cli.dump.subprocess.run", return_value=failed), \
         patch("auraforge_cli.build_info.get_build_sha", return_value="b2f477a3"):
