...
dashboard-spawned ``auraforge update`` was mid-``npm install``, which then failed
...
@pytest.fixture
def marker(tmp_path):
    return tmp_path / ".auraforge-update-in-progress"
...
def test_marker_path_follows_process_hermes_home(tmp_path, monkeypatch):
    ...
    assert update_marker_path() == tmp_path / ".auraforge-update-in-progress"
...
class TestHandoffFromOrchestratingUpdater:
    """The Tauri updater holds the marker, then spawns ``auraforge update``.

    The regression: the child saw its own parent's live marker and exited 2,
    so every GUI update failed with "Aura Forge is still running" and retrying
...
class TestAncestryHandoff:
    ...
    ``auraforge-setup`` under ``~/.auraforge`` is only refreshed by a full installer
