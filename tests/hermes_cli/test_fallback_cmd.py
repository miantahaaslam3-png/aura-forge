"""Tests for `auraforge fallback` — chain reading, add/remove/clear, legacy migration."""
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".auraforge" / "config.yaml"
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from auraforge_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from auraforge_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestListCommand:
    def test_list_empty(self, isolated_home, capsys):
        ...
        from auraforge_cli.fallback_cmd import cmd_fallback_list
        cmd_fallback_list(types.SimpleNamespace())
        out = capsys.readouterr().out
        assert "No fallback providers configured" in out
        assert "auraforge fallback add" in out
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("auraforge_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("auraforge_cli.main._require_tty"):
            from auraforge_cli.fallback_cmd import cmd_fallback_add
...
class TestRemoveCommand:
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        with patch("auraforge_cli.setup._curses_prompt_choice", return_value=1):
            from auraforge_cli.fallback_cmd import cmd_fallback_remove
...
class TestClearCommand:
    ...
    def test_clear_with_confirmation(self, isolated_home, capsys, monkeypatch):
        ...
        from auraforge_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    """Verify `auraforge fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "auraforge_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
