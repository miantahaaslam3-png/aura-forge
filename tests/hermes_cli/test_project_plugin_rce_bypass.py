...
1. ``auraforge_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.auraforge/plugins/`` source via
...
2. ``auraforge_cli.web_server._mount_plugin_api_routes`` then imported
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        """Plant a project-source plugin under CWD's ``.auraforge/plugins``
