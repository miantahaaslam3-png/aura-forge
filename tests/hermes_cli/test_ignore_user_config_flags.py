"""Tests for --ignore-user-config and --ignore-rules flags on `auraforge chat`.
...
files. In Aura Forge the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.auraforge/config.yaml`` in ``load_cli_config()``
