...
prefix (e.g. ``mission-control.tilos.com/auraforge/*`` -> local Caddy ->
:9119), injecting ``X-Forwarded-Prefix: /auraforge`` on every request.
...
honour it too:
  ...
  1. The gate's ``Location:`` redirect to /login (in
     ``_unauth_response``) needs to be ``/auraforge/login`` so the browser
  ...
  3. ``_redirect_uri`` (the OAuth callback URL handed to the IDP) must
     ...
     ``/auraforge/auth/callback`` and the user gets 404.
  4. Cookies must use ``Path=/auraforge`` when behind a prefix so they
...
class TestGateRedirectsCarryPrefix:
    def test_html_redirect_to_login_carries_prefix(self, gated_app_proxied):
        ...
        assert r.headers["location"].startswith("/auraforge/auth/login"), (
    ...
    def test_api_401_envelope_login_url_carries_prefix(self, gated_app_proxied):
        ...
        # include the prefix.
        assert body["login_url"].startswith("/auraforge/login"), (
...
class TestOAuthRedirectUriRespectsPrefix:
    def test_redirect_uri_includes_prefix_in_authorize_url(
        ...
        ``https://mission-control.tilos.com/auraforge/auth/callback`` — the
        ...
        assert parsed.path == "/auraforge/auth/callback", (
...
class TestPublicUrlOverride:
    ...
    def test_scheme_less_public_url_env_warns_operator(
        ...
        patch_config(None)
        monkeypatch.setenv("HERMES_DASHBOARD_PUBLIC_URL", "auraforge.domain.com")
...
class TestCookiePathRespectsPrefix:
    ...
    def test_cookies_read_back_round_trip_through_prefix(
        ...
        __Secure- prefix AND Path=/auraforge, so the next request under
        ...
        (``/auraforge/auth/login``, ``/auraforge/auth/callback``). A cookie
        set with ``Path=/auraforge`` would therefore NOT be sent back on
        the second request through TestClient even though it WOULD be
        sent by a real browser hitting ``/auraforge/auth/callback``. To
        ...
        # automatically send a Path=/auraforge cookie to a /auth/callback
        # request path.
        r2 = gated_app_proxied.get(
            f"/auth/callback?code=stub_code&state={state}",
            headers={
                "x-forwarded-prefix": "/auraforge",
                "cookie": pkce_kv,
            },
            follow_redirects=False,
        )
        ...
        assert "Path=/auraforge" in at_cookies[0]
