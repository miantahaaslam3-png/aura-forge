"""Tests for auraforge_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    ...
    import importlib
    from auraforge_cli import model_catalog
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from auraforge_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://auraforge-agent.nousresearch.com/docs/api/model-catalog.json"
    FALLBACK = (
...
class TestDefaultModelFromCache:
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import auraforge_cli.model_catalog as model_catalog
        from auraforge_cli.models import PREFERRED_SILENT_DEFAULT_MODEL
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in auraforge_cli.models."""
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``auraforge model`` CLI — not the live
        ...
        import importlib
        from auraforge_cli import model_catalog
        from auraforge_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("auraforge_cli.models.check_nous_free_tier", return_value=False), patch(
                "auraforge_cli.models.union_with_portal_free_recommendations",
