"""Tests for the post-pull syntax guard in ``auraforge update``.

When a bad commit lands on ``main`` with a syntax error in a critical file
(e.g. orphan merge-conflict markers in ``auraforge_cli/config.py``), the CLI
becomes unbootable — every ``auraforge`` invocation imports those files at
...
markers in ``auraforge_cli/config.py``; users who ran ``auraforge update`` in
the 7-minute window before #28458 landed could not run any ``auraforge``
command afterward.
"""
...
def test_validate_critical_files_syntax_tolerates_missing_files(tmp_path):
    ...
    # Populate everything except auraforge_constants.py
    for relpath in hermes_main._UPDATE_CRITICAL_FILES:
        if relpath == "auraforge_constants.py":
            continue
...
# release; if a future ``auraforge update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
