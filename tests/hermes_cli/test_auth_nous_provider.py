...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `auraforge model` → Nous Portal → Skip (keep current) after
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `auraforge auth add nous`
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Aura Forge must surface an
    ...
    re-authenticate" string led users to report this as a Aura Forge persistence
    ...
    assert "auraforge auth add nous" in message.lower()
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "auraforge-cli"
...
class TestNousDeviceAuthTimeoutMessage:
    def test_timeout_message_mentions_captcha_login_and_retry(self):
        ...
        assert "auraforge portal" in msg
