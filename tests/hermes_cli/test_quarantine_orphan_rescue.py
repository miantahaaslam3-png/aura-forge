"""Regression tests: a failed quarantine restore must never strand `auraforge`.

On Windows the updater renames the live ``auraforge*.exe`` shims aside
(``auraforge.exe.old.<unix-ms>``) so uv can write replacements. Gaps in the
recovery path ended with ``auraforge`` gone from PATH — and, because the command
that repairs it IS ``auraforge update``, unrecoverable without a manual reinstall
(#75584):
...
def test_cleanup_rescue_reports_when_it_cannot_recover(tmp_path, capsys):
    ...
    assert "FAILED to restore auraforge.exe" in captured.err
...
def test_cleanup_rescue_is_quiet_when_another_process_wins(tmp_path, capsys):
    ...
    orphan = scripts / f"auraforge.exe.old.{_stamp()}"
    orphan.write_bytes(b"MZ-orphan")
    original = scripts / "auraforge.exe"
...
def test_cleanup_rescues_newest_by_parsed_stamp_not_lexicographic(tmp_path):
    ...
    """
    scripts = _make_scripts_dir(tmp_path)
    (scripts / "auraforge.exe.old.999").write_bytes(b"MZ-stray-short-stamp")
    (scripts / f"auraforge.exe.old.{_stamp(60_000)}").write_bytes(b"MZ-genuine")

    with _run_cleanup(scripts):
        cli_main._cleanup_quarantined_exes(scripts)

    assert (scripts / "auraforge.exe").read_bytes() == b"MZ-genuine"


def test_cleanup_ignores_names_it_did_not_create(tmp_path):
    """An unparseable suffix is not ours: never rescued, never deleted."""
    scripts = _make_scripts_dir(tmp_path)
    (scripts / "auraforge.exe").write_bytes(b"MZ-live")
    foreign = scripts / "auraforge.exe.old.backup"
    ...
    assert (scripts / "auraforge.exe").read_bytes() == b"MZ-live"
...
def test_cleanup_does_not_rescue_from_a_foreign_name(tmp_path):
    ...
    assert not (scripts / "auraforge.exe").exists()
...
def test_cleanup_leaves_fresh_quarantine_for_concurrent_update(tmp_path):
    ...
    scripts = _make_scripts_dir(tmp_path)
    (scripts / "auraforge.exe").write_bytes(b"MZ-live")
    fresh = scripts / f"auraforge.exe.old.{_stamp()}"
...
def test_cleanup_still_sweeps_genuinely_stale_quarantine(tmp_path):
    ...
    stale = scripts / f"auraforge.exe.old.{_stamp(ancient_ms)}"
...
def test_quarantine_stamp_ms_parses_and_rejects():
    assert cli_main._quarantine_stamp_ms(Path("auraforge.exe.old.1787020473885")) == 1787020473885
    assert cli_main._quarantine_stamp_ms(Path("auraforge.exe.old.backup")) is None
    assert cli_main._quarantine_stamp_ms(Path("auraforge.exe")) is None
...
def test_helper_retries_then_succeeds(tmp_path):
    scripts = _make_scripts_dir(tmp_path)
    quarantined = scripts / "auraforge.exe.old.123"
    quarantined.write_bytes(b"MZ-old-auraforge")
    original = scripts / "auraforge.exe"
    ...
    assert failed == []
    assert original.read_bytes() == b"MZ-old-auraforge"
    assert calls["n"] >= 2
...
def test_helper_reports_failure_and_returns_the_pair(tmp_path, capsys):
    ...
    assert "FAILED to restore auraforge.exe" in captured.err
    assert "auraforge.exe.old.123" in captured.err
