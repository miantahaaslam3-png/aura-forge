"""Tests for the multi-board kanban layer (``auraforge kanban boards …``).
...
* CLI surface: ``auraforge kanban boards list/create/switch/rm``.
...
def fresh_home(tmp_path, monkeypatch):
    ...
    # Also reset auraforge_constants cache so get_default_hermes_root() re-reads.
    try:
...
class TestSlugValidation:
    @pytest.mark.parametrize("good", [
        "default", "atm10-server", "auraforge-agent", "proj_1", "a",
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``auraforge kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
    env["PYTHONPATH"] = str(_WORKTREE)
    if env_extra:
        env.update(env_extra)
    return subprocess.run(
        [sys.executable, "-m", "auraforge_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
