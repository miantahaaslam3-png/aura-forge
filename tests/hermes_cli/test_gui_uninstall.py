...
def _make_gui_build(hermes_home: Path) -> None:
    """Create the source-built GUI artifacts a `auraforge desktop` run produces."""
    desktop = hermes_home / "auraforge-agent" / "apps" / "desktop"
    ...
    (hermes_home / "auraforge-agent" / "node_modules").mkdir(parents=True)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `auraforge desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    hermes_home = tmp_path / ".auraforge"
    _make_agent(hermes_home)
    icon = lde.icon_path(hermes_home / "auraforge-agent")
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".auraforge")
