"""Tests for the per-profile Projects store (auraforge_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Aura Forge Agent", folders=["/tmp/auraforge"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "auraforge-agent"
    assert proj.name == "Aura Forge Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/auraforge"
    assert [f.path for f in proj.folders] == ["/tmp/auraforge"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "auraforge-agent").id == pid
