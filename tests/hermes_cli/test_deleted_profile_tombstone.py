...
from auraforge_constants import named_profile_home
from auraforge_logging import setup_logging
...
def _delete(name: str) -> None:
    with patch("auraforge_cli.profiles._cleanup_gateway_service"), patch(
        "auraforge_cli.profiles._stop_profile_backends"
    ):
...
class TestNamedProfileHome:
    def test_logs_under_named_profile_resolve_to_profile_home(self, tmp_path):
        # tmp_path acts as a real Aura Forge home (Docker/custom layout): it
    ...
    def test_unrelated_profiles_dir_is_not_named(self, tmp_path):
        ...
        # its parent is not a Aura Forge home, so logging must keep mkdir-ing.
