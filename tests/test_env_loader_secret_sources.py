"""Tests for the secret-source tracking in ``auraforge_cli.env_loader``.

These cover the small public surface that lets `auraforge model` / `auraforge setup`
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    ...
    hot modules (cli.py, auraforge_cli/main.py, run_agent.py, ...).  The
    ...
    # cli.py, auraforge_cli/main.py, run_agent.py, trajectory_compressor.py,
