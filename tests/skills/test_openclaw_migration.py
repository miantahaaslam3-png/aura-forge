...
"""
    ...
    assert "Tyler Williams > Active Projects: Aura Forge Agent" in entries
...
def test_readable_config_keeps_every_pre_existing_key(tmp_path: Path):
    ...
    assert merged["model"] == "auraforge-4-405b"
...
def test_symlinked_config_stays_a_symlink(tmp_path: Path):
    """Managed deployments symlink ~/.auraforge/config.yaml into a dotfiles repo.
    ...
    real.write_text("model: auraforge-4-405b\ncommand_allowlist:\n  - /usr/bin/*\n",
                    encoding="utf-8")
    ...
    assert "auraforge-4-405b" in real.read_text(encoding="utf-8")
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # user at ~/.auraforge/config.yaml in its post-migration summary — it never
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Aura Forge``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Aura Forge prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Aura Forge to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Aura Forge config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Aura Forge uses tools well"
    # All-lowercase matches → lowercase ``auraforge``; this preserves the
    # real filesystem path ``~/.auraforge`` (Aura Forge home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "auraforge should always respond concisely"
