...
def test_frontmatter_required_fields():
    ...
    auraforge = fm["metadata"]["auraforge"]
    assert auraforge["tags"]
    assert "related_skills" in auraforge
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Aura Forge Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["auraforge"]["related_skills"]:
