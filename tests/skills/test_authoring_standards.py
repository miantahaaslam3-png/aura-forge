...
def test_required_frontmatter_fields(p):
    ...
    auraforge = (fm.get("metadata") or {}).get("auraforge") or {}
    if not (auraforge.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.auraforge.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    auraforge = (fm.get("metadata") or {}).get("auraforge") or {}
    dangling = [
        rs for rs in (auraforge.get("related_skills") or []) if rs not in _all_names()
    ]
