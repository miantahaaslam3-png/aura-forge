...
- Storage: a job created under profile ``coder`` writes to
  ``~/.auraforge/profiles/coder/cron/jobs.json`` — NOT the shared default root.
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    monkeypatch.setattr(
        auraforge_constants, "_get_platform_default_hermes_home", lambda: root
    )
...
def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    ...
    assert auraforge_constants.get_hermes_home().resolve() == profile_home.resolve()
    assert auraforge_constants.get_default_hermes_root().resolve() == root.resolve()
