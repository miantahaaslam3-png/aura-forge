...
class TestToolsetConsistency:
    ...
    def test_hermes_platforms_share_core_tools(self):
        """All auraforge-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on auraforge-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["auraforge-cli", "auraforge-telegram", "auraforge-discord", "auraforge-whatsapp", "auraforge-slack", "auraforge-signal", "auraforge-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_hermes_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("auraforge-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("auraforge-cli")
        second = resolve_toolset("auraforge-cli")
        ...
        assert (
            "auraforge-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("auraforge-cli", include_registry=False)
        second = resolve_toolset("auraforge-cli", include_registry=False)
        assert first == second
