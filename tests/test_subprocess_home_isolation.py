...
Aura Forge state stays profile-scoped through HERMES_HOME. Host subprocesses should
...
See: https://github.com/miantahaaslam3-png/aura-forge/issues/25114
See: https://github.com/miantahaaslam3-png/aura-forge/issues/36144
See: https://github.com/miantahaaslam3-png/aura-forge/issues/29015
"""
...
class TestGetSubprocessHome:
    """Unit tests for auraforge_constants.get_subprocess_home()."""
    ...
    def _container_mode(self, monkeypatch):
        monkeypatch.setattr(auraforge_constants, "is_container", lambda: True)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("HERMES_REAL_HOME", raising=False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        hermes_home = real_home / ".auraforge" / "profiles" / "coder"
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        from auraforge_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        self._container_mode(monkeypatch)
        base = tmp_path / ".auraforge" / "profiles"
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from auraforge_cli.profiles import _PROFILE_DIRS
