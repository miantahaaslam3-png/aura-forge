...
(SQLITE_PROTOCOL — typical on NFS/SMB), Aura Forge must fall back to
...
def _reset_last_init_error():
    ...
    auraforge_state._set_last_init_error(None)
    yield
    auraforge_state._set_last_init_error(None)
...
class TestApplyWalWithFallback:
    ...
    def test_error_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see auraforge_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("auraforge_state.sqlite3.connect", side_effect=gated_connect):
            with pytest.raises(sqlite3.OperationalError):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_adds_nfs_hint_for_locking_protocol(self):
        ...
        auraforge_state._set_last_init_error("OperationalError: locking protocol")
