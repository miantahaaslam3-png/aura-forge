...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/miantahaaslam3-png/aura-forge/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    hermes_meta = (fm.get("metadata") or {}).get("auraforge") or {}
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `auraforge skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
    ...
    return (
        ...
        "\n"
        ":::info\n"
        "The following is the complete skill definition that Aura Forge loads when this skill is triggered. This is what the agent sees as instructions when the skill is active.\n"
        ":::\n"
        "\n"
        f"{body_clean}\n"
    )
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Aura Forge Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Aura Forge ships with a large built-in skill library copied into `~/.auraforge/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Aura Forge also syncs bundled skills on `auraforge update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.auraforge/skills/` tree, it is still shipped with Aura Forge; restore it with `auraforge skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with auraforge-agent — install via auraforge skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with auraforge-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "auraforge skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "auraforge skills install official/blockchain/solana",
        "auraforge skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "auraforge skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
