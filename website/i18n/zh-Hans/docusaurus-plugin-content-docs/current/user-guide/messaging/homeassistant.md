...
description: 通过 Home Assistant 集成，使用 Aura Forge Agent 控制您的智能家居。
...
Aura Forge Agent 通过以下两种方式与 [Home Assistant](https://www.home-assistant.io/) 集成：
...
3. 滚动至**长期访问令牌**
4. 点击**创建令牌**，命名为"Aura Forge Agent"
5. 复制令牌
...
Aura Forge Agent 注册了四个智能家居控制工具：
...
Agent 发出的消息将以 **Home Assistant 持久通知**的形式推送（通过 `persistent_notification.create`），标题为"Aura Forge Agent"，显示在 HA 通知面板中。
