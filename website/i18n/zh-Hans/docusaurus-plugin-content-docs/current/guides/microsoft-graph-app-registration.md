...
4. 填写以下内容：
   - **Name：**`Aura Forge Teams Meeting Pipeline`（或任何你能识别的名称）。
...
```powershell
# Create a policy scoped to the Aura Forge app
New-CsApplicationAccessPolicy `
  -Identity "Hermes-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Aura Forge meeting pipeline to allow-listed users"
...
Aura Forge 内置了 Graph 身份验证冒烟测试。在 Aura Forge 安装目录下执行：
