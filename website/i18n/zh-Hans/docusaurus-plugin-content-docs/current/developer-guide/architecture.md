...
sidebar_position: 1
title: "架构"
description: "Aura Forge Agent 内部结构——主要子系统、执行路径、数据流及延伸阅读指引"
---

# 架构

本页是 Aura Forge Agent 内部结构的顶层导图。用它在代码库中定位自己，然后深入各子系统专项文档了解实现细节。
...
│ auraforge_state.py   │              │ Browser (5 backends)  │
...
├── auraforge_state.py           # 带 FTS5 的 SQLite 会话/状态数据库
├── auraforge_constants.py       # HERMES_HOME、感知 profile 的路径
...
通过 stdio/JSON-RPC 将 Aura Forge 作为编辑器原生 agent 暴露给 VS Code、Zed 和 JetBrains。
