...
1. `auraforge_cli/auth.py` decides how credentials are found.
2. `auraforge_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `auraforge_cli/models.py` and `auraforge_cli/main.py` make the provider show up in the CLI. (`auraforge_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `auraforge_cli/auth.py`
2. `auraforge_cli/models.py`
3. `auraforge_cli/runtime_provider.py`
4. `auraforge_cli/main.py`
...
:::tip
`auraforge_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `hermes setup`.
:::
...
- `PROVIDER_REGISTRY` in `auraforge_cli/auth.py`
- `_PROVIDER_LABELS` in `auraforge_cli/models.py`
- `_PROVIDER_ALIASES` in both `auraforge_cli/auth.py` and `auraforge_cli/models.py`
- CLI `--provider` choices in `auraforge_cli/main.py`
...
## Step 2: Add auth metadata in `auraforge_cli/auth.py`
...
## Step 3: Add model catalog and aliases in `auraforge_cli/models.py`
...
## Step 4: Resolve runtime data in `auraforge_cli/runtime_provider.py`
...
:::tip
`auraforge_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `hermes model` and `hermes setup` automatically.
:::
...
- `tests/auraforge_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/auraforge_cli/test_model_switch_custom_providers.py` (and adjacent `tests/auraforge_cli/test_model_switch_*.py`)
- `tests/auraforge_cli/test_setup_model_provider.py`
...
python -m pytest tests/auraforge_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/auraforge_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -q
```
...
```bash
source venv/bin/activate
python -m auraforge_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m auraforge_cli.main model
...
- [ ] `ProviderConfig` added in `auraforge_cli/auth.py`
- [ ] aliases added in `auraforge_cli/auth.py` and `auraforge_cli/models.py`
- [ ] model catalog added in `auraforge_cli/models.py`
- [ ] runtime branch added in `auraforge_cli/runtime_provider.py`
- [ ] CLI wiring added in `auraforge_cli/main.py` (setup.py inherits automatically)
