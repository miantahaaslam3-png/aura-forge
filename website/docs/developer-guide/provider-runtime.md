...
- `auraforge_cli/runtime_provider.py` — credential resolution, custom-endpoint runtime resolution
- `auraforge_cli/auth.py` — provider registry, `resolve_provider()`
- `auraforge_cli/model_switch.py` — shared `/model` switch pipeline (CLI + gateway)
...
4. **Config flow**:
   - CLI: reads the fallback chain via `auraforge_cli/fallback_config.get_fallback_chain()` → passes to `AIAgent(fallback_model=...)`
...
- `tests/auraforge_cli/test_fallback_cmd.py` — the `/fallback` CLI command
