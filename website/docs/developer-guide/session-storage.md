...
Source file: `auraforge_state.py`
...
Abridged — see `SCHEMA_SQL` in `auraforge_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `auraforge_state.py` for the exact SQL.
...
This is derived from `auraforge_constants.get_hermes_home()` which resolves to
