...
def _(rid, params: dict) -> dict:
    from auraforge_cli.input_sanitize import sanitize_user_prompt_text
    ...
    # over another app when they are back in Aura Forge.
        ...
        if (
            ...
            if not is_truthy_value(params.get("confirm_truncate")):
                ...
                return _err(
                    ...
                    "(update your Aura Forge client if a rewind was intended)",
                )
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from auraforge_cli.clipboard import has_clipboard_image, save_clipboard_image
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Aura Forge Desktop. If it serves Aura Forge/Desktop UI or another unrelated app, stop that process and report failure.",
...
def _(rid, params: dict) -> dict:
    # `text` is a JSON string describing the OS window underneath the Aura Forge
