...
def _(rid, params: dict) -> dict:
    ...
    ``auraforge_cli.config.reload_env``, matching classic CLI's ``/reload``
    ...
    """
    try:
        from auraforge_cli.config import reload_env
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        try:
            from auraforge_cli.plugins import get_plugin_commands
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m auraforge_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "auraforge_cli.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m auraforge_cli.main` (can drive the agent) →
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from auraforge_cli.moa_config import moa_usage, normalize_moa_config
    ...
    if name == "loop":
        ...
        if not session:
            return _err(rid, 4001, "no active session")
        try:
            from auraforge_cli.loops import LoopManager, dispatch_loop_command
        except Exception as exc:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "browse":
            from auraforge_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from auraforge_cli.skills_hub import inspect_skill
...
# (auraforge_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
...
# All persistence reuses auraforge_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from auraforge_cli.mcp_config import _remove_mcp_server
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native Aura Forge plugin.
