...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `aura --tui` keeps real _stdio.
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `aura --tui`, so it must NOT count as dead here (doing
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    # The desktop app and `aura --tui` both land here. See
        ...
        if missing_skills:
            ...
            if loaded_skills:
                logger.warning(
                    ...
                    "Continuing with: %s. "
                    "List available skills with `aura skills list`.",
                    missing_display,
                    ", ".join(loaded_skills),
                )
            else:
...
def _(rid, params: dict) -> dict:
    ...
    # ``aura --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``aura gateway`` process, whose
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Aura Forge profile home
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Aura Forge Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Aura Forge/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Aura Forge Desktop. If it serves Hermes/Desktop UI or another unrelated app, stop that process and report failure.",
...
def _is_repo_junk(root: str) -> bool:
    ...
    anything under HERMES_HOME (~/.aura by default) — config/sessions/skills,
...
def _discover_repos_payload(db, *, conn=None, backfill: bool = True) -> list[dict]:
    ...
    repos even with zero aura sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (aura home subtree + bare
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Aura Forge provider is configured.",
                },
            )
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `hermes` is interactive — use `/aura chat -q …` or run `hermes` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`aura setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`aura gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`aura sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`aura config edit` needs $EDITOR in a real terminal"
    return None
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``aura journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``aura plugins`` / the dashboard, so the three surfaces
