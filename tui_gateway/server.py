...
# appends every unhandled exception to ~/.auraforge/logs/tui_gateway_crash.log
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    """Prepend Aura Forge' managed bin, the venv bin dir, and the user-local
    bin dir to PATH so slash_worker child processes can resolve
    Aura Forge-managed CLIs (browser-use, uvx, uv) even when the parent
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # Prepend the Aura Forge venv bin dir and the user-local bin dir to PATH so
        # slash_worker child processes can resolve Aura Forge-managed CLIs
...
_SESSION_OWNERSHIP_UNAVAILABLE = (
    "Aura Forge could not safely reserve this session. Try again."
)
        ...
        with _session_resume_lock:
            with _sessions_lock:
                ...
                if current.get("close_on_disconnect"):
                    ...
                    # `auraforge --tui` keeps real _stdio. UNLESS another window
...
# A `auraforge serve` killed mid-update used to lose every un-flushed in-memory
...
def _flush_sessions_before_exit(budget_s: float | None = None) -> int:
    ...
    worker = threading.Thread(target=_run, daemon=True, name="auraforge-exit-flush")
    worker.start()
    worker.join(budget)
...
def install_exit_flush_signal_handlers() -> bool:
    ...
    Called by ``auraforge serve`` / dashboard startup before uvicorn takes over
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `auraforge --tui`, so it must NOT count as dead here (doing
...
def _start_backend_heartbeat_refresher() -> None:
    ...
    thread = threading.Thread(
        ...
        args=(stop_event,),
        name="auraforge-gateway-heartbeat",
        daemon=True,
    )
    thread.start()
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``auraforge dashboard`` was launched) instead of the configured
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `auraforge` shows the prompt before constructing AIAgent; the TUI used
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``auraforge`` / ``auraforge --tui`` / CLI) is started from a
...
_TOUR_BRIDGE_UNAVAILABLE = json.dumps(
    {
        "success": False,
        "error": (
            "No Aura Forge Desktop window answered the tour request. The tour is "
            ...
            "nothing listening. Update the Aura Forge Desktop app and start a new "
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``auraforge config set display.skin``) OR edited the active skin's colors in
...
def _ensure_skin_watcher() -> None:
    ...
    Aura Forge activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
    ...
    threading.Thread(target=_loop, name="auraforge-change-watcher", daemon=True).start()
...
def _config_model_target() -> tuple[str, str]:
    ...
    (`auraforge --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # HERMES_INFERENCE_MODEL, which `auraforge --tui -m <model>` sets as a
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    the same backend told the model it was "chatting inside the Aura Forge desktop
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    # Coding posture (base Aura Forge): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `auraforge --tui` both land here. See
    ...
    try:
        ...
        # recovery above — which keys off auraforge-cli's tool universe — can't
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `auraforge --tui -m`
...
def _agent_cbs(sid: str) -> dict:
    callbacks = {
        ...
        # directly underneath the Aura Forge window, and answers
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Aura Forge has to support both the legacy 8-row petdex atlas and the current
...
def _is_repo_junk(root: str) -> bool:
    ...
    :func:`_non_workspace_dirs`) or anything under HERMES_HOME (~/.auraforge by
...
def _scan_discovered_repos_remote(conn, policy: dict) -> bool:
    ...
    zero Aura Forge sessions still appear in the sidebar (#81723). Mirrors the
        ...
        try:
            for dirpath, dirnames, _filenames in os.walk(root):
                ...
                if ".git" in dirnames:
                    ...
                    # Not a repo: skip hidden dirs (e.g. .auraforge) and node_modules.
...
def _discover_repos_payload(
    ...
    repos even with zero auraforge sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (auraforge home subtree + bare
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero Aura Forge sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with Aura Forge
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `auraforge` is interactive — use `/auraforge chat -q …` or run `auraforge` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`auraforge setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`auraforge gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`auraforge sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`auraforge config edit` needs $EDITOR in a real terminal"
    return None
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Aura Forge" if role == "assistant" else role.title()
