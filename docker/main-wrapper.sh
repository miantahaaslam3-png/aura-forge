...
#   first arg is anything else    → exec `aura <args>` (subcommand passthrough)
#
# Drop to aura via s6-setuidgid, but skip it when already non-root.
set -e

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid aura "$@"; exec "$@"; }
...
# an arbitrary non-root, non-aura UID, the bootstrap was skipped and the
# baked image dirs (owned by the aura build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u hermes)" ]; then
    cat >&2 <<EOF
[hermes] ERROR: container started with --user $cur_uid (an arbitrary, non-aura UID) — not supported.
...
# to the aura user's home before dropping privileges so libraries that
...
# the user's command so `aura chat` starts in the Docker -w
...
# Aura Forge subcommand pass-through.
drop aura "$@"
