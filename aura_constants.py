"""Shared constants for Aura Forge.
...
_UNSET = object()
_AURA_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_AURA_HOME_OVERRIDE", default=_UNSET
)


def set_aura_home_override(path: str | Path | None) -> Token:
    """Set a context-local Aura home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _AURA_HOME_OVERRIDE.set(value)


def reset_aura_home_override(token: Token) -> None:
    """Restore the previous context-local Aura home override."""
    _AURA_HOME_OVERRIDE.reset(token)


def get_aura_home_override() -> str | None:
    """Return the active context-local Aura home override, if any."""
    override = _AURA_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_aura_home() -> Path:
    """Return the platform-native default Aura home path."""
...
def get_aura_home() -> Path:
    """Return the Aura home directory (default: platform-native path).

    Reads AURA_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``AURA_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``AURA_HOME`` explicitly (see the systemd
    template in ``aura_cli/gateway.py`` and the kanban dispatcher in
    ``aura_cli/kanban_db.py``).  See https://github.com/miantahaaslam3-png/aura-forge/issues/18594.
    """
    override = get_aura_home_override()
    if override:
        return Path(override)

    val = os.environ.get("AURA_HOME", "").strip()
    if val:
        return Path(val)
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_aura_home()
        ...
        if active and active != "default":
            ...
            msg = (
                f"[AURA_HOME fallback] AURA_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass AURA_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
    ...
    return _get_platform_default_aura_home()


def get_default_aura_root() -> Path:
    """Return the root Aura directory for profile-level operations.

    In standard deployments this is the platform-native Aura home
    (``~/.aura-forge`` on POSIX, ``%LOCALAPPDATA%\\aura-forge`` on native Windows).

    In Docker or custom deployments where ``AURA_HOME`` points outside
    ``~/.aura-forge`` (e.g. ``/opt/data``), returns ``AURA_HOME`` directly
    — that IS the root.

    In profile mode where ``AURA_HOME`` is ``<root>/profiles/<name>``,
    ...
    """
    native_home = _get_platform_default_aura_home()
    ...
    try:
        ...
        # AURA_HOME is under ~/.aura-forge (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — AURA_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Aura is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``AURA_OPTIONAL_SKILLS``.
    """
    override = os.getenv("AURA_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_aura_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``AURA_OPTIONAL_MCPS``.
    """
    override = os.getenv("AURA_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_aura_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``AURA_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<AURA_HOME>/skills`` last-resort
    """
    override = os.getenv("AURA_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_aura_home() / "skills"


def get_aura_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve an Aura subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to AURA_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to AURA_HOME (e.g. ``"image_cache"``).
...
get_hermes_dir = get_aura_dir

def iter_aura_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Aura-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\aura-forge\\node``. POSIX installs use
    ``$AURA_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_aura_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with auraManagedNodePathEntries() in
...
_AURA_NODE_TARGET_MAJOR = int(os.environ.get("AURA_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Aura-managed Node trees live under ``$AURA_HOME/node`` (or a profile's
    ``AURA_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_aura_node_executable``
    used to trust file presence alone, so ``aura update`` would pick that
    ...
    try:
        from aura_cli._subprocess_compat import windows_hide_flags

        result = subprocess.run(
            [path, "--version"],
            capture_output=True,
            timeout=10,
            env=with_aura_node_path(),
...
def aura_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Aura-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_aura_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%AURA_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    home = get_aura_home()
    index_url = f"https://nodejs.org/dist/latest-v{_AURA_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_AURA_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def heal_aura_managed_node() -> bool:
    """Redownload Aura-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not aura_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "AURA_HOME": str(get_aura_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_aura_node_executable(command: str) -> str | None:
    """Return a Aura-managed Node/npm executable path, healing broken trees."""
    names = _candidate_node_command_names(command)
    broken_present = False
    for directory in iter_aura_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_aura_managed_node():
        for directory in iter_aura_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Aura-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Aura-managed installs.

    This is for Aura-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_aura_node_executable(command)
    if managed:
        return managed
    if aura_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_aura_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Aura-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_aura_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``aura update`` — leaving a **dangling symlink** that ``which`` still
...
def display_aura_home() -> str:
    """Return a user-friendly display string for the current AURA_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.aura-forge``
        profile:  ``~/.hermes/profiles/coder``
        custom:   ``/opt/aura-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.aura-forge``.  For code that needs a real ``Path``, use
    :func:`get_aura_home` instead.
    """
    home = get_aura_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``AURA_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/miantahaaslam3-png/aura-forge/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{AURA_HOME}/home`` when the profile-home directory exists."""
    aura_home = get_aura_home_override() or (env or {}).get("AURA_HOME") or os.getenv("AURA_HOME")
    if not aura_home:
        return None
    profile_home = os.path.join(aura_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("AURA_REAL_HOME") or os.getenv("AURA_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Aura profile HOME.

    ``AURA_HOME`` scopes Aura state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={AURA_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{AURA_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{AURA_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Aura's subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["AURA_REAL_HOME"] = real_home
...
def is_container() -> bool:
    ...
    See: miantahaaslam3-png/aura-forge#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under AURA_HOME.

    Replaces the ``get_aura_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, aura_logging.py, aura_time.py, etc.).
    """
    return get_aura_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under AURA_HOME."""
    return get_aura_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under AURA_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_aura_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._aura_ipv4_patched = True  # type: ignore[attr-defined]
