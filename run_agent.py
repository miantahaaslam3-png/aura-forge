...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``auraforge -c`` / ``--resume`` (relaunch
...
from auraforge_cli.timeouts import (
...
_loaded_env_paths = load_hermes_dotenv(aura_forge_home=_hermes_home, project_env=_project_env)
...
# (auraforge_state._rows_to_conversation), so any code that mutates a loaded or
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Aura Forge-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.auraforge/sessions/``.  Without
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[auraforge-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # `auraforge --resume`. set_session_yolo() no-ops on a missing row, so
    ...
    def _ensure_lmstudio_runtime_loaded(
        ...
        from auraforge_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``auraforge chat -q``. In that mode,
    ...
    def _should_emit_quiet_tool_messages(self) -> bool:
        ...
        ``auraforge chat -Q``) always wins: those flows neutralize the rendering
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        auraforge-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See auraforge-agent#21444 for symptom history."
        )
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Aura Forge against xai-oauth.  Lots of
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        while current is not None and id(current) not in seen:
            ...
            if any(
                ...
                return (
                    "Aura Forge can't reach the model provider. You may be offline. "
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        try:
            from auraforge_cli import lifecycle as _lifecycle
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.auraforge/sessions/session_{sid}.json``
        ...
        # untrusted input (X-Aura Forge-Session-Id header) and must not escape
    ...
    def interrupt(
        ...
        *,
        ...
        # interrupt event rather than Aura Forge' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal Aura Forge model request this cancels only that request;
    ...
    def _record_file_mutation_result(
        ...
        if landed:
            ...
            # can tell Aura Forge-authored content from later user hand-edits.
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.auraforge/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _format_turn_completion_explanation(
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "turn_lease":
                return (
                    prefix
                    + "the turn was stopped because another Aura Forge process "
            ...
            if cause == "locked":
                return (
                    ...
                    "(another Aura Forge process was writing to the state "
            ...
            if cause == "corrupt":
                return (
                    ...
                    "1. Run `auraforge doctor --fix`\n"
                    "2. Salvage with: sqlite3 ~/.auraforge/state.db \".recover\" "
                    "(then replace state.db)\n"
                    "3. Restore from a backup in ~/.auraforge/backups/\n"
            ...
            return (
                ...
                "Check the state database health (`auraforge doctor`), then "
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        if _dev:
            ...
            # `auraforge logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when Aura Forge YOLO selected unrestricted
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``auraforge auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ~/.auraforge/.env credential/base-url edits at the turn boundary.

        A Settings save (desktop ``PUT /api/env``, ``auraforge setup``) updates
        ...
        if (
            ...
            if self.provider == "kimi-coding":
                from auraforge_cli.auth import _resolve_kimi_base_url

                base_url = _resolve_kimi_base_url(
                    api_key, pconfig.inference_base_url, env_url
                ).rstrip("/")
            elif self.provider == "zai":
                from auraforge_cli.auth import _resolve_zai_base_url
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from auraforge_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from auraforge_cli.models import lmstudio_model_reasoning_options
    ...
    def _ollama_supports_thinking_cached(self) -> bool:
        ...
        try:
            from auraforge_cli.models import ollama_model_supports_thinking
    ...
    def _compress_context(
        ...
        *,
        ...
        try:
            ...
            # id rotation updated auraforge_logging._session_context (a
    ...
    def run_conversation(
        ...
        from auraforge_cli.observability.relay_shared_metrics import (
            finish_task_run,
            start_task_run,
        )
        ...
        try:
            # Serialize the full load -> run -> flush region across Aura Forge
            ...
            if (
                ...
                def _on_session_turn_lease_wait(elapsed: float) -> None:
                    ...
                    if elapsed < 1.0:
                        self._emit_status(
                            "⏳ Another Aura Forge process is using this session; "
                            "waiting for it to finish before starting your turn..."
                        )
                    else:
                        self._emit_status(
                            "⏳ Still waiting for the other Aura Forge process on "
                ...
                if not _turn_db.acquire_session_turn_lease(
                    ...
                    if getattr(self, "_interrupt_requested", False):
                        ...
                        interrupt_msg = (
                            "Stopped waiting for another Aura Forge process on "
                    ...
                    timeout_msg = (
                        "⏳ Another Aura Forge process kept this session busy too "
