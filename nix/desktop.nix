# nix/desktop.nix — Aura Forge Desktop (Electron) app build + wrapper
#
...
# uses our fully wrapped binary at step 4 ("existing Aura Forge CLI").
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    # binary so the desktop's resolver step 4 ("existing Aura Forge CLI on
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Aura Forge Agent";
