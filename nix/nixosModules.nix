...
#
# Container mode: aura runs from /nix/store bind-mounted read-only into a
...
# and read by aura at startup — no container recreation needed for env changes.
#
# Tool resolution: the aura wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    ...
    deepConfigType = lib.types.mkOptionType {
      ...
      description = "Aura Forge YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    # aura user + sudo on first boot (writable layer persists), then
    ...
    containerEntrypoint = pkgs.writeShellScript "hermes-container-entrypoint" ''
      ...
      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/aura ]; then
  ...
  in {
    options.services.hermes-agent = with lib; {
      enable = mkEnableOption "Aura Forge Agent gateway service";
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Aura Forge config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Aura Forge reads this file on every startup via load_hermes_dotenv().
        '';
      };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `aura gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the aura user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the aura plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Aura Forge discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          with the same Python interpreter as aura (python312).
        '';
        example = literalExpression ''
          [
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the aura CLI to environment.systemPackages and export
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.aura symlink to the service
            stateDir. These users are automatically added to the aura group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the aura CLI to system PATH and export HERMES_HOME system-wide
      ...
      # Wire extraPackages into the aura user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            ...
            Set addToSystemPackages = true or ensure aura is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.aura        2770 ${cfg.user} ${cfg.group} - -"
      ...
      # ── Activation: link config + auth + documents ────────────────────
      {
        system.activationScripts."hermes-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          ...
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.aura ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.aura ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.aura -maxdepth 1 \
    ...
    HERMES_CONTAINER_MODE_EOF
          ...
          # Create ~/.aura -> stateDir/.aura for each hostUser so the
          ...
          # Aura Forge reads $HERMES_HOME/.env at startup via load_hermes_dotenv(),
    ...
    HERMES_NIX_ENV_EOF
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.hermes-agent = {
          description = "Aura Forge Agent Gateway";
          ...
          serviceConfig = {
            ...
            # so interactive users in the aura group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.hermes-agent = {
          description = "Aura Forge Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ${containerBin} create \
                ...
                --env HERMES_HOME=${containerDataDir}/.aura \
                --env HERMES_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/aura gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
