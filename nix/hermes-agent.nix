# nix/hermes-agent.nix — Overridable Aura Forge Agent package
#
        ...
        for di in sp.glob('*.dist-info'):
            ...
            for line in meta.read_text().splitlines():
                if line.startswith('Name:'):
                    ...
                    if pkg in core:
                        print(f'ERROR: plugin package \"{pkg}\" collides with a package in aura sealed venv', file=sys.stderr)
...
stdenv.mkDerivation (finalAttrs: {
  ...
  passthru = {
    ...
    # ("existing aura on PATH") then picks up the fully wrapped
