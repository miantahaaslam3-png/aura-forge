# Aura Forge Agent - Development Guide
...
Aura Forge is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Aura Forge ships a **lot** — most merges are bug fixes to
...
one behind a plain URL + token, or Aura Forge Cloud. Only the first two are spawned
...
├── auraforge_state.py       # SessionDB — SQLite session store (FTS5 search)
├── auraforge_constants.py   # get_hermes_home(), display_hermes_home() — profile-aware paths
├── auraforge_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── auraforge_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
Applies to TypeScript across Aura Forge: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`auraforge_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `HermesCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.hermes/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`auraforge_cli/commands.py`)
...
1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `auraforge_cli/commands.py`:
```python
...
The dashboard embeds the real `hermes --tui` — **not** a rewrite.  See `auraforge_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `auraforge_cli/web_server.py`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Aura Forge built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Aura Forge core.
...
core Aura Forge tool that should ship in the base system.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `auraforge_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `auraforge_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `hermes tools`, `hermes setup`, most CLI subcommands | `auraforge_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`auraforge_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
auraforge_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
...
- `default` — Classic Aura Forge gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `auraforge_cli/skin_engine.py`:
...
Aura Forge has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.hermes/plugins/` and pip-installed entry points.

### General plugins (`auraforge_cli/plugins.py` + `plugins/<name>/`)
...
(`run_agent.py`, `cli.py`, `gateway/run.py`, `auraforge_cli/main.py`, etc.).
...
The desktop "Bots" experience ships bundled in-tree. Each bot is a Aura Forge
...
2. **Tools referenced in SKILL.md prose must be native Aura Forge tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Aura Forge Agent" is the secondary collaborator. If the
   contributor's commit shows "Aura Forge Agent" as author (because they
   used Aura Forge to draft the skill), replace it with their actual name
...
Aura Forge returns a delegation id immediately and the result re-enters the
...
- **CLI:** `auraforge_cli/curator.py` wires `hermes curator <verb>` where
...
- **CLI:** `auraforge_cli/kanban.py` wires `hermes kanban` with verbs
...
- **Plan** (`auraforge_cli/update_inventory.py`, `hermes update --plan`):
...
- **Snapshot** (`auraforge_cli/backup.py`): pre-update quick snapshot for
...
Aura Forge supports **profiles** — multiple fully isolated instances, each with its own
`HERMES_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `auraforge_cli/main.py` sets
`HERMES_HOME` before any module imports. All `get_hermes_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_hermes_home()` for all HERMES_HOME paths.** Import from `auraforge_constants`.
...
2. **Use `display_hermes_home()` for user-facing messages.** Import from `auraforge_constants`.
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line`
  (gateway run), `auraforge_cli.update_cmd._hermes_holder_subcommand`
  (top-level subcommand of any Aura Forge argv). Never hand-roll token scans.
...
Use `get_hermes_home()` from `auraforge_constants` for code paths. Use `display_hermes_home()`
for user-facing print/log messages. Hardcoding `~/.hermes` breaks profiles — each profile
has its own `HERMES_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### All CLI menu-pickers MUST use curses.
Interactive menus must use `auraforge_cli/curses_ui.py`. See `auraforge_cli/tools_config.py` for an example.
...
Use the pattern from `tests/auraforge_cli/test_profiles.py`:
```python
@pytest.fixture
...
Aura Forge supports Linux, macOS and native Windows, and plenty of its behaviour
...
`tests/auraforge_cli/test_venv_holder_windows_live.py` on a real
