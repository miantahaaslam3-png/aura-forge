"""Shared constants for Aura Forge Agent.
...
def set_hermes_home_override(path: str | Path | None) -> Token:
    """Set a context-local Aura Forge home override and return its reset token.
...
def reset_hermes_home_override(token: Token) -> None:
    """Restore the previous context-local Aura Forge home override."""
    _HERMES_HOME_OVERRIDE.reset(token)


def get_hermes_home_override() -> str | None:
    """Return the active context-local Aura Forge home override, if any."""
...
def _get_platform_default_hermes_home() -> Path:
    """Return the platform-native default Aura Forge home path."""
...
def _hermes_home_from_env() -> Path:
    ...
    """
    # Aura Forge: prefer AURA_FORGE_HOME (set by installer), then HERMES_HOME
    val = os.environ.get("AURA_FORGE_HOME", "").strip()
    if not val:
...
def get_hermes_home() -> Path:
    """Return the Aura Forge home directory (default: platform-native path).
    ...
    template in ``auraforge_cli/gateway.py`` and the kanban dispatcher in
    ``auraforge_cli/kanban_db.py``).  See https://github.com/miantahaaslam3-png/aura-forge/issues/18594.
    """
...
def hermes_home_key(path: str | Path | None = None) -> str:
    """Return a stable key for a Aura Forge home/profile directory.
...
def get_process_hermes_home() -> Path:
    """Return the Aura Forge home for the running process, ignoring task overrides.
...
# Files whose presence marks a directory as a real Aura Forge home. A fresh home
...
def get_hermes_dir(
    ...
    *,
    ...
    """Resolve a Aura Forge subdirectory with backward compatibility.
    ...
    Args:
        ...
        home: Optional explicit Aura Forge home. Profile-aware callers that manage
...
# Install tree root (this file lives at <install_root>/auraforge_constants.py).
...
def bootstrap_hermes_managed_node() -> str | None:
    ...
    Aura Forge never modifies a toolchain it does not own, so instead it provisions
...
def secure_parent_dir(path: Path) -> None:
    ...
    See https://github.com/miantahaaslam3-png/aura-forge/issues/25821 and
    https://github.com/miantahaaslam3-png/aura-forge/pull/93050.
    """
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Aura Forge profile HOME.

    ``HERMES_HOME`` scopes Aura Forge state. ``HOME`` is reserved for the OS/user
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Aura Forge' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Aura Forge itself runs inside WSL.
...
def get_config_path() -> Path:
    ...
    in 7+ files (skill_utils.py, auraforge_logging.py, auraforge_time.py, etc.).
    """
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``auraforge_cli`` modules using three different Windows
    ...
    A few sites outside ``auraforge_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``auraforge_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of Aura Forge itself. An ImportError
...
# `auraforge_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Aura Forge.
...
# Aura Forge backward-compatible aliases — re-exported by auraforge_cli.config
