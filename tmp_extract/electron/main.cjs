...
function resolveHermesHome() {
  // Aura Forge: prefer AURA_FORGE_HOME (set by the installer), then fall back
  // to HERMES_HOME for backward-compat with the original Hermes install.
  ...
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'aura-forge-home')
  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so an AURA_FORGE_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Consult the live
    // User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('AURA_FORGE_HOME') || readWindowsUserEnvVar('HERMES_HOME')
    if (fromRegistry) return normalizeHermesHomeRoot(fromRegistry)
  }
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'aura-forge')
  }
  // POSIX / fallback: ~/.aura-forge
  return path.join(app.getPath('home'), '.aura-forge')
}
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_HERMES_ROOT, '.aura-forge-bootstrap-complete')
