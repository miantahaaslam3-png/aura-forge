...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Aura Forge convention — don't recursively snapshot siblings)
    ".worktrees/",
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file`` / ``patch`` produced, so restores can tell "Aura Forge wrote
...
def _git_env(
    ...
    The shared store is internal Aura Forge infrastructure — it must NOT inherit
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file Aura Forge just successfully wrote.
        ...
        recorded hash was hand-edited by the user after Aura Forge last touched
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        ...
        still matches what Aura Forge last wrote (per the agent-write ledger)
        and ``skipped`` lists files the user hand-edited after Aura Forge'
        last write or that Aura Forge never wrote at all.
        """
        ...
        ledger = _load_ledger(store, dir_hash)
        if not ledger:
            # No agent-write ledger yet (pre-existing store, or Aura Forge has
        ...
        for rel in names_out.splitlines():
            ...
            if recorded is None:
                # Aura Forge never wrote this file (or the ledger predates it) —
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has Aura Forge changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        Aura Forge changed", not an exact per-session ledger.
        ...
        if restore_paths is not None:
            ...
            for rel in restore_paths:
                ...
                if ok_in_commit:
                    ...
                    # Aura Forge created it. Deleting it would not restore a prior
