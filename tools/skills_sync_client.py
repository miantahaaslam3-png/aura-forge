...
is toggled via ``auraforge sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.auraforge/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``auraforge sync enable`` — it is
...
# namespace from auraforge-agent's local ``content_hash`` (skills_guard.py:846),
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a Aura Forge Cloud instance can be set
...
def sync_feature_enabled() -> bool:
    ...
    switch a Aura Forge Cloud deployment sets to opt its instances into sync by
...
def sync_org_auto_propose() -> bool:
    ...
    runs ``auraforge sync propose <skill>``. The skill keeps working with the
...
def sync_default_opt_in() -> bool:
    ...
    ``auraforge sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    Aura Forge Cloud deployment wants. Per the design notes, this default is
...
#
# Only agent-created + user-authored skills under ~/.auraforge/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.auraforge/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (Aura Forge Cloud "on by default"):** every *eligible* skill syncs
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.auraforge/skills/ containing
...
def stable_device_id() -> str:
    ...
    ~/.auraforge/skills/.sync_device_id.
    ...
    Use ``set_device_name()`` / ``auraforge sync device --name`` to set an explicit
    label."""
    ...
    # Aura Forge Cloud (and any templated deployment) can seed the label
    ...
    # is then persisted, so a later `auraforge sync device --name` (or editing the
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.auraforge/skills/.sync_device_id, overwriting
...
# ~/.auraforge/skills/.sync_state as JSON.
#
...
# path under ~/.auraforge/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.auraforge/skills/ (posix), or None."""
    try:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this Aura Forge speaks "
            f"{WIRE_VERSION} — update Aura Forge to sync with it"
        )
...
def push_skills(
    ...
    *,
    ...
    message: str = "auraforge skill sync",
) -> Dict[str, Any]:
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (auraforge sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.auraforge/skills/. Only paths the user has opted into (``sync: true``) are
...
def maybe_push_skills(*, message: str = "auraforge skill sync") -> Optional[Dict[str, Any]]:
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``auraforge sync status``. Never raises."""
    status: Dict[str, Any] = {
        "nous_admin": False,
        "logged_in": False,
...
#
# Org skills live under a DISTINCT local namespace, ~/.auraforge/skills/_org/
...
#
# `auraforge sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.auraforge/skills/_org/<org_id>/``.
