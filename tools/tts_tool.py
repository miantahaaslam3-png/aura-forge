...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.auraforge/config.yaml``. Aura Forge
...
Configuration is loaded from ~/.auraforge/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``auraforge_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from auraforge_cli.config import get_env_value as _get_env_value
    except ImportError:
...
# Base URL now resolved via auraforge_cli.models.deepinfra_base_url (shared).
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.auraforge/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.auraforge/config.yaml.
    ...
    try:
        ...
        logger.debug("auraforge_cli.config not available, using default TTS config")
        return {}
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``auraforge tools``); otherwise the historical Edge backend remains active.
...
# custom voice-cloning scripts, etc.) into Aura Forge without any Python code
...
#           output_format: wav
#
# Aura Forge writes the input text to a temp UTF-8 file, runs the command with
...
def _command_provider_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of Aura Forge secrets by
...
def _run_command_tts(
    ...
    Child env is scrubbed of Aura Forge secrets (salvage of #56332) while still
...
# file, so retired models disappear from auraforge the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``auraforge_cli.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = _resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `auraforge setup` to configure, "
    ...
    from auraforge_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `auraforge model` or set XAI_API_KEY.")
    ...
    # output_format when Aura Forge actually needs a non-default format/override.
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            import auraforge_cli as _hermes_cli
        ...
        headers["X-Goog-Api-Client"] = f"auraforge-agent/{_hermes_version}"
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Aura Forge caches Piper voice models.

    Resolves to ``~/.auraforge/cache/piper-voices/`` under the active
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `auraforge setup` to install Mistral support."
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run auraforge setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'auraforge setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'auraforge tools' and select Piper under TTS, or install manually: "
