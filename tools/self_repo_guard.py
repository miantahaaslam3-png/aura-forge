...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite Aura Forge's live source checkout "
        ...
        "checkout, stop Aura Forge, run the command externally, then restart "
        "Aura Forge."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(hermes_home).expanduser() if hermes_home else Path.home() / ".auraforge"
