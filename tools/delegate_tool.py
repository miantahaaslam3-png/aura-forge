...
# Must match auraforge_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``auraforge-cli`` (which bundles
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``auraforge-cli`` because those also contain
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. auraforge-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (auraforge-cli,
    # auraforge-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # child Aura Forge/Qwen subagent onto the parent's Claude Messages wire (or the
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.auraforge/logs/subagent-<sid>-<ts>.log``
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from auraforge_cli.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    # request-scoped chat_id binding (the raw X-Aura Forge-Session-Id on
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-Aura Forge-Session-Id once the batch completes.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `auraforge -z`, a cron job, a Kanban "
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'auraforge auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Aura Forge config.
    ...
    ``HERMES_IGNORE_USER_CONFIG=1`` (``auraforge chat --ignore-user-config``) is
