...
# means a compromised transitive dependency could read every Aura Forge secret
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active Aura Forge profile."""
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Aura Forge silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Aura Forge CLI where sys.stdout and sys.stderr both report
...
def _real_profile_cdp() -> tuple:
    ...
    with _real_profile_cdp_lock:
        ...
        # the same launch path Aura Forge' built-in local browsing already uses,
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Aura Forge
