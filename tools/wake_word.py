"""Wake-word ("Hey Aura Forge") detection — hands-free session trigger.
...
wake word, Aura Forge opens a fresh session and captures voice via the existing
...
* **openwakeword** (default, free, no API key) — loads an ONNX model. Defaults
  to the bundled "hey auraforge" model (``tools/wakewords/``) so the wake word
...
# Minimum gap between two consecutive wake fires, so one "hey auraforge" can't
...
# Bundled "hey auraforge" model (tools/wakewords/) — the default, so the wake word
# works out of the box. Config names in _ALIASES resolve to it, not a built-in.
_BUNDLED_MODEL_NAME = "hey_hermes"
_BUNDLED_MODEL_ALIASES = frozenset({"", "hey_hermes", "hey auraforge", "auraforge"})
...
def wake_phrase(cfg: Optional[Dict[str, Any]] = None) -> str:
    ...
    return str(_get(cfg, "phrase")) or "hey auraforge"
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return (
            "Microphone delivers only silence. Grant the Aura Forge backend "
...
class _SherpaKwsEngine(_Engine):
    ...
    model's vocabulary, so "hey auraforge", "hey coder", or any other phrase works
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # wake any profile ("hey auraforge" / "hey coder" / ...). display-name →
        # profile is kept for routing the match back.
        phrase = str(_get(cfg, "phrase") or "hey auraforge").strip()
        ...
        kw = tempfile.NamedTemporaryFile(
            mode="w", suffix=".txt", prefix="auraforge-kws-", delete=False, encoding="utf-8"
        )
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if provider == "porcupine" and not (os.getenv("PORCUPINE_ACCESS_KEY") or "").strip():
        ...
        hint = (f"Wake word needs {missing} configured — run `auraforge tools` "
