...
def _pinned_guard(name: str) -> Optional[str]:
    ...
    try:
        ...
        if name in ESSENTIAL_SKILLS:
            return (
                f"Skill '{name}' is essential to Aura Forge (the agent's own "
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Aura Forge profiles.
...
def _attach_lint_findings(result: Dict[str, Any], skill_md: Path) -> None:
    ...
    result["lint_hint"] = (
        ...
        "to match Aura Forge skill standards."
    )
...
def _patch_skill(
    ...
    if not old_string:
        ...
        # how to recover. Upstream: miantahaaslam3-png/aura-forge#33064.
        return {
            "success": False,
            "error": (
