...
# playback-phase barge transcript is treated as a self-capture of Aura Forge'
...
def is_tts_echo(
    ...
    Aura Forge just spoke using a character-level similarity ratio, which works
    across languages without word-tokenization. A genuine user interjection
    is very unlikely to closely match Aura Forge' own words, so a high ratio is
...
def _play_audio_file_impl(file_path: str) -> bool:
    ...
    # A unique suffix prevents concurrent Aura Forge TTS calls from colliding on
