...
teammate agent (another Aura Forge profile on this install, or an agent on a
...
DM each other via a prompt-injected ``auraforge -p <bot> chat ...`` shellout.
...
- Everything here is additive. The legacy protocol transports
  (``auraforge -p`` / ``auraforge peer dm``) keep working for older prompts.

The transports themselves are unchanged and proven:
- local teammate  → ``auraforge -p <name> chat --in ~ -c "Bot Chat"
  --create-if-missing -Q --query-file <tmp>`` (one turn, reply on stdout)
- peer teammate   → ``auraforge peer dm <peer>[/<name>] < <tmp>``
                ...
                "properties": {
                    "target": {
                        ...
                        "description": (
                            ...
                            "('researcher', 'auraforge' for the default agent), or "
...
def _resolve_local_name(target: str, roster: list[str]) -> Optional[str]:
    """Map a target handle to a profile name ('auraforge' → 'default')."""
    want = target.strip()
    if not want:
        return None
    if want.lower() == "auraforge":
...
def cleanup_bot_dm_cache(
    ...
    locations: list[tuple[Path, str]] = [
        (temp_root, "auraforge-dm-*.txt"),
        (temp_root, "auraforge-relay-dm-*.txt"),
    ]
    try:
...
def _delivery_lock(argv: list[str], *, stdin_file: bool):
    ...
    Local deliveries (``auraforge -p <profile> chat …``) collide with relay
    ...
    # resolves the venv-relative auraforge next to this gateway's interpreter
    ...
    if (
        stdin_file
        or len(argv) < 3
        or cli not in ("auraforge", "auraforge.exe")
        or argv[1] != "-p"
    ):
...
def _agent_home(agent: Any) -> str:
    ...
    try:
        ...
        pass
    return os.getenv("HERMES_HOME") or os.path.expanduser("~/.auraforge")
