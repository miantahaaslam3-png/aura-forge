#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Aura Forge Skills Hub.
...
Used by auraforge_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class OptionalSkillSource(SkillSource):
    ...
    OFFICIAL_REPO = "miantahaaslam3-png/aura-forge"
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``auraforge_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
def _load_hermes_index() -> Optional[dict]:
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Aura Forge index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Aura Forge index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Aura Forge index fetch failed: %s", e)
            return _load_stale_index_cache()
...
class HermesIndexSource(SkillSource):
    """Skill source backed by the centralized Aura Forge Skills Index.
