...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Aura Forge exit
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    for failure in sudo_failures:
        if failure in output:
            from auraforge_constants import display_hermes_home as _dhh
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    # interactive Aura Forge password prompt or the sudo -S password-pipe path.
    ...
    # silently without Aura Forge prompting.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Aura Forge processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Aura Forge processes a generous grace
...
def _docker_persistent_profile_scoped() -> bool:
    ...
    ``container_persistent: true`` is ONE long-lived container per Aura Forge
...
def _current_session_profile() -> str:
    """Return the active session's Aura Forge profile name, or "" when unset.
...
def _ensure_terminal_env_bridged() -> None:
    ...
    try:
        from auraforge_cli.config import apply_terminal_config_to_env, read_raw_config
...
def _get_env_config() -> Dict[str, Any]:
    ...
    from auraforge_cli.config import _is_ssh_remote_tilde_cwd
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Aura Forge processes that hit SIGKILL / OOM / a closed terminal
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "notify_on_complete=true) so Aura Forge tracks the process, then run readiness "
...
def terminal_tool(
    ...
    try:
        ...
        # auraforge_cli/gateway.py and the cron-path guard in auraforge_cli/cron.py,
            ...
            if contains_launchctl_submit_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Use Aura Forge cron for one-shot delayed work, or install an "
