      ...
      remote_api:
        ...
        identity_header:       # optional per-user identity header attached
          ...
          value: "alice"       # required for static; profile mode uses the
                               # active Aura Forge profile name
        timeout: 180
...
def sdk_httpx():
    ...
    side by side with Aura Forge' own pinned ``httpx``. Every object that crosses
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Aura Forge' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Aura Forge explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Aura Forge and
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            candidates = [
                ...
                # image (which the Aura Forge Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Aura Forge MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Aura Forge gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Aura Forge document cache; returns a marker with
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`auraforge_cli.mcp_config._unwrap_exception_group` with
...
def _resolve_identity_header(server_name: str, config: dict):
    ...
    mode resolves the value to the active Aura Forge profile name once at
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Aura Forge' existing approval
...
class MCPServerTask:
    ...
    def _make_logging_callback(self):
        ...
        server into Aura Forge' logging (agent.log via auraforge_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        # ungraceful exit of this Aura Forge process (kill -9, crash, force-quit)
        ...
        try:
            async with stdio_client(server_params, errlog=_errlog) as (
                ...
                if new_pids:
                    ...
                    for _pid in new_pids:
                        try:
                            from auraforge_cli.process_identity import register_child
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Aura Forge' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Aura Forge is fully
...
def _http_status_error_types() -> tuple:
    ...
    mcp >= 2.0) or by Aura Forge' pinned ``httpx``, and the two define unrelated
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Aura Forge processes (e.g.
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Aura Forge config file.
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Aura Forge' MEDIA: tag convention and out to messaging
            ...
            # Aura Forge' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Aura Forge's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Aura Forge uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Aura Forge registry schema format.
