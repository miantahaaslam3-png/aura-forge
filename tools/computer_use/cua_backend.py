...
def _action_result_from(
    ...
    *,
    ...
    cua-driver's mcp-tool-notes and miantahaaslam3-png/aura-forge#67052.
    """
...
def _cua_no_overlay() -> bool:
    """True when Aura Forge should pass ``--no-overlay`` to cua-driver.
...
def _cua_telemetry_disabled() -> bool:
    """True when Aura Forge should disable cua-driver telemetry for this user.
...
def _cua_configured_permission_mode() -> str:
    ...
    per-session Aura Forge YOLO toggle so a stale config line can never
...
def _cua_grant_existing_profile() -> bool:
    ...
    cua-driver's trusted-launcher grant. Aura Forge passes
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command when Aura Forge runs in WSL.

    Windows cua-driver manifests can report ``C:\\Users\\...\\cua-driver.exe``
    even though the Aura Forge process uses POSIX subprocess spawning inside WSL.
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one Aura Forge session's YOLO choice
    ...
    * ``unrestricted`` — explicit Aura Forge YOLO; launch-time risk
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of miantahaaslam3-png/aura-forge#47072: instead of hardcoding
    ...
    # A Windows-installed cua-driver can hand a WSL-hosted Aura Forge an absolute
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    if sys.platform == "win32":
        ...
        candidates.extend([
            ...
            # misses it until every Aura Forge process is restarted.
            os.path.join(
...
def cua_driver_runtime_contract_status(binary: Optional[str] = None) -> Dict[str, Any]:
    """Report whether a local driver can host Aura Forge' 0.20 integration."""
    ...
    if version < _CUA_DRIVER_RUNTIME_CONTRACT_MIN:
        return {
            ...
            "binary": resolved,
            "version": raw_version,
            "reason": "Aura Forge computer use requires cua-driver 0.20.0 or newer",
        }
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of miantahaaslam3-png/aura-forge#47072: read the canonical
...
class _CuaDriverSession:
    ...
    def __init__(
        ...
        self._started = False
        # Surface 4 of miantahaaslam3-png/aura-forge#47072: per-tool
    ...
    def _call_tool_via_cli(self, name: str, args: Dict[str, Any], timeout: float) -> Dict[str, Any]:
        ...
        try:
            for attempt in range(attempts):
                ...
                # Aura Forge talks to the direct `cua-driver mcp` runtime
    ...
    def _unknown_transport_outcome(name: str, exc: Exception) -> Dict[str, Any]:
        message = (
            ...
            "unknown, so Aura Forge did not replay it. Take fresh state before "
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    miantahaaslam3-png/aura-forge#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        # Surface 6 of miantahaaslam3-png/aura-forge#47072: per-snapshot
        ...
        #   - Gets a distinct agent-cursor color per Aura Forge run, with
        ...
        # one Aura Forge run = one backend = one label — and pass it as
    ...
    def start(self) -> None:
        ...
        if not contract.get("ready"):
            # An installed-but-incompatible driver (e.g. predating a Aura Forge
        ...
        # `computer-use` / `mcp` extras), not part of Aura Forge' minimal core.
    ...
    def capture(
        ...
        # Surface 3 of miantahaaslam3-png/aura-forge#47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of miantahaaslam3-png/aura-forge#47072: prefer the
    ...
    def _apply_delivery(
        ...
        to proceed. See miantahaaslam3-png/aura-forge#67052 phase B.
        """
    ...
    def click(
        ...
        *,
        ...
        # miantahaaslam3-png/aura-forge#47072). cua-driver-rs gained an explicit
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        separately approved by the Aura Forge adapter, and uses cua-driver's
