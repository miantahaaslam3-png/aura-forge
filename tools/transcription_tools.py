...
# DeepInfra STT base URL now resolved via auraforge_cli.models.deepinfra_base_url (shared).
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    """Return whether *provider* is exempt from Aura Forge's remote upload cap."""
...
def _apply_pre_transcription_hook(
    *,
    ...
    """
    try:
        from auraforge_cli.plugins import has_hook, invoke_hook
    ...
    *,
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="auraforge-local-stt-") as output_dir:
            ...
            # Scrub Aura Forge secrets from the child env (sibling path to #56332 /
...
def _transcribe_deepinfra(
    ...
    *,
    ...
    ``auraforge_cli.models`` helpers so every DeepInfra surface resolves the
