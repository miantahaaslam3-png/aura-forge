...
Lets the LLM write a Python script that calls Aura Forge tools via RPC,
...
# available on every platform Aura Forge itself runs on.
...
# operational marker: without it, a sandbox script that spawns/imports Aura Forge
...
"""Auto-generated Aura Forge tools RPC stubs."""
...
"""Auto-generated Aura Forge tools RPC stubs (file-based transport)."""
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    # HERMES_TIMEZONE is an internal Aura Forge setting and must not leak
    ...
    # Aura Forge's own package root is useful too, but only when the child
    # uses the same Python environment. Project mode can select an
    # external venv; exposing Aura Forge's site-packages to that interpreter
...
# Interpreter paths already reported as outside the Aura Forge environment —
...
def _uses_hermes_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Aura Forge's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    if mode == "strict":
        ...
        cwd_note = (
            ...
            "Aura Forge's own python (the common case — stdlib plus Aura Forge's "
    ...
    description = (
        "Run Python that calls Aura Forge tools programmatically. Use when you "
