...
def _is_hermes_provider_credential(name: str) -> bool:
    ...
    Non-Aura Forge API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Aura Forge credential into the execute_code child.
    """
    try:
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Non-Aura Forge third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if _is_hermes_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Aura Forge provider "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                if _is_hermes_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Aura Forge "
