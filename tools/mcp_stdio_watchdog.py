...
<url>``) is spawned as a direct child of the Aura Forge process. Aura Forge's own
...
Aura Forge process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Aura Forge startup's opt-in
