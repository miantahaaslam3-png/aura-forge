...
One ``CDPSupervisor`` runs per Aura Forge ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Aura Forge logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "auraforge-dialog-bridge.invalid"
...
(() => {
  ...
  const ENDPOINT = "http://auraforge-dialog-bridge.invalid/";
