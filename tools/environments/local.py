...
# project's dependencies into the Aura Forge venv path instead of the project's own
# ``.venv`` — silently clobbering the Aura Forge environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Aura Forge venv stays reachable via PATH (its bin dir is first), so stripping
...
# system/venv Pythons — to the Aura Forge venv's stdlib, which crashes with
# version-mismatch errors before a child script even imports a package
# (#75018). Aura Forge itself treats PYTHONHOME as contamination in its own
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    # the separate Aura Forge runtime venv.  The filter validates that relationship
...
# legitimate child Aura Forge spawns needs them, and they are the highest-value
...
def hermes_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Aura Forge spawns legitimately needs them.
...
def build_subprocess_env(
    ...
    *,
    ...
    Aura Forge secret-scrub policy have a single owner.  History: ~11 separate
...
def _find_bash() -> str:
    ...
    raise RuntimeError(
        "Git Bash not found. Aura Forge Agent requires Git for Windows on Windows.\n"
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Aura Forge. If the override is blocked or later re-applied, "
...
def _managed_runtime_path_entries() -> list[str]:
    ...
    itself, so on a machine where Aura Forge provisioned its own toolchain a
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Aura Forge
...
def _build_hermes_repo_root_aliases(
    ...
    """Return exact repo-root spellings emitted by Aura Forge launchers.
...
# --- Aura Forge venv / repo-root detection (module-level, computed once) ---
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    ``<Aura Forge repo>/venv`` layout and a real venv marker; require both before
...
def _get_hermes_site_packages(env: dict) -> list[Path]:
    """Return exact site-packages dirs owned by the Aura Forge runtime.
...
def _strip_hermes_owned_pythonpath(env: dict) -> None:
    ...
    for entry in pp.split(os.pathsep):
        ...
        # --- Check 1: Aura Forge venv site-packages ---
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Aura Forge trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    # Commands run on the Aura Forge host itself — controller-side platform
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        # Honored ahead of the generic TMPDIR so users can redirect Aura Forge' temp
