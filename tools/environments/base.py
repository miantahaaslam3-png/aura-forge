"""Base class for all Aura Forge execution environment backends.
...
# set), not Aura Forge' per-turn session identity.
#
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Aura Forge backends.
    ...
    # True only when commands execute on the SAME host as the Aura Forge process
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # inherited from the Aura Forge process.  ``${VAR:-default}`` semantics:
        ...
        # Restrict Aura Forge metadata files without changing the user's command
