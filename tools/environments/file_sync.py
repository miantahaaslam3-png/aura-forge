...
def iter_sync_files(container_base: str = "/root/.auraforge") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.auraforge to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.auraforge", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync_back(self, hermes_home: Path | None = None) -> None:
        ...
        Downloads the remote ``.auraforge/`` directory as a tar archive,
    ...
    def _sync_back_transaction(self, hermes_home: Path | None = None) -> None:
        ...
        # against an uninitialized remote .auraforge/ directory.
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="auraforge-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.auraforge/skills/a.md`` →
        ``~/.auraforge/skills/a.md``, a new remote file at
        ``/root/.auraforge/skills/b.md`` maps to ``~/.auraforge/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
