...
def _get_active_profile_name() -> str:
    """Return the active Aura Forge profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Aura Forge
    process leaves its container behind permanently because there's no
    subsequent Aura Forge process scheduled to reuse that exact (task, profile)
    pair.
    """
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # container shared across sessions").  If a prior Aura Forge process
    ...
    def _find_reusable_container(
        ...
        # container) but can happen if a previous Aura Forge process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Aura Forge exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Aura Forge process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Aura Forge startup. That covers the
        ...
        # outlives Aura Forge processes, processes inside it stay alive, and
