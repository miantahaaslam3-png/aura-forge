...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``auraforge_constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from auraforge_constants import get_subprocess_home
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as Aura Forge.
...
def _get_real_hermes_home() -> str | None:
    """Return the realpath of the authoritative Aura Forge home (cached)."""
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    container's sandbox mirror of Aura Forge state.

    Two detectors (both #32049): these catch writes that would be
    SILENTLY LOST — the host Aura Forge process never reads the mirror, so
