# Aura Forge Agent Security Policy

This document describes Aura Forge Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/miantahaaslam3-png/aura-forge/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Aura Forge Agent does not operate a bug
bounty program.**
...
Aura Forge Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Aura Forge Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Aura Forge Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Aura Forge Agent's documentation
...
Aura Forge Agent supports two OS-level isolation postures. They address
...
Aura Forge Agent supports this in two ways:

- **Aura Forge Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Aura Forge Agent's in-process heuristics
...
Aura Forge Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Aura Forge Agent
itself. Bugs in Aura Forge Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Aura Forge Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Aura Forge Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Aura Forge Agent has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Aura Forge Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Aura Forge Agent. Bugs in Aura Forge
...
- Aura Forge Agent includes supply-chain guards for MCP server
