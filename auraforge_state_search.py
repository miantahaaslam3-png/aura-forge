...
``auraforge_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import auraforge_state (cycle) — shared
module-level constants live in auraforge_state_common.
"""
...
# Moved methods logged under the "auraforge_state" logger before the split;
...
class SessionSearchMixin:
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Aura Forge surface and jump
